...
The tracked W95FA and Fixedsys FONs are preloaded into every browser and CLI
process and installed lazily by WAT. Stock variable UI fonts, common Win9x UI
aliases, all three fixed stock objects, and explicit Fixedsys requests therefore
use this path without calling the four Canvas text-policy imports; explicit
scalable/document faces and shaped text retain the fallback.
