...
- Heroes' audible menu/map still use about10–16% renderer CPU. Its core spin
  is fixed. User explicitly accepts roughly15% for synthesis: "synth is not
  free." This residual is no longer an acceptance blocker; preserve the music
  and animation rather than making audio optimization a prerequisite.
...
## Local integration, source305

Prepared in `/private/tmp/wa-idle-integrate-20260910` from committed main
453a00c2 plus the complete isolated phone/idle series through51c8b2a0. Preserve
main's symbolic region owners, larger DLL tables and newer tests; retain the
Win16 builtin-name extent needed by the phone fix. Regenerate the map rather
than accepting either parent's generated hash.

Main's guest/profiling clock split requires one semantic integration repair:
helper park deadlines and the host main-Sleep remainder now read `_waitNow`,
the clock that created those deadlines. Input grace still uses host time.
Both new split-clock assertions failed before this repair and pass afterward.

Full build gates pass (933 test files accounted for, not933 executed tests),
canonical1067238/compat1067695 bytes, matching layout `e5ea7699f58899e4`.
Focused tests pass: browser park/scheduler, cooperative deadlines, clock43+
contexts/batch, Win16 WaitMessage, dialog timer/command, cross-instance modal,
Worker scheduler48, ThreadManager, touch controls and single-app layout.

Visible three-app smoke (`wa-idle-integrated-multi305`),15s: FreeCell game1467,
Four Stones and Blackjack notice all alive/error-free, renderer1.51%. FreeCell
and Four Stones main slices stay9 ->9; Blackjack186 ->480 queue parks. Rodent
visible smoke (`wa-idle-integrated-main305`)1.19% renderer, real board rendered.
These verify the integrated local build, not a production change.
Heroes menu smoke in the same output directory:13.12% renderer with music,
no errors; screenshot inspected. This is within the user's accepted synth
cost, with the clock-context park fix retained.
