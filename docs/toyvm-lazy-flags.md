...
walks the finished region backwards — across block edges, from the successors
it emitted — and swaps them in. Up to 16% of a program's dispatches land on a
flagless op; the throughput difference is under this box's noise floor, and it
is kept because the two arms are the same ops doing strictly less work.
