...
(function () {
  ...
  let _guestPathComponent;
  function guestPathComponent(name) {
    if (!_guestPathComponent) {
      if (typeof require === 'function') {
        _guestPathComponent = require('./zip-mount').guestPathComponent;
      } else if (typeof window !== 'undefined' && window.ZipMount) {
        _guestPathComponent = window.ZipMount.guestPathComponent;
      }
      if (typeof _guestPathComponent !== 'function') {
        throw new Error('iso9660: zip-mount.js path sanitizer is unavailable');
      }
    }
    try {
      return _guestPathComponent(name, `ISO directory identifier "${name}"`);
    } catch (error) {
      throw new Error(`iso9660: unsafe directory identifier "${name}": ${error.message}`);
    }
  }
  ...
  // Directory identifiers are length-delimited, not padded descriptor text:
  // retain every code unit so the Win32 component sanitizer can reject an
  // embedded NUL/control or trailing space instead of silently trimming it.
  function ucs2Identifier(bytes, offset, length) {
    if (length & 1) throw new Error('iso9660: odd-length Joliet directory identifier');
    let out = '';
  ...
  function parseDirectoryRecord(bytes, offset, joliet) {
    ...
    else name = joliet ? ucs2Identifier(bytes, idOff, idLen) : asciiString(bytes, idOff, idLen);
  ...
  function parseIso(source, opts = {}) {
    ...
    const walk = (lba, byteLength, prefix, depth) => {
      ...
      for (const rec of directoryRecords(provider, lba, byteLength, info.joliet, info.blockSize)) {
        ...
        if (guestPaths.has(foldedPath)) {
          throw new Error(`iso9660: directory entries "${guestPaths.get(foldedPath)}" and ` +
            `"${path}" both mount at ${foldedPath}; Win32 paths are case-insensitive`);
        }
        guestPaths.set(foldedPath, path);
        files.push({
          path,
          name: rec.name,
          lba: rec.lba,
  ...
  function mountIso(vfs, source, opts = {}) {
    ...
    // Total logical blocks from the PVD, so GetDiskFreeSpace can report the
    // disc's real size rather than a fixed-disk guess.
    if (!vfs.volumeSizes) vfs.volumeSizes = new Map();
    vfs.volumeSizes.set(drive.toLowerCase(), iso.volumeSpaceSize >>> 0);
