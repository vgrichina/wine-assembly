...
function stageAndLoadPe(exports, memoryBuffer, exeBytes, log) {
  ...
  if (staged < exeBytes.length) {
    ...
    for (const section of image.sections) {
      // PointerToRawData=0 and IMAGE_SCN_CNT_UNINITIALIZED_DATA are BSS in the
      // WAT loader. Do not turn their declared raw extent into initialized data.
      if (!section.rawOff || (section.chr & 0x80)) continue;
