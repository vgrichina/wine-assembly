// Where the vendored open fonts get mounted so a guest can find them.
//
// WAT asks the VFS for the same filenames Win98 had — C:\WINDOWS\FONTS\
// ARIAL.TTF, TAHOMABD.TTF — and never learns which font actually answers.
// That keeps face substitution, and with it every licensing and vendoring
// decision, entirely on the host side, exactly like the bundled .FON strikes
// already work. fonts/substitutions.json is the single map both hosts read;
// this module turns it into the mount list each one applies its own way
// (readFileSync in the CLI harness, fetch in the browser).

const FONT_DIR = 'c:\\windows\\fonts\\';
const SUBSET_DIR = 'subset/';

// The bitmap strikes every host mounts unconditionally, named as they appear in
// fonts/. These are the floor under all guest text: an unnamed LOGFONT and the
// Win98 stock fonts resolve to one of these, and no host font path exists
// underneath them, so a host that fails to mount these draws no text at all.
const BUNDLED_BITMAP_FONTS = [
  'System.fon', 'MSSansSerif.fon', 'Fixedsys.fon', 'Courier.fon', 'Terminal.fon',
];

// Where the deployed subset of a vendored font lives. Derived rather than
// listed in the manifest so the two cannot disagree; test/test-font-subsets.js
// checks that every derived path exists and carries the same metrics as the
// full font it came from.
function subsetPath(file) {
  return SUBSET_DIR + file.slice(file.lastIndexOf('/') + 1);
}

// [{ vfsPath, file, face, style }] — `file` is relative to fonts/.
//
// With { subset: true } the mounts point at the CP1252 subsets instead of the
// full TTFs: 278 KB rather than 4.7 MB, which matters because the browser
// fetches all of them before a guest can draw its first character.
function fontMounts(manifest, options) {
  const useSubset = !!(options && options.subset);
  const mounts = [];
  for (const face of manifest.faces || []) {
    if (!face.win98Files) continue;
    for (const style of Object.keys(face.win98Files)) {
      const file = face.styles && face.styles[style];
      // test/test-font-substitutions.js already rejects a manifest that maps a
      // style with no substitute behind it. Skipping here too means a stale
      // deployed manifest degrades to "face unavailable" rather than mounting
      // `undefined` and failing later inside the parser.
      if (!file) continue;
      mounts.push({
        vfsPath: FONT_DIR + face.win98Files[style].toLowerCase(),
        file: useSubset ? subsetPath(file) : file,
        source: file,
        face: face.win98,
        style,
        charset: face.charset,
      });
    }
  }
  return mounts;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { fontMounts, subsetPath, FONT_DIR, SUBSET_DIR, BUNDLED_BITMAP_FONTS };
} else if (typeof window !== 'undefined') {
  window.fontMounts = fontMounts;
  window.fontSubstitutions = {
    fontMounts, subsetPath, FONT_DIR, SUBSET_DIR, BUNDLED_BITMAP_FONTS };
}
