...
const VIRTUAL_MAP_STATE = 0x07F02400;
const VIRTUAL_MAP_TABLE = 0x07F02410;
// CreateDIBSection hands the guest pointers into a dedicated high range backed
// by the final 64MB of linear memory. Must match $DIB_GUEST_BASE /
 ...
 * @param {ArrayBuffer|SharedArrayBuffer|WebAssembly.Memory} [memory] - optional memory for sparse VirtualAlloc maps
 * @returns {number} WASM linear memory offset
 */
function g2w(guestAddr, imageBase, memory) {
  const wa = ((guestAddr >>> 0) - (imageBase >>> 0) + GUEST_BASE) >>> 0;
  if (wa < DIRECT_GUEST_WINDOW) return wa;
  // Same order as WAT's $g2w: the DIB arena is tested only after the direct
  // window misses, and before the sparse VirtualAlloc scan.
  const dibOff = ((guestAddr >>> 0) - DIB_GUEST_BASE) >>> 0;
  if (dibOff < DIB_GUEST_CAPACITY) return (DIB_BACKING_BASE + dibOff) >>> 0;
  const buffer = memory && memory.buffer ? memory.buffer : memory;
  if (buffer) {
    const dv = new DataView(buffer);
    const count = dv.getUint32(VIRTUAL_MAP_STATE, true) >>> 0;
    for (let i = 0; i < count; i++) {
      const rec = VIRTUAL_MAP_TABLE + i * 16;
...
function g2wSpan(guestAddr, max, imageBase, memory) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (addr >= base && addr < ((base + size) >>> 0)) {
      return Math.min(max, (base + size - addr) >>> 0);
    }
  }
  // Unmapped: the caller gets the NULL sentinel, which is 4 bytes wide.
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { readStrA, readStrW, g2w, g2wSpan, walkStackFrame, formatFrames, decodeMfcCString, GUEST_BASE };
} else if (typeof window !== 'undefined') {
  window.memUtils = { readStrA, readStrW, g2w, g2wSpan, walkStackFrame, formatFrames, decodeMfcCString, GUEST_BASE };
}
