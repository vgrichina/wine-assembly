...
function parseDIB(buf) {
  ...
  if (bpp === 16) {
    masks = compression === 3
      ? [r32(40) >>> 0, r32(44) >>> 0, r32(48) >>> 0]
      : [0x7C00, 0x03E0, 0x001F];
    if (compression !== 0 && compression !== 3) return null;
    if (compression === 3 && biSize === 40) maskBytes = 12;
    if (compression === 3 && biSize !== 40 && biSize < 52) return null;
    const valid = masks.every(mask => {
      if (!mask || (mask & 0xFFFF0000)) return false;
      const low = mask & -mask;
      return ((mask + low) & mask) === 0;
    }) && !(masks[0] & masks[1]) && !(masks[0] & masks[2]) && !(masks[1] & masks[2]);
    if (!valid) return null;
  }
  const pixelOff = paletteOff + numColors * palEntrySize + maskBytes;
  ...
  for (let row = 0; row < absH; row++) {
    ...
    if (bpp === 1) {
      ...
      for (let x = 0; x < w; x++) {
        let idx;
        if (rleIndices) {
          idx = rleIndices[row * w + x];
        } else {
          const byte = buf[srcOff + (x >> 1)];
          idx = (x & 1) ? (byte & 0xF) : (byte >> 4);
        }
        pixels[dstOff + x * 4] = pal[idx * 4];
        pixels[dstOff + x * 4 + 1] = pal[idx * 4 + 1];
        pixels[dstOff + x * 4 + 2] = pal[idx * 4 + 2];
      ...
      for (let x = 0; x < w; x++) {
        const si = srcOff + x * 2;
        const px = buf[si] | (buf[si + 1] << 8);
        pixels[dstOff + x * 4] = Math.round(((px & info[0].mask) >>> info[0].shift) * 255 / info[0].max);
        pixels[dstOff + x * 4 + 1] = Math.round(((px & info[1].mask) >>> info[1].shift) * 255 / info[1].max);
        pixels[dstOff + x * 4 + 2] = Math.round(((px & info[2].mask) >>> info[2].shift) * 255 / info[2].max);
        pixels[dstOff + x * 4 + 3] = 255;
      }
    } else if (bpp === 24) {
      for (let x = 0; x < w; x++) {
  ...
  // For 1bpp mono bitmaps, expose the foreground colour (palette[0]) so
  // BitBlt's mono→colour expansion can distinguish foreground from background
  // even when palette[0] isn't black (e.g. hearts/diamonds use red).
  const monoFg = (bpp === 1 && pal.length >= 4) ? { r: pal[0], g: pal[1], b: pal[2] } : null;
  return { w, h: absH, bpp, pixels, indices, paletteBGRA, monoFg, masks };
}
