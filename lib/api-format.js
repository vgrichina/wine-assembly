...
//   WASM_TEXT_N WASM_GDI_DC WASM_GDI_TEXT_AUX
//                                (host text / canonical GDI state in WASM memory)
...
const HOST_TRACE_TABLE = {
  ...
  gdi_text_bind: {
    name: 'gdi_text_bind',
    args: [
      { name: 'token', type: 'HANDLE' },
      { name: 'dc', type: 'WASM_GDI_DC' },
      { name: 'originX', type: 'INT' },
      { name: 'originY', type: 'INT' },
      { name: 'aux', type: 'WASM_GDI_TEXT_AUX' },
      { name: 'clipBandsWa', type: 'DWORD' },
      { name: 'clipCount', type: 'INT' },
    ],
    ret: 'BOOL',
  },
  gdi_patblt: {
...
function fmtArg(type, val, ctx, meta = {}, argVals = [], argSpecs = []) {
  ...
  switch (type) {
    ...
    case 'WASM_GDI_DC': {
      if (!v || v + 92 > ctx.dv.byteLength) return ctx.hex(v);
      const color = off => `#${(ctx.dv.getUint32(v + off, true) & 0xFFFFFF).toString(16).padStart(6, '0')}`;
      return `&{text=${color(20)} bk=${color(24)} bkMode=${ctx.dv.getInt32(v + 28, true)}` +
        ` align=${ctx.hex(ctx.dv.getUint32(v + 32, true))}` +
        ` vpOrg=${ctx.dv.getInt32(v + 56, true)},${ctx.dv.getInt32(v + 60, true)}` +
        ` font=${ctx.hex(ctx.dv.getUint32(v + 88, true))}}`;
    }
    case 'WASM_GDI_TEXT_AUX': {
      if (!v || v + 32 > ctx.dv.byteLength) return ctx.hex(v);
      return `&{charExtra=${ctx.dv.getInt32(v + 20, true)}` +
        ` justifyExtra=${ctx.dv.getInt32(v + 24, true)}` +
        ` justifyCount=${ctx.dv.getInt32(v + 28, true)}}`;
    }
    case 'LPOLESTRARRAY': {
      if (v === 0) return 'NULL';
      try {
