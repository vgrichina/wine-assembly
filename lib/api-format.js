...
//   WASM_TEXT_N                  (host text in WASM memory)
...
// The interfaces an OLE container and its objects actually negotiate. A name
// here turns "QueryInterface failed" into "it wanted IPersistStorage".
const IID_NAMES = {
  ...
  '00000118-0000-0000-c000-000000000046': 'IOleClientSite',
  '0000011b-0000-0000-c000-000000000046': 'IOleContainer',
  '0000011e-0000-0000-c000-000000000046': 'IOleCache',
  '00000128-0000-0000-c000-000000000046': 'IRunnableObject',
  '0000012a-0000-0000-c000-000000000046': 'IViewObject',
  ...
  '00000102-0000-0000-c000-000000000046': 'IEnumFORMATETC',
...
const HOST_TRACE_TABLE = {
  // A window surface is re-created whenever its canonical storage moves, and
  // creation seeds the pixels with COLOR_BTNFACE -- so "who created which
  // surface, with what backing" is the difference between a painted window and
  // a grey one. Trace it with the same detail as the uploads.
  gdi_surface_create: {
    name: 'gdi_surface_create',
    args: [
      { name: 'surface', type: 'HANDLE' },
      { name: 'width', type: 'INT' },
      { name: 'height', type: 'INT' },
      { name: 'bpp', type: 'INT' },
      { name: 'bits', type: 'DWORD' },
      { name: 'stride', type: 'INT' },
      { name: 'topDown', type: 'INT' },
      { name: 'palette', type: 'DWORD' },
      { name: 'paletteCount', type: 'INT' },
    ],
    ret: 'BOOL',
  },
  gdi_surface_attach: {
  ...
  gdi_surface_delete: {
    name: 'gdi_surface_delete',
    args: [{ name: 'surface', type: 'HANDLE' }],
    ret: 'BOOL',
  },
  gdi_surface_upload: {
    name: 'gdi_surface_upload',
    args: [
      { name: 'surface', type: 'HANDLE' },
      { name: 'left', type: 'INT' },
      { name: 'top', type: 'INT' },
...
const VT_NAMES = {
  ...
  21: 'VT_UI8', 22: 'VT_INT', 23: 'VT_UINT',
};

// A VARIANT the guest passed us. The payload alone is unreadable in a trace —
// what matters is the vt (our IDispatch stubs reject types they do not store,
// which surfaces as a bare DISP_E_TYPEMISMATCH) and, for a BSTR, the text.
// Follows one level of VT_BYREF, which is how VB6 passes most property puts.
function fmtVariant(ctx, addr, depth = 0) {
  try {
    const wa = ctx.g2w(addr);
    const vt = ctx.dv.getUint16(wa, true);
    const base = vt & 0x0fff;
    const byref = (vt & 0x4000) !== 0;
    const name = (VT_NAMES[base] || `vt${base}`) + (byref ? '|BYREF' : '') +
      ((vt & 0x2000) ? '|ARRAY' : '');
    const payload = ctx.dv.getUint32(wa + 8, true) >>> 0;
    if (byref && payload && depth === 0) {
      // The pointed-to value carries no vt of its own except for VT_VARIANT,
      // where the target is a whole VARIANT and recursing gives the real type.
      if (base === 12) return `${name}->${fmtVariant(ctx, payload, depth + 1)}`;
      const tw = ctx.g2w(payload);
      const deref = (base === 2 || base === 11) ? ctx.dv.getInt16(tw, true)
        : ctx.dv.getUint32(tw, true) >>> 0;
      return base === 8
        ? `${name}=${fmtBstr(ctx, deref)}`
        : `${name}=${ctx.hex(deref)}`;
    }
    if (base === 8) return `${name}=${fmtBstr(ctx, payload)}`;
    if (base === 11) return `${name}=${payload ? 'True' : 'False'}`;
    if (base === 5) return `${name}=${ctx.dv.getFloat64(wa + 8, true)}`;
    if (base === 0 || base === 1) return name;
    return `${name}=${payload < 0x10000 ? payload : ctx.hex(payload)}`;
  } catch {
    return ctx.hex(addr);
  }
}

function fmtBstr(ctx, ptr) {
  if (!ptr) return 'NULL';
  const s = readWStr(ctx.memory, ctx.g2w, ptr, 160);
  return s === null || s === undefined ? ctx.hex(ptr) : `"${s}"`;
}
  ...
  switch (type) {
    ...
    case 'DISPPARAMS': {
      ...
      try {
        ...
        if (rgvarg && cArgs) {
          const parts = [];
          for (let i = 0; i < Math.min(cArgs, 8); i++) {
            parts.push(fmtVariant(ctx, rgvarg + i * 16));
          }
          args = ` args=[${parts.join(', ')}]`;
        }
        return `&{cArgs=${cArgs} cNamedArgs=${cNamedArgs} rgvarg=${ctx.hex(rgvarg)}${args}}`;
      } catch {
        return ctx.hex(v);
      }
    }
    ...
    // COM's two workhorses. An HRESULT read as a bare hex number tells you
    // nothing about whether the call worked; the sign bit is the whole answer,
    // and the handful of names below cover almost every failure worth tracing.
    // A QueryInterface trace is unreadable without this: the whole content of
    // the call is which interface was asked for, and that is 16 bytes behind a
    // pointer. Known IIDs print by name, the rest as a GUID.
    // Which format is being asked for is the entire content of a GetData or
    // QueryGetData call; the pointer tells you nothing.
    case 'LPFORMATETC': {
      if (!v) return 'NULL';
      try {
        const w = ctx.g2w(v);
        const cf = ctx.dv.getUint16(w, true);
        const aspect = ctx.dv.getUint32(w + 8, true);
        const tymed = ctx.dv.getUint32(w + 16, true);
        const names = { 1: 'CF_TEXT', 2: 'CF_BITMAP', 3: 'CF_METAFILEPICT', 6: 'CF_TIFF',
                        7: 'CF_OEMTEXT', 8: 'CF_DIB', 9: 'CF_PALETTE', 13: 'CF_UNICODETEXT',
                        14: 'CF_ENHMETAFILE' };
        const tymeds = [];
        for (const [bit, name] of [[1, 'HGLOBAL'], [2, 'FILE'], [4, 'ISTREAM'],
                                   [8, 'ISTORAGE'], [16, 'GDI'], [32, 'MFPICT'], [64, 'ENHMF']]) {
          if (tymed & bit) tymeds.push(name);
        }
        const fmt = names[cf] || (cf >= 0xC000 ? `registered:${ctx.hex(cf)}` : ctx.hex(cf));
        return `&{${fmt} aspect=${aspect} tymed=${tymeds.join('|') || '0'}}`;
      } catch {
        return ctx.hex(v);
      }
    }
    case 'REFIID': {
      ...
      try {
        const w = ctx.g2w(v);
        const d1 = ctx.dv.getUint32(w, true), d2 = ctx.dv.getUint16(w + 4, true);
        const d3 = ctx.dv.getUint16(w + 6, true);
        const rest = [];
        for (let i = 8; i < 16; i++) rest.push(ctx.dv.getUint8(w + i).toString(16).padStart(2, '0'));
        const guid = `${d1.toString(16).padStart(8, '0')}-${d2.toString(16).padStart(4, '0')}-` +
          `${d3.toString(16).padStart(4, '0')}-${rest.slice(0, 2).join('')}-${rest.slice(2).join('')}`;
        return IID_NAMES[guid] ? `${IID_NAMES[guid]} {${guid}}` : `{${guid}}`;
      } catch {
        return ctx.hex(v);
      }
    }
    case 'HRESULT':
      return HRESULT_NAMES[v >>> 0] || ((v & 0x80000000) ? `FAILED ${ctx.hex(v)}` : ctx.hex(v));
    // An out-parameter that receives a pointer (IFoo**, void**, DWORD*).
    // Printing the pointer the callee wrote is the point; the address of the
    // caller's variable never is.
    case 'LPDWORD':
    case 'LPVOIDPTR': {
      if (!v) return 'NULL';
      try {
        return `&${ctx.hex(ctx.dv.getUint32(ctx.g2w(v), true) >>> 0)}`;
      } catch {
        return ctx.hex(v);
      }
    }
...
function fmtStruct(type, ptr, ctx) {
  ...
  try {
    switch (type) {
      ...
      // Every CreateDIBSection/CreateDIBitmap/StretchDIBits failure is a
      // rejected header, and the header is the whole content of the call: a
      // bare pointer says nothing about which field the guest got wrong.
      // BITMAPCOREHEADER (size 12) packs width/height as WORDs.
      case 'LPBITMAPINFO': {
        const w = g2w(ptr);
        const size = dv.getUint32(w, true);
        if (size === 12) {
          return `&{core ${dv.getUint16(w + 4, true)}x${dv.getUint16(w + 6, true)}`
            + ` planes=${dv.getUint16(w + 8, true)} bpp=${dv.getUint16(w + 10, true)}}`;
        }
        const COMPRESSION = { 0: 'RGB', 1: 'RLE8', 2: 'RLE4', 3: 'BITFIELDS', 4: 'JPEG', 5: 'PNG' };
        const comp = dv.getUint32(w + 16, true);
        // BI_BITFIELDS puts three DWORD masks right after the 40-byte header
        // (or inside a V4/V5 header at the same offset), and which channel
        // layout was asked for is exactly what decides whether the DIB is
        // accepted — print them.
        let masks = '';
        if (comp === 3) {
          masks = ` masks=${ctx.hex(dv.getUint32(w + 40, true))}/`
            + `${ctx.hex(dv.getUint32(w + 44, true))}/${ctx.hex(dv.getUint32(w + 48, true))}`;
        }
        return `&{size=${size} ${dv.getInt32(w + 4, true)}x${dv.getInt32(w + 8, true)}`
          + ` planes=${dv.getUint16(w + 12, true)} bpp=${dv.getUint16(w + 14, true)}`
          + ` comp=${COMPRESSION[comp] || comp} sizeImage=${dv.getUint32(w + 20, true)}`
          + ` clrUsed=${dv.getUint32(w + 32, true)}${masks}}`;
      }
      case 'LPPOINT': {
        const w = g2w(ptr);
        return `&{x=${dv.getInt32(w, true)} y=${dv.getInt32(w + 4, true)}}`;
      }
      // Virtual LAN sockets: the address, the readiness set, and the
      // timeout are the three structs worth reading in a network trace.
      // WsControl's TDIObjectID. Which entity is being asked about, and for
      // what, is the whole content of the call - the raw pointer says nothing.
      case 'LPTDIOBJ': {
        const w = g2w(ptr);
        const ent = dv.getUint32(w, true), inst = dv.getUint32(w + 4, true);
        const cls = dv.getUint32(w + 8, true), ty = dv.getUint32(w + 12, true);
        const id = dv.getUint32(w + 16, true);
        const ENTITY = { 0x200: 'IF', 0x280: 'AT', 0x300: 'CO_NL', 0x301: 'CL_NL',
                         0x400: 'CO_TL', 0x401: 'CL_TL', 0: 'GENERIC' };
        return `&{entity=${ENTITY[ent] || ctx.hex(ent)}.${inst} class=${ctx.hex(cls)} ` +
               `type=${ctx.hex(ty)} id=${ctx.hex(id)}}`;
      }
      case 'LPSOCKADDR': {
        const w = g2w(ptr);
        const fam = dv.getUint16(w, true);
        if (fam !== 2) return `&{family=${fam}}`;
        const port = dv.getUint16(w + 2, false);
        const o = [0, 1, 2, 3].map(i => dv.getUint8(w + 4 + i));
        return `&${o.join('.')}:${port}`;
      }
      case 'LPFDSET': {
        const w = g2w(ptr);
        const n = Math.min(dv.getUint32(w, true), 64);
        const out = [];
        for (let i = 0; i < n; i++) out.push(ctx.hex(dv.getUint32(w + 4 + i * 4, true)));
        return `&[${out.join(' ')}]`;
      }
      case 'LPTIMEVAL': {
        const w = g2w(ptr);
        return `&{${dv.getInt32(w, true)}s+${dv.getInt32(w + 4, true)}us}`;
      }
      case 'LPSIZE': {
        const w = g2w(ptr);
      ...
      case 'LPWNDCLASSA': {
        ...
        // cbWndExtra decides what GWL offset 4 means: a class that reserves at
        // least DLGWINDOWEXTRA bytes owns a dialog's per-window block, so
        // SetWindowLong(hwnd, DWLP_DLGPROC) lands there instead of in ordinary
        // class-extra storage. Storm registers SDlgDialog that way.
        const clsExtra = dv.getUint32(w + 8, true);
        const wndExtra = dv.getUint32(w + 12, true);
        const cursor = dv.getUint32(w + 24, true);
        const background = dv.getUint32(w + 28, true);
        ...
        // hbrBackground decides whether anything erases this class's windows
        // at all: a NULL brush means the app owns every pixel, which is how
        // apps put windows over a DirectDraw primary.
        return `&{style=${ctx.hex(style)} wndProc=${ctx.hex(wndProc)} class=${classStr} ` +
          `cbClsExtra=${clsExtra} cbWndExtra=${wndExtra} ` +
          `menu=${menuStr} cursor=${ctx.hex(cursor)} hbrBackground=${ctx.hex(background)}}`;
      }
    }
  } catch {}
  return ctx.hex(ptr);
...
function fmtCharFormat(ptr, ctx, wide = false) {
  if (!ptr) return 'NULL';
  const { dv, g2w, hex } = ctx;
  try {
    const w = g2w(ptr);
    const cb = dv.getUint32(w, true) >>> 0;
    const mask = dv.getUint32(w + 4, true) >>> 0;
    const effects = dv.getUint32(w + 8, true) >>> 0;
    const yHeight = dv.getInt32(w + 12, true) | 0;
    const yOffset = dv.getInt32(w + 16, true) | 0;
    const color = dv.getUint32(w + 20, true) >>> 0;
    const charset = dv.getUint8(w + 24);
    const pitch = dv.getUint8(w + 25);
    let face = '';
    for (let i = 0; i < 32; i++) {
      const ch = wide ? dv.getUint16(w + 26 + i * 2, true) : dv.getUint8(w + 26 + i);
      if (!ch) break;
      face += String.fromCharCode(ch);
    }
    const flags = [];
    if (effects & 0x1) flags.push('bold');
    if (effects & 0x2) flags.push('italic');
    if (effects & 0x4) flags.push('underline');
    if (effects & 0x40000000) flags.push('autocolor');
    return `${hex(ptr)} &{cb=${cb} mask=${hex(mask)} effects=${hex(effects)}${flags.length ? ':' + flags.join('|') : ''} yHeight=${yHeight} yOffset=${yOffset} color=${hex(color)} charset=${charset} pitch=${pitch} face=${JSON.stringify(face)}}`;
  } catch {
    return hex(ptr);
  }
}
...
function formatCall(entry, esp, ctx) {
  ...
  for (let i = 0; i < entry.args.length; i++) {
    ...
    let formatted = fmtArg(a.type, argVals[i], ctx, a, argVals, entry.args);
    if ((entry.name === 'SendMessageA' || entry.name === 'SendMessageW') &&
        i === 3 && (argVals[1] === 0x043A || argVals[1] === 0x0444)) {
      formatted = fmtCharFormat(argVals[i] >>> 0, ctx, entry.name === 'SendMessageW');
    }
