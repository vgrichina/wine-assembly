...
function createAudioHost(ctx, shared) {
  ...
  const _mciFetchLate = (dev) => {
    if (!dev || dev.smf || !dev.element || !_readVfsFileAsync || !ctx.readFileAsync) return;
    dev.lateFetch = true;
    _readVfsFileAsync(dev.element).then(bytes => {
      dev.lateFetch = false;
      if (!bytes || dev.smf || !_mci.devices.has(dev.id)) return;
  ...
  const _mciStringCommand = (cmdWa, retWa, retLen) => {
    ...
    if (verb === 'open') {
      ...
      if (isSeq && !dev.smf) _mciFetchLate(dev);
      if (dev.type === 'sequencer') {
  ...
  const _mciOpen = (deviceTypeOrWa, elementWa, flags, isWide) => {
    ...
    _mci.devices.set(id, dev);
    if (isMidi && !dev.smf) _mciFetchLate(dev);
    if (isMidi) {
