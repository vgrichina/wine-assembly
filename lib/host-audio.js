  ...
  if (ctx.sharedAudio && ctx.sharedAudio.voices) {
    ...
    _voices = ctx._voices = {
      ...
      isPlaying(id) {
        ...
        if (!v) return false;
        if (v.mode === 'snapshot') {
          if (v.currentSrc) return true;
          if (!v.snapshotPlaying) return false;
          if (v.snapshotLoop) return true;
          // With no AudioContext there is no source.onended callback. Retire
          // a one-shot from the same guest clock that advances its cursor.
          const nowMs = _audioClockMs();
