...
function createAudioHost(ctx, shared) {
  ...
  const _tapGain = (v) => {
    const voiceGain = (v && v.gain && v.gain.gain && Number.isFinite(v.gain.gain.value))
      ? v.gain.gain.value : 1;
    ...
    const pan = (v && v.pan && v.pan.pan && Number.isFinite(v.pan.pan.value)) ? v.pan.pan.value : 0;
  ...
  if (ctx.sharedAudio && ctx.sharedAudio.voices) {
    ...
    _voices = ctx._voices = {
      ...
      _ensureCtx(rate) {
        ...
        if (!AC) return null;
        if (typeof window !== 'undefined' && window.claimAudioSession) window.claimAudioSession();
  ...
  // the waveOut/DirectSound PCM seams above.  Frozen CLI recordings do not
  // have a real-time AudioContext destination to capture, so render the same
  // parsed note timeline into a compact mono buffer for the guest-clock tap.
  // The browser path remains untouched because _audioTap() is null there.
  const _tapMidiSequence = (dev, firstStart) => {
    ...
    const rate = 22050;
    const duration = Math.max(0, Math.min(15 * 60,
      (dev.smf.duration || 0) - firstStart));
    if (!(duration > 0)) return;
    const mix = new Float32Array(Math.ceil(duration * rate));
    let noiseState = 0x6D2B79F5;
    for (const n of notes) {
      const relativeStart = n.start - firstStart;
      const noteEnd = relativeStart + Math.min(n.dur, 8);
      if (noteEnd <= 0 || relativeStart >= duration) continue;
      const from = Math.max(0, Math.floor(relativeStart * rate));
      const to = Math.min(mix.length, Math.ceil(noteEnd * rate));
      const freq = 440 * Math.pow(2, (n.note - 69) / 12);
      const level = (n.vel / 127) * _midiSequencerNoteGain;
      for (let i = from; i < to; i++) {
        const age = i / rate - relativeStart;
        const left = noteEnd - i / rate;
        const env = Math.min(1, age / 0.012, left / 0.10);
        let wave;
        if (n.ch === 9) {
          noiseState = (Math.imul(noiseState, 1664525) + 1013904223) | 0;
          wave = ((noiseState >>> 8) / 0x800000 - 1) * Math.exp(-age * 14);
        } else {
          const phase = age * freq;
          wave = Math.sin(phase * Math.PI * 2);
          // A quiet harmonic keeps the headless rendition musical without
          // trying to impersonate the browser's full General MIDI soundfont.
          wave += 0.22 * Math.sin(phase * Math.PI * 4 + n.ch * 0.17);
        }
        mix[i] += wave * level * Math.max(0, env);
      }
    }
    const pcm = new Uint8Array(mix.length * 2);
    ...
    const master = _audioMixerState.mixerMutes[0] ? 0 : _mixerGain(_audioMixerState.mixerVolumes[0] >>> 0);
    const gain = midi * master * Math.SQRT1_2;
    try {
      tap.pcm({
