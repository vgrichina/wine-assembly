...
function createAudioHost(ctx, shared) {
  ...
  function _trailingPcmSilenceFrames(mem, ptr, len, channels, bits) {
    if (!mem || !channels || (bits !== 8 && bits !== 16)) return 0;
    const bps = bits / 8;
    const frames = (len / (bps * channels)) | 0;
    let silent = 0;
    for (let frame = frames - 1; frame >= 0; frame--) {
      let frameSilent = true;
      for (let ch = 0; ch < channels; ch++) {
        const off = ptr + (frame * channels + ch) * bps;
        if (bits === 8 ? mem[off] !== 0x80 : (mem[off] !== 0 || mem[off + 1] !== 0)) {
          frameSilent = false;
          break;
        }
      }
      if (!frameSilent) break;
      silent++;
    }
    return silent;
  }
  ...
  const _updateVoiceSpatialNode = (v) => {
    ...
    try {
      panner.refDistance = Math.max(0.000001, state.minDistance);
      panner.maxDistance = Math.max(panner.refDistance, state.maxDistance);
  ...
  if (ctx.sharedAudio && ctx.sharedAudio.voices) {
    ...
    _voices = ctx._voices = {
      ...
      playRing(id, ptr, len, startOff, loop) {
        ...
        // Miles retunes its fixed 32 KiB mono mixer buffer immediately before
        // starting full-length effects such as Heroes II town construction.
        // It still passes DSBPLAY_LOOPING, but never services or stops that
        // buffer afterward. Consume this per-play marker so those full-ring
        // effects finish once; continuously mixed/ambient rings do not use
        // this stopped-buffer SetFrequency sequence.
        const retunedMilesOneShot = !!loop && v.retunedWhileStopped &&
          v.rate === 22050 && v.channels === 1 && v.bits === 8 && playLength === 32768;
        v.retunedWhileStopped = false;
        ...
        try {
          ...
          const src = ac.createBufferSource();
          src.buffer = buf;
          // Miles starts both its continuously mixed ambience and individual
          // effects with DSBPLAY_LOOPING. A one-shot occupies the front of a
          // fixed 32 KiB buffer and leaves a substantial 0x80/zero-filled
          // tail; looping that allocation repeats pickups and UI effects
          // forever. A true mixer/ambience ring carries PCM through its final
          // segment and remains looped/refreshed through Lock/Unlock.
          const trailingSilentFrames = loop
            ? _trailingPcmSilenceFrames(pcm, ptr + (startOff | 0), playLength,
                                        v.channels, v.bits)
            : 0;
          const totalFrames = buf.length | 0;
          src.loop = !!loop && !retunedMilesOneShot &&
            trailingSilentFrames < Math.max(1, totalFrames >> 3);
          ...
          v.currentSrc = src;
          src.onended = () => { if (v.currentSrc === src) v.currentSrc = null; };
        } catch (_) {}
      },
      stop(id) {
        const v = this._map[id]; if (!v) return;
      ...
      isPlaying(id) {
        ...
        if (!v) return false;
        if (v.mode === 'snapshot') return !!v.currentSrc;
      ...
      setFreq(id, hz) {
        const v = this._map[id]; if (!v) return;
        if (!v.currentSrc) v.retunedWhileStopped = true;
        v.freq = hz || v.rate;
  ...
  const _mciCommand = (hostId, command, flags, paramsWa) => {
    ...
    switch (command >>> 0) {
      ...
      case 0x0804: // MCI_CLOSE
        if (dev.type === 'waveaudio') _waveStop(dev);
        else _midiStop(dev);
  ...
  const _mciStringCommand = (cmdWa, retWa, retLen) => {
    ...
    if (verb === 'open') {
      ...
      if (tokens[1] && lower[1] !== 'type' && lower[1] !== 'alias' &&
          lower[1] !== 'sequencer' && lower[1] !== 'waveaudio') element = tokens[1];
      ...
      const dev = {
        id,
        type: isSeq ? 'sequencer' : isWave ? 'waveaudio' : (type || 'auto'),
        element,
    ...
    if (verb === 'status') {
      ...
      let value = '';
      _mciRefreshPlaybackState(dev);
      if (item === 'length') value = String(_mciLengthMs(dev));
  ...
  const _mciOpen = (deviceTypeOrWa, elementWa, flags, isWide) => {
    ...
    const dev = {
      id,
      type: isMidi ? 'sequencer' : isWave ? 'waveaudio' : (type || 'auto'),
      element,
