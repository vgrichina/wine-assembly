...
function createDebugMidi(deps) {
  ...
  async function play(url) {
    ...
    try {
      ...
      // primeAudio() only builds a context once something has asked for audio
      // (host.js). Playing a MIDI file by hand is exactly that request, and
      // there is no guest here to make it.
      if (audioHost.markAudioRequested) audioHost.markAudioRequested();
