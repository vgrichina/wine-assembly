...
(function (root) {
  ...
  function createWorkerImports(memory, sigs, post, opts) {
    ...
    const apiNameAt = (ptr, len) => {
      try {
        const start = ptr >>> 0;
        const count = Math.min(len >>> 0, 64, memory.buffer.byteLength - start);
        if (start >= memory.buffer.byteLength || count <= 0) return '';
    ...
    names.forEach((name, id) => {
      ...
      // The dispatch brackets every GL call with API logging hooks. In normal
      // browser runs both hooks are no-ops, yet forwarding them emitted two
      // extra messages per vertex. Recognise only GL dispatch names locally;
      // verbose/API-trace sessions opt back into the ordinary broker path.
      if (name === 'log' && !opts.forwardGlLogs) {
        host[name] = (ptr, len) => {
          const apiName = apiNameAt(ptr, len);
          suppressGlApiExit = /^(?:gl|wgl)/.test(apiName) || apiName === 'SwapBuffers';
          if (suppressGlApiExit) { stats.local++; return; }
          stats.async++;
          post({ t: 'call', id, args: [ptr, len], slot });
        };
        return;
      }
      if (name === 'log_api_exit' && !opts.forwardGlLogs) {
        host[name] = (...args) => {
          if (suppressGlApiExit) {
            suppressGlApiExit = false;
            stats.local++;
            return;
          }
          stats.async++;
      ...
      host[name] = (...args) => {
        ...
        const out = returnsF64 ? v.f64[0] : i32Result;
        return out;
      };
    });
