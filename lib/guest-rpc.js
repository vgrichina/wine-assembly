...
(function (root) {
  ...
  function readInputPending(memory) {
    return Atomics.load(views(memory).i32, SLOT.INPUT_PENDING) | 0;
  }
  ...
  function createMainBroker(memory, hostImports, sigs, opts) {
    ...
    return {
      ...
      publish(state) {
        ...
        if (state.inputPending !== undefined) Atomics.store(v.i32, SLOT.INPUT_PENDING, state.inputPending | 0);
  ...
  const api = { RPC_BASE, SLOT, CTRL_INTS, STATUS_IDLE, STATUS_REQ, STATUS_RESP, readInputPending,
