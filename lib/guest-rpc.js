...
(function (root) {
  ...
  const RPC_MAX_SLOTS = Regions.SIZE.THREAD_RPC / RPC_STRIDE;
  const I32 = n => n * 4;
    ...
    names.forEach((name, id) => {
      ...
      if (name === 'gpu_gl_call' && glEncoder) {
        host[name] = (opcode, stackWa, aux) => {
          stats.local++;
          // Shader compilation consumes shared source memory synchronously;
          // never encode this private D3D command as a GL opcode.
          if (opcode >= 0x30000 && opcode <= 0x30005) return brokerCall(id, [opcode, stackWa, aux], false);
          if (opts.d3dCommands) {
            const d3dResult = opts.d3dCommands.call(opcode, stackWa, aux);
            if (d3dResult >= 0) return d3dResult | 0;
          }
          // Reserved for the opt-in D3DIM software render Worker. A normal
          // Worker instance reports "not queued" so WAT renders synchronously.
