...
const DLL_PATHS = {
  ...
  'd3dxof.dll': 'binaries/dlls/d3dxof.dll',
  'cards.dll': 'binaries/entertainment-pack/cards.dll',

  // The Visual Basic 6 runtime, from Microsoft's own SP6 redistributable. A
  // VB6 executable is almost nothing but imports of this one DLL -- without it
  // there is no guest code worth running, which is why Rodent's Revenge and
  // JigSawed trapped on their first instruction. The other three come out of
  // the same package and are what msvbvm60 itself loads.
  'msvbvm60.dll': 'binaries/dlls/msvbvm60.dll',
  'olepro32.dll': 'binaries/dlls/olepro32.dll',
  'asycfilt.dll': 'binaries/dlls/asycfilt.dll',
  'comcat.dll': 'binaries/dlls/comcat.dll',
};
...
const APP_LOCAL_DLLS = [
  'msvcp50.dll',
  // Blizzard's Storm runtime is shipped beside StarCraft/Diablo and exports
  // game-specific ordinal APIs. Treating it like a stubbed system DLL turns
  // every ordinal import into an unimplemented generic thunk.
  'storm.dll',
  // Diablo's other two private DLLs, written beside diablo.exe by the retail
  // CD installer. Without these the sibling-DLL discovery never even asks for
  // them and the game traps on UiAppActivate the moment its window is up.
  'diabloui.dll', 'smackw32.dll',
  // The Win98 shell: Explorer's window, desktop and taskbar live in SHELL32
  // (entered through ordinal 244) and SHDOCVW.
  'shell32.dll', 'shlwapi.dll', 'shdocvw.dll', 'shdoc401.dll', 'browseui.dll',
  'ole32.dll',
  'kvdd.dll', 'sdl.dll',
