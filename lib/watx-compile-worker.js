...
(function () {
  ...
  function decodeInputs(msg) {
    ...
    const compiler = (msg.compilerNames || []).map(name => ({ name, text: take() }));
    const entry = take();
    for (const name of (msg.sourceNames || [])) addSource(name, take());
    return { compiler, entry, vfs };
  }
  ...
  function handle(msg) {
    ...
    try {
      const t05 = now();
      const input = decodeInputs(msg);
      decodeMs = now() - t05;

      const t1 = now();
      const { compile, logs } = loadCompiler(input.compiler);
      input.compiler = null;
      evalMs = now() - t1;
      bumpPeak();
