...
(function () {
  ...
  function handle(msg) {
    ...
    try {
      ...
      // THE PLACED MAP TRAVELS WITH THE BYTES.
      // A source build's wasm and lib/region-map.generated.js are two halves of
      // one memory map, and pairing halves that disagree does not fail — every
      // host import reads at the address the mirror names, the guest wrote it
      // somewhere else, and the app draws a plausible wrong picture. The
      // build-time artifacts carry a layout fingerprint in a custom section for
      // exactly this comparison, but this path cannot: hashing needs
      // crypto.subtle, which is undefined outside a secure context, and
      // `http://<lan-ip>:8099/?compile-wat` is a workflow we actually use
      // (tools/ios-selftest-server.js). So the PLACEMENT itself is sent instead
      // of a digest of it — 174 short triples, no crypto, and a mismatch can
      // name the region that moved rather than two opaque hashes. The host
      // compares; formatting lives there so there is one implementation of it.
      const placed = (result.regions && result.regions.regions) || null;
      send({
        type: 'result',
        ok: valid !== false,
        valid,
        ...
        // `kind` travels too: a `span` is a transparent named LIMIT that owns
        // no bytes and has no mirror entry, so a comparison that does not know
        // which entries are spans reports $DIRECT_WINDOW as a missing region
        // every single time and the check gets switched off as noisy.
        layout: placed && placed.map(r => ({ name: r.name, kind: r.kind, base: r.base, size: r.size })),
