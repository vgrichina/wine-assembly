...
(function () {
  ...
  const HINT_MS = 1500;      // when the footer with the next actions fades in
  ...
  const STYLE = `
    ...
    /* The "what now" footer under the safe-to-turn-off bitmap. It fades in
       after a pause so the screen reads as a dead monitor first, and it is
       the only thing on that screen that does anything: a stray tap must
       not restart the machine. */
    #wine-power-screen .footer {
      position: absolute; left: 0; right: 0; bottom: 4vh; text-align: center;
      font: 13px/1.4 "Microsoft Sans Serif", Tahoma, Arial, sans-serif;
      color: #a0a0a0; opacity: 0; transition: opacity 0.6s ease;
      padding: 0 12px; z-index: 1;
    }
    #wine-power-screen .footer.on { opacity: 1; }
    #wine-power-screen .footer p { margin: 0 0 10px; }
    #wine-power-screen .footer .actions {
      display: flex; flex-wrap: wrap; justify-content: center; gap: 10px;
    }
    #wine-power-screen .footer .actions button,
    #wine-power-screen .footer .actions a {
      box-sizing: border-box; min-width: 180px; padding: 6px 14px;
      font: inherit; color: #f0f0f0; text-decoration: none;
      background: #202020; border: 1px solid #7a7a7a; border-radius: 0;
      cursor: pointer;
    }
    #wine-power-screen .footer .actions button:hover,
    #wine-power-screen .footer .actions a:hover { background: #2a2a2a; color: #fff; }
    #wine-power-screen .footer .actions button:focus-visible,
    #wine-power-screen .footer .actions a:focus-visible { outline: 1px dotted #fff; outline-offset: -4px; }
  `;
  ...
  function install(deps) {
    ...
    function paintAllFrom(wine) {
      let any = false;
      for (const kind of [SCREEN_WAIT, SCREEN_OFF]) {
    ...
    function paintAhead() {
      if (painted.has(SCREEN_WAIT) && painted.has(SCREEN_OFF)) return;
    ...
    function showOff() {
      // The machine is off. Nothing on this screen wakes it except the one
      // button that says so -- a tap anywhere used to reload the page, and a
      // reload is not something to hand out for a stray click.
      const restart = el('button', { type: 'button', class: 'restart', text: 'Restart the computer' });
      const visit = el('a', {
        class: 'visit', href: 'https://berrry.app', target: '_blank', rel: 'noopener',
        text: 'berrry.app',
      });
      const footer = el('div', { class: 'footer' }, [
        el('p', { text: 'Thanks for running Wine-Assembly.' }),
        el('div', { class: 'actions' }, [restart, visit]),
      ]);
      showScreen('off', [logoCanvas(SCREEN_OFF), footer]);
      after(HINT_MS, () => footer.classList.add('on'));
      restart.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); powerOn(); });
      // The screen's pointerdown handler refocuses the invisible key trap,
      // which would steal focus from these controls mid-click.
      for (const node of [restart, visit]) node.addEventListener('pointerdown', e => e.stopPropagation());
    }
