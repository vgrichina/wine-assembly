...
// S-expression parser sees it. The emulator source is deliberately split
// across files: 01-header.wat opens `(module` and 13-exports.wat closes it, so
// validating each part in isolation would reject the real build. Keep lexer
// state across parts instead and require the complete stream to balance.
//
...
async function compileWat(readFile, options = {}) {
  ...
  const cacheKey = `${options.tailCalls === false ? 'compat-no-tail-calls' : 'tail-calls'}:${version}`;
