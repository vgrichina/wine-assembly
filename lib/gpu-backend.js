...
(function (root, factory) {
  ...
  class WebGLBackend {
    ...
    updateBuffer(buffer, target, data, usage) {
      ...
      gl.bufferData(target || gl.ARRAY_BUFFER, data, usage || gl.STREAM_DRAW);
    }
    ...
    useProgram(program) {
      this.gl.useProgram(program && program.handle || null);
    }
    ...
    setViewport(x, y, width, height) { this.gl.viewport(x, y, width, height); }
    setScissor(x, y, width, height) { this.gl.scissor(x, y, width, height); }
    setDepthFunc(value) { this.gl.depthFunc(value); }
    setDepthMask(value) { this.gl.depthMask(!!value); }
    setDepthRange(nearValue, farValue) { this.gl.depthRange(nearValue, farValue); }
    setBlendFunc(src, dst) { this.gl.blendFunc(src, dst); }
    setCullFace(value) { this.gl.cullFace(value); }
    setFrontFace(value) { this.gl.frontFace(value); }
    setLineWidth(value) { this.gl.lineWidth(value); }
    ...
    draw(command) {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, command.vertexBuffer);
      ...
      for (const attribute of command.attributes) {
        ...
        gl.vertexAttribPointer(location, attribute.size, attribute.type || gl.FLOAT,
          !!attribute.normalized, stride, attribute.offset);
      }
      if (command.indexBuffer) {
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, command.indexBuffer);
