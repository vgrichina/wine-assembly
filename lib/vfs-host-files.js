...
function guestPathInTree(treeRoot, hostPath) {
  const relative = path.relative(path.resolve(treeRoot), path.resolve(hostPath));
  if (!relative || relative === '..' || relative.startsWith('..' + path.sep) ||
      path.isAbsolute(relative)) return null;
  return 'C:\\' + relative.replace(/\//g, '\\');
}
...
module.exports = { expandIncludePatterns, guestPathInTree, globRegex, normalizePattern };
