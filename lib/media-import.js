...
(function () {
  ...
  // The data track is a provider window over MODE1/2352 payload bytes; audio
  // files remain unopened until MCI plays a track.
  async function analyzeCueBundle(values, opts) {
    ...
    if (dataTracks.length && dataTracks[0].type !== 'MODE1/2352') {
      throw new Error(`media import: data track type ${dataTracks[0].type} is not supported (expected MODE1/2352)`);
    }
    ...
    if (dataTracks.length) {
      ...
      const mode1 = C.mode1RawProvider(raw, {
        rawOffset: range.rawOffset,
