...
(function () {
  ...
  async function materializeIniFiles(vfs, paths) {
    ...
    for (const path of paths || []) {
      // InstallShield's language/profile file uses the same INI syntax and is
      // consumed through GetPrivateProfile* despite its historical .LID
      // suffix. Alpha Centauri's SETUP.EXE is one such browser-only failure:
      // a Blob-backed ISO cannot synchronously fill SETUP.LID after the host
      // import has already started.
      if (!/\.(?:ini|lid)$/i.test(path)) continue;
  ...
  // Some preservation images contain an AUTORUN.INF that faithfully points
  // at a broken mastering-time wrapper rather than the installer on the disc.
  // Keep these exceptions as narrow disc-shape matches: a volume label plus
  // both the bad target and the known-good target must agree.  The wrapper
  // remains selectable; it simply stops being the default.
  function applyIsoCompatibility(volumeLabel, candidates) {
    const byPath = path => candidates.find(candidate =>
      candidate.path.toUpperCase() === path.toUpperCase());
    if (String(volumeLabel || '').toUpperCase() === 'SMAC-E1_0Z') {
      const broken = byPath('D:\\AUTOMENU.EXE');
      const game = byPath('D:\\PROGRAMS\\TERRAN.EXE');
      const patch = byPath('D:\\PATCH\\SMACP4E.EXE');
      const setup = byPath('D:\\SETUP.EXE');
      if (broken && broken.autorun && game && patch && setup) {
        broken.brokenAutorun = true;
        delete broken.autorun;
        setup.recommended = true;
        setup.compatibilityLabel = 'original installer';
        patch.compatibilityLabel = 'official v4 updater';
        return [setup, patch, ...candidates.filter(candidate =>
          candidate !== setup && candidate !== patch)];
      }
    }
    return candidates;
  }
  ...
  async function analyzeIso(base, options, prepared) {
    ...
    // Retail Diablo's disc is uniquely identified by its root DIABDAT.MPQ.
    // Storm's load/provider loop depends on timeSetEvent callbacks continuing
    // between guest slices, just like the registered shareware build.
    const asyncMultimediaTimer = paths.some(path =>
      path.toUpperCase() === `${root}DIABDAT.MPQ`.toUpperCase());
    ...
    exeCandidates = applyIsoCompatibility(parsed.volumeLabel, exeCandidates);
    return {
      ...base,
      ...
      async mount(vfs) {
        ...
        await materializeIniFiles(vfs, paths);
        // Diablo performs MPQ reads on a cooperative worker while the main
        // thread is inside a synchronous dialog callback. Browser File reads
        // cannot suspend that callback without losing its guest stack, so
        // stage the one identified data archive before execution begins.
        if (asyncMultimediaTimer) await vfs.materialize(`${root}DIABDAT.MPQ`);
        return mounted;
      },
    };
  }
  ...
  function makeAppEntry(plan, opts) {
    ...
    return {
      ...
      mediaName: plan.name,
      asyncMultimediaTimer: !!plan.asyncMultimediaTimer,
      // Windows starts a program with its executable directory as the current
      // directory. Disc installers use that to find adjacent setup payloads;
      // leaving imported media at VirtualFS's C:\ default makes D:\SETUP.EXE
      // manufacture C:\SETUP.INS and strand its D:\_SETUP.LIB companions.
      workingDirectory: exePath.replace(/[^\\]+$/, ''),
