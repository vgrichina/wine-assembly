...
function parseResources(pe) {
  ...
  function parseDialog(buf) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (ctrl.className === 'Button') {
        ...
        else if (bs === 0x04) ctrl.type = 'radiobutton';
        else if (bs === 0x09) ctrl.type = 'autoradiobutton';
  ...
  function parseDIB(buf) {
    ...
    const w = bufR32(buf, 4);
    const h = bufR32(buf, 8);  // signed — positive = bottom-up
    const bpp = bufR16(buf, 14);
    const absH = Math.abs(h | 0);  // handle signed
    const bottomUp = (h | 0) > 0;
    const numColors = bpp <= 8 ? (1 << bpp) : 0;
    const paletteOff = biSize;
    const pixelOff = paletteOff + numColors * 4;
    const rowBytes = Math.ceil(w * bpp / 32) * 4;  // DWORD-aligned
    const pixels = new Uint8Array(w * absH * 4);
    // Read palette
    const pal = [];
    for (let i = 0; i < numColors; i++) {
      const b = buf[paletteOff + i * 4], g = buf[paletteOff + i * 4 + 1], r = buf[paletteOff + i * 4 + 2];
      pal.push(r, g, b, 255);
    }
    ...
    return { w, h: absH, bpp, pixels };
  }

  // --- Build output ---
  const result = { menus: {}, dialogs: {}, strings: {}, icons: {}, accelerators: {}, bitmaps: {} };
