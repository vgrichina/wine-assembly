...
function seedExeImage(vfs, exeBytes, exeName, guestPath) {
  ...
  let exact = null;
  if (guestPath) {
    exact = String(guestPath).replace(/\//g, '\\').toLowerCase();
    if (!/^[a-z]:\\/.test(exact)) exact = 'c:\\' + exact.replace(/^\\+/, '');
    if (!paths.includes(exact)) paths.push(exact);
  }
  for (const p of paths) {
    if (vfs.ensureParentDirs) vfs.ensureParentDirs(p);
    vfs.files.set(p, { data, attrs: 0x20 });
  }
