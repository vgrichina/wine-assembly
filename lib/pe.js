...
function readPE(fileOrBuffer) {
  const isNodeBuffer = typeof Buffer !== 'undefined' && Buffer.isBuffer(fileOrBuffer);
  let buf = fileOrBuffer;
  if (typeof fileOrBuffer === 'string') {
    if (typeof require !== 'function') throw new Error('PE paths are only available in Node');
    buf = require('fs').readFileSync(fileOrBuffer);
  } else if (!isNodeBuffer && fileOrBuffer instanceof ArrayBuffer) {
    buf = new Uint8Array(fileOrBuffer);
  }
  if (!buf || typeof buf.length !== 'number') throw new Error('PE input must be a path or byte buffer');
  const data = new DataView(buf.buffer, buf.byteOffset || 0, buf.byteLength);
  const u16 = off => data.getUint16(off, true);
  const u32 = off => data.getUint32(off, true);
  if (buf.length < 0x40 || u16(0) !== 0x5A4D) {
    throw new Error('not a PE image (no MZ signature)');
  }
  const peOff = u32(0x3C);
  if (u32(peOff) !== 0x00004550) {
    throw new Error('not a PE image (no PE signature at e_lfanew)');
  }
  const numSect = u16(peOff + 6);
  const optSize = u16(peOff + 20);
  const imageBase = u32(peOff + 52);
  const entryRva = u32(peOff + 40);
...
const peApi = { readPE, nameLooksCode, IMAGE_SCN_CNT_CODE, IMAGE_SCN_MEM_EXECUTE };
if (typeof module !== 'undefined' && module.exports) module.exports = peApi;
if (typeof window !== 'undefined') window.peLib = peApi;
