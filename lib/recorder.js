...
(function () {
  ...
  function pickRecordSource() {
    ...
    if (recordTarget === 'screen') {
      ...
      return {
        w: sw,
        h: sh,
        draw: (ctx, dw, dh) => ctx.drawImage(screen, 0, 0, sw, sh, 0, 0, dw, dh),
      };
    }
    ...
    return {
      ...
      draw: (ctx, dw, dh) => {
        if (win._backCanvas) {
          try { ctx.drawImage(win._backCanvas, 0, 0, sw, sh, 0, 0, dw, dh); return; }
          catch (_) {}
        }
        if (screen) {
  ...
  function start(options) {
    ...
    recCanvas.width = source.w * SCALE;
    recCanvas.height = source.h * SCALE;
