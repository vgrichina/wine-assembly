...
(function () {
  ...
  function paintFrame() {
    ...
    if (win._backCanvas) {
      try { recCtx.drawImage(win._backCanvas, 0, 0, w, h); return; }
      catch (_) {}
    }
    try { recCtx.drawImage(screen, win.x, win.y, w, h, 0, 0, w, h); }
    catch (_) {}
  }
  ...
  function start() {
    ...
    recCanvas.width = Math.max(1, win.w | 0);
    recCanvas.height = Math.max(1, win.h | 0);
