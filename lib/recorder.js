...
(function () {
  ...
  let recCtx = null;
  let rafId = 0;
  let audioTaps = [];   // [{ ac, master, dest, source, sink }]
  let audioMixer = null;
  ...
  function paintFrame() {
    ...
    if (!source) return;
    // MediaRecorder's encoded dimensions are established at start. Resizing
    // its canvas when a game enters fullscreen can make Safari stretch the
    // new 4:3 picture into the initial desktop-shaped video. Keep the capture
    // surface fixed for the session and contain every source at its own ratio.
    const dw = recCanvas.width;
    const dh = recCanvas.height;
    const fit = Math.min(dw / source.w, dh / source.h);
    const pw = source.w * fit;
    const ph = source.h * fit;
    recCtx.fillStyle = '#000';
    recCtx.fillRect(0, 0, dw, dh);
    recCtx.save();
    recCtx.translate((dw - pw) / 2, (dh - ph) / 2);
    try { source.draw(recCtx, pw, ph); }
    catch (_) {}
    finally { recCtx.restore(); }
  }

  function findAllAudioCtxs() {
    const set = new Set();
    const shell = typeof window !== 'undefined' && window.wineShell;
    const apps = (shell && shell.runningApps) || ((typeof runningApps !== 'undefined') ? runningApps
  ...
  // The audio track must exist BEFORE MediaRecorder starts, even if the guest
  // has not requested sound yet. Keep its mixer independent of guest lifetime
  // and attach newly created guest contexts while recording. Adding a track to
  // an already recording MediaStream is not a supported way to start audio.
  function refreshAudioTaps() {
    if (!audioMixer || !audioDestination) return;
    for (let i = audioTaps.length - 1; i >= 0; i--) {
      const tap = audioTaps[i];
      if (tap.ac.state !== 'closed') continue;
      releaseAudioTap(tap);
      audioTaps.splice(i, 1);
    }
    for (const ac of findAllAudioCtxs()) {
      if (ac.state === 'closed' || audioTaps.some(tap => tap.ac === ac)) continue;
      ...
      try {
        ...
        source = audioMixer.createMediaStreamSource(bridge.stream);
        source.connect(audioDestination);
        master.connect(bridge);
        audioTaps.push({ ac, master, dest: bridge, source, sink: audioDestination });
        console.log(`[record] attached audio context (sampleRate=${ac.sampleRate})`);
      } catch (e) {
        releaseAudioTap({ master, dest: bridge, source, sink: audioDestination });
        console.warn('[record] audio tap failed:', e);
      }
    }
  }

  function releaseAudioTap(tap) {
    if (tap.dest) {
      try { tap.master.disconnect(tap.dest); } catch (_) {}
      for (const track of tap.dest.stream.getTracks()) { try { track.stop(); } catch (_) {} }
    }
  ...
  function collectAudioTracks() {
    ...
    try {
      audioMixer = new AC();
      audioDestination = audioMixer.createMediaStreamDestination();
      if (audioMixer.state === 'suspended') audioMixer.resume().catch(() => {});
      refreshAudioTaps();
      return audioDestination.stream.getAudioTracks();
    } catch (e) {
      console.warn('[record] audio mixer failed:', e);
      return [];
    }
  }
  ...
  function start(options) {
    ...
    active = true;
    audioTimer = setInterval(refreshAudioTaps, 250);
    startedAt = Date.now();
    updateButton();
  ...
  function cleanup() {
    ...
    outStream = null;
    captureStream = null;
    for (const tap of audioTaps) releaseAudioTap(tap);
    audioTaps = [];
    if (audioMixer) { try { audioMixer.close().catch(() => {}); } catch (_) {} }
    audioMixer = null;
