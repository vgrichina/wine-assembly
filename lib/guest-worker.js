...
function threadSendYield(ex, trapped) {
  ...
  const base = { trapped: trapped || null, yield: y };
  if (y === 10) {
...
function driveThreadSend(ex) {
  ...
  for (let round = 0; round < 64; round++) {
    ...
    if (!ex.get_eip || !(ex.get_eip() >>> 0)) {
      ...
      const frame = threadSendFrames.pop();
      if (frame) restoreThreadSend(ex, frame);
...
function abortThreadSend(ex, restore) {
  ...
  if (restore !== false) restoreThreadSend(ex, frame);
  return true;
}
...
const handleMessage = async (msg) => {
  try {
    ...
    if (msg.t === 'dispatchThreadSend') {
      ...
      threadSendFrames.push(frame);
      const asyncDispatch = ex.thread_send_begin(
        msg.hwnd | 0, msg.msg | 0, msg.wparam | 0, msg.lparam | 0) | 0;
      if (!asyncDispatch) {
        const result = ex.get_eax ? ex.get_eax() | 0 : 0;
        threadSendFrames.pop();
        restoreThreadSend(ex, frame);
