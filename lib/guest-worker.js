...
if (IS_WEB_WORKER) {
  importScripts('gl-command-stream.js?v=5');
  importScripts('d3d-command-stream.js?v=1');
  ...
  try {
    if (msg.t === 'init') {
      ...
      if (IS_WEB_WORKER && msg.d3dRenderWorker && self.D3DCommandStream) {
        d3dCommands = new self.D3DCommandStream.Encoder({
          ...
          workerUrl: 'd3d-render-worker.js?v=1',
    ...
    if (msg.t === 'slice') {
      ...
      send({
        ...
        d3d: d3dCommands ? Object.assign({}, d3dCommands.stats, {
          ready: d3dCommands.ready(),
        }) : null,
      });
      return;
    }
