  ...
  class GuestThreadHost {
    ...
    _driveLocalThreadSend() {
      ...
      for (let round = 0; round < 64; round++) {
        ...
        if (y) return { blocked: true, yield: y };
        if (!(ex.get_eip() >>> 0)) {
          const result = ex.thread_send_end() | 0;
          const frame = this._localSendFrames.pop();
          if (frame) this._restoreLocalSend(ex, frame);
    ...
    async _dispatchLocalThreadSend(args) {
      ...
      this._localSendFrames.push(frame);
      if (!(ex.thread_send_begin(args.hwnd | 0, args.msg | 0,
        args.wparam | 0, args.lparam | 0) | 0)) {
        const result = ex.get_eax() | 0;
        this._localSendFrames.pop();
        this._restoreLocalSend(ex, frame);
        return { done: true, result };
      }
    ...
    async _abortLocalThreadSend(restore = true) {
      const ex = this.localMainExports();
      const frame = this._localSendFrames.pop();
      if (!frame) return { aborted: false };
      ex.thread_send_end();
      if (restore !== false) this._restoreLocalSend(ex, frame);
      return { aborted: true };
    }
    ...
    async _dispatchThreadSendToLink(link, args, depth) {
      ...
      for (let polls = 0; polls < 10000; polls++) {
        ...
        if (r && r.blocked && r.yield !== 0 && r.yield !== 9) {
