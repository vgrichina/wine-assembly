    ...
    _onMessage(msg) {
      switch (msg.t) {
        ...
        case 'rpc':
          ...
          // this returns, so it must not be deferred. The slot says which
          // thread's control block to read — with N guest threads there are N.
          this.broker.serveRpc(msg.slot || 0);
          break;
        case 'call':
