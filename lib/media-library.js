...
// the next visit rather than accumulating. A completed row from an older
// schema is not crash debris: it and its bytes stay intact for the database
// upgrade path to migrate.
//
...
(function () {
  ...
  // The catalog schema may change before the OPFS naming convention does.
  // Keep this deliberately tolerant: an older single-file row has no `parts`,
  // while schema-1 bundles name each opaque OPFS file explicitly. If a future
  // migration encounters an unfamiliar/empty parts shape, protecting row.id is
  // safer than interpreting the row as an orphan and deleting user bytes.
  function storageNamesFor(row) {
    if (!row) return [];
    if (Array.isArray(row.parts)) {
      const names = row.parts.map(part => part && part.storageName).filter(Boolean);
  ...
  class MediaLibrary {
    ...
    async remove(id) {
      ...
      const names = row ? storageNamesFor(row) : [id];
    ...
    async cleanupOrphans() {
      ...
      for (const row of (all || [])) {
        // Schema is migration metadata, not a crash marker. openDb() performs
        // migrations before this sweep; if a completed older row remains, it
        // must survive for a later/repair migration rather than losing both
        // its catalog record and irreplaceable media bytes here.
        if (row.state !== 'complete') {
          await this.remove(row.id).catch(() => {});
          removed.records.push(row.id);
          continue;
        }
        for (const name of storageNamesFor(row)) known.add(name);
      }
