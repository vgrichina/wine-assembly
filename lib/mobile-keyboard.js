...
//
//   manual   the user asked for the keyboard from the on-screen toggle, and it
//            stays up until they ask again. This OVERRIDES the caret, and has
//            to: a fullscreen DirectDraw game draws its own text field with its
//            own blinking bar and never calls CreateCaret, so `hasCaret` is
//            false the whole time Diablo II is asking for a character name.
//            Without the override the 500ms resync blurs the proxy a fraction
//            of a second after the toggle focused it and the keyboard drops
...
function keyboardProxyAction(state) {
  ...
  if (s.manual) return proxyFocused ? 'none' : 'focus';
