...
// pops its stdcall frame. Only ReadFile does that (see $handle_ReadFile and
// $io_block in src/09a-handlers.wat). Every *other* consumer of a lazy entry
// runs to completion in the same turn and therefore needs the bytes now:
//
//     _lread / _hread          src/09a-handlers.wat — continues immediately
//     MapViewOfFile + writeback  this file — takes entry.data directly
...
// or unwinding through nested WASM frames. The same rule covers spawned guest
// threads, which broker host imports to the main thread and cannot park at
// all.
...
class VirtualFS {
  ...
  createFile(path, access, creation) {
    ...
    // and its Data\*.drs).
    if (!exists && (creation === 3 || creation === 4 || creation === 5)) {
  ...
  setProviderFile(path, { provider, offset = 0, length, attrs = 0x20, chunkCache } = {}) {
    ...
    const size = length === undefined
      ? Math.max(0, (cache.size | 0) - (offset | 0))
      : (length | 0);
    const norm = this._normPath(path);
  ...
  // to any consumer other than ReadFile — see the contract note above
  // defineProviderData. Resolves to the materialized Uint8Array.
  materialize(path) {
    const norm = this._resolvePath(path);
    const entry = this.files.get(norm);
    if (!entry) return Promise.reject(new Error(`materialize: no such file ${path}`));
    if (!entry._provider) return Promise.resolve(entry.data);
    const size = entry._size;
    return entry._provider.fill(entry._offset || 0, size).then(() => entry.data);
  }
  ...
  const writeStrA = (guestAddr, str) => {
    ...
    while (offset < total) {
      ...
      for (let i = 0; i < chunk.len; i++) {
        const pos = offset + i;
        chunk.view[i] = pos < str.length ? str.charCodeAt(pos) & 0xFF : 0;
      }
      offset += chunk.len;
    }
    return str.length;
