...
class VirtualFS {
  ...
  createFile(path, access, creation) {
    ...
    const h = (this._nextHandle++) | 0;  // signed i32 to match WASM
  ...
  setCurrentDirectory(path) {
    this.cwd = this._resolvePath(path) + '\\';
 ...
 */
const { readStrA: _readStrA, readStrW: _readStrW, g2w: _g2w } = typeof require !== 'undefined' ? require('./mem-utils') : (typeof window !== 'undefined' && window.memUtils || {});
...
function createFilesystemImports(ctx) {
  ...
  const readStrA = (wasmAddr, maxLen = 260) => _readStrA(ctx.getMemory(), wasmAddr, maxLen);
  const readStrW = (wasmAddr, maxLen = 260) => _readStrW(ctx.getMemory(), wasmAddr, maxLen);
  const readStr = (wasmAddr, isWide) => isWide ? readStrW(wasmAddr) : readStrA(wasmAddr);

  const g2w = (guestAddr) => {
    const exports = ctx.exports;
    return exports ? _g2w(guestAddr, exports.get_image_base()) : guestAddr;
  };
  ...
  return {
    ...
    fs_map_view_of_file: (hMapping, access, offsetHi, offsetLo, size) => {
      ...
      _mappedViews.set(guestAddr, mapSize);
      if (ctx.log) ctx.log(`MapViewOfFile → 0x${guestAddr.toString(16)} (${mapSize} bytes)`);
      return guestAddr;
    },
