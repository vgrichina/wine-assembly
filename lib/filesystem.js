      ...
      if (_traceFs()) {
        ...
        if (!path || path.length < 3) {
          try {
            ...
            // Walk a stdcall-ish frame: the CreateFileA wrapper (0x744a9003) pushes 7 args, then +ret.
            // After the wrapper's own `add esp, 0x14; ret`, the next RA is 20+4=24 bytes up.
            // We don't know the exact frame sizes, so dump raw + mark entries that look like call-ret sites.
            const mem = new Uint8Array(ctx.getMemory());
            const stackBase = esp - imageBase + 0x12000;
            const rawDump = [];
            for (let i = 0; i < 64; i++) {
              const w = dv.getUint32(stackBase + i*4, true) >>> 0;
              const wOff = w - imageBase + 0x12000;
              let tag = '';
              if (w >= 0x74400000 && w < 0x76000000 && wOff >= 5 && wOff < mem.length) {
                const b5 = mem[wOff - 5], b2 = mem[wOff - 2], b6 = mem[wOff - 6];
                if (b5 === 0xE8) tag = '*R';   // preceded by E8 rel32 call
                else if (b2 === 0xFF || b6 === 0xFF) tag = '*i'; // indirect call
              }
              if (tag) rawDump.push(`+${(i*4).toString(16)}:0x${w.toString(16)}${tag}`);
            }
            if (rawDump.length) caller += ` frame=[${rawDump.slice(0, 16).join(',')}]`;
          } catch (_) {}
        }
