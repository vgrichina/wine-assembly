...
function createFilesystemImports(ctx) {
  ...
  const MAP_ALLOC_BASE = 0x02462200; // after DLL table
  ...
  return {
    ...
    fs_map_view_of_file: (hMapping, access, offsetHi, offsetLo, size) => {
      ...
      // Allocate from the file mapping bump region (avoids tiny heap limit)
      const wasmAddr = mapAlloc(mapSize);
      if (!wasmAddr) return 0;
      const guestAddr = wasmAddr + w2g(); // WASM → guest address
