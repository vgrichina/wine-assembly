    ...
    const collect = (searchDir) => {
      ...
      for (const [path, entry] of this.files) {
        if (this._parentDir(path) === searchDir && regex.test(this._fileName(path))) {
          found.push({ name: this._fileName(path), size: entrySize(entry), attrs: entry.attrs || 0x80 });
        }
      }
    ...
    if (results.length === 0 && !pat.includes('*') && !pat.includes('?') &&
      ...
      for (const [path, entry] of this.files) {
        if (this._fileName(path) === pat) {
          results.push({ name: pat, size: entrySize(entry), attrs: entry.attrs || 0x80 });
          break;
        }
      }
    }
      ...
      if (view) {
        ...
        if (mapping) {
          ...
          if (entry) {
            ...
            // Same split as the map side: a sparse-backed view is not one
            // contiguous run in linear memory.
            let synced = 0;
            while (synced < toSync) {
              const chunk = guestChunk(baseAddr + synced, toSync - synced);
              entry.data.set(chunk.view, view.offset + synced);
              synced += chunk.len;
            }
          }
        }
