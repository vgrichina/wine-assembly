...
function createFilesystemImports(ctx) {
  ...
  const _patchRichEditRtfSentinelSize = (handle, data, nToWrite) => {
    ...
    let changed = false;
    const out = new Uint8Array(nToWrite);
    let dst = 0;

    for (let i = 0; i < src.length;) {
      if (_matchesBytes(src, i, upSentinel)) {
        dst = _writeAscii(out, dst, '\\up0');
        i += upSentinel.length;
        changed = true;
        continue;
      }
      if (_matchesBytes(src, i, fsSentinel)) {
        dst = _writeAscii(out, dst, '\\fs' + halfPoints);
        i += fsSentinel.length;
        changed = true;
        continue;
      }
  ...
  const MAP_ALLOC_BASE = 0x07392400; // after DLL table + DLL resource table
  ...
  return {
    ...
    fs_write_file: (handle, bufGA, nToWrite, nWrittenGA) => {
      let data = new Uint8Array(ctx.getMemory(), g2w(bufGA), nToWrite);
