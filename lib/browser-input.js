...
(function () {
  ...
  function wireCanvasInput(canvas, renderer, deps) {
    ...
    function wantsRelativeMouse(canvasX, canvasY) {
      if (renderer.wantsRelativeMouse && renderer.wantsRelativeMouse(canvasX, canvasY)) return true;
      // A few legacy games decide to hide/clip the Win32 cursor late in their
      // frame loop. Let their app manifest declare the stable intent rather
      // than making browser capture depend on whether that guest API call won
      // the race with this click. Still require an exclusive presentation so
      // an ordinary window from the same process never captures the desktop.
      if (!renderer._exclusiveTransform && !renderer._exclusivePresentationViewport) return false;
