...
(function () {
  ...
  const TouchCursor = {
    ...
    tick() {
      ...
      const visible = !!shape && (booting || running);
      if (shape && shape !== this._shape) {
        this._shape = shape;
        this._render();
      }
    ...
    _render() {
      ...
      const badgeScale = badge ? Math.max(1, Math.round(scale * 0.6)) : 0;
      const badgeSize = badge ? artSize(badge.art) : { w: 0, h: 0 };
      const badgeOx = badge ? size.w * scale : 0;
      const pad = scale;  // room for the drop shadow

      const w = Math.max(size.w * scale, badgeOx + badgeSize.w * badgeScale) + pad;
      const h = Math.max(size.h * scale, badgeSize.h * badgeScale) + pad;
      this.el.width = w;
      this.el.height = h;
      ...
      drawArt(ctx, shape.art, scale, 0, 0);
      ctx.restore();
      if (badge) drawArt(ctx, badge.art, badgeScale, badgeOx, 0);

      this._hotX = shape.hot[0] * scale;
      this._hotY = shape.hot[1] * scale;
    },
    ...
    setShape(css) {
      const shape = normalizeShape(css);
      if (!shape || shape === this._shape) return;
      this._shape = shape;
      this._render();
      this._place();
    },
