...
(function () {
  ...
  // The touch-control overlay changes what the sprite is FOR. A dpad game is
  // played with the pad, and the arrow left sitting in the middle of the board
  // after the one tap that reached a menu is noise -- there is no pointer in
  // that game to describe. So for an app that declares `touchControls`, the
  // sprite is shown only while a finger is actually on the game surface, and
  // fades out on release. Everything else -- a pointer app, a boot, an app
  // with no overlay -- keeps the sticky sprite exactly as before, because
  // there the hourglass after a tap is the whole point.
  const FADE_MS = 180;
  ...
  const TouchCursor = {
    ...
    install(options) {
      ...
      el.style.cssText = [
        ...
        'opacity:1', 'transition:opacity ' + FADE_MS + 'ms linear',
      ].join(';');
      ...
      window.addEventListener('touchstart', e => {
        this._noteTouchStart(e);
        if (this._suppressed() && !this._canvasTouches.size) return;
      ...
      // No touchend handler on the sprite POSITION, on purpose: it stays where
      // the finger left it, which is the whole point -- the hourglass appears
      // after the tap that started the work. The end handlers below only close
      // the canvas-touch bookkeeping the overlay policy needs.
      const endTouch = e => { this._noteTouchEnd(e); };
      window.addEventListener('touchend', endTouch, touchOpts);
      window.addEventListener('touchcancel', endTouch, touchOpts);
    ...
    // True while a touch-controls overlay is up. Read live, because it comes
    // and goes with the running app.
    _suppressed() {
      if (this.forceSuppressed !== undefined) return !!this.forceSuppressed;
      const tc = typeof window !== 'undefined' ? window.TouchControls : null;
      if (!tc || typeof tc.isVisible !== 'function') return false;
      try { return !!tc.isVisible(); } catch (_) { return false; }
    },

    // A touch that landed on an overlay widget is not a pointer gesture: it is
    // a button press, and drawing a cursor over the dpad describes nothing.
    // Zones count as overlay too -- they swallow the touch, so no click ever
    // reaches the guest there.
    _isOverlayTarget(node) {
      for (let n = node, i = 0; n && i < 8; n = n.parentNode, i++) {
        if (n.id === 'touch-controls') return true;
        const cls = typeof n.className === 'string' ? n.className : '';
        if (/\btc-(btn|dpad|zone|nub|corner|row|key|mode|cross|arrow)\b/.test(cls)) return true;
      }
      return false;
    },

    _isCanvasTarget(node) {
      for (let n = node, i = 0; n && i < 8; n = n.parentNode, i++) {
        if (n === this.canvas) return true;
        if (n.id === 'screen' || n.id === 'screen-present' || n.id === 'screen-wrap') {
          return true;
        }
      }
      return false;
    ...
    _noteTouchStart(e) {
      ...
      for (let i = 0; list && i < list.length; i++) {
        const t = list[i];
        const target = e.target || (t && t.target);
        if (this._isOverlayTarget(target)) continue;
        if (!this._isCanvasTarget(target)) continue;
        this._canvasTouches.add(t.identifier);
      }
      if (this._canvasTouches.size && this._fadeTimer) {
        clearTimeout(this._fadeTimer);
        this._fadeTimer = null;
        if (this.el) this.el.style.opacity = '1';
      }
      if (this._canvasTouches.size) this.tick();
    },
    ...
    _noteTouchEnd(e) {
      ...
      let changed = false;
      for (let i = 0; list && i < list.length; i++) {
        if (this._canvasTouches.delete(list[i].identifier)) changed = true;
      }
      if (!changed || this._canvasTouches.size || !this._suppressed()) return;
      // Fade rather than blink: a tap-and-release is a real gesture and the
      // sprite acknowledging it reads better than one that was never there.
    ...
    tick() {
      ...
      let visible = !!(custom || shape) && (booting || running);
      // A boot still shows its hourglass: there is no guest to ask yet, and
      // that is exactly when a visitor needs telling something is happening.
      if (visible && !booting && this._suppressed()) {
        visible = this._canvasTouches.size > 0 || !!this._fadeTimer;
      }
