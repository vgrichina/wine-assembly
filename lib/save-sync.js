...
(function () {
  ...
  async function request(options, url, init) {
    ...
    if (typeof AbortController === 'undefined') {
      return doFetch(url, Object.assign({ credentials: 'same-origin' }, init));
    }
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), ms);
    try {
      return await doFetch(url, Object.assign(
        { credentials: 'same-origin' }, init, { signal: controller.signal }));
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  async function getUser(options) {
    options = options || {};
    const response = await request(options, `${baseUrl(options.endpoint)}/api/auth/user`,
