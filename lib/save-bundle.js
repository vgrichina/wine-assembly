...
(function () {
  ...
  function collectSaveFiles(vfs, patterns, options) {
    ...
    for (const [rawPath, entry] of vfs.files.entries()) {
      ...
      // Looking at `.data` is not a harmless type probe for a provider-backed
      // entry: it asks the VFS to materialize the entire file and throws
      // VfsPendingError when an async provider is not resident. Such an entry
      // is mounted content, not save state. A guest write drops `_provider`
      // through the entry's data setter, at which point it is eager state and
      // belongs in the bundle normally.
      if (!entry || entry._provider) continue;
      const data = entry.data;
      if (!(data instanceof Uint8Array)) continue;
      if (data.length > maxFileBytes) {
        throw new Error(
          `save bundle: ${path} is ${data.length} bytes, over the ` +
