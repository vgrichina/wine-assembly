...
`;
  ...
  function install(opts) {
    ...
    function bindCdPlayer(value, mount) {
    ...
    async function restoreLibrary() {
      ...
      for (const row of rows) {
        if (isOpticalMedia(row)) bindCdPlayer(row, keptMount(row));
    ...
    async function applyChoice(plan, source, choice) {
      ...
      if (choice.keep && row) bindCdPlayer(row, keptMount(row));
      else bindCdPlayer(plan, async (vfs) => plan.mount(vfs));
