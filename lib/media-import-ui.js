...
// line to avoid. A drop while a guest owns the canvas is accepted the same
// way; the import dialog is a page-level overlay, so it does not have to fight
// the guest for input.
//
...
`;
  ...
  const GLYPH = { iso: '💿', zip: '🗜️', exe: '🪟', unknown: '📄' };
  ...
  function install(opts) {
    ...
    function keptEntryFor(row) {
      return {
        ...
        mounts: [async (vfs) => {
          const file = await library.fileFor(row.id);
          const plan = await mediaImport.analyze(file, { name: row.name });
    ...
    async function applyChoice(plan, file, choice) {
      let mediaId = null;
      let row = null;
      ...
      const entry = (choice.keep && row)
        ...
        : registerSession(plan, file, choice, mediaId);
      if (choice.launch) launch(plan.id);
      return plan.id;
    }

    function registerSession(plan, file, choice, mediaId) {
      ...
      apps[plan.id] = entry;
      sessionItems.set(plan.id, { plan, entry, file, iconUrl: choice.iconUrl || null });
    ...
    async function keepSession(appId, onProgress) {
      ...
      if (!persistState) persistState = await libraryModule.MediaLibrary.requestPersist();
      const row = await lib.add(item.file, {
        name: item.plan.name,
        kind: item.plan.kind,
        volumeLabel: item.plan.volumeLabel,
        onProgress,
        extra: {
    ...
    if (dropTarget) {
      ...
      overlay.appendChild(el('div', null, 'Drop to insert'));
