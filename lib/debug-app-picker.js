(function (root, factory) {
  ...
  const MAX_RECENT = 5;
  const OTHER_APP_IDS = ['sndrec32_xp', 'mspaint', 'mspaint_ep', 'winamp'];
  const OTHER_GAME_IDS = [
    'ski32', 'winmine', 'pinball', 'pinball_plus95',
    'liquid_war', 'liquid_war_server',
  ];
  const CATEGORY_DEFS = [
    {
      ...
      sections: [
        ...
        'Win98 Accessories',
        'XP',
        { group: 'Other', label: 'Other Apps', include: OTHER_APP_IDS },
      ],
    },
    {
      label: 'Classic Games',
      sections: [
        ...
        'Plus! 98',
        { group: 'Other', label: 'Other Games', include: OTHER_GAME_IDS },
      ],
    },
    {
      label: '16-bit Games',
  ...
  function categorizeCatalog(catalog) {
    ...
    function buildSection(rawSection) {
      ...
      if (definition.include) {
        const included = new Set(definition.include);
        entries = entries.filter(entry => included.has(entry.value));
      }
