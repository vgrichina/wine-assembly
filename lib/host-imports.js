...
function createHostImports(ctx) {
  ...
  function _excludeChildrenClip(t, hwnd) {
    ...
    // rect subtraction. Needed because evenodd breaks at overlap centers
    // (logged above; real apps like mspaint's tool button grid trip it).
    const c = t.ctx;
    c.save();
  ...
  const host = {
    ...
    move_window: (hwnd, x, y, w, h, flags) => {
      ...
      if (flags & 0x40) win.visible = true;
