...
function createHostImports(ctx) {
  ...
  const _flushGdiSurfacePresentation = (presentation, syncForComposite = false) => {
    ...
    try {
      ...
      // Both hosts count it here so the CLI's frame pacing and the browser's
      // are literally the same measurement and can be compared directly.
      _noteGuestFrame(dirty);
