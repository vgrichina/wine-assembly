...
function createHostImports(ctx) {
  ...
  // ---- Audio master node ------------------------------------------------
  // Single GainNode per AudioContext that all voices/synths/sfx connect to.
  // Routing through a master lets recorder.js tap a MediaStreamDestination
  // alongside ac.destination without having to track every source node.
    ...
    _voices = ctx._voices = {
      ...
      open(rate, channels, bits) {
        ...
        if (ac) {
          ...
          const _master = _getAudioMaster(ac);
      ...
      writeStream(id, ptr, len) {
        ...
        try {
          ...
          try {
            ...
            src.buffer = buf;
            src.connect(v.gain || _getAudioMaster(ac));
  ...
  const _ensureTinySynth = (ac) => {
    ...
    try {
      ...
      if (synth.setAudioContext) synth.setAudioContext(ac, _getAudioMaster(ac));
  ...
  const _midiSchedule = (dev) => {
    ...
    master.connect(_getAudioMaster(ac));
  ...
  const host = {
    ...
    message_beep: (uType) => {
      try {
        ...
        osc.connect(gain);
        gain.connect(_getAudioMaster(audioCtx));
        osc.start(now);
        osc.stop(now + 0.13);
      } catch (_) {}
    },
    play_sound: (wasmPtr, length) => {
      ...
      try {
        ...
        audioCtx.decodeAudioData(wavData.buffer).then(audioBuffer => {
          ...
          source.connect(_getAudioMaster(audioCtx));
          source.start();
        }).catch(() => {});
      } catch (_) {}
    },
    ...
    wave_in_start: (handle) => {
      ...
      if (!getUserMedia) return 0; // Headless tests feed PCM through wave_in_feed_pcm.
      ...
      Promise.resolve(getUserMedia({ audio: {
        ...
        device.silentGain.gain.value = 0;
        ...
        device.acquirePending = false;
        device.lastError = error && error.message ? error.message : String(error || 'microphone unavailable');
        console.warn('[waveIn] microphone acquisition failed:', device.lastError);
      });
      return 0;
    },
    wave_in_stop: (handle) => {
    ...
    // --- Font support ---
    note_richedit_charformat_size: (yHeightTwips) => {
