    ...
    host.dx_trace = (kind, slot, a, b, c) => {
      switch (kind) {
        ...
        case 5: { // dx_present: slot=surface, a=bpp, b=dib_wa, c=primary_pal_wa
          ...
          console.log(`[dx] Present slot=${slot} bpp=${a} dib=${hex(b)} firstNonZero=${where} pal=${hex(c)} first4=${palFirst}`);
          break;
        }
        case 6: // Flip: slot=front, a=back_slot, b=new_front_dib, c=old_front_dib
          ...
          break;
        case 8: // BltPtr: slot=dst_dib, a=src_dib, b=dst_pitch, c=src_pitch
          console.log(`[dx] BltPtr dstDib=${hex(slot)} srcDib=${hex(a)} dstPitch=${b} srcPitch=${c}`);
          break;
        case 7: { // BltRect: slot=dst, a=dx<<16|dy, b=dw<<16|dh, c=sw<<16|sh
          const i16 = v => (v << 16) >> 16;
          console.log(`[dx] BltRect dst=${slot} d=(${i16(a>>16)},${i16(a&0xFFFF)}) dsz=(${i16(b>>16)},${i16(b&0xFFFF)}) ssz=(${i16(c>>16)},${i16(c&0xFFFF)})`);
          break;
        }
        default:
