  ...
  const _presentDxSurfaceToMainWindow = (surface) => {
    ...
    if (ctx.renderer && ctx.renderer.getWindowCanvas) {
      ...
      if (wc && wc.ctx) {
        ...
        // A windowed (non-exclusive) DirectDraw app owns only its client area:
        // it Blts to the primary in screen coordinates through the clipper it
        // attached with SetHWnd, and the caption, border and menu bar next to
        // that client area belong to us. Presenting the whole primary over the
        // window erased all of it -- dx_tunnel and dx_twist lost their chrome
        // to a 640x480 frame layer. Keep only the part of the primary that
        // lies under the client rect and leave the rest of the layer
        // transparent so the chrome underneath shows through.
        const exclusiveHwnd = (typeof e.get_dx_exclusive_hwnd === 'function')
          ? e.get_dx_exclusive_hwnd() >>> 0 : 0;
        let clientClip = null;
        if (dxLayer && win && exclusiveHwnd !== hwnd &&
            typeof ctx.renderer._computeClientRect === 'function') {
          ctx.renderer._computeClientRect(win);
          const cr = win.clientRect;
          if (cr && cr.w > 0 && cr.h > 0 &&
              (cr.x || cr.y || cr.w !== surface.w || cr.h !== surface.h)) {
            const sx = Math.max(0, Math.min((win.x | 0) + (cr.x | 0), surface.w));
            const sy = Math.max(0, Math.min((win.y | 0) + (cr.y | 0), surface.h));
            const cw = Math.max(0, Math.min(cr.w | 0, surface.w - sx));
            const ch = Math.max(0, Math.min(cr.h | 0, surface.h - sy));
            if (cw > 0 && ch > 0) clientClip = { sx, sy, cw, ch };
          }
        }
        if (clientClip) {
          dxLayer.ctx.clearRect(0, 0, dxLayer.w, dxLayer.h);
          targetCtx.putImageData(img, -(win.x | 0), -(win.y | 0),
        ...
        if (ctx.trace && ctx.trace.has('dx')) {
          ...
          console.log(`[dx] Upload  slot=${surface.slot} ${surface.w}x${surface.h} ` +
            ...
            ` clip=${clientClip ? `${clientClip.sx},${clientClip.sy} ${clientClip.cw}x${clientClip.ch}` : 'none'}`);
        }
        if (win) win.clientPainted = true;
