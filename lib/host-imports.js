...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    gdi_bitblt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, rop) => {
      ...
      if (globalThis.process && globalThis.process.env && globalThis.process.env.BLT_DBG) {
        let nz = 0, cr = 0, cg = 0, cb = 0;
        for (let i = 0; i < srcData.data.length; i += 4) {
          if (srcData.data[i] || srcData.data[i+1] || srcData.data[i+2]) { nz++; cr += srcData.data[i]; cg += srcData.data[i+1]; cb += srcData.data[i+2]; }
        }
        const N = srcData.data.length / 4;
        const srcBH = !isSrcWindow ? _getDC(srcDC).selectedBitmap : 0;
        const srcBmp = srcBH ? _gdiObjects[srcBH] : null;
        const bmpInfo = srcBmp ? `srcBmp=${srcBH.toString(16)} bpp=${srcBmp.bpp||'?'} mono=${!!srcBmp.mono}` : 'srcBmp=?';
        const dstBH = !isDstWindow ? _getDC(dstDC).selectedBitmap : 0;
        const dstBmpS = dstBH ? _gdiObjects[dstBH] : null;
        const dstInfo = dstBmpS ? `dstBmp=${dstBH.toString(16)} bpp=${dstBmpS.bpp||'?'}` : 'dstBmp=?';
        console.log(`[BLT2] rop=${rop.toString(16)} srcDC=${srcDC.toString(16)}(${bmpInfo}) dstDC=${dstDC.toString(16)}(${dstInfo}) src-area: nz=${nz}/${N} avg(${N>0?Math.round(cr/N):0},${N>0?Math.round(cg/N):0},${N>0?Math.round(cb/N):0})`);
      }
