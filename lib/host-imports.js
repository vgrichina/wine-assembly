...
function createHostImports(ctx) {
      ...
      while (state.p < end) {
        ...
        if (status === 0xFF) {
          ...
          if (type === 0x51 && len === 3 && state.p + 3 <= end) {
            tempos.push({ tick, usPerQn: (u8[state.p] << 16) | (u8[state.p + 1] << 8) | u8[state.p + 2] });
          }
          state.p += len;
          continue;
        }
        ...
        if (op === 0x90 && b > 0) events.push({ tick, type: 'on', ch, note: a, vel: b });
        else if (op === 0x80 || (op === 0x90 && b === 0)) events.push({ tick, type: 'off', ch, note: a });
        else if (op === 0xC0) events.push({ tick, type: 'program', ch, program: a });
        else if (op === 0xB0) events.push({ tick, type: 'cc', ch, cc: a, value: b });
      }
      p = end;
    }
  ...
  const _midiSchedule = (dev) => {
    ...
    for (const n of dev.smf.notes) {
      ...
      gain.gain.exponentialRampToValueAtTime(Math.max(0.002, (n.vel / 127) * 0.12), start + 0.01);
  ...
  const _midiOutEnsureMaster = (dev, ac) => {
    ...
    const master = ac.createGain();
    master.gain.value = 0.18 * _midiOutGainValue(dev.volume);
    master.connect(ac.destination);
    dev.master = master;
    return master;
  };
  ...
  const _midiOutNoteOn = (dev, ch, note, vel) => {
    ...
    gain.gain.exponentialRampToValueAtTime(Math.max(0.002, (vel / 127) * 0.16), now + 0.01);
    osc.connect(gain);
    gain.connect(master);
    try {
      osc.start(now);
  ...
  const _mciOpen = (deviceTypeOrWa, elementWa, flags, isWide) => {
    ...
    const element = elementWa ? read(elementWa, 260) : '';
    ...
    midi_out_set_volume: (handle, volume) => {
      ...
      if (!dev) {
        ...
        for (const d of _midiOut.devices.values()) {
          d.volume = volume >>> 0;
          if (d.master && d.master.gain) d.master.gain.value = 0.18 * _midiOutGainValue(d.volume);
        }
        return 0;
      }
      dev.volume = volume >>> 0;
      if (dev.master && dev.master.gain) dev.master.gain.value = 0.18 * _midiOutGainValue(dev.volume);
      return 0;
    },
