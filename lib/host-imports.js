...
function createHostImports(ctx) {
  ...
  const _dibToSurfaceCanvas = (slot) => {
    ...
    if (bpp === 32) {
      ...
      // 8bpp / 4bpp not yet supported — paint opaque black so GDI draws on a
      // known surface. Correct palette handling is follow-up work.
  ...
  const _surfaceCanvasToDib = (slot) => {
    ...
    // 8bpp/4bpp: silent drop — GDI output on palettized surfaces not yet supported.
  };
