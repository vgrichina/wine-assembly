...
function createHostImports(ctx) {
  ...
  function _excludeChildrenClip(t, hwnd) {
    ...
    if (_trace.has('clip')) {
      console.log(`[clip] hwnd=${_hex(hwnd)} walk1(WND_RECORDS children): ${_walk1Raw.length ? _walk1Raw.join(' ') : '(none)'}`);
    }
    ...
    if (me && topWin) {
      ...
      // Descendant test: walk up from candidate; true if we hit hwnd before top.
      ...
      // Only apply descendants-always-clip when hwnd IS the top-level
      // (genuine WS_CLIPCHILDREN semantics for frame→palette protection).
      // For non-toplevel parents (e.g. Tools palette painting its own
      // button glyphs via its own DC), do NOT clip descendants — the
      // parent intentionally draws into its children's screen area.
      const hwndIsTopLevel = (hwnd >>> 0) === (topHwnd >>> 0);
      for (const k of Object.keys(r.windows)) {
        ...
        const desc = hwndIsTopLevel && isDescendantOf(cand);
  ...
  if (trace.has('wave') || trace.has('audio-stats')) {
    ...
    const stats = ctx._waveStats = { open: 0, write: 0, writeBytes: 0, close: 0, lastFmt: null, lastWriteAt: 0 };
    const waveWrap = (name, fn, fmt) => {
      if (!host[name]) return;
      const orig = host[name];
      host[name] = (...args) => {
        const r = orig(...args);
        if (traceWave) console.log(`[wave] ${fmt(args, r)}`);
        return r;
      };
    };
