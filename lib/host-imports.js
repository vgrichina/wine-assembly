  ...
  function _excludeChildrenClip(t, hwnd) {
    ...
    if (me && topWin) {
      ...
      for (const k of Object.keys(r.windows)) {
        ...
        // Descendants of hwnd are always clipped (WS_CLIPCHILDREN — zOrder
        // only sorts siblings; a brought-to-front parent can have higher
        // zOrder than its children but they still occlude its surface).
        // Cousins/siblings are clipped only if their zOrder is above us.
        if (!isDescendantOf(cand) && (cw.zOrder | 0) <= myZ) continue;
