...
function createHostImports(ctx) {
  ...
  const _markDrawTargetDirty = (target, dirtyRect) => {
    if (!target || target.surfaceSlot === undefined) return;
    let rect = dirtyRect;
  ...
  const _firstNonZeroByte = (wa, len) => {
    if (!wa || len <= 0) return -1;
    try {
      const mem = new Uint8Array(ctx.getMemory(), wa, len);
  ...
  const _presentBestDxOffscreen = (force = false) => {
    ...
    const visiblePrimary = primary.find(p => _firstNonZeroByte(p.dibWa, p.bytes) >= 0);
    if (visiblePrimary) return presentIfChanged(visiblePrimary);

    for (const p of primary) {
      const candidate = offscreen.find(s =>
        s.w === p.w &&
        s.h === p.h &&
        s.bpp === p.bpp &&
        _firstNonZeroByte(s.dibWa, s.bytes) >= 0);
      if (candidate) return presentIfChanged(candidate);
    }

    const fallback = offscreen.find(s =>
      s.w >= 320 &&
      s.h >= 200 &&
      _firstNonZeroByte(s.dibWa, s.bytes) >= 0);
    if (fallback) return presentIfChanged(fallback);
  ...
  const _legacyGetDrawTarget = (hdc, hwnd) => {
    ...
    if (bmp && bmp.type === 'bitmap') {
      ...
      if (bmp.dibSection) _syncDibSection(bmp);
      if (bmp.canvas) {
        return { ctx: bmp.canvas.getContext('2d'), ox: (dc.vpOrgX | 0) - (dc.winOrgX | 0), oy: (dc.vpOrgY | 0) - (dc.winOrgY | 0), canvas: bmp.canvas, opaque: !!bmp._opaque };
      }
    }
    return null;
  };
  ...
  const host = {
    ...
    alloc_screen_dc: () => {
      // GetDC(NULL) — paints onto the topmost back-canvas. Resolved at
      // draw time via the same renderer.windows lookup the legacy path
      // does for hwnd=0.
      const rec = Object.assign(_defaultDcState(), {
    ...
    gdi_create_compat_bitmap: (hdc, w, h) => {
      ...
      return _gdiAlloc({ type: 'bitmap', w, h, pixels: _opaqueBlackPixels(w, h), canvas, _opaque: true });
    },
    gdi_create_bitmap: (w, h, bpp, lpBitsWasm) => {
      ...
      if (!lpBitsWasm) {
        const canvas = _createOffscreen(w, h);
        _initOpaqueBlack(canvas, w, h);
        return _gdiAlloc({ type: 'bitmap', w, h, pixels: _opaqueBlackPixels(w, h), canvas, mono: isMono, _opaque: true });
      }
      const pixels = new Uint8Array(w * h * 4);
      const mem = new Uint8Array(ctx.getMemory());
      ...
      return _gdiAlloc({ type: 'bitmap', w, h, pixels, canvas, mono: isMono, _opaque: true });
    },
  ...
  return { host, readStr, gdi: { _gdiObjects, _dcState, _gdiAlloc, _getDC, _getClientOrigin, handleBox: _handleBox, _dcTable, dcHandleBox: _dcHandleBox, pixelCache: _pixelCache, presentBestDxOffscreen: _presentBestDxOffscreen } };
}
