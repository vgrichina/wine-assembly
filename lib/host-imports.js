...
function createHostImports(ctx) {
  ...
  const _getDrawTarget = (hdc, hwnd) => {
    if (_isWindowDC(hdc)) {
      ...
      return { ctx: ctx.renderer.ctx, ox: o.x, oy: o.y, hwnd: h };
    }
  ...
  const host = {
    ...
    erase_background: (hwnd, hbrBackground) => {
      ...
      c.fillStyle = color;
      _withZClip(hwnd, () => c.fillRect(x, y, w, h));
    ...
    gdi_scroll_window: (hwnd, dx, dy) => {
      ...
      _clippedPut(c, imgData, o.x + dx, o.y + dy);
  ...
  // Wrap GDI draw functions with z-order clipping for window DCs
  const _gdiDrawFns = ['gdi_fill_rect', 'gdi_rectangle', 'gdi_ellipse', 'gdi_line_to',
    'gdi_arc', 'gdi_bitblt', 'gdi_stretch_blt', 'gdi_text_out', 'gdi_draw_text',
    'gdi_set_pixel', 'gdi_frame_rect', 'gdi_set_dib_to_device', 'gdi_scroll_window'];
  for (const name of _gdiDrawFns) {
    const orig = host[name];
    if (!orig) continue;
    host[name] = (...args) => {
      const hdc = args[0];
      if (!_isWindowDC(hdc) || !ctx.renderer) return orig(...args);
      const hwnd = _hwndFromDC(hdc);
      return _withZClip(hwnd, () => orig(...args));
    };
  }
