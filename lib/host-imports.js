...
function createHostImports(ctx) {
  ...
  const _midiSchedule = (dev) => {
    if (!dev || !dev.smf || !dev.smf.notes.length) return 0;
    ...
    if (tiny && dev.smf.events && dev.smf.events.length) {
      ...
      dev.timer = setTimeout(() => {
        ...
        if (dev.state === 'playing') dev.state = 'stopped';
        dev.timer = null;
      }, Math.max(1, (dev.smf.duration - firstStart) * 1000 + 100));
      return 0;
    }
    _midiStop(dev);
    dev.state = 'playing';
    ...
    dev.timer = setTimeout(() => {
      ...
      if (dev.state === 'playing') dev.state = 'stopped';
      dev.timer = null;
    }, Math.max(1, dev.smf.duration * 1000 + 100));
    return 0;
  };
