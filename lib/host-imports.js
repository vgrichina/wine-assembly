...
function createHostImports(ctx) {
  ...
  // Canvas text call. Legacy host-only callers retain a minimal fallback.
  ...
  const host = {
    ...
    gdi_text_bind: (token, dcWa, originX, originY, auxWa, clipBandsWa, clipCount) => {
      ...
      if (presentation) {
        const { width, height, surface, canvas } = presentation;
        if (token >= 0x200000 && token < 0x300000 &&
            presentation.paletteWa && presentation.paletteCount > 0) {
          const mem = new Uint8Array(ctx.getMemory());
          const count = Math.min(presentation.paletteCount, 1 << surface.bpp);
          surface.palette = [];
          for (let i = 0; i < count; i++) {
            const p = presentation.paletteWa + i * 4;
            surface.palette.push([mem[p], mem[p + 1], mem[p + 2]]);
          }
  ...
  return { host, readStr, gdi: { _gdiObjects, _dcState, _gdiAlloc, _getDC, _getClientOrigin, handleBox: _handleBox, surfacePresentations: _gdiSurfacePresentations, pixelCache: _pixelCache, presentBestDxOffscreen: _presentBestDxOffscreen } };
}
