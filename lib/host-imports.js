...
function createHostImports(ctx) {
  ...
  function _excludeChildrenClip(t, hwnd) {
    ...
    if (me && topWin) {
      ...
      // Descendant test: walk up from candidate until we hit hwnd or top/root.
      ...
      for (const k of Object.keys(r.windows)) {
        ...
        if ((cw.zOrder | 0) <= myZ) continue;
        if (_resolveTopHwnd(cand) !== topHwnd) continue;
        if (isDescendantOf(cand)) continue;
    ...
    // Kids may overlap. Evenodd on overlapping holes breaks parity — a triple
    // overlap becomes "drawable" again and wipes the palette center. We handle
    // two cases:
    //
    //   CONTAINMENT  — one rect fully inside another. Cheap: drop the inner.
    //                  Safe with evenodd because only one rect remains.
    //                  Most real cases fall here (parent container listed
    //                  alongside its own child buttons; MDI frame + inner view).
    //
    //   PARTIAL OVERLAP — rects cross without either containing the other.
    //                     Evenodd breaks and we'd need full region union
    //                     (rect subtraction / sweep-line merge). We don't do
    //                     that yet — instead we LOG it so we can tell whether
    //                     it happens on real apps, and fall through to evenodd
    //                     (producing the known bug if it fires).
    //
    // Drop fully-contained rects.
    const filtered = [];
    ...
    // Warn on partial overlaps that survived containment-drop. Throttled so
    // a persistent overlap doesn't spam on every draw.
    if (!globalThis._clipPartialWarned) globalThis._clipPartialWarned = new Set();
    for (let i = 0; i < filtered.length; i++) {
      for (let j = i + 1; j < filtered.length; j++) {
        ...
        if (ix < ax && iy < ay) {
          const key = `${a.x},${a.y},${a.w},${a.h}|${b.x},${b.y},${b.w},${b.h}`;
          if (!globalThis._clipPartialWarned.has(key)) {
            globalThis._clipPartialWarned.add(key);
            console.warn(`[clip] partial overlap (full union needed): hwnd=0x${hwnd.toString(16)} A=(${a.x},${a.y})+${a.w}x${a.h} B=(${b.x},${b.y})+${b.w}x${b.h}`);
          }
        }
      }
    }
    ...
    c.beginPath();
    c.rect(-t.ox, -t.oy, cw, ch);
    for (const k of filtered) c.rect(k.x, k.y, k.w, k.h);
  ...
  const host = {
    ...
    gdi_fill_rect: (hdc, left, top, right, bottom, hbrush) => {
     if (globalThis.process && globalThis.process.env && globalThis.process.env.FR_DBG) {
       console.log(`[FR] hdc=0x${hdc.toString(16)} rect=(${left},${top})-(${right},${bottom}) brush=0x${hbrush.toString(16)}`);
     }
     _drawWithClip(hdc, (t) => {
      const c = t.ctx;
    ...
    apply_window_clip: (hdc, hwnd) => {
      ...
      // Our renderer has no per-child surfaces: children draw into the parent's
      // back-canvas, so any parent draw that overlaps a child will wipe it and
      // children won't naturally repaint themselves. Always exclude children —
      // CLIPCHILDREN becomes the default, not the opt-in.
      const clipChildren = true;
    ...
    gdi_load_bitmap: (hInstance, resourceId) => {
      ...
      // LoadBitmap of a 1bpp DIB produces a monochrome DDB: at BitBlt time
      // the two bit-values expand to the destination DC's text/bg colors
      // (mono→color expansion). This lets apps use the same mask bitmap
      // against different surfaces with inverted text/bg to pre-AND masks
      // into color sprites (classic Win98 cartoon-screensaver trick).
      return _gdiAlloc({ type: 'bitmap', w: bmp.w, h: bmp.h, pixels, canvas,
        bpp: bmp.bpp, indices: bmp.indices, paletteBGRA: bmp.paletteBGRA,
        mono: (bmp.bpp === 1), monoFg: bmp.monoFg });
    },
    ...
    gdi_draw_text: (hdc, textPtr, nCount, rectWA, uFormat, isWide) => {
      ...
      const tw = Math.round(t.ctx.measureText(text).width);
      const th = fontHeight;
      ...
      _drawWithClip(hdc, (tt) => {
        ...
        if (bkMode === 2) { // OPAQUE
          const br = bkColor & 0xFF, bg2 = (bkColor >> 8) & 0xFF, bb = (bkColor >> 16) & 0xFF;
          c.fillStyle = `rgb(${br},${bg2},${bb})`;
          c.fillRect(tt.ox + x, tt.oy + y, tw, th);
        }
        c.fillStyle = `rgb(${r},${g},${b})`;
        c.textBaseline = 'top';
        c.fillText(text, tt.ox + x, tt.oy + y);
        if (_accelIdx >= 0) {
          ...
          const ux = tt.ox + x + Math.round(prefixIncl - chWidth);
          ...
          const uy = tt.oy + y + Math.round(glyphBottom) + 1;
    ...
    host.dx_trace = (kind, slot, a, b, c) => {
      switch (kind) {
        ...
        case 8: { // Execute entry: slot=bufPtr(guest), a=instr_off, b=instr_len
          ...
          console.log(`[dx] ExecIn bufGuest=${hex(slot)} instrOff=${a} instrLen=${b} caller=${hex(retAddr)}`);
          break;
        }
