...
function createHostImports(ctx) {
  ...
  const _defaultDcState = () => ({
    penColor: 0x000000, penWidth: 1, brushColor: 0xC0C0C0,
  ...
  function _watWindowStyle(hwnd) {
    const we = ctx.renderer && ctx.renderer.wasm && ctx.renderer.wasm.exports;
    if (!we || !we.wnd_get_style_export) return 0;
    try { return we.wnd_get_style_export(hwnd) >>> 0; } catch (_) { return 0; }
  }

  function _ctrlRect(hwnd) {
    const we = ctx.renderer && ctx.renderer.wasm && ctx.renderer.wasm.exports;
    if (!we || !we.ctrl_get_class || !we.ctrl_get_xy || !we.ctrl_get_wh) return null;
    try {
      if (!we.ctrl_get_class(hwnd)) return null;
  ...
  function _childRect(hwnd) {
    const r = ctx.renderer;
    if (!r) return null;
    const win = r.windows[hwnd];
    if (win) return { x: win.x | 0, y: win.y | 0, w: win.w | 0, h: win.h | 0, isDialog: !!win.isDialog };
    const cr = _ctrlRect(hwnd);
    if (cr) return { ...cr, isDialog: false };
    return null;
  }

  function _parentHwnd(hwnd) {
    const r = ctx.renderer;
    const win = r && r.windows[hwnd];
    if (win) return win.parentHwnd || 0;
    const we = r && r.wasm && r.wasm.exports;
    if (!we || !we.wnd_get_parent) return 0;
    try { return we.wnd_get_parent(hwnd) >>> 0; } catch (_) { return 0; }
  }

  function _absClientOrigin(hwnd) {
    const r = ctx.renderer;
    if (!r) return null;
    let x = 0, y = 0, h = hwnd, guard = 0;
    while (h && guard++ < 32) {
      const win = r.windows[h];
      if (win) {
        if (win.isChild) {
          x += win.x | 0;
          y += win.y | 0;
        ...
        if (win.clientRect) {
          x += (win.clientRect.x - win.x) | 0;
          y += (win.clientRect.y - win.y) | 0;
        }
        return { x, y };
      }
      const cr = _ctrlRect(h);
      if (cr) {
        x += cr.x | 0;
        y += cr.y | 0;
        h = _parentHwnd(h);
  ...
  function _autoClipInfo(hdc, t) {
    const r = ctx.renderer;
    if (!r || !t || !t.canvas) return null;
    const hwnd = _hwndFromDC(hdc);
    if (!hwnd) return null;
    const rects = [];
    const excludes = [];

    const own = r.windows[hwnd]
      ? { x: t.ox | 0, y: t.oy | 0, w: r.windows[hwnd].w | 0, h: r.windows[hwnd].h | 0 }
      : (() => {
          const cr = _ctrlRect(hwnd);
          return cr ? { x: t.ox | 0, y: t.oy | 0, w: cr.w | 0, h: cr.h | 0 } : null;
        })();
    if (own && own.w > 0 && own.h > 0) rects.push(own);

    let cur = hwnd, guard = 0;
    while (cur && guard++ < 32) {
      const parent = _parentHwnd(cur);
      if (!parent) break;
      const parentOrigin = _absClientOrigin(parent);
      const curRect = _childRect(cur);
      if (parentOrigin && curRect) {
        rects.push({
          x: parentOrigin.x + curRect.x,
          y: parentOrigin.y + curRect.y,
          w: curRect.w,
          h: curRect.h,
        });
      }

      const curWin = r.windows[cur];
      if (curWin && curWin.isChild && parentOrigin) {
        const we = r.wasm && r.wasm.exports;
        if (we && we.wnd_next_child_slot && we.wnd_slot_hwnd) {
          let slot = 0, scanGuard = 0;
          while (scanGuard++ < 512) {
            slot = we.wnd_next_child_slot(parent, slot) | 0;
            if (slot < 0) break;
            const sib = we.wnd_slot_hwnd(slot) >>> 0;
            slot++;
            if (!sib || sib === cur) continue;
            const sr = _childRect(sib);
            if (!sr || sr.w <= 0 || sr.h <= 0) continue;
            const swin = r.windows[sib];
            const visible = swin ? !!swin.visible : !!(_watWindowStyle(sib) & 0x10000000);
            if (!visible) continue;
            excludes.push({
              x: parentOrigin.x + sr.x,
              y: parentOrigin.y + sr.y,
              w: sr.w,
              h: sr.h,
            });
          }
    ...
    if (!rects.length && !excludes.length) return null;
    return { rects, excludes };
  }

  function _withAutoClip(hdc, t, drawFn) {
    const info = _autoClipInfo(hdc, t);
    if (!info) {
      drawFn(t);
      return;
    }
    ...
    try {
      for (const r of info.rects) {
        c.beginPath();
        c.rect(r.x, r.y, Math.max(0, r.w), Math.max(0, r.h));
        c.clip();
      }
      if (info.excludes.length) {
        const p = new Path2D();
        p.rect(0, 0, t.canvas.width, t.canvas.height);
        for (const r of info.excludes) p.rect(r.x, r.y, Math.max(0, r.w), Math.max(0, r.h));
        c.clip(p, 'evenodd');
      }
      drawFn(t);
    } finally {
  ...
  function _drawWithClip(hdc, drawFn) {
    ...
    if (rgn.simpleRect) {
      ...
      c.clip();
      try { _withAutoClip(hdc, t, drawFn); } finally { c.restore(); }
      return true;
    }
  ...
  const host = {
    ...
    get_window_rect: (hwnd, rectPtr) => {
      ...
      if (!win) {
        ...
        if (we && we.ctrl_get_wh && we.ctrl_get_class && we.ctrl_get_class(hwnd) !== 0
          ...
          if (pwin) {
            if (typeof ctx.renderer._computeClientRect === 'function') {
              ctx.renderer._computeClientRect(pwin);
            }
    ...
    // ((0,0)=client top-left). The chrome-offset and child-chain walk
    // currently performed inside _legacyGetDrawTarget will be done at
    // alloc time and stored in rec.ox/rec.oy. For now, return a record
    // with kind+hwnd and let resolution recompute on each draw.
    alloc_window_dc: (hwnd, whole) => {
      ...
      // Compute static client/chrome offset for the window-DC case so the
      // record carries the resolved origin. Surface DCs (memory, etc.) get
      // their own kinds; this path is just the per-window back-canvas.
      if (ctx.renderer && !whole) {
        const topHwnd = _resolveTopHwnd(hwnd);
        const topWin = ctx.renderer.windows[topHwnd];
        if (topWin && topWin.clientRect) {
          rec.ox = topWin.clientRect.x - topWin.x;
          rec.oy = topWin.clientRect.y - topWin.y;
        }
        const childWin = ctx.renderer.windows[hwnd];
        if (childWin && childWin.isChild) {
          let cw = childWin;
          while (cw && cw.isChild) {
            rec.ox += cw.x;
  ...
  return { host, readStr, gdi: { _gdiObjects, _dcState, _gdiAlloc, _getDC, _getClientOrigin, handleBox: _handleBox, _dcTable, dcHandleBox: _dcHandleBox } };
}
