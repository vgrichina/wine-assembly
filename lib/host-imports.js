...
function createHostImports(ctx) {
  ...
  const GDI_PALETTE_TABLE = 0x07F0B000;
  const GDI_PALETTE_SELECTED = GDI_PALETTE_TABLE + 0x20;
  const GDI_PALETTE_ENTRIES = GDI_PALETTE_TABLE + 0x40;
  const DIB_PAGE_STATE = 0x07E10000;
  const DIB_BACKING_BASE = 0x1C000000;
  const DIB_BACKING_SIZE = 0x04000000;
  const _traceFmtCtx = () => ({
  ...
  const _gdiObjects = _sharedGdi ? _sharedGdi._gdiObjects : {
    ...
    0x30007: { type: 'bitmap', w: 1, h: 1, pixels: new Uint8Array(4) },
    ...
    0x30015: { type: 'brush', color: 0xFFFFFF, null: true }, // NULL_BRUSH / HOLLOW_BRUSH (5)
    0x30016: { type: 'pen', color: 0xFFFFFF, width: 1 }, // WHITE_PEN (6)
    0x30017: { type: 'pen', color: 0x000000, width: 1 }, // BLACK_PEN (7)
    0x30018: { type: 'pen', color: 0x000000, width: 1, null: true }, // NULL_PEN (8)
  ...
  const _screenCanvasState = _sharedGdi
    ? (_sharedGdi.screenCanvasState || (_sharedGdi.screenCanvasState = {}))
    : {};
  ...
  // --- DC table (Phase B groundwork) ---
  // Replaces the legacy hwnd+0x40000 / hwnd+0xC0000 encoding with explicit
  // records. Handles allocated from 0x300001+ (above the 0x200000 DDraw
  // surface range, below the 0x400001 GDI handle range used by _handleBox).
  // _getDrawTarget checks _dcTable first; legacy decoders still serve any
  // hdc that wasn't allocated via _dcAlloc.
  const _dcTable = _sharedGdi ? _sharedGdi._dcTable : new Map();
  const _releasedDcTable = _sharedGdi
    ? (_sharedGdi._releasedDcTable || (_sharedGdi._releasedDcTable = new Map()))
    : new Map();
  const _windowDcDefaults = _sharedGdi
    ? (_sharedGdi._windowDcDefaults || (_sharedGdi._windowDcDefaults = new Map()))
    : new Map();
  const _dcHandleBox = _sharedGdi ? _sharedGdi.dcHandleBox : { next: 0x300001, free: [] };
  ...
  const _dcAlloc = (rec) => {
    const h = _dcHandleBox.next++;
    _releasedDcTable.delete(h);
    _dcTable.set(h, rec);
    return h;
  };
  const _dcFree = (hdc) => {
    const rec = _dcTable.get(hdc);
    if (!rec) return 0;
    _dcTable.delete(hdc);
    _releasedDcTable.set(hdc, rec);
    while (_releasedDcTable.size > 512) {
      const oldest = _releasedDcTable.keys().next().value;
      _releasedDcTable.delete(oldest);
    }
    return 1;
  };
  // Returns the per-DC GDI state (penColor, brushColor, ...). For _dcTable
  // records, the record itself carries the state. For legacy hdcs, falls
  // back to _dcState via _getDC.
  const _getDcState = (hdc) => {
    const rec = _dcTable.get(hdc);
    if (rec) return rec;
    const released = _releasedDcTable.get(hdc);
    if (released) return released;
    return _getDC(hdc);
  };
  const _clientSizeForHwnd = (hwnd) => {
    const win = ctx.renderer && ctx.renderer.windows && ctx.renderer.windows[hwnd >>> 0];
    const cr = win && win.clientRect;
    return cr ? { w: cr.w | 0, h: cr.h | 0 } : null;
  };
  const _rememberWindowDcDefaults = (hdc) => {
    const rec = _dcTable.get(hdc) || _releasedDcTable.get(hdc);
    if (!rec || rec.kind !== 'window' || !rec.hwnd) return;
    _windowDcDefaults.set(rec.hwnd >>> 0, {
      vpOrgX: rec.vpOrgX | 0, vpOrgY: rec.vpOrgY | 0,
      winOrgX: rec.winOrgX | 0, winOrgY: rec.winOrgY | 0,
      vpExtX: rec.vpExtX || 1, vpExtY: rec.vpExtY || 1,
      winExtX: rec.winExtX || 1, winExtY: rec.winExtY || 1,
      vpTracksClient: !!rec._vpTracksClient,
    });
  };
  const _applyWindowDcDefaults = (rec) => {
    if (!rec || rec.kind !== 'window' || !rec.hwnd) return;
    const d = _windowDcDefaults.get(rec.hwnd >>> 0);
    if (!d) return;
    rec.vpOrgX = d.vpOrgX | 0; rec.vpOrgY = d.vpOrgY | 0;
    rec.winOrgX = d.winOrgX | 0; rec.winOrgY = d.winOrgY | 0;
    rec.vpExtX = d.vpExtX || 1; rec.vpExtY = d.vpExtY || 1;
    rec.winExtX = d.winExtX || 1; rec.winExtY = d.winExtY || 1;
    rec._vpTracksClient = !!d.vpTracksClient;
    if (rec._vpTracksClient) {
      const cs = _clientSizeForHwnd(rec.hwnd);
      if (cs && cs.w > 0 && cs.h > 0) {
        rec.vpExtX = cs.w;
        rec.vpExtY = cs.h;
      }
  ...
  const _brushColorRef = (hbrush, fallback = 0x000000) => {
    if (hbrush === 0) return fallback;
    if (hbrush >= 0x30010 && hbrush <= 0x30015) {
      const stockColors = {
        0x30010: 0xFFFFFF, // WHITE_BRUSH
        0x30011: 0xC0C0C0, // LTGRAY_BRUSH
        0x30012: 0x808080, // GRAY_BRUSH
        0x30013: 0x404040, // DKGRAY_BRUSH
        0x30014: 0x000000, // BLACK_BRUSH
      };
      if (stockColors[hbrush] !== undefined) return stockColors[hbrush];
      return null;
    }
    if (hbrush <= 30) {
      const sysRGB = {
        1: 0xC0C0C0, 2: 0x808000, 3: 0x800000, 4: 0x808080,
        5: 0xC0C0C0, 6: 0xFFFFFF, 7: 0x000000, 14: 0x800000,
        15: 0xFFFFFF, 16: 0xC0C0C0,
      };
      return sysRGB[hbrush] || 0xC0C0C0;
    }
    const obj = _gdiObjects[hbrush];
    return (obj && obj.type === 'brush') ? obj.color : fallback;
  };
  const _colorRefCss = (bc) => `rgb(${bc & 0xFF},${(bc >> 8) & 0xFF},${(bc >> 16) & 0xFF})`;

  const _gdiAlloc = (obj) => { const h = _handleBox.next++; _gdiObjects[h] = obj; return h; };
  const _surfaceForBitmap = (bmp) => {
    if (!bmp || bmp.type !== 'bitmap' || (!bmp.dibSection && !bmp.softwareBitmap) ||
        !bmp.lpBitsWa || !_gdiSurface.GdiSurface) return null;
    const memory = ctx.getMemory();
    if (bmp.surface && bmp.surface.storage && bmp.surface.storage.buffer === memory) return bmp.surface;
    const rowBytes = ((bmp.w * bmp.bpp + 31) >> 5) << 2;
    try {
      bmp.surface = new _gdiSurface.GdiSurface({
        width: bmp.w,
        height: bmp.h,
        bpp: bmp.bpp,
        stride: rowBytes,
        topDown: bmp.bottomUp === false,
        storage: new Uint8Array(memory),
        storageOffset: bmp.lpBitsWa >>> 0,
        palette: bmp.palette,
        format: bmp.bpp === 32 ? 'bgra32' : `dib${bmp.bpp}`,
      });
      return bmp.surface;
    } catch (_) {
      return null;
  ...
  const _surfaceTargetForDc = (hdc) => {
    const dc = _getDcState(hdc);
    const bitmap = dc && dc.selectedBitmap ? _gdiObjects[dc.selectedBitmap] : null;
    const surface = _surfaceForBitmap(bitmap);
    if (!surface) return null;
    return {
      surface,
      bitmap,
      originX: (dc.vpOrgX | 0) - (dc.winOrgX | 0),
      originY: (dc.vpOrgY | 0) - (dc.winOrgY | 0),
      dc,
    };
  };
  const _clipSurfaceRect = (target, x, y, width, height) => {
    let left = x | 0;
    let top = y | 0;
    let right = left + (width | 0);
    let bottom = top + (height | 0);
    const clip = target.dc.clipRgn && target.dc.clipRgn.simpleRect;
    if (clip) {
      left = Math.max(left, clip.l | 0);
      top = Math.max(top, clip.t | 0);
      right = Math.min(right, clip.r | 0);
      bottom = Math.min(bottom, clip.b | 0);
    }
    left += target.originX;
    right += target.originX;
    top += target.originY;
    bottom += target.originY;
    left = Math.max(0, left);
    top = Math.max(0, top);
    right = Math.min(target.surface.width, right);
    bottom = Math.min(target.surface.height, bottom);
    if (right <= left || bottom <= top) return null;
    return { x: left, y: top, w: right - left, h: bottom - top };
  };
  const _presentSolidSurfaceRect = (target, rect) => {
    const bmp = target.bitmap;
    const stored = target.surface.readPixel(rect.x, rect.y);
    if (stored === null) return;
    if (bmp.canvas) {
      const canvasContext = _bitmapCtx2d(bmp);
      canvasContext.fillStyle = _colorRefCss(stored);
      canvasContext.fillRect(rect.x, rect.y, rect.w, rect.h);
      _invalidatePixelCache(bmp.canvas);
    }
    target.surface.takeDirtyRect();
    _updateDibSectionSyncHash(bmp);
  };
  const _fillSurfaceRect = (target, x, y, width, height, color) => {
    const rect = _clipSurfaceRect(target, x, y, width, height);
    if (!rect) return true;
    _syncDibSection(target.bitmap);
    target.surface.fillRect(rect.x, rect.y, rect.w, rect.h, color);
    _presentSolidSurfaceRect(target, rect);
    return true;
  };
  ...
  const _getDC = (hdc) => {
    if (!_dcState[hdc]) _dcState[hdc] = _defaultDcState();
    return _dcState[hdc];
  };
  ...
  const _bitmapCanvasPool = _sharedGdi
    ? (_sharedGdi.bitmapCanvasPool || (_sharedGdi.bitmapCanvasPool = new Map()))
    : new Map();
  const _takeBitmapCanvas = (w, h) => {
    const key = `${w | 0}x${h | 0}`;
    const bucket = _bitmapCanvasPool.get(key);
    const canvas = bucket && bucket.pop();
    if (bucket && bucket.length === 0) _bitmapCanvasPool.delete(key);
    return canvas || _createOffscreen(w, h);
  };
  const _releaseBitmapCanvas = (bmp) => {
    if (!bmp || bmp.type !== 'bitmap' || !bmp.canvas || bmp.dibSection) return;
    const w = bmp.w | 0, h = bmp.h | 0;
    if (w <= 0 || h <= 0 || w * h > 1024 * 1024) return;
    const key = `${w}x${h}`;
    let bucket = _bitmapCanvasPool.get(key);
    if (!bucket) _bitmapCanvasPool.set(key, bucket = []);
    if (bucket.length < 32) bucket.push(bmp.canvas);
  };
  const _bitmapCtx2d = (bmp) => {
    if (!bmp || !bmp.canvas) return null;
    if (!bmp._ctx2d) bmp._ctx2d = bmp.canvas.getContext('2d');
    return bmp._ctx2d;
  };
  const _drawImageNearest = (ctx2d, ...args) => {
    const prev = ctx2d.imageSmoothingEnabled;
    try {
      _setNearestCanvasContext(ctx2d);
      return ctx2d.drawImage(...args);
    } finally {
      ctx2d.imageSmoothingEnabled = prev;
    }
  };
  const _getScreenDcTarget = () => {
    const r = ctx.renderer;
    const w = Math.max(1, (r && r.canvas && r.canvas.width) || 640);
    const h = Math.max(1, (r && r.canvas && r.canvas.height) || 480);
    if (!_screenCanvasState.canvas || _screenCanvasState.w !== w || _screenCanvasState.h !== h) {
      const canvas = _createOffscreen(w, h);
      if (!canvas) return null;
      const c = canvas.getContext('2d');
      const base = (r && r.colors && r.colors.desktop) || '#008080';
      const colors = [base, '#007878', '#008c8c', '#006c6c', '#149494'];
      c.fillStyle = base;
      c.fillRect(0, 0, w, h);
      for (let y = 0; y < h; y += 32) {
        for (let x = 0; x < w; x += 32) {
          c.fillStyle = colors[(((x / 32) | 0) + ((y / 32) | 0)) % colors.length];
          c.fillRect(x, y, 32, 32);
        }
      }
      _screenCanvasState.canvas = canvas;
      _screenCanvasState.ctx = c;
      _screenCanvasState.w = w;
      _screenCanvasState.h = h;
    }
    return {
      ctx: _screenCanvasState.ctx,
      ox: 0,
      oy: 0,
      hwnd: 0,
      canvas: _screenCanvasState.canvas,
      opaque: true,
    };
  };
  const _initOpaqueBlack = (canvas, w, h) => {
    if (!canvas) return;
    const c = canvas.getContext('2d');
    c.fillStyle = '#000000';
    c.fillRect(0, 0, w, h);
  };
  const _opaqueBlackPixels = (w, h) => {
    const pixels = new Uint8Array(w * h * 4);
    for (let i = 3; i < pixels.length; i += 4) pixels[i] = 255;
    return pixels;
  };
  const _makeCommonControlToolbarBitmap = (resourceId) => {
    resourceId = resourceId >>> 0;
    const specs = {
      0: { name: 'std-small', iconW: 16, iconH: 15, count: 15 },
      1: { name: 'std-large', iconW: 24, iconH: 24, count: 15 },
      4: { name: 'view-small', iconW: 16, iconH: 15, count: 12 },
      5: { name: 'view-large', iconW: 24, iconH: 24, count: 12 },
      8: { name: 'hist-small', iconW: 16, iconH: 15, count: 5 },
      9: { name: 'hist-large', iconW: 24, iconH: 24, count: 5 },
    };
    const spec = specs[resourceId];
    if (!spec) return 0;
    const w = spec.iconW * spec.count;
    const h = spec.iconH;
    const canvas = _takeBitmapCanvas(w, h);
    if (!canvas) return 0;
    const c = canvas.getContext('2d');
    c.fillStyle = '#c0c0c0';
    c.fillRect(0, 0, w, h);
    const accents = [
      '#0040c0', '#008040', '#c06000', '#800080', '#008080',
      '#c00000', '#000080', '#808000', '#0060a0', '#a00060',
      '#408000', '#6040c0', '#804000', '#0080c0', '#c00040',
    ];
    for (let i = 0; i < spec.count; i++) {
      const ox = i * spec.iconW;
      const iw = spec.iconW;
      const ih = spec.iconH;
      const scale = Math.max(1, Math.floor(iw / 16));
      const x0 = ox + Math.max(2, 3 * scale);
      const y0 = Math.max(1, 2 * scale);
      const boxW = Math.max(7, iw - 7 * scale);
      const boxH = Math.max(7, ih - 5 * scale);
      const accent = accents[(i + resourceId) % accents.length];

      c.fillStyle = '#ffffff';
      c.fillRect(x0, y0, boxW, boxH);
      c.strokeStyle = '#000000';
      c.strokeRect(x0 + 0.5, y0 + 0.5, boxW - 1, boxH - 1);
      c.fillStyle = accent;
      c.fillRect(x0 + scale, y0 + scale, Math.max(2, boxW - 2 * scale), Math.max(2, 2 * scale));

      c.fillStyle = (i % 3) === 0 ? '#000080' : ((i % 3) === 1 ? '#008000' : '#800000');
      c.fillRect(x0 + 2 * scale, y0 + 4 * scale, Math.max(2, boxW - 4 * scale), scale);
      if (ih >= 15) c.fillRect(x0 + 2 * scale, y0 + 6 * scale, Math.max(2, boxW - 5 * scale), scale);
      if ((i & 1) === 0) {
        c.fillStyle = '#ffff00';
        c.fillRect(ox + iw - 5 * scale, Math.max(1, ih - 6 * scale), 3 * scale, 3 * scale);
        c.strokeStyle = '#000000';
        c.strokeRect(ox + iw - 5 * scale + 0.5, Math.max(1, ih - 6 * scale) + 0.5, 3 * scale - 1, 3 * scale - 1);
      } else {
        c.strokeStyle = '#000000';
        c.beginPath();
        c.moveTo(ox + iw - 4 * scale, Math.max(2, 3 * scale));
        c.lineTo(ox + iw - 2 * scale, Math.max(4, 5 * scale));
        c.lineTo(ox + iw - 5 * scale, Math.max(4, 5 * scale));
        c.stroke();
      }
    }
  ...
  const _surfaceCanvases = new Map(); // slot -> { canvas, w, h, ctx2d, dirty... }
  const SURFACE_DIRTY_RECT_LIMIT = 64;
  ...
  const _clearSurfaceCanvasDirty = (sc) => {
    if (!sc) return;
    sc.dirty = false;
    sc.dirtyFull = false;
    sc.dirtyRects = [];
  ...
  const _markSurfaceSlotDirty = (slot, rect) => {
    ...
    let w = Math.ceil(Number(rect.w) || 0);
    let h = Math.ceil(Number(rect.h) || 0);
    if (x < 0) { w += x; x = 0; }
    if (y < 0) { h += y; y = 0; }
    if (x + w > sc.w) w = sc.w - x;
    if (y + h > sc.h) h = sc.h - y;
    if (w <= 0 || h <= 0) return;
    if (x === 0 && y === 0 && w === sc.w && h === sc.h) {
      sc.dirtyFull = true;
      sc.dirtyRects = [];
      return;
    }
    if (sc.dirtyFull) return;
    sc.dirtyRects.push({ x, y, w, h });
    if (sc.dirtyRects.length > SURFACE_DIRTY_RECT_LIMIT) {
      sc.dirtyFull = true;
      sc.dirtyRects = [];
    }
  };
  ...
  const _markDrawTargetDirty = (target, dirtyRect) => {
    ...
    if (target.surfaceSlot !== undefined) {
      if (rect) {
        _markSurfaceSlotDirty(target.surfaceSlot, {
          x: (target.ox | 0) + (rect.x | 0),
          y: (target.oy | 0) + (rect.y | 0),
          w: rect.w | 0,
          h: rect.h | 0,
        });
      } else {
        _markSurfaceSlotDirty(target.surfaceSlot, null);
      }
    }
    if (target.selectedBitmap) {
      const bmp = _gdiObjects[target.selectedBitmap];
      if (bmp && (bmp.dibSection || bmp.softwareBitmap)) {
        _syncCanvasToSoftwareBitmap(bmp, target, rect);
      }
    }
  };

  const _gdiMapScalar = (value, winOrg, winExt, vpExt) => {
    const wx = winExt || 1;
    const vx = vpExt || 1;
    return Math.round(winOrg + ((value - winOrg) * vx) / wx);
  };
  const _gdiMapPoint = (dc, x, y) => ({
    x: _gdiMapScalar(x, dc.winOrgX | 0, dc.winExtX | 0, dc.vpExtX | 0),
    y: _gdiMapScalar(y, dc.winOrgY | 0, dc.winExtY | 0, dc.vpExtY | 0),
  });
  ...
  const _paletteSignature = (mem, palWa) => {
    const pal = new Uint8Array(mem, palWa, 1024);
    let h = 2166136261 >>> 0;
  ...
  const _palMatcher = (palWa) => {
    const mem = ctx.getMemory();
    const sig = _paletteSignature(mem, palWa);
    let c = _palMatchCache.get(palWa);
    if (!c || c.sig !== sig) {
      c = { sig, map: new Map(), pal: new Uint8Array(mem, palWa, 1024).slice() };
  ...
  const _surfaceCanvasToDib = (slot) => {
    ...
    const mem = ctx.getMemory();
    const dib = new Uint8Array(mem, dibWa, pitch * h);
    const rects = sc.dirtyFull || !sc.dirtyRects.length
      ? [{ x: 0, y: 0, w, h }]
      : sc.dirtyRects;
    const palWa = bpp === 8 ? _getPrimaryPalWa() : 0;
    const matchPal = palWa ? _palMatcher(palWa) : null;
    for (const r of rects) {
      const rx = r.x | 0;
      const ry = r.y | 0;
      const rw = Math.min(r.w | 0, w - rx);
      const rh = Math.min(r.h | 0, h - ry);
      if (rw <= 0 || rh <= 0) continue;
      const img = sc.ctx2d.getImageData(rx, ry, rw, rh);
      const rgba = img.data;
      if (bpp === 32) {
        for (let yy = 0; yy < rh; yy++) {
          const dibRow = (ry + yy) * pitch + rx * 4;
          const rgbaRow = yy * rw * 4;
          for (let xx = 0; xx < rw; xx++) {
            const di = dibRow + xx * 4, si = rgbaRow + xx * 4;
            dib[di]     = rgba[si + 2];
            dib[di + 1] = rgba[si + 1];
            dib[di + 2] = rgba[si];
            dib[di + 3] = 0;
        ...
        for (let yy = 0; yy < rh; yy++) {
          const dibRow = (ry + yy) * pitch + rx * 2;
          const rgbaRow = yy * rw * 4;
          for (let xx = 0; xx < rw; xx++) {
            const di = dibRow + xx * 2, si = rgbaRow + xx * 4;
      ...
      } else if (bpp === 8 && matchPal) {
        for (let yy = 0; yy < rh; yy++) {
          const dibRow = (ry + yy) * pitch + rx;
          const rgbaRow = yy * rw * 4;
          for (let xx = 0; xx < rw; xx++) {
            const si = rgbaRow + xx * 4;
            dib[dibRow + xx] = matchPal(rgba[si], rgba[si + 1], rgba[si + 2]);
          }
        }
      }
      // No palette yet for 8bpp, or 4bpp/1bpp: drop write-back.
    }
    _clearSurfaceCanvasDirty(sc);
  ...
  const _usesOwnWindowSurface = (hwnd) => {
    const r = ctx.renderer;
    const win = r && r.windows[hwnd];
    if (!win) return false;
    if (r && typeof r._usesOwnWindowSurface === 'function') return r._usesOwnWindowSurface(win);
    return !!(win.isChild && win.isDialog);
  };

  const _resolveDrawSurfaceHwnd = (hwnd) => {
    const r = ctx.renderer;
    if (!r) return hwnd;
    const we = ctx.exports || (r.wasm && r.wasm.exports);
    let cur = hwnd >>> 0;
    for (let guard = 0; cur && guard < 64; guard++) {
      const win = r.windows[cur];
      if (win) {
        if (_usesOwnWindowSurface(cur) || !win.isChild) return cur;
        cur = win.parentHwnd >>> 0;
        continue;
      }
      if (!we || !we.wnd_get_parent) break;
      let parent = 0;
      try { parent = we.wnd_get_parent(cur) >>> 0; } catch (_) { parent = 0; }
      if (!parent || parent === cur) break;
      cur = parent;
    }
    return _resolveTopHwnd(hwnd);
  };

  // Get the canvas context + origin for a given DC handle.
  // Dual-path: _dcTable records (Phase B) take precedence; otherwise fall
  // through to the legacy hwnd+0x40000 / hwnd+0xC0000 decoder. Step 1 wires
  // the record path but no caller populates _dcTable yet — the legacy
  // branch still services every live hdc.
  const _getDrawTarget = (hdc, hwnd) => {
    const rec = _dcTable.get(hdc);
    if (rec) return _resolveDcRecord(rec, hdc, hwnd);
    const released = _releasedDcTable.get(hdc);
    if (released) return _resolveDcRecord(released, hdc, hwnd);
    // Some Win98-era MFC games keep a CDC wrapper around and later reach GDI
    // with CDC::m_hDC already NULL. Native GDI would reject the call, but the
    // intended coordinates are still client-relative; without this the host
    // reports success and drops the frame. Route that legacy null DC to the
    // current main window's client surface so direct-draw tick paths remain
    // visible.
    if (hdc === 0 && ctx.renderer) {
      const we = ctx.exports || (ctx.renderer.wasm && ctx.renderer.wasm.exports);
      let main = 0;
      try { main = we && we.get_main_hwnd ? (we.get_main_hwnd() >>> 0) : 0; } catch (_) {}
      if (main) {
        return _resolveDcRecord(Object.assign(_defaultDcState(), {
          kind: 'window', hwnd: main, ox: 0, oy: 0,
        }), hdc, hwnd);
      }
    }
    return _legacyGetDrawTarget(hdc, hwnd);
  };

  // Resolve a DcRecord to { canvas, ctx, ox, oy, hwnd }. Mirrors the legacy
  // path but pulls hwnd/origin from the record, not from hdc arithmetic.
  // Surface and memory kinds reuse the same canvas-lookup helpers.
  const _resolveDcRecord = (rec, hdc, hwndIn) => {
    if (rec.kind === 'surface') {
      const sc = _getSurfaceCanvas(rec.surfaceSlot);
      if (!sc) return null;
      return { ctx: sc.ctx2d, ox: 0, oy: 0, hwnd: 0, canvas: sc.canvas, surfaceSlot: rec.surfaceSlot };
    }
    if (rec.kind === 'mem') {
      const bmpH = rec.selectedBitmap;
      const bmp = bmpH ? _gdiObjects[bmpH] : null;
      if (bmp && bmp.type === 'bitmap') {
        if (!bmp.canvas) bmp.canvas = _createOffscreen(bmp.w || 1, bmp.h || 1);
        if (bmp.dibSection) _syncDibSection(bmp);
        if (bmp.canvas) {
          return { ctx: _bitmapCtx2d(bmp),
                   ox: (rec.vpOrgX | 0) - (rec.winOrgX | 0),
                   oy: (rec.vpOrgY | 0) - (rec.winOrgY | 0),
                   canvas: bmp.canvas,
                   selectedBitmap: bmpH,
                   opaque: !!bmp._opaque };
        }
    ...
    if (rec.kind === 'screen') {
      return _getScreenDcTarget();
    }
    // window / whole / screen — top-level back-canvas by default. Child
    // dialogs keep a separate surface so tab/page content does not erase
    // pixels owned by the parent dialog.
    if (!ctx.renderer) return null;
    const acd = ctx.renderer._activeChildDraw;
    if (acd && rec.hwnd === acd.hwnd) {
      return { ctx: acd.ctx, ox: acd.ox, oy: acd.oy, hwnd: acd.hwnd, canvas: acd.canvas };
    }
    const ownSurface = _usesOwnWindowSurface(rec.hwnd);
    const targetHwnd = ownSurface ? rec.hwnd : _resolveDrawSurfaceHwnd(rec.hwnd);
    const wc = ctx.renderer.getWindowCanvas(targetHwnd);
    if (!wc) return null;
    const win = ctx.renderer.windows[rec.hwnd];
    if (win) win.clientPainted = true;
    ctx.renderer.scheduleRepaint();
    let ox = rec.ox | 0, oy = rec.oy | 0;
    const we = ctx.exports || (ctx.renderer.wasm && ctx.renderer.wasm.exports);
    if (we && we.wnd_window_screen_x && we.wnd_window_screen_y) {
      try {
        if (ownSurface) {
          ox = 0;
          oy = 0;
          if (rec.kind === 'window' && we.wnd_client_screen_x && we.wnd_client_screen_y) {
            ox = (we.wnd_client_screen_x(rec.hwnd) | 0) - (we.wnd_window_screen_x(rec.hwnd) | 0);
            oy = (we.wnd_client_screen_y(rec.hwnd) | 0) - (we.wnd_window_screen_y(rec.hwnd) | 0);
          }
        } else {
          ox = (we.wnd_window_screen_x(rec.hwnd) | 0) - (we.wnd_window_screen_x(targetHwnd) | 0);
          oy = (we.wnd_window_screen_y(rec.hwnd) | 0) - (we.wnd_window_screen_y(targetHwnd) | 0);
          if (rec.kind === 'window' && we.wnd_client_screen_x && we.wnd_client_screen_y) {
            ox = (we.wnd_client_screen_x(rec.hwnd) | 0) - (we.wnd_window_screen_x(targetHwnd) | 0);
            oy = (we.wnd_client_screen_y(rec.hwnd) | 0) - (we.wnd_window_screen_y(targetHwnd) | 0);
          }
        }
      } catch (_) {}
    } else if (ownSurface) {
      ox = 0;
      oy = 0;
      if (rec.kind === 'window' && win && win.clientRect) {
        ox = win.clientRect.x - win.x;
        oy = win.clientRect.y - win.y;
      }
    }
    ox += (rec.vpOrgX | 0) - (rec.winOrgX | 0);
    oy += (rec.vpOrgY | 0) - (rec.winOrgY | 0);
    return { ctx: wc.ctx, ox, oy, hwnd: rec.hwnd, canvas: wc.canvas };
  };

  // Legacy: All window drawing goes into one full-window-sized back canvas
  // per window. GetWindowDC: (0,0)=window top-left. GetDC/BeginPaint:
  // (0,0)=client top-left. Memory DCs draw into their bitmap's canvas at (0,0).
  const _legacyGetDrawTarget = (hdc, hwnd) => {
    if (_isSurfaceDC(hdc)) {
      ...
      return { ctx: sc.ctx2d, ox: 0, oy: 0, hwnd: 0, canvas: sc.canvas, surfaceSlot: sc.info.slot };
    }
    if (_isWindowDC(hdc)) {
      if (!ctx.renderer) return null;
      const acd = ctx.renderer._activeChildDraw;
      ...
      const we = ctx.exports || (ctx.renderer.wasm && ctx.renderer.wasm.exports);
      let h = resolvedHwnd;
      let topHwnd = (we && we.wnd_top_level && h) ? (we.wnd_top_level(h) >>> 0) : _resolveTopHwnd(h);
      ...
      const ownSurface = _usesOwnWindowSurface(h);
      const targetHwnd = ownSurface ? h : _resolveDrawSurfaceHwnd(h);
      const wc = ctx.renderer.getWindowCanvas(targetHwnd);
      ...
      let ox = 0, oy = 0;
      if (we && we.wnd_window_screen_x && we.wnd_window_screen_y && targetHwnd) {
        if (ownSurface) {
          if (_isWholeWindowDC(hdc)) {
            ox = 0;
            oy = 0;
          } else if (we.wnd_client_screen_x && we.wnd_client_screen_y) {
            ox = (we.wnd_client_screen_x(h) | 0) - (we.wnd_window_screen_x(h) | 0);
            oy = (we.wnd_client_screen_y(h) | 0) - (we.wnd_window_screen_y(h) | 0);
          }
        } else {
          const topX = we.wnd_window_screen_x(targetHwnd) | 0;
          const topY = we.wnd_window_screen_y(targetHwnd) | 0;
          if (_isWholeWindowDC(hdc)) {
            ox = (we.wnd_window_screen_x(h) | 0) - topX;
            oy = (we.wnd_window_screen_y(h) | 0) - topY;
          } else if (we.wnd_client_screen_x && we.wnd_client_screen_y) {
            ox = (we.wnd_client_screen_x(h) | 0) - topX;
            oy = (we.wnd_client_screen_y(h) | 0) - topY;
          }
        }
      } else if (!ownSurface && !(_isWholeWindowDC(hdc) && h === targetHwnd)) {
        const topWin = ctx.renderer.windows[targetHwnd];
        if (topWin && topWin.clientRect) {
          ox = topWin.clientRect.x - topWin.x;
          oy = topWin.clientRect.y - topWin.y;
        }
      } else if (ownSurface && !_isWholeWindowDC(hdc)) {
        const childWin = ctx.renderer.windows[h];
        if (childWin && childWin.clientRect) {
          ox = childWin.clientRect.x - childWin.x;
          oy = childWin.clientRect.y - childWin.y;
        }
      }
      const wdc = _getDcState(hdc);
    ...
    if (bmp && bmp.type === 'bitmap') {
      ...
      if (bmp.dibSection) _syncDibSection(bmp);
      if (bmp.canvas) {
        return { ctx: _bitmapCtx2d(bmp), ox: (dc.vpOrgX | 0) - (dc.winOrgX | 0), oy: (dc.vpOrgY | 0) - (dc.winOrgY | 0), canvas: bmp.canvas, selectedBitmap: bmpH, opaque: !!bmp._opaque };
      }
    }
    return null;
  };
  ...
  function _drawWithClip(hdc, drawFn, dirtyRect) {
    ...
    if (!rgn) {
      t.clipActive = false;
      drawFn(t);
      _markDrawTargetDirty(t, dirtyRect);
      return true;
    }
    const c = t.ctx;
    if (rgn.simpleRect) {
    ...
    if (drew) _markDrawTargetDirty(t, dirtyRect);
    return true;
  }
  ...
  const _dibTrackedPageRange = (bmp) => {
    if (!bmp || !bmp.dibSection) return null;
    const start = (bmp.lpBitsWa >>> 0) - DIB_BACKING_BASE;
    if (start < 0 || start >= DIB_BACKING_SIZE) return null;
    const rowBytes = ((bmp.w * bmp.bpp + 31) >> 5) << 2;
    const totalBytes = rowBytes * bmp.h;
    if (totalBytes <= 0 || start + totalBytes > DIB_BACKING_SIZE) return null;
    return {
      first: start >>> 12,
      last: (start + totalBytes - 1) >>> 12,
    };
  };

  // Claim dirty pages before reading their pixels. Host imports run
  // synchronously with the calling guest, and a later guest store marks the
  // page dirty again for the next bitmap read.
  const _claimDibGuestDirty = (bmp) => {
    const range = _dibTrackedPageRange(bmp);
    if (!range) return null;
    const states = new Uint8Array(ctx.getMemory(), DIB_PAGE_STATE, DIB_BACKING_SIZE >>> 12);
    let dirty = !bmp._syncedOnce;
  ...
  // Host GDI writes are copied to guest memory immediately, so those pages
  // are coherent rather than guest-dirty.
  const _markDibGuestClean = (bmp) => {
    const range = _dibTrackedPageRange(bmp);
    if (!range) return false;
    const states = new Uint8Array(ctx.getMemory(), DIB_PAGE_STATE, DIB_BACKING_SIZE >>> 12);
    for (let page = range.first; page <= range.last; page++) {
      if (states[page]) states[page] = 1;
    }
    bmp._syncedOnce = true;
    return true;
  };

  // Rebuild bmp.pixels + bmp.canvas from live DIB memory only after guest
  // writes. Conversion is deliberately whole-bitmap: most software renderers
  // update complete frames, and this keeps the synchronization contract exact.
  const _syncDibSection = (bmp) => {
    ...
    const trackedDirty = _claimDibGuestDirty(bmp);
    if (trackedDirty === false) return;
    // Compatibility fallback for host-created DIBs outside the WAT arena.
    // Emulator-created CreateDIBSection objects always use page tracking.
    if (trackedDirty === null) {
      let hash = 0 | 0;
      const step = totalBytes <= 1024 * 1024 ? 1 : Math.max(1, (totalBytes / 4096) | 0);
  ...
  const _updateDibSectionSyncHash = (bmp) => {
    if (!bmp || !bmp.dibSection || !bmp.lpBitsWa) return;
    // Tracked DIBs were marked clean before the host-to-guest copy, so there
    // is no hash to refresh afterward.
    if (_dibTrackedPageRange(bmp)) {
      bmp._syncedOnce = true;
      return;
    }
    const { w, h, bpp, lpBitsWa } = bmp;
    const mem = new Uint8Array(ctx.getMemory());
  ...
  const _syncCanvasToSoftwareBitmap = (bmp, target, dirtyRect) => {
    if (!bmp || (!bmp.dibSection && !bmp.softwareBitmap) || !bmp.canvas || !bmp.lpBitsWa) return;
    const { w, h, bpp, lpBitsWa, palette } = bmp;
    if (w <= 0 || h <= 0) return;
    const mem = new Uint8Array(ctx.getMemory());
    const rowBytes = ((w * bpp + 31) >> 5) << 2;
    const totalBytes = rowBytes * h;
    if (lpBitsWa + totalBytes > mem.length) return;
    _markDibGuestClean(bmp);
    let rx = 0, ry = 0, rw = w, rh = h;
    if (dirtyRect) {
      rx = (target.ox | 0) + (dirtyRect.x | 0);
      ry = (target.oy | 0) + (dirtyRect.y | 0);
      rw = dirtyRect.w | 0;
      rh = dirtyRect.h | 0;
      if (rx < 0) { rw += rx; rx = 0; }
      if (ry < 0) { rh += ry; ry = 0; }
    ...
    const ctx2d = bmp.canvas.getContext('2d');
    const img = ctx2d.getImageData(rx, ry, rw, rh);
    const rgba = img.data;
    const bottomUp = bmp.bottomUp !== false;
    const matchPal = (() => {
      if (bpp !== 8 || !palette || !palette.length) return null;
      const cache = bmp._paletteMatchCache || (bmp._paletteMatchCache = new Map());
      return (r, g, b) => {
        const key = (r << 16) | (g << 8) | b;
    ...
    for (let yy = 0; yy < rh; yy++) {
      const y = ry + yy;
      const dstY = bottomUp ? (h - 1 - y) : y;
      const rowBase = lpBitsWa + dstY * rowBytes;
      const rgbaRow = yy * rw * 4;
      for (let xx = 0; xx < rw; xx++) {
        const x = rx + xx;
        const si = rgbaRow + xx * 4;
        if (bpp === 32) {
          ...
          mem[rowBase + x] = matchPal(rgba[si], rgba[si + 1], rgba[si + 2]);
        }
      }
    }
    _updateDibSectionSyncHash(bmp);
  ...
  const _putImageDataToTarget = (target, imgData, x, y) => {
    if (!target || !target.ctx) return;
    if (!target.clipActive) {
      target.ctx.putImageData(imgData, x, y);
      return;
    }
    _clippedPut(target.ctx, imgData, x, y);
  };
  const _targetIsDisplayBacked = (target) => !!(
    target && (target.hwnd || target.surfaceSlot !== undefined)
  );
  ...
  const host = {
    ...
    create_window: (hwnd, style, x, y, cx, cy, titlePtr, menuId) => {
      ...
      if (ctx.renderer) ctx.renderer.createWindow(hwnd, style, x, y, cx, cy, title, menuId, ctx.instance || null, ctx.wasmMemory || null);
      return hwnd;
    },
    sys_command: (hwnd, sc) => {
    ...
    erase_background: (hwnd, hbrBackground) => {
      ...
      const hdc = host.alloc_window_dc(hwnd, 0);
      const we = ctx.exports || (ctx.renderer.wasm && ctx.renderer.wasm.exports);
      if (we && we.dc_apply_client_erase_clip) we.dc_apply_client_erase_clip(hdc, hwnd);
      else if (we && we.dc_apply_client_clip) we.dc_apply_client_clip(hdc, hwnd);
      _drawWithClip(hdc, (t) => {
        t.ctx.fillStyle = color;
        t.ctx.fillRect(t.ox, t.oy, w, h);
      });
      host.release_dc(hdc);
    ...
    gdi_create_compat_dc: (hdcSrc) => {
      ...
      _dcState[h] = _defaultDcState();
      return h;
    },
    // --- Phase B DC allocators (no WAT caller yet; wired in step 2) ---
    // Allocate a window-DC record. whole=1 → GetWindowDC semantics
    // ((0,0)=window top-left); whole=0 → GetDC/BeginPaint semantics
    // ((0,0)=client top-left). Geometry is resolved from WAT when drawing;
    // the record stores only persistent DC state and the target HWND.
    alloc_window_dc: (hwnd, whole) => {
      hwnd = hwnd >>> 0;
      if (hwnd === 0x10000) {
        return _dcAlloc(Object.assign(_defaultDcState(), {
          kind: 'screen', hwnd: 0, ox: 0, oy: 0,
        }));
      }
      const rec = Object.assign(_defaultDcState(), {
        kind: whole ? (whole === 2 ? 'nc' : 'whole') : 'window',
        hwnd, ox: 0, oy: 0,
      });
      _applyWindowDcDefaults(rec);
      // Origin resolution is WAT-owned in _resolveDcRecord; the record only
      // remembers the HWND and DC kind.
      if (ctx.renderer && !whole) {
        const we = ctx.exports || (ctx.renderer.wasm && ctx.renderer.wasm.exports);
        if (we && we.wnd_client_screen_x && we.wnd_client_screen_y && we.wnd_window_screen_x && we.wnd_top_level) {
          try {
            const topHwnd = we.wnd_top_level(hwnd) >>> 0;
            rec.ox = (we.wnd_client_screen_x(hwnd) | 0) - (we.wnd_window_screen_x(topHwnd) | 0);
            rec.oy = (we.wnd_client_screen_y(hwnd) | 0) - (we.wnd_window_screen_y(topHwnd) | 0);
          } catch (_) {}
        }
      }
      return _dcAlloc(rec);
    ...
    alloc_screen_dc: () => {
      // GetDC(NULL) / GetDC(GetDesktopWindow()) — expose a stable desktop
      // backing surface for screen-capture style BitBlt callers.
      const rec = Object.assign(_defaultDcState(), {
        kind: 'screen', hwnd: 0, ox: 0, oy: 0,
      });
      return _dcAlloc(rec);
    },
    release_dc: (hdc) => _dcFree(hdc >>> 0),
    gdi_create_compat_bitmap: (hdc, w, h, lpBitsWa) => {
      w = w | 0; h = h | 0;
      if (w <= 0) w = 1;
      if (h <= 0) h = 1;
      const canvas = _takeBitmapCanvas(w, h);
      _initOpaqueBlack(canvas, w, h);
      // Direct host callers predate the WAT allocator. Production
      // CreateCompatibleBitmap always supplies private linear-memory backing.
      if (!lpBitsWa) {
        return _gdiAlloc({ type: 'bitmap', w, h, bpp: 32,
          pixels: _opaqueBlackPixels(w, h), canvas, _opaque: true });
      }
      lpBitsWa >>>= 0;
      const byteLength = w * h * 4;
      if (!Number.isSafeInteger(byteLength) || byteLength <= 0 ||
          lpBitsWa + byteLength > ctx.getMemory().byteLength) {
        _releaseBitmapCanvas({ type: 'bitmap', w, h, canvas });
        return 0;
      }
      return _gdiAlloc({
        type: 'bitmap', w, h, bpp: 32,
        pixels: _opaqueBlackPixels(w, h), canvas,
        softwareBitmap: true,
        lpBitsWa,
        bottomUp: false,
        _opaque: true,
    ...
    gdi_create_bitmap: (w, h, bpp, lpBitsWasm) => {
      ...
      if (!lpBitsWasm) {
        const canvas = _takeBitmapCanvas(w, h);
        _initOpaqueBlack(canvas, w, h);
        return _gdiAlloc({ type: 'bitmap', w, h, bpp, pixels: _opaqueBlackPixels(w, h), canvas, mono: isMono, _opaque: true });
      }
      const pixels = new Uint8Array(w * h * 4);
      const mem = new Uint8Array(ctx.getMemory());
      ...
      return _gdiAlloc({ type: 'bitmap', w, h, bpp, pixels, canvas, mono: isMono, _opaque: true });
    },
    gdi_create_dib_section: (w, h, bpp, lpBitsWa, lpbmiWa) => {
      ...
      const maxColors = bpp <= 8 ? (1 << bpp) : 0;
      const numColors = maxColors ? Math.min(clrUsed || maxColors, maxColors) : 0;
      const palette = [];
    ...
    gdi_create_dib_bitmap: (lpbmiWa, lpbInitWa, fdwInit) => {
      ...
      return _gdiAlloc({ type: 'bitmap', w, h, pixels, canvas, mono: (bpp === 1), _opaque: true });
      },
    ...
    gdi_select_object: (hdc, hObj) => {
      ...
      let prev = 0;
      if (obj) {
        if (obj.type === 'pen') { prev = dc.selectedPen || 0x30017; dc.selectedPen = hObj; dc.penColor = obj.color; dc.penWidth = obj.width || 1; }
        else if (obj.type === 'brush') { prev = dc.selectedBrush || 0x30010; dc.selectedBrush = hObj; dc.brushColor = obj.color; }
    ...
    gdi_get_current_object: (hdc, objType) => {
      ...
      if (objType === 1) return dc.selectedPen || 0x30017;
      if (objType === 2) return dc.selectedBrush || 0x30010;
    ...
    gdi_delete_object: (h) => {
      // GetStockObject returns process-owned objects. Real Win32 keeps them
      // valid when an application passes one to DeleteObject (which MFC does
      // while replacing temporary fonts). Our synthetic stock handles must do
      // the same or later controls/menus silently lose DEFAULT_GUI_FONT and
      // fall back to the canvas monospace default.
      if ((h >= 0x30010 && h <= 0x30022) || h === 0x30001 || h === 0x30002 || h === 0x30007) return 1;
      const obj = _gdiObjects[h];
      if (obj && obj.type === 'bitmap') _releaseBitmapCanvas(obj);
      delete _gdiObjects[h];
      return 1;
    },
    gdi_delete_dc: (hdc) => { _dcFree(hdc >>> 0); delete _dcState[hdc]; delete _gdiObjects[hdc]; return 1; },
    gdi_get_clip_box: (hdc) => {
      const rec = _dcTable.get(hdc);
      if (rec) {
        if (rec.kind === 'mem' && rec.selectedBitmap) {
          const bmp = _gdiObjects[rec.selectedBitmap];
          if (bmp) return (bmp.w & 0xFFFF) | ((bmp.h & 0xFFFF) << 16);
        }
        if (rec.kind === 'screen') {
          const r = ctx.renderer;
          const w = r ? r.canvas.width : 640;
        ...
        if (ctx.renderer && ctx.renderer.windows) {
          const w = ctx.renderer.windows[rec.hwnd];
          if (w) {
            const width = rec.kind === 'whole' ? w.w : ((w.clientRect && w.clientRect.w) || w.w);
            const height = rec.kind === 'whole' ? w.h : ((w.clientRect && w.clientRect.h) || w.h);
      ...
      if (_isWindowDC(hdc)) {
        ...
        if (ctx.renderer && ctx.renderer.windows) {
          ...
          if (w && w.clientRect) return (w.clientRect.w & 0xFFFF) | ((w.clientRect.h & 0xFFFF) << 16);
        }
        return 640 | (480 << 16);
      }
      const dc = _getDcState(hdc);
    ...
    gdi_fill_rect: (hdc, left, top, right, bottom, hbrush) => {
     const dc = _getDcState(hdc);
     const rr = _gdiMapRect(dc, left, top, right, bottom);
     const bc = _brushColorRef(hbrush, 0);
     if (bc === null) return 1;
     const surfaceTarget = _surfaceTargetForDc(hdc);
     if (surfaceTarget && (!dc.clipRgn || dc.clipRgn.simpleRect)) {
      _fillSurfaceRect(surfaceTarget, rr.x, rr.y, rr.w, rr.h, bc);
      return 1;
     }
     _drawWithClip(hdc, (t) => {
      const c = t.ctx;
      const rr = _gdiMapRect(dc, left, top, right, bottom);
      const x = t.ox + rr.x, y = t.oy + rr.y, w = rr.w, h = rr.h;
      c.fillStyle = _colorRefCss(bc);
      if (bc === 0 && ctx.renderer && ctx.renderer.rememberChildExposureSnapshot) {
        ctx.renderer.rememberChildExposureSnapshot(t.hwnd, x, y, w, h);
      }
      c.fillRect(x, y, w, h);
     });
     return 1;
    ...
    gdi_fill_rgn: (hdc, hrgn, hbrush) => {
      ...
      let bc;
      if (hbrush === 0) {
        bc = _getDcState(hdc).brushColor || 0;
      ...
      _markDrawTargetDirty(t, {
        x: rgn.bbox.l,
        y: rgn.bbox.t,
        w: rgn.bbox.r - rgn.bbox.l,
        h: rgn.bbox.b - rgn.bbox.t,
      });
      return 1;
    },
    gdi_frame_rgn: (hdc, hrgn, hbrush, width, height) => {
      const rgn = _gdiObjects[hrgn];
      if (!rgn || rgn.type !== 'region') return 0;
      const t = _getDrawTarget(hdc);
      if (!t) return 1;
      const bc = _brushColorRef(hbrush, _getDcState(hdc).brushColor || 0);
      if (bc === null) return 1;
      const c = t.ctx;
      const lineWidth = Math.max(1, width | 0, height | 0);
      for (const chain of rgn.branches || []) {
        c.save();
        try {
          c.translate(t.ox, t.oy);
          ...
          c.strokeStyle = _colorRefCss(bc);
          c.lineWidth = lineWidth;
          if (rgn.simpleRect) {
            const rr = rgn.simpleRect;
            c.strokeRect(rr.l + lineWidth / 2, rr.t + lineWidth / 2,
              Math.max(0, rr.r - rr.l - lineWidth),
              Math.max(0, rr.b - rr.t - lineWidth));
          } else {
            for (const { path, polarity } of chain) {
              if (polarity > 0) c.stroke(path);
            }
          }
        } finally {
          c.restore();
      ...
      _markDrawTargetDirty(t, {
        x: rgn.bbox.l - lineWidth,
        y: rgn.bbox.t - lineWidth,
        w: rgn.bbox.r - rgn.bbox.l + lineWidth * 2,
        h: rgn.bbox.b - rgn.bbox.t + lineWidth * 2,
      });
      _traceRgn('FrameRgn', '0x'+hrgn.toString(16), 'brush=', '0x'+hbrush.toString(16), 'w=', width, 'h=', height);
      return 1;
    },
    ...
    gdi_draw_focus_rect: (hdc, left, top, right, bottom) => {
      ...
      _drawWithClip(hdc, (t) => {
        const c = t.ctx;
        const rr = _gdiMapRect(dc, left, top, right, bottom);
        const x0 = t.ox + rr.l, y0 = t.oy + rr.t;
        const x1 = t.ox + rr.r - 1, y1 = t.oy + rr.b - 1;
        c.save();
        c.fillStyle = '#000';
    ...
    gdi_rectangle: (hdc, left, top, right, bottom) => {
      ...
      _drawWithClip(hdc, (t) => {
        ...
        const rr = _gdiMapRect(dc, left, top, right, bottom);
        const x = t.ox + rr.x, y = t.oy + rr.y, w = rr.w, h = rr.h;
        const brush = _gdiObjects[dc.selectedBrush];
        if (!brush || !brush.null) {
          const bc = dc.brushColor || 0;
          c.fillStyle = `rgb(${bc & 0xFF},${(bc >> 8) & 0xFF},${(bc >> 16) & 0xFF})`;
          c.fillRect(x, y, w, h);
        }
        const pen = _gdiObjects[dc.selectedPen];
        if (!pen || !pen.null) {
    ...
    gdi_round_rect: (hdc, left, top, right, bottom, ellipseWidth, ellipseHeight) => {
      ...
      _drawWithClip(hdc, (t) => {
        const c = t.ctx;
        const rr = _gdiMapRect(dc, left, top, right, bottom);
        const x = t.ox + rr.x, y = t.oy + rr.y;
        const w = rr.w, h = rr.h;
        if (w <= 0 || h <= 0) return;
        const rx = Math.max(0, Math.min(Math.abs(ellipseWidth || 0) / 2, Math.abs(w) / 2));
        const ry = Math.max(0, Math.min(Math.abs(ellipseHeight || 0) / 2, Math.abs(h) / 2));
        c.beginPath();
        if (!rx || !ry) {
          c.rect(x, y, w, h);
        } else {
    ...
    gdi_ellipse: (hdc, left, top, right, bottom) => {
      ...
      _drawWithClip(hdc, (t) => {
        const c = t.ctx;
        const rr = _gdiMapRect(dc, left, top, right, bottom);
        const cx = t.ox + (rr.l + rr.r) / 2, cy = t.oy + (rr.t + rr.b) / 2;
        const rx = (rr.r - rr.l) / 2, ry = (rr.b - rr.t) / 2;
        c.beginPath();
    ...
    gdi_polygon: (hdc, pointsWA, nCount) => {
      ...
      _drawWithClip(hdc, (t) => {
        const c = t.ctx;
        c.beginPath();
        const first = _gdiMapPoint(dc, dv.getInt32(pointsWA, true), dv.getInt32(pointsWA + 4, true));
        c.moveTo(t.ox + first.x, t.oy + first.y);
        for (let i = 1; i < nCount; i++) {
          const pt = _gdiMapPoint(dc, dv.getInt32(pointsWA + i * 8, true), dv.getInt32(pointsWA + i * 8 + 4, true));
          c.lineTo(t.ox + pt.x, t.oy + pt.y);
        }
        c.closePath();
        const bc = dc.brushColor;
        c.fillStyle = `rgb(${bc & 0xFF},${(bc >> 8) & 0xFF},${(bc >> 16) & 0xFF})`;
    ...
    gdi_poly_bezier: (hdc, pointsWA, nCount, fromCurrent) => {
      nCount |= 0;
      const firstPoint = fromCurrent ? 0 : 1;
      if (nCount < firstPoint + 3 || ((nCount - firstPoint) % 3) !== 0) return 0;
      const dv = new DataView(ctx.getMemory());
      const dc = _getDcState(hdc);
      _drawWithClip(hdc, (t) => {
        const c = t.ctx;
        ...
        let pointIndex = 0;
        if (fromCurrent) {
          const start = _gdiMapPoint(dc, dc.posX | 0, dc.posY | 0);
          c.moveTo(t.ox + start.x, t.oy + start.y);
        } else {
          const start = _gdiMapPoint(dc,
            dv.getInt32(pointsWA, true), dv.getInt32(pointsWA + 4, true));
          c.moveTo(t.ox + start.x, t.oy + start.y);
          pointIndex = 1;
        }
        for (; pointIndex + 2 < nCount; pointIndex += 3) {
          const p1 = _gdiMapPoint(dc,
            dv.getInt32(pointsWA + pointIndex * 8, true),
            dv.getInt32(pointsWA + pointIndex * 8 + 4, true));
          const p2 = _gdiMapPoint(dc,
            dv.getInt32(pointsWA + (pointIndex + 1) * 8, true),
            dv.getInt32(pointsWA + (pointIndex + 1) * 8 + 4, true));
          const p3 = _gdiMapPoint(dc,
            dv.getInt32(pointsWA + (pointIndex + 2) * 8, true),
            dv.getInt32(pointsWA + (pointIndex + 2) * 8 + 4, true));
          c.bezierCurveTo(t.ox + p1.x, t.oy + p1.y,
            t.ox + p2.x, t.oy + p2.y, t.ox + p3.x, t.oy + p3.y);
          if (fromCurrent) {
            dc.posX = dv.getInt32(pointsWA + (pointIndex + 2) * 8, true);
            dc.posY = dv.getInt32(pointsWA + (pointIndex + 2) * 8 + 4, true);
          }
        }
        const pen = _gdiObjects[dc.selectedPen];
        if (!pen || !pen.null) {
          c.strokeStyle = _colorRefCss(dc.penColor || 0);
          c.lineWidth = dc.penWidth || 1;
          c.stroke();
        }
      });
    ...
    gdi_polyline_to: (hdc, pointsWA, nCount) => {
      nCount |= 0;
      ...
      _drawWithClip(hdc, (t) => {
        ...
        for (let i = 0; i < nCount; i++) {
          const gx = dv.getInt32(pointsWA + i * 8, true);
          const gy = dv.getInt32(pointsWA + i * 8 + 4, true);
          const p = _gdiMapPoint(dc, gx, gy);
          c.lineTo(t.ox + p.x, t.oy + p.y);
          dc.posX = gx;
          dc.posY = gy;
        }
        const pen = _gdiObjects[dc.selectedPen];
    ...
    gdi_polyline: (hdc, pointsWA, nCount) => {
      nCount |= 0;
      if (nCount < 2) return nCount === 1 ? 1 : 0;
      const dv = new DataView(ctx.getMemory());
      const dc = _getDcState(hdc);
      _drawWithClip(hdc, (t) => {
        const c = t.ctx;
        const first = _gdiMapPoint(dc,
          dv.getInt32(pointsWA, true), dv.getInt32(pointsWA + 4, true));
        c.beginPath();
        c.moveTo(t.ox + first.x + 0.5, t.oy + first.y + 0.5);
        for (let i = 1; i < nCount; i++) {
          const p = _gdiMapPoint(dc,
            dv.getInt32(pointsWA + i * 8, true), dv.getInt32(pointsWA + i * 8 + 4, true));
          c.lineTo(t.ox + p.x + 0.5, t.oy + p.y + 0.5);
        }
        const pen = _gdiObjects[dc.selectedPen];
        if (!pen || !pen.null) {
          c.strokeStyle = _colorRefCss(dc.penColor || 0);
    ...
    gdi_set_region_bands: (hrgn, rectsWA, count) => {
      const rgn = _gdiObjects[hrgn];
      if (!rgn || rgn.type !== 'region' || count < 0 || count > 208) return 0;
      const dv = new DataView(ctx.getMemory());
      const rects = [];
      for (let i = 0; i < count; i++) {
    ...
    // WAT owns Win32 update regions. These imports remain only as compatibility
    // shims for stale builds; current WAT calls host.invalidate solely to
    // schedule canvas compositing.
    ...
    gdi_line_to: (hdc, x, y) => {
      ...
      _drawWithClip(hdc, (t) => {
        ...
        c.lineWidth = dc.penWidth;
        c.beginPath();
        const from = _gdiMapPoint(dc, dc.posX, dc.posY);
        const to = _gdiMapPoint(dc, x, y);
        c.moveTo(t.ox + from.x + 0.5, t.oy + from.y + 0.5);
        c.lineTo(t.ox + to.x + 0.5, t.oy + to.y + 0.5);
        c.stroke();
      });
      dc.posX = x; dc.posY = y;
      return 1;
    },
    gdi_get_line_descriptor: (hdc, descWa) => {
      const dc = _getDcState(hdc);
      const bmp = dc && dc.selectedBitmap ? _gdiObjects[dc.selectedBitmap] : null;
      const pen = dc && dc.selectedPen ? _gdiObjects[dc.selectedPen] : null;
      const penWidth = pen ? Math.max(1, pen.width | 0) : 1;
      const penStyle = pen ? pen.style | 0 : 0;
      const wideStyleIsSolid = penWidth > 1 &&
        ((penStyle >= 0 && penStyle <= 4) || penStyle === 6);
      const thinStyleIsSupported = penWidth === 1 &&
        ((penStyle >= 0 && penStyle <= 4) || penStyle === 6);
      const wideMappingIsUnit = Math.abs(dc.winExtX || 1) === Math.abs(dc.vpExtX || 1) &&
        Math.abs(dc.winExtY || 1) === Math.abs(dc.vpExtY || 1);
      if (!descWa || !bmp || (!bmp.dibSection && !bmp.softwareBitmap) || !bmp.lpBitsWa ||
          (bmp.bpp !== 24 && bmp.bpp !== 32) || !pen || pen.type !== 'pen' ||
          pen.null || (!thinStyleIsSupported && !wideStyleIsSolid) ||
          (pen.width | 0) < 0 || (pen.width | 0) > 64 ||
          (penWidth > 1 && !wideMappingIsUnit)) return 0;
      const dv = new DataView(ctx.getMemory());
      const stride = ((bmp.w * bmp.bpp + 31) >> 5) << 2;
      // Geometry and pixels are interpreted by WAT. This import only describes
      // the selected browser-backed object state at the API boundary.
      const values = [
        bmp.lpBitsWa >>> 0, bmp.w | 0, bmp.h | 0, stride | 0,
        bmp.bpp | 0, bmp.bottomUp === false ? 1 : 0,
        dc.penColor & 0xFFFFFF, penWidth,
        dc.winOrgX | 0, dc.winOrgY | 0, dc.winExtX || 1, dc.winExtY || 1,
        dc.vpOrgX | 0, dc.vpOrgY | 0, dc.vpExtX || 1, dc.vpExtY || 1,
        wideStyleIsSolid ? 0 : penStyle,
      ];
      for (let i = 0; i < values.length; i++) dv.setInt32(descWa + i * 4, values[i], true);
      return 1;
    },
    gdi_present_dib_rect: (hdc, left, top, right, bottom) => {
      const dc = _getDcState(hdc);
      const bmp = dc && dc.selectedBitmap ? _gdiObjects[dc.selectedBitmap] : null;
      const surface = _surfaceForBitmap(bmp);
      if (!bmp || !surface || !bmp.canvas) return 0;
      left = Math.max(0, left | 0); top = Math.max(0, top | 0);
      right = Math.min(bmp.w, right | 0); bottom = Math.min(bmp.h, bottom | 0);
      if (right <= left || bottom <= top) return 1;
      const rgba = surface.rgbaRect(left, top, right - left, bottom - top);
      const canvasContext = _bitmapCtx2d(bmp);
      const image = canvasContext.createImageData(right - left, bottom - top);
      image.data.set(rgba);
      canvasContext.putImageData(image, left, top);
      bmp.pixels = bmp.pixels || new Uint8Array(bmp.w * bmp.h * 4);
      for (let y = top; y < bottom; y++) {
        const src = (y - top) * (right - left) * 4;
        bmp.pixels.set(rgba.subarray(src, src + (right - left) * 4), (y * bmp.w + left) * 4);
      }
      bmp._opaque = true;
      bmp._syncedOnce = true;
      _markDibGuestClean(bmp);
    ...
    gdi_arc: (hdc, left, top, right, bottom, xStart, yStart, xEnd, yEnd) => {
      ...
      _drawWithClip(hdc, (t) => {
        const c = t.ctx;
        const rr = _gdiMapRect(dc, left, top, right, bottom);
        const ps = _gdiMapPoint(dc, xStart, yStart);
        const pe = _gdiMapPoint(dc, xEnd, yEnd);
        const cx = t.ox + (rr.l + rr.r) / 2, cy = t.oy + (rr.t + rr.b) / 2;
        const rx = (rr.r - rr.l) / 2, ry = (rr.b - rr.t) / 2;
        const startAngle = Math.atan2(ps.y - (rr.t + rr.b) / 2, ps.x - (rr.l + rr.r) / 2);
        const endAngle = Math.atan2(pe.y - (rr.t + rr.b) / 2, pe.x - (rr.l + rr.r) / 2);
        c.beginPath();
    ...
    gdi_bitblt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, rop) => {
      ...
      if (globalThis.process && globalThis.process.env && globalThis.process.env.BLT_DBG) {
        ...
        const limit = parseInt(globalThis.process.env.BLT_DBG_LIMIT || '40', 10);
        const minW = parseInt(globalThis.process.env.BLT_DBG_MIN_W || '0', 10) || 0;
        const minH = parseInt(globalThis.process.env.BLT_DBG_MIN_H || '0', 10) || 0;
        if (w >= minW && bh >= minH && globalThis._bltDbg++ < limit) {
          const sd = srcTarget && srcTarget.canvas ? `${srcTarget.canvas.width}x${srcTarget.canvas.height}+${srcTarget.ox},${srcTarget.oy}` : 'none';
          const dd = dstTarget && dstTarget.canvas ? `${dstTarget.canvas.width}x${dstTarget.canvas.height}+${dstTarget.ox},${dstTarget.oy}` : 'none';
          let pix = '';
          if (globalThis.process.env.BLT_DBG_PIXELS && srcTarget && srcTarget.ctx) {
            try {
              const img = srcTarget.ctx.getImageData((srcTarget.ox | 0) + sx, (srcTarget.oy | 0) + sy, w, bh).data;
              let nonBlack = 0, nonGray = 0;
              for (let i = 0; i < img.length; i += 4) {
                const r = img[i], g = img[i + 1], b = img[i + 2];
                if (r || g || b) nonBlack++;
                if (!(r === g && g === b)) nonGray++;
              }
              pix = ` srcNonBlack=${nonBlack}/${w * bh} srcNonGray=${nonGray}`;
            } catch (_) {}
            try {
              const dc = _getDcState(srcDC);
              const bmp = dc && dc.selectedBitmap ? _gdiObjects[dc.selectedBitmap] : null;
              if (bmp && bmp.dibSection && bmp.lpBitsWa) {
                const mem = new Uint8Array(ctx.getMemory());
                const rowBytes = ((bmp.w * bmp.bpp + 31) >> 5) << 2;
                const total = rowBytes * bmp.h;
                let rawNonZero = 0;
                for (let i = 0; i < total; i++) if (mem[bmp.lpBitsWa + i]) rawNonZero++;
                pix += ` rawNonZero=${rawNonZero}/${total} bpp=${bmp.bpp} pal=${bmp.palette ? bmp.palette.length : 0}`;
              }
            } catch (_) {}
          }
          console.log(`[BLT] dst=${dstDC.toString(16)}(${isDstWindow?'win':'mem'}) src=${srcDC.toString(16)}(${isSrcWindow?'win':'mem'}) rop=${rop.toString(16)} dstT=${dstTarget?'Y':'N'} srcT=${srcTarget?'Y':'N'} dstC=${dd} srcC=${sd} src@${sx},${sy} dst@${dx},${dy} ${w}x${bh}${pix}`);
        }
      }
      ...
      const dirtyRect = () => ({ x: dx, y: dy, w, h: bh });
      ...
      if (rop === WHITENESS || rop === BLACKNESS || rop === PATCOPY || rop === DSTINVERT) {
        ...
        const dc = _getDcState(dstDC);
        const surfaceTarget = _surfaceTargetForDc(dstDC);
        if (rop !== DSTINVERT && surfaceTarget && (!dc.clipRgn || dc.clipRgn.simpleRect)) {
          const color = rop === WHITENESS ? 0xFFFFFF : rop === PATCOPY ? (dc.brushColor || 0) : 0;
          _fillSurfaceRect(surfaceTarget, dx, dy, w, bh, color);
          return 1;
        }
        if (rop === DSTINVERT) {
          ...
          _drawWithClip(dstDC, (t) => {
            _putImageDataToTarget(t, imgData, t.ox + dx, t.oy + dy);
          }, dirtyRect);
        } else {
          let fr = 0, fg = 0, fb = 0;
      ...
      // Native GDI clips the source rectangle too. Canvas getImageData()
      // throws when the requested source region overflows its bitmap, which
      // shows up in cards.dll's selected-card copy path.
      if (srcTarget && srcTarget.canvas) {
        const sw = srcTarget.canvas.width, sh = srcTarget.canvas.height;
        let eX = (srcTarget.ox | 0) + sx, eY = (srcTarget.oy | 0) + sy;
        if (eX < 0) { const d = -eX; sx += d; dx += d; w -= d; eX = 0; }
        if (eY < 0) { const d = -eY; sy += d; dy += d; bh -= d; eY = 0; }
        if (eX + w > sw) w = sw - eX;
        if (eY + bh > sh) bh = sh - eY;
        if (w <= 0 || bh <= 0) return 1;
      }

      // Check if destination is a monochrome bitmap — color→mono conversion needed
      const dstDCState = !isDstWindow ? _getDcState(dstDC) : null;
      ...
      const srcDCState2 = !isSrcWindow ? _getDcState(srcDC) : null;
      ...
      // Old sprite pipelines commonly draw a mask with SRCAND followed by a
      // color image with SRCPAINT. For small color memory bitmaps, canvas
      // compositing is much faster than round-tripping every frame through
      // ImageData and preserves the intended black/white-mask behavior.
      if (!_env.NO_GDI_ROP_COMPOSITE && !isSrcWindow && !isDstWindow &&
          !srcIsMono && !dstIsMono && srcTarget.canvas !== dstTarget.canvas &&
          (rop === SRCAND || rop === SRCPAINT) && w * bh <= 65536) {
        _drawWithClip(dstDC, (t) => {
          const c = t.ctx;
          c.save();
          c.globalCompositeOperation = rop === SRCAND ? 'multiply' : 'lighter';
          c.drawImage(
            srcTarget.canvas,
            srcTarget.ox + sx, srcTarget.oy + sy, w, bh,
            t.ox + dx, t.oy + dy, w, bh
          );
          c.restore();
        }, dirtyRect);
        return 1;
      ...
      // SRCCOPY: plain color copies can stay on the canvas fast path. Mono
      // expansion and color→mono still need pixel access.
      if (rop === SRCCOPY) {
        if (!_env.NO_GDI_FAST_COPY && !srcIsMono && !dstIsMono && srcTarget.opaque &&
            srcTarget.canvas !== dstTarget.canvas &&
            (_targetIsDisplayBacked(srcTarget) || _targetIsDisplayBacked(dstTarget))) {
          _drawWithClip(dstDC, (t) => {
            t.ctx.drawImage(
              srcTarget.canvas,
              srcTarget.ox + sx, srcTarget.oy + sy, w, bh,
        ...
        if (srcIsMono && !dstIsMono) {
          ...
          const srcDCState = _getDcState(srcDC);
      ...
      _drawWithClip(dstDC, (t) => {
        _putImageDataToTarget(t, dstData, t.ox + dx, t.oy + dy);
      }, dirtyRect);
      return 1;
    },
    gdi_transparent_blt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, colorKey) => {
      if (w <= 0 || bh <= 0) return 1;
      const isDstWindow = _isWindowDC(dstDC);
      const srcTarget = _getDrawTarget(srcDC);
      const dstTarget = _getDrawTarget(dstDC);
      ...
      // Match the destination/source clipping rules used by BitBlt so callers
      // can swap this in for toolbar/image-list SRCCOPY tiles without needing
      // separate bounds checks in WAT.
      if (isDstWindow && !_isWholeWindowDC(dstDC) && ctx.renderer) {
        const dstHwnd = _hwndFromDC(dstDC);
        const win = ctx.renderer.windows[dstHwnd];
        if (win && win.clientRect) {
          const cr = win.clientRect;
          if (dx < 0) { sx -= dx; w += dx; dx = 0; }
      ...
      const imgData = srcTarget.ctx.getImageData(srcTarget.ox + sx, srcTarget.oy + sy, w, bh);
      const kr = colorKey & 0xFF;
      const kg = (colorKey >> 8) & 0xFF;
      const kb = (colorKey >> 16) & 0xFF;
      for (let i = 0; i < imgData.data.length; i += 4) {
        if (imgData.data[i] === kr && imgData.data[i + 1] === kg && imgData.data[i + 2] === kb) {
          imgData.data[i + 3] = 0;
        } else {
          imgData.data[i + 3] = 255;
        }
      ...
      const tmp = _createOffscreen(w, bh);
      _drawWithClip(dstDC, (t) => {
        if (tmp) {
          tmp.getContext('2d').putImageData(imgData, 0, 0);
          t.ctx.drawImage(tmp, t.ox + dx, t.oy + dy);
        } else {
          const dstData = t.ctx.getImageData(t.ox + dx, t.oy + dy, w, bh);
          for (let i = 0; i < imgData.data.length; i += 4) {
            if (imgData.data[i + 3]) {
    ...
    gdi_disabled_blt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, colorKey) => {
      if (w <= 0 || bh <= 0) return 1;
      const srcTarget = _getDrawTarget(srcDC);
      const dstTarget = _getDrawTarget(dstDC);
      if (!srcTarget || !dstTarget) return 1;

      const srcX = (srcTarget.ox | 0) + sx;
      const srcY = (srcTarget.oy | 0) + sy;
      const readW = Math.min(w, srcTarget.canvas.width - srcX);
      const readH = Math.min(bh, srcTarget.canvas.height - srcY);
      if (readW <= 0 || readH <= 0) return 1;
      const src = srcTarget.ctx.getImageData(srcX, srcY, readW, readH).data;
      const kr = colorKey & 0xFF;
      const kg = (colorKey >> 8) & 0xFF;
      const kb = (colorKey >> 16) & 0xFF;
      const tmp = _createOffscreen(readW + 1, readH + 1);
      if (!tmp) return 1;
      const tc = tmp.getContext('2d');
      const out = tc.createImageData(readW + 1, readH + 1);
      const setMask = (x, y, gray) => {
        const p = (y * (readW + 1) + x) * 4;
        out.data[p] = out.data[p + 1] = out.data[p + 2] = gray;
        out.data[p + 3] = 255;
      };
      // Draw the lower-right highlight first, then the upper-left shadow.
      // This is the classic DrawState DSS_DISABLED embossed mask treatment.
      for (let y = 0; y < readH; y++) {
        for (let x = 0; x < readW; x++) {
          const p = (y * readW + x) * 4;
          if (src[p] === kr && src[p + 1] === kg && src[p + 2] === kb) continue;
          setMask(x + 1, y + 1, 255);
        }
      }
      for (let y = 0; y < readH; y++) {
      ...
      _drawWithClip(dstDC, t => {
        t.ctx.drawImage(tmp, t.ox + dx, t.oy + dy);
      }, { x: dx, y: dy, w: readW + 1, h: readH + 1 });
      return 1;
    },
    gdi_stretch_blt: (dstDC, dx, dy, dw, dh, srcDC, sx, sy, sw, sh, rop) => {
      // Win32 treats empty blits as a successful no-op. Canvas backends throw
      // on zero-sized ImageData, and CARDS.dll emits 0x0 setup blits before
      // its first real draw.
      ...
      const sbSrcDC = !isSrcWindow ? _getDcState(srcDC) : null;
      ...
      // When src and dst sizes match, plain color copies can stay on the
      // canvas fast path. Mono expansion still needs pixel access.
      if (sw === dw && sh === dh) {
        if (rop === SRCCOPY) {
          if (!_env.NO_GDI_FAST_COPY && !sbSrcMono && srcTarget.opaque &&
              srcTarget.canvas !== dstTarget.canvas &&
              (_targetIsDisplayBacked(srcTarget) || _targetIsDisplayBacked(dstTarget))) {
            _drawWithClip(dstDC, (t) => {
              t.ctx.drawImage(
                srcTarget.canvas,
                srcTarget.ox + sx, srcTarget.oy + sy, sw, sh,
                t.ox + dx, t.oy + dy, dw, dh
              );
            });
            return 1;
          }
      ...
      if (!tmp) return 1;
      const tc = tmp.getContext('2d');
      _drawImageNearest(tc, srcTarget.canvas, srcTarget.ox + sx, srcTarget.oy + sy, sw, sh, 0, 0, dw, dh);
    ...
    gdi_scroll_window: (hwnd, dx, dy, prcScroll, prcClip) => {
      hwnd >>>= 0; dx |= 0; dy |= 0; prcScroll >>>= 0; prcClip >>>= 0;
      if (!ctx.renderer) return 1;
      const win = ctx.renderer.windows[hwnd];
      if (!win) return 1;
      if (!dx && !dy) return 1;

      const ownSurface = _usesOwnWindowSurface(hwnd);
      const targetHwnd = ownSurface ? hwnd : _resolveDrawSurfaceHwnd(hwnd);
      const wc = ctx.renderer.getWindowCanvas(targetHwnd);
      if (!wc || !wc.canvas || !wc.ctx) return 1;

      const we = ctx.exports || (ctx.renderer.wasm && ctx.renderer.wasm.exports);
      const clientSize = _clientSizeForHwnd(hwnd);
      let clientX = 0, clientY = 0;
      let clientW = (win.isChild ? 0 : (clientSize && clientSize.w)) || (win.w | 0);
      let clientH = (win.isChild ? 0 : (clientSize && clientSize.h)) || (win.h | 0);
      if (we && we.wnd_window_screen_x && we.wnd_window_screen_y) {
        try {
          const baseX = we.wnd_window_screen_x(targetHwnd) | 0;
          const baseY = we.wnd_window_screen_y(targetHwnd) | 0;
          if (we.wnd_client_screen_x && we.wnd_client_screen_y) {
            clientX = (we.wnd_client_screen_x(hwnd) | 0) - baseX;
            clientY = (we.wnd_client_screen_y(hwnd) | 0) - baseY;
          } else {
            clientX = (we.wnd_window_screen_x(hwnd) | 0) - baseX;
            clientY = (we.wnd_window_screen_y(hwnd) | 0) - baseY;
          }
          if (win.isChild && we.wnd_screen_w && we.wnd_screen_h) {
            clientW = we.wnd_screen_w(hwnd) | 0;
            clientH = we.wnd_screen_h(hwnd) | 0;
          }
        } catch (_) {}
      } else if (!ownSurface && win.isChild && win.parentHwnd) {
        const target = ctx.renderer.windows[targetHwnd];
        const targetCr = target && target.clientRect;
        clientX = (targetCr ? targetCr.x - target.x : 0) + (win.x | 0);
        clientY = (targetCr ? targetCr.y - target.y : 0) + (win.y | 0);
      } else if (ownSurface && win.clientRect) {
        clientX = win.clientRect.x - win.x;
        clientY = win.clientRect.y - win.y;
      } else if (win.clientRect) {
        clientX = win.clientRect.x - win.x;
        clientY = win.clientRect.y - win.y;
      }

      const readRect = (ptr) => {
        if (!ptr || !we || !we.get_image_base || !_mu1.g2w || !ctx.getMemory) return null;
        try {
          const memory = ctx.getMemory();
          const wa = _mu1.g2w(ptr >>> 0, we.get_image_base() >>> 0, memory);
          const dv = new DataView(memory && memory.buffer ? memory.buffer : memory);
          if (wa + 16 > dv.byteLength) return null;
          return {
            l: dv.getInt32(wa, true) | 0,
            t: dv.getInt32(wa + 4, true) | 0,
            r: dv.getInt32(wa + 8, true) | 0,
            b: dv.getInt32(wa + 12, true) | 0,
          };
        } catch (_) {
          return null;
        }
      ...
      const scrollRect = readRect(prcScroll) || { l: 0, t: 0, r: clientW, b: clientH };
      let l = Math.max(0, scrollRect.l | 0);
      let t = Math.max(0, scrollRect.t | 0);
      let r = Math.min(clientW | 0, scrollRect.r | 0);
      let b = Math.min(clientH | 0, scrollRect.b | 0);
      const clipRect = readRect(prcClip);
      if (clipRect) {
        l = Math.max(l, clipRect.l | 0);
        t = Math.max(t, clipRect.t | 0);
        r = Math.min(r, clipRect.r | 0);
        b = Math.min(b, clipRect.b | 0);
      }
      if (r <= l || b <= t) return 1;

      let x = clientX + l, y = clientY + t;
      let w = r - l, h = b - t;

      const cw = wc.canvas.width | 0;
      const ch = wc.canvas.height | 0;
      ...
      const srcW = w - Math.abs(dx);
      const srcH = h - Math.abs(dy);
      const c = wc.ctx;
      const snapshot = _createOffscreen(w, h);
      if (!snapshot) return 1;
      snapshot.getContext('2d').drawImage(wc.canvas, x, y, w, h, 0, 0, w, h);
      ...
      if (srcW > 0 && srcH > 0) {
        const sx = dx < 0 ? -dx : 0;
        const sy = dy < 0 ? -dy : 0;
        const tx = x + (dx > 0 ? dx : 0);
        const ty = y + (dy > 0 ? dy : 0);
        c.drawImage(snapshot, sx, sy, srcW, srcH, tx, ty, srcW, srcH);
      }
      c.restore();
      ctx.renderer.scheduleRepaint();
      return 1;
    ...
    gdi_load_bitmap: (hInstance, resourceId) => {
      if ((hInstance | 0) === -1) {
        return _makeCommonControlToolbarBitmap(resourceId);
      }
      ...
      // Win98 Paint resource 859 uses palette bright red as the transparent
      // tool-strip background. MFC normally maps that placeholder to
      // COLOR_BTNFACE while constructing its tool palette. Doing the mapping
      // here keeps later XOR/mask repaints deterministic across canvas
      // backends; dark red remains available for actual icon pixels.
      if ((resourceId >>> 0) === 859 && bmp.bpp === 4) {
      ...
      return _gdiAlloc({ type: 'bitmap', w: bmp.w, h: bmp.h, pixels, canvas,
        ...
        mono: isStdMono, monoFg: bmp.monoFg, _opaque: true });
    },
    ...
    gdi_get_object_bits: (hObj) => {
      const obj = _gdiObjects[hObj];
      if (!obj || obj.type !== 'bitmap' || !obj.dibSection) return 0;
      return obj.lpBitsWa >>> 0;
    },
    gdi_get_object_storage: (hObj) => {
      const obj = _gdiObjects[hObj];
      if (!obj || obj.type !== 'bitmap' || (!obj.dibSection && !obj.softwareBitmap)) return 0;
      return obj.lpBitsWa >>> 0;
    },
    gdi_get_object_bpp: (hObj) => {
      const obj = _gdiObjects[hObj];
      if (!obj || obj.type !== 'bitmap') return 0;
      return (obj.bpp || (obj.mono ? 1 : 32)) >>> 0;
    },
    ...
    gdi_get_text_align: (hdc) => {
      ...
      return dc.textAlign !== undefined ? dc.textAlign : 0;
	    ...
	    gdi_set_viewport_org: (hdc, x, y) => {
	      ...
	      dc.vpOrgX = x | 0;
	      dc.vpOrgY = y | 0;
	      _rememberWindowDcDefaults(hdc);
	      return 0;
	    },
    gdi_get_viewport_org_x: (hdc) => _getDcState(hdc).vpOrgX | 0,
    gdi_get_viewport_org_y: (hdc) => _getDcState(hdc).vpOrgY | 0,
	    gdi_set_viewport_ext: (hdc, x, y) => {
	      const dc = _getDcState(hdc);
	      dc.vpExtX = x | 0;
	      dc.vpExtY = y | 0;
	      const rec = _dcTable.get(hdc) || _releasedDcTable.get(hdc);
	      const cs = rec && rec.hwnd ? _clientSizeForHwnd(rec.hwnd) : null;
	      dc._vpTracksClient = !!(cs && (x | 0) === cs.w && (y | 0) === cs.h);
	      _rememberWindowDcDefaults(hdc);
	      return 1;
	    },
    gdi_get_viewport_ext_x: (hdc) => _getDcState(hdc).vpExtX || 1,
    gdi_get_viewport_ext_y: (hdc) => _getDcState(hdc).vpExtY || 1,
    ...
    gdi_get_window_org_x: (hdc) => _getDcState(hdc).winOrgX | 0,
    gdi_get_window_org_y: (hdc) => _getDcState(hdc).winOrgY | 0,
	    gdi_set_window_ext: (hdc, x, y) => {
	      const dc = _getDcState(hdc);
	      dc.winExtX = x | 0;
	      dc.winExtY = y | 0;
	      _rememberWindowDcDefaults(hdc);
    ...
    gdi_get_window_ext_x: (hdc) => _getDcState(hdc).winExtX || 1,
    gdi_get_window_ext_y: (hdc) => _getDcState(hdc).winExtY || 1,
      ...
      _drawWithClip(hdc, (tt) => {
        ...
        try {
          for (let li = 0; li < lines.length; li++) {
            ...
            c.fillStyle = `rgb(${r},${g},${b})`;
            c.fillText(ln, tt.ox + lx, tt.oy + ly + ascent);
    ...
    gdi_get_pixel: (hdc, x, y) => {
      const surfaceTarget = _surfaceTargetForDc(hdc);
      if (surfaceTarget) {
        const point = _gdiMapPoint(surfaceTarget.dc, x | 0, y | 0);
        const color = surfaceTarget.surface.readPixel(
          surfaceTarget.originX + point.x,
          surfaceTarget.originY + point.y,
        );
      ...
      try {
        let cache = _pixelCache.get(t.canvas);
        if (!cache || cache.w !== t.canvas.width || cache.h !== t.canvas.height) {
          cache = { w: t.canvas.width, h: t.canvas.height, data: t.ctx.getImageData(0, 0, t.canvas.width, t.canvas.height).data };
          _pixelCache.set(t.canvas, cache);
        }
        const off = ((py * cache.w + px) << 2) >>> 0;
    ...
    gdi_set_pixel: (hdc, x, y, color) => {
      const dc = _getDcState(hdc);
      const point = _gdiMapPoint(dc, x | 0, y | 0);
      const r = color & 0xFF, g = (color >> 8) & 0xFF, b = (color >> 16) & 0xFF;
      const surfaceTarget = _surfaceTargetForDc(hdc);
      if (surfaceTarget && !dc.clipRgn) {
        const sx = surfaceTarget.originX + point.x;
        const sy = surfaceTarget.originY + point.y;
        // Preserve other guest writes that may share this DIB/page before the
        // one-pixel host update makes the Canvas presentation current.
        _syncDibSection(surfaceTarget.bitmap);
        const stored = surfaceTarget.surface.writePixel(sx, sy, color);
        if (stored === null) return 0xFFFFFFFF;
        surfaceTarget.bitmap._syncedOnce = true;
        if (surfaceTarget.bitmap.canvas) {
          const [sr, sg, sb] = [stored & 0xFF, (stored >>> 8) & 0xFF, (stored >>> 16) & 0xFF];
          const canvasContext = _bitmapCtx2d(surfaceTarget.bitmap);
          canvasContext.fillStyle = `rgb(${sr},${sg},${sb})`;
          canvasContext.fillRect(sx, sy, 1, 1);
          _invalidatePixelCache(surfaceTarget.bitmap.canvas);
        }
        surfaceTarget.surface.takeDirtyRect();
        _updateDibSectionSyncHash(surfaceTarget.bitmap);
        return stored;
      }
      _drawWithClip(hdc, (t) => {
        t.ctx.fillStyle = `rgb(${r},${g},${b})`;
        t.ctx.fillRect(t.ox + point.x, t.oy + point.y, 1, 1);
      }, { x: point.x, y: point.y, w: 1, h: 1 });
      return color;
    },
    gdi_ext_flood_fill: (hdc, x, y, color, fillType) => {
      const target = _getDrawTarget(hdc, 0);
      if (!target || !target.canvas || !target.ctx) return 0;
      const dc = _getDcState(hdc);
      const point = _gdiMapPoint(dc, x | 0, y | 0);
      const startX = (target.ox | 0) + point.x;
      const startY = (target.oy | 0) + point.y;
      const width = target.canvas.width | 0;
      const height = target.canvas.height | 0;
      if (startX < 0 || startY < 0 || startX >= width || startY >= height) return 0;

      const brush = _gdiObjects[dc.selectedBrush];
      if (brush && brush.null) return 0;
      const fill = dc.brushColor & 0xFFFFFF;
      const fillR = fill & 0xFF;
      const fillG = (fill >> 8) & 0xFF;
      const fillB = (fill >> 16) & 0xFF;
      const matchColor = color & 0xFFFFFF;
      const matchR = matchColor & 0xFF;
      const matchG = (matchColor >> 8) & 0xFF;
      const matchB = (matchColor >> 16) & 0xFF;
      const image = target.ctx.getImageData(0, 0, width, height);
      const pixels = image.data;
      const surfaceMode = (fillType | 0) === 1; // FLOODFILLSURFACE
      const matches = (px, py) => {
        const off = (py * width + px) * 4;
        const equal = pixels[off] === matchR && pixels[off + 1] === matchG && pixels[off + 2] === matchB;
        return surfaceMode ? equal : !equal; // FLOODFILLBORDER fills until color
      };
      if (!matches(startX, startY)) return 0;
      if (surfaceMode && fillR === matchR && fillG === matchG && fillB === matchB) return 1;

      // Scanline flood fill bounds memory to the image plus a small span stack;
      // Paint commonly fills most of a 640x480 DIB, where per-pixel JS objects
      // would otherwise cause a large transient allocation.
      ...
      while (stack.length) {
        ...
        let spanUp = false;
        let spanDown = false;
        for (let px = left; px < width && !visited[sy * width + px] && matches(px, sy); px++) {
          const vi = sy * width + px;
          visited[vi] = 1;
          const off = vi * 4;
          pixels[off] = fillR;
          pixels[off + 1] = fillG;
          ...
          if (sy > maxY) maxY = sy;
          if (sy > 0) {
            const canFillUp = !visited[vi - width] && matches(px, sy - 1);
            if (canFillUp && !spanUp) { stack.push(px, sy - 1); spanUp = true; }
            else if (!canFillUp) spanUp = false;
          }
          if (sy + 1 < height) {
            const canFillDown = !visited[vi + width] && matches(px, sy + 1);
            if (canFillDown && !spanDown) { stack.push(px, sy + 1); spanDown = true; }
            else if (!canFillDown) spanDown = false;
          }
        }
      }
      if (!changed) return 0;

      const dirty = { x: minX - (target.ox | 0), y: minY - (target.oy | 0),
        w: maxX - minX + 1, h: maxY - minY + 1 };
      const snapshot = _createOffscreen(width, height);
      if (!snapshot) return 0;
      snapshot.getContext('2d').putImageData(image, 0, 0);
      _drawWithClip(hdc, (t) => {
        t.ctx.drawImage(snapshot, minX, minY, dirty.w, dirty.h,
          minX, minY, dirty.w, dirty.h);
      }, dirty);
      return 1;
    },
    ...
    gdi_set_dib_to_device: (hdc, xDest, yDest, w, h, xSrc, ySrc, startScan, cLines, bitsWA, bmiWA, colorUse) => {
      ...
      _markDrawTargetDirty(t, { x: xDest, y: yDest, w, h });
      if (_profOn) {
    ...
    gdi_stretch_dib_bits: (hdc, xDst, yDst, wDst, hDst, xSrc, ySrc, wSrc, hSrc, bitsWA, bmiWA, colorUse, rop) => {
      // Perf instrumentation: set `globalThis.PROF_SDB = true` in DevTools
      // (or PROF_SDB=1 env in node). Logs running stats every ~240 calls.
      const _profOn = (typeof globalThis !== 'undefined' && globalThis.PROF_SDB) || _env.PROF_SDB;
      const _t0 = _profOn ? performance.now() : 0;
      ...
      const biBitCount = dv.getUint16(bmiWA + 14, true);
      const biCompression = dv.getUint32(bmiWA + 16, true);
      const biSizeImage = dv.getUint32(bmiWA + 20, true);
      const biSize = dv.getUint32(bmiWA, true);
      const imgW = Math.abs(biWidth), imgH = Math.abs(biHeight);
      const topDown = biHeight < 0;
      ...
      if (biBitCount <= 8) {
        ...
        if (colorUse === 1) {
          ...
          // SelectPalette mirrors the current palette index (0-3) in GDI_PALETTE_TABLE so we can
          // pick the right table here — pinball selects between several palettes per blit.
          let palIdx = dv.getUint32(GDI_PALETTE_SELECTED, true);
          if (palIdx > 3) palIdx = 0;
          const palBase = GDI_PALETTE_ENTRIES + palIdx * 1024;
          for (let i = 0; i < numColors; i++) {
      ...
      // BI_RLE8 / BI_RLE4 store runs rather than DWORD-aligned scanlines.
      // Decode once into top-down palette indices, then let the normal source
      // rectangle and scaling path below consume those indices. In particular,
      // do not derive row addresses from rowBytes for compressed data: that
      // reads beyond biSizeImage and turns RLE command bytes into visual noise.
      let rleIndices = null;
      if ((biCompression === 1 && biBitCount === 8) ||
          (biCompression === 2 && biBitCount === 4)) {
        rleIndices = new Uint8Array(imgW * imgH);
        let p = bitsWA;
        const end = Math.min(mem.length, biSizeImage ? bitsWA + biSizeImage : mem.length);
        let x = 0, y = 0;
        const putIndex = (idx) => {
          const visualY = topDown ? y : (imgH - 1 - y);
          if (x >= 0 && x < imgW && visualY >= 0 && visualY < imgH) {
            rleIndices[visualY * imgW + x] = idx;
          }
          x++;
        };
        while (p + 1 < end && y < imgH) {
          const count = mem[p++];
          const value = mem[p++];
          if (count) {
            if (biCompression === 1) {
              for (let i = 0; i < count; i++) putIndex(value);
            } else {
              const hi = value >> 4, lo = value & 0x0F;
              for (let i = 0; i < count; i++) putIndex((i & 1) ? lo : hi);
            }
            continue;
          }
          if (value === 0) { // end of line
            ...
            x += mem[p++];
            y += mem[p++];
          } else if (biCompression === 1) { // absolute RLE8 run
            const available = Math.min(value, end - p);
            for (let i = 0; i < available; i++) putIndex(mem[p + i]);
            p += available;
            if (value & 1) p++;
          } else { // absolute RLE4 run; payload is WORD-aligned
            const byteCount = (value + 1) >> 1;
            const available = Math.min(byteCount, end - p);
            const nibbles = Math.min(value, available * 2);
            for (let i = 0; i < nibbles; i++) {
              const byte = mem[p + (i >> 1)];
              putIndex((i & 1) ? (byte & 0x0F) : (byte >> 4));
            }
            p += available;
            if (byteCount & 1) p++;
          }
      ...
      const rowBytes = Math.ceil((imgW * biBitCount) / 32) * 4;
      const pixelBytes = sw * sh * 4;
      let pixels = globalThis._sdbPixBuf;
      if (!pixels || pixels.length < pixelBytes) {
        pixels = globalThis._sdbPixBuf = new Uint8Array(pixelBytes);
      }
      pixels.fill(0, 0, pixelBytes);
      for (let row = 0; row < sh; row++) {
        ...
        const srcVisualRow = topDown ? srcRowIdx : (imgH - 1 - srcRowIdx);
        for (let x = 0; x < sw; x++) {
          const sx = xSrc + x;
          ...
          if (biBitCount === 1) {
            ...
            const nibble = rleIndices
              ? rleIndices[srcVisualRow * imgW + sx]
              : ((sx & 1) ? (mem[srcRow + (sx >> 1)] & 0xF) : ((mem[srcRow + (sx >> 1)] >> 4) & 0xF));
            const c = palette[nibble] || 0;
            r = (c >> 16) & 0xFF; g = (c >> 8) & 0xFF; b = c & 0xFF;
          } else if (biBitCount === 8) {
            const idx = rleIndices ? rleIndices[srcVisualRow * imgW + sx] : mem[srcRow + sx];
      ...
      const _tDecode = _profOn ? performance.now() : 0;
      try {
        ...
        const dstW = Math.abs(wDst), dstH = Math.abs(hDst);
        let drew = false;
        const dcForClip = _getDcState(hdc);
        if (!dcForClip.clipRgn && sw === dstW && sh === dstH && typeof t.ctx.putImageData === 'function') {
          const cacheKey = `${sw}x${sh}`;
          const imgMap = globalThis._sdbImageDataCache || (globalThis._sdbImageDataCache = new Map());
          let imgData = imgMap.get(cacheKey);
          if (!imgData) {
            imgData = t.ctx.createImageData(sw, sh);
            imgMap.set(cacheKey, imgData);
          }
          imgData.data.set(pixels.subarray(0, pixelBytes));
          t.ctx.putImageData(imgData, t.ox + xDst, t.oy + yDst);
          drew = true;
        }
        if (!drew) {
          const cacheKey = `${sw}x${sh}`;
          const cacheMap = globalThis._sdbCanvasCache || (globalThis._sdbCanvasCache = new Map());
          ...
          if (!cache) {
            ...
            cache.imgData = cache.ctx.createImageData(sw, sh);
            cacheMap.set(cacheKey, cache);
          }
          const { canvas: tmpCanvas, ctx: tmpCtx, imgData } = cache;
          imgData.data.set(pixels.subarray(0, pixelBytes));
          tmpCtx.putImageData(imgData, 0, 0);
          _drawImageNearest(t.ctx, tmpCanvas, 0, 0, sw, sh, t.ox + xDst, t.oy + yDst, dstW, dstH);
        }
      } catch (e) { if (typeof process !== 'undefined' && process.env && process.env.SDB_DEBUG) console.log('[sdb] err', e.message); }
      _markDrawTargetDirty(t, { x: xDst, y: yDst, w: Math.abs(wDst), h: Math.abs(hDst) });
      if (_profOn) {
        const tEnd = performance.now();
        const dtDecode = _tDecode - _t0;
        const dtDraw = tEnd - _tDecode;
        const dtAll = tEnd - _t0;
        const p = globalThis._sdbProf || (globalThis._sdbProf = {
          n: 0, decodeSum: 0, drawSum: 0, max: 0,
          winN: 0, winDecode: 0, winDraw: 0, winMax: 0, winStart: tEnd,
        });
        p.n++; p.decodeSum += dtDecode; p.drawSum += dtDraw;
        if (dtAll > p.max) p.max = dtAll;
        p.winN++; p.winDecode += dtDecode; p.winDraw += dtDraw;
        if (dtAll > p.winMax) p.winMax = dtAll;
        if (p.n % 240 === 0) {
          const wall = tEnd - p.winStart;
          const rate = wall > 0 ? (p.winN * 1000 / wall).toFixed(0) : '?';
          console.log(
            `[PROF sdb] n=${p.n} ${biBitCount}bpp ${sw}x${sh}->${Math.abs(wDst)}x${Math.abs(hDst)} ` +
            `last240 avg=${((p.winDecode + p.winDraw) / p.winN).toFixed(2)}ms ` +
            `(decode ${(p.winDecode / p.winN).toFixed(2)} + draw ${(p.winDraw / p.winN).toFixed(2)}) ` +
            `max=${p.winMax.toFixed(1)}ms blit-rate=${rate}/s ` +
            `overall avg=${((p.decodeSum + p.drawSum) / p.n).toFixed(2)}ms max=${p.max.toFixed(1)}ms`
          );
          p.winN = 0; p.winDecode = 0; p.winDraw = 0; p.winMax = 0; p.winStart = tEnd;
        }
      }
      ...
      if (ctx.dumpSdb) {
        ...
        const palIdx = (colorUse === 1) ? (dv.getUint32(GDI_PALETTE_SELECTED, true) & 3) : -1;
          ...
          for (let row = 0; row < imgH; row++) {
            ...
            for (let x = 0; x < imgW; x++) {
              ...
              if (biBitCount === 1) {
                ...
                const nib = rleIndices
                  ? rleIndices[row * imgW + x]
                  : ((x & 1) ? (mem[rowOff + (x >> 1)] & 0xF) : ((mem[rowOff + (x >> 1)] >> 4) & 0xF));
                const c = palette[nib] || 0;
                r = (c >> 16) & 0xFF; g = (c >> 8) & 0xFF; b = c & 0xFF;
              } else if (biBitCount === 8) {
                const idx = rleIndices ? rleIndices[row * imgW + x] : mem[rowOff + x];
                const c = palette[idx] || 0;
                r = (c >> 16) & 0xFF; g = (c >> 8) & 0xFF; b = c & 0xFF;
              } else if (biBitCount === 24) {
                const si = rowOff + x * 3;
    ...
    gdi_frame_rect: (hdc, left, top, right, bottom, hbrush, hwnd) => {
      ...
      if (!t) return 1;
      const c = t.ctx;
      const bc = _brushColorRef(hbrush, 0x000000);
      if (bc === null) return 1;
      const ox = t.ox, oy = t.oy;
      c.fillStyle = _colorRefCss(bc);
      // Draw 1px frame
      ...
      _markDrawTargetDirty(t, { x: left, y: top, w: right - left, h: bottom - top });
      return 1;
    },
    ...
    create_font: (height, weight, italic, facePtr) => {
      ...
      return _gdiAlloc({ type: 'font', height: _normalizeFontHeight(height), weight, italic, face, css });
    },
  ...
  if (trace.has('gdi')) {
    ...
    const wrap = (name, fn, fmt) => {
      host[name] = (...args) => {
        const r = fn(...args);
        console.log(`[gdi] ${_formatHostTrace(name, args, r) || fmt(args, r)}`);
        return r;
      };
    };
    ...
    const bmpDesc = (h) => {
      const o = _gdiObjects[h];
      if (!o || o.type !== 'bitmap') return '';
      const res = o.resourceId !== undefined ? ` res=${o.resourceId}` : '';
      return ` ${o.w}x${o.h}${res}`;
    };
    const selectedBmpDesc = (dc) => {
      const d = _getDcState(dc);
      return d && d.selectedBitmap ? `${hex(d.selectedBitmap)}${bmpDesc(d.selectedBitmap)}` : 'none';
    };
    wrap('gdi_select_object', host.gdi_select_object,
      ([dc,obj], r) => `SelectObject(${hex(dc)}, ${hex(obj)}${bmpDesc(obj)}) → ${hex(r)}${bmpDesc(r)}`);
    ...
    wrap('gdi_bitblt', host.gdi_bitblt,
      ([ddc,dx,dy,w,h,sdc,sx,sy,rop], r) =>
        `BitBlt(dst=${hex(ddc)} bmp=${selectedBmpDesc(ddc)} (${dx},${dy}) ${w}x${h} ← src=${hex(sdc)} bmp=${selectedBmpDesc(sdc)} (${sx},${sy}) ${ropNames[rop]||hex(rop)}) → ${r}`);
    wrap('gdi_transparent_blt', host.gdi_transparent_blt,
      ([ddc,dx,dy,w,h,sdc,sx,sy,key], r) =>
        `TransparentBlt(dst=${hex(ddc)} bmp=${selectedBmpDesc(ddc)} (${dx},${dy}) ${w}x${h} ← src=${hex(sdc)} bmp=${selectedBmpDesc(sdc)} (${sx},${sy}) key=${hex(key)}) → ${r}`);
    ...
    if (host.gdi_round_rect) wrap('gdi_round_rect', host.gdi_round_rect,
      ([dc,l,t,r2,b,w,h], r) => `RoundRect(${hex(dc)} (${l},${t})-(${r2},${b}) ${w}x${h}) → ${r}`);
    ...
    if (host.gdi_poly_bezier) wrap('gdi_poly_bezier', host.gdi_poly_bezier,
      ([dc,pts,n,current], r) => `${current ? 'PolyBezierTo' : 'PolyBezier'}(${hex(dc)} pts=${hex(pts)} n=${n}) → ${r}`);
    if (host.gdi_polyline_to) wrap('gdi_polyline_to', host.gdi_polyline_to,
      ([dc,pts,n], r) => `PolylineTo(${hex(dc)} pts=${hex(pts)} n=${n}) → ${r}`);
    ...
    if (host.gdi_ext_flood_fill) wrap('gdi_ext_flood_fill', host.gdi_ext_flood_fill,
      ([dc,x,y,c,m], r) => `ExtFloodFill(${hex(dc)} ${x},${y} color=${hex(c)} mode=${m}) → ${r}`);
      ...
      ([dc,m], r) => `SetBkMode(${hex(dc)} ${m}) → ${r}`);
	    if (host.gdi_draw_text) wrap('gdi_draw_text', host.gdi_draw_text,
	      ([dc,tp,n,rc,fmt,w], r) => {
	        ...
	        try {
	          const dv = new DataView(ctx.getMemory());
	          rr = `${dv.getInt32(rc, true)},${dv.getInt32(rc + 4, true)},${dv.getInt32(rc + 8, true)},${dv.getInt32(rc + 12, true)}`;
	        } catch (_) {}
	        return `DrawText(${hex(dc)} "${s}" rect=${hex(rc)} [${rr}] fmt=${hex(fmt)}) → ${r}`;
	      });
    ...
    if (host.gdi_ext_text_out) wrap('gdi_ext_text_out', host.gdi_ext_text_out,
      ([dc,x,y,opts,rc,tp,n,w], r) => {
        let s = '';
        let rr = '?';
        try { const bytes = w ? Math.min(n, 32) * 2 : Math.min(n, 32); const mem = new Uint8Array(ctx.getMemory(), tp, bytes); s = new TextDecoder(w ? 'utf-16le' : 'ascii').decode(mem).replace(/\0.*/,''); } catch (_) {}
        try {
          const dv = new DataView(ctx.getMemory());
          rr = rc ? `${dv.getInt32(rc, true)},${dv.getInt32(rc + 4, true)},${dv.getInt32(rc + 8, true)},${dv.getInt32(rc + 12, true)}` : 'null';
        } catch (_) {}
        return `ExtTextOut(${hex(dc)} ${x},${y} opts=${hex(opts)} rect=${hex(rc)} [${rr}] "${s}" n=${n}${w?'W':''}) → ${r}`;
      });
  ...
  return { host, readStr, gdi: { _gdiObjects, _dcState, _gdiAlloc, _getDC, _getClientOrigin, handleBox: _handleBox, _dcTable, _releasedDcTable, _windowDcDefaults, bitmapCanvasPool: _bitmapCanvasPool, dcHandleBox: _dcHandleBox, screenCanvasState: _screenCanvasState, pixelCache: _pixelCache, presentBestDxOffscreen: _presentBestDxOffscreen } };
}
