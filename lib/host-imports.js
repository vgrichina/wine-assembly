...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    log: () => {},
    log_i32: (v) => { if (process.env.DBG_INV) console.log('[LOG_I32]', '0x' + (v >>> 0).toString(16), v); },
    log_api_exit: () => {},
    log_block: () => {},
    ...
    invalidate: (hwnd) => {
      if (process.env.DBG_INV) console.log('[INVALIDATE] hwnd=0x' + hwnd.toString(16));
    ...
    erase_background: (hwnd, hbrBackground) => {
      if (process.env.DBG_INV) console.log('[ERASE] hwnd=0x' + hwnd.toString(16) + ' brush=0x' + hbrBackground.toString(16));
