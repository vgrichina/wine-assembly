  ...
  function _subtractBranchesFromBranch(A, Bs) {
    ...
    for (const P of Bs) {
      ...
      if (acc.length > RGN_BRANCH_CAP * 4) {
        throw new Error('CombineRgn branch explosion in subtract — needs Approach B scratch canvas');
      }
    }
    return acc;
  }
  function _combineRgn(mode, A, B) {
    ...
    if (mode === 1) {
      ...
      if (branches.length > RGN_BRANCH_CAP) {
        throw new Error('CombineRgn AND branch explosion — needs Approach B scratch canvas');
      }
    ...
    if (mode === 4) {
      ...
      if (branches.length > RGN_BRANCH_CAP) {
        throw new Error('CombineRgn DIFF branch explosion — needs Approach B scratch canvas');
      }
      return {
    ...
    if (mode === 2) {
      ...
      for (const b of B.branches) {
        ...
        if (branches.length > RGN_BRANCH_CAP) {
          throw new Error('CombineRgn OR branch explosion — needs Approach B scratch canvas');
        }
      }
