...
function createHostImports(ctx) {
  ...
  const _markDrawTargetDirty = (target, dirtyRect) => {
    ...
    if (target.canonicalSurface) {
      ...
      if (w > 0 && h > 0) {
        const rgba = target.ctx.getImageData(x, y, w, h).data;
        ...
        presentation.surface.markDirty(x, y, w, h);
      }
    }
  };
  ...
  const host = {
    ...
    gdi_set_region_bands: (hrgn, rectsWA, count) => {
      if (count === -1) {
        delete _gdiObjects[hrgn];
        return 1;
      }
      if (count < 0 || count > 208) return 0;
      let rgn = _gdiObjects[hrgn];
      if (!rgn) rgn = _gdiObjects[hrgn] = _makeRectRgn(0, 0, 0, 0);
    ...
    gdi_surface_create: (id, width, height, bpp, bitsWa, stride, topDown,
      ...
      if (bpp <= 8 && paletteWa && paletteCount > 0) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          palette.push([mem[p + 2], mem[p + 1], mem[p]]);
        }
      }
      ...
      try {
        const surface = new _gdiSurface.GdiSurface({
          ...
          palette, format: bpp === 32 ? 'bgra32' : `dib${bpp}`,
        });
        _gdiSurfacePresentations.set(id, {
    ...
    gdi_surface_upload: (id, left, top, right, bottom) => {
      ...
      const { width, height, surface, canvas, bitsWa, byteLength } = presentation;
      const arenaOffset = bitsWa - DIB_BACKING_BASE;
      if (arenaOffset >= 0 && arenaOffset + byteLength <= DIB_BACKING_SIZE) {
        const states = new Uint8Array(ctx.getMemory(), DIB_PAGE_STATE,
          DIB_BACKING_SIZE >>> 12);
        const firstPage = arenaOffset >>> 12;
        const lastPage = (arenaOffset + byteLength - 1) >>> 12;
      ...
      _invalidatePixelCache(canvas);
      if (presentation.targetHwnd && ctx.renderer) {
        const win = ctx.renderer.windows && ctx.renderer.windows[presentation.targetHwnd];
        if (win) win.clientPainted = true;
        if (ctx.renderer.scheduleRepaint) ctx.renderer.scheduleRepaint();
      }
      return 1;
    },
    gdi_surface_attach: (id, hwnd) => {
      const presentation = _gdiSurfacePresentations.get(id >>> 0);
      hwnd >>>= 0;
      if (!presentation || !hwnd || !ctx.renderer || !ctx.renderer.getWindowCanvas) return 0;
      const target = ctx.renderer.getWindowCanvas(hwnd);
      if (!target || !target.canvas) return 0;
      presentation.canvas = target.canvas;
    ...
    gdi_text_bind: (token, dcWa, originX, originY) => {
      token >>>= 0; dcWa >>>= 0; originX |= 0; originY |= 0;
      ...
      if (presentation) {
        const { width, height, surface, canvas } = presentation;
        const rgba = surface.rgbaRect(0, 0, width, height);
        const canvasContext = canvas.getContext('2d');
  ...
  return { host, readStr, gdi: { _gdiObjects, _dcState, _gdiAlloc, _getDC, _getClientOrigin, handleBox: _handleBox, _dcTable, _releasedDcTable, _windowDcDefaults, bitmapCanvasPool: _bitmapCanvasPool, dcHandleBox: _dcHandleBox, screenCanvasState: _screenCanvasState, surfacePresentations: _gdiSurfacePresentations, pixelCache: _pixelCache, presentBestDxOffscreen: _presentBestDxOffscreen } };
}
