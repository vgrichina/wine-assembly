...
function createHostImports(ctx) {
  ...
  const _syncDibSection = (bmp) => {
    ...
    // Small/medium DIBSections are often software-rendered directly through
    // ppvBits, then presented with BitBlt. Hash every byte there; sparse
    // sampling can miss animated indexed-color writes and keep the canvas stale.
    let hash = 0 | 0;
