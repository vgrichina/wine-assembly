...
function createHostImports(ctx) {
  ...
  // --- Canvas font resources and derived GDI presentations ---
  // Thread workers share font resources and derived surface presentations.
  ...
  const _flushGdiSurfacePresentation = (presentation, syncForComposite = false) => {
    ...
    try {
      ...
      // skia-canvas can defer putImageData past an immediate cross-canvas
      // drawImage. Force its command queue visible before the Node compositor
      // consumes this presentation; browser Canvas updates synchronously.
  ...
  // Font/mapping state is published from the canonical WAT DC immediately
  // before a Canvas measurement or mask request. A direct host-only
  // measurement gets immutable defaults.
  ...
  const _resolveFont = (hdc) => {
    ...
    if (font && font.css) {
        ...
        if (Number.isFinite(sy) && sy > 0 && Math.abs(sy - 1) > 0.01) {
          // LOGFONT height is expressed in logical units. Once an anisotropic
          // DC maps it into device space, sub-8px preview glyphs are valid.
          // Do not feed that device height back through _buildCssFont: its 8px
          // floor protects malformed CreateFont input, but made print-preview
          // lines overlap after their origins had already been scaled down.
          const deviceHeight = Math.max(1, Math.round((font.height || 13) * sy));
          return font.css.replace(/\d+(?:\.\d+)?px/, `${deviceHeight}px`);
        }
        return font.css;
    }
  ...
  // Canvas exposes no switch for monochrome glyph rasterization. Scalable text
  // is rendered into this reusable alpha mask and thresholded into caller-owned
  // WAT bytes. Canvas never composites those bytes into a destination surface.
  ...
  const host = {
    ...
    gdi_text_bind: (token, dcWa, originX, originY, auxWa, clipBandsWa, clipCount,
      fontHeight, fontWeight, fontItalic, fontFaceWa) => {
      token >>>= 0; dcWa >>>= 0; originX |= 0; originY |= 0; auxWa >>>= 0;
      clipBandsWa >>>= 0; clipCount |= 0;
      fontHeight |= 0; fontWeight |= 0; fontItalic |= 0; fontFaceWa >>>= 0;
      if (!token || !dcWa || dcWa + 92 > ctx.getMemory().byteLength) return 0;
      const dv = new DataView(ctx.getMemory());
      const face = fontFaceWa ? readStr(fontFaceWa, 32) : '';
      const font = {
        type: 'font',
        height: _normalizeFontHeight(fontHeight),
        weight: fontWeight,
        italic: fontItalic,
        face,
        css: _buildCssFont(fontHeight, fontWeight, fontItalic, face),
      };
      const selectedFont = dv.getUint32(dcWa + 88, true);
      const clipBands = [];
    ...
    // Rasterize an unavailable scalable face into caller-owned one-bit mask
    // bytes. WAT converts those bytes into retained path geometry; Canvas does
    // not create a path and never reads or modifies the destination surface.
    gdi_text_mask: (token, x, y, textPtr, nCount, isWide,
      left, top, width, height, maskWa, stride) => {
      token >>>= 0; x |= 0; y |= 0; textPtr >>>= 0; nCount |= 0; isWide |= 0;
      left |= 0; top |= 0; width |= 0; height |= 0; maskWa >>>= 0; stride |= 0;
      const memory = ctx.getMemory();
      if (!token || nCount < 0 || nCount > 65536 || width <= 0 || height <= 0 ||
          stride < width || !maskWa || maskWa + stride * height > memory.byteLength) return 0;
      const mem = new Uint8Array(memory);
      const dv = new DataView(memory);
      let text = '';
      if (textPtr && nCount > 0) {
        if (isWide) {
          if (textPtr + nCount * 2 > memory.byteLength) return 0;
          ...
          if (textPtr + nCount > memory.byteLength) return 0;
          for (let i = 0; i < nCount && mem[textPtr + i]; i++) {
            text += String.fromCharCode(mem[textPtr + i]);
          }
        }
      }
      if (!_binaryTextMask) _binaryTextMask = _createOffscreen(width, height);
      if (!_binaryTextMask) return 0;
      if (_binaryTextMask.width !== width) _binaryTextMask.width = width;
      if (_binaryTextMask.height !== height) _binaryTextMask.height = height;
      const mask = _binaryTextMask.getContext('2d');
      const dc = _getDcState(token);
      const font = _resolveFont(token);
      const ta = dc.textAlign | 0;
      const hAlign = ta & 6;
      const vAlign = ta & 24;
      const canvasBaseline = vAlign === 24 ? 'alphabetic' : vAlign === 8 ? 'bottom' : 'top';
      const point = _gdiMapPoint(dc, x, y);
      ...
      try {
        ...
        // Every glyph sits at a whole-pixel advance, because that is where WAT
        // measured it: GDI applies no kerning and rounds each advance, so a
        // whole-string fillText — which kerns and accumulates fractions — would
        // draw text that does not line up with the extent the guest laid out
        // with, and would drift further with every character.
        mask.textAlign = 'left';
        mask.textBaseline = canvasBaseline;
        const advance = ch => Math.round(mask.measureText(ch).width);
        let totalWidth = justifyExtra + Math.max(0, text.length - 1) * charExtra;
        for (const ch of text) totalWidth += advance(ch);
        totalWidth = Math.max(0, totalWidth);
        let cursor = hAlign === 2 ? point.x - totalWidth
          : hAlign === 6 ? point.x - totalWidth / 2 : point.x;
        let remainingBreaks = justifyCount;
        let remainingExtra = justifyExtra;
        for (let i = 0; i < text.length; i++) {
          const ch = text[i];
          mask.fillText(ch, cursor, point.y);
          cursor += advance(ch);
      ...
      let image;
      try { image = mask.getImageData(0, 0, width, height); }
      catch (_) { return 0; }
      const rgba = image.data;
      mem.fill(0, maskWa, maskWa + stride * height);
      for (let row = 0; row < height; row++) {
        const dst = maskWa + row * stride;
        const src = row * width * 4;
        for (let column = 0; column < width; column++) {
          mem[dst + column] = rgba[src + column * 4 + 3] >= 96 ? 1 : 0;
        }
      }
      return 1;
    },
    ...
    measure_text: (token, textPtr, nCount, isWide) => {
      ...
      const font = _resolveFont(token);
      const c = ctx.renderer && (ctx.renderer.ctx ||
        (ctx.renderer.canvas && ctx.renderer.canvas.getContext('2d')));
      if (!c) {
