...
function createHostImports(ctx) {
  ...
  // Whole-window DCs (GetWindowDC) draw into a full-window-sized canvas at (0,0).
  // Memory DCs draw into their bitmap's canvas at (0,0).
  const _getDrawTarget = (hdc, hwnd) => {
    if (_isWindowDC(hdc)) {
      if (!ctx.renderer) return null;
      // WAT-child paint override: when the renderer is dispatching WM_PAINT
      // to a WAT-managed child control, it has already chosen the canvas and
      // child-relative origin. Route the GDI calls there directly so the
      // child doesn't need a JS-side window record.
      ...
      // GetWindowDC: use full-window canvas (0,0) = window top-left
      if (_isWholeWindowDC(hdc)) {
        const topHwnd = _resolveTopHwnd(h);
        const topWin = ctx.renderer.windows[topHwnd];
        if (!topWin) return null;
        const fw = Math.max(1, topWin.w);
        const fh = Math.max(1, topWin.h);
        if (!topWin._fullBackCanvas || topWin._fullBackW !== fw || topWin._fullBackH !== fh) {
          topWin._fullBackCanvas = ctx.renderer._createOffscreen(fw, fh);
          topWin._fullBackCtx = topWin._fullBackCanvas.getContext('2d');
          topWin._fullBackW = fw;
          topWin._fullBackH = fh;
        }
        return { ctx: topWin._fullBackCtx, ox: 0, oy: 0, hwnd: h, canvas: topWin._fullBackCanvas, isFullWindow: true };
      }

      // GetDC/BeginPaint: use client-area canvas
      const topHwnd = _resolveTopHwnd(h);
