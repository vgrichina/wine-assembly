...
function createHostImports(ctx) {
  ...
  const _guestToWasm = pointer => {
    ...
    return ((pointer >>> 0) - imageBase + _regionMap.GUEST_BASE) >>> 0;
  };
  ...
  const host = {
    ...
    cxx_throw: (argsPtr) => {
      ...
      try {
        ...
        const g2w = ctx.g2w || (addr => addr - imageBase + _regionMap.GUEST_BASE);
    ...
    host.dx_trace = (kind, slot, a, b, c) => {
      ...
      switch (kind) {
        ...
        case 8: { // Execute entry: slot=bufPtr(guest), a=instr_off, b=instr_len
          ...
          try {
            const esp = e.get_esp();
            const retWa = esp - imageBase + _regionMap.GUEST_BASE;
          ...
          try {
            const ebp = e.get_ebp();
            if (ebp) {
              const callerRetWa = ebp + 4 - imageBase + _regionMap.GUEST_BASE;
          ...
          if (trace.has('dx-raw') && b > 0 && b < 65536) {
            try {
              const wa = slot - imageBase + _regionMap.GUEST_BASE + a;
        ...
        case 12: { // BltRect: slot=dst, a=src, b=lpDestRectGuest, c=lpSrcRectGuest
          ...
          const readRect = (g) => {
            if (!g) return 'NULL';
            try {
              const wa = g - imageBase + _regionMap.GUEST_BASE;
        ...
        case 10: { // Device2 DrawPrimitive: slot=primType, a=vtxType, b=count, c=lpvVerts(guest)
          ...
          const vWa = c - imageBase + _regionMap.GUEST_BASE;
