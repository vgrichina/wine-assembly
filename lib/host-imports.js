...
function createHostImports(ctx) {
  ...
  // Win3.x/Win9x .FON files are NE containers whose RT_FONT resources contain
  // FNT bitmap strikes. Keep the original one-bit glyph rows so Canvas never
  // substitutes or antialiases a platform font for these faces.
  const _bitmapFonts = _sharedGdi
    ? (_sharedGdi.bitmapFonts || (_sharedGdi.bitmapFonts = new Map()))
    : new Map();
  const _bitmapFontPaths = _sharedGdi
    ? (_sharedGdi.bitmapFontPaths || (_sharedGdi.bitmapFontPaths = new Map()))
    : new Map();

  const _parseFntStrike = (bytes, base, resourceLength) => {
    const end = Math.min(bytes.length, base + resourceLength);
    if (base < 0 || base + 118 > end) return null;
    const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const u16 = p => dv.getUint16(base + p, true);
    const u32 = p => dv.getUint32(base + p, true);
    const version = u16(0);
    if (version !== 0x0200 && version !== 0x0300) return null;
    const declaredSize = u32(2);
    const size = Math.min(resourceLength, declaredSize || resourceLength);
    if (size < 118 || base + size > bytes.length) return null;
    const firstChar = bytes[base + 95];
    const lastChar = bytes[base + 96];
    if (lastChar < firstChar) return null;
    const height = u16(88);
    if (!height || height > 256) return null;
    const faceOffset = u32(105);
    if (!faceOffset || faceOffset >= size) return null;
    let face = '';
    for (let p = base + faceOffset; p < base + size && bytes[p]; p++) {
      face += String.fromCharCode(bytes[p]);
    }
    if (!face) return null;

    const tableOffset = version === 0x0200 ? 118 : 148;
    const entrySize = version === 0x0200 ? 4 : 6;
    const count = lastChar - firstChar + 1;
    if (tableOffset + (count + 1) * entrySize > size) return null;
    const glyphs = new Array(count);
    for (let i = 0; i < count; i++) {
      const p = tableOffset + i * entrySize;
      const width = u16(p);
      const offset = version === 0x0200 ? u16(p + 2) : u32(p + 2);
      const rowBytes = Math.ceil(width / 8);
      if (!width || offset >= size || offset + rowBytes * height > size) return null;
      glyphs[i] = { width, offset, rowBytes };
    }
    return {
      version, face, firstChar, lastChar, glyphs, height,
      ascent: u16(74), internalLeading: u16(76), externalLeading: u16(78),
      weight: u16(83), charset: bytes[base + 85],
      avgWidth: u16(91), maxWidth: u16(93),
      defaultChar: firstChar + bytes[base + 97],
      bits: bytes.slice(base, base + size),
    };
  };

  const _parseFonStrikes = input => {
    if (!input || input.length < 118) return [];
    const bytes = input instanceof Uint8Array ? input : new Uint8Array(input);
    const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const strikes = [];
    const rawVersion = dv.getUint16(0, true);
    if (rawVersion === 0x0200 || rawVersion === 0x0300) {
      const strike = _parseFntStrike(bytes, 0, bytes.length);
      return strike ? [strike] : [];
    }
    if (bytes.length < 64 || bytes[0] !== 0x4D || bytes[1] !== 0x5A) return strikes;
    const ne = dv.getUint32(0x3C, true);
    if (ne + 0x28 > bytes.length || bytes[ne] !== 0x4E || bytes[ne + 1] !== 0x45) return strikes;
    let p = ne + dv.getUint16(ne + 0x24, true);
    if (p + 2 > bytes.length) return strikes;
    const shift = dv.getUint16(p, true);
    if (shift > 24) return strikes;
    p += 2;
    while (p + 8 <= bytes.length) {
      ...
      p += 8;
      for (let i = 0; i < count && p + 12 <= bytes.length; i++, p += 12) {
        if ((type & 0x7FFF) !== 8) continue; // RT_FONT
        const offset = dv.getUint16(p, true) << shift;
        const length = dv.getUint16(p + 2, true) << shift;
        if (offset < bytes.length && length) {
          const strike = _parseFntStrike(bytes, offset, Math.min(length, bytes.length - offset));
  ...
  const _removeBitmapFontPath = pathKey => {
    const installed = _bitmapFontPaths.get(pathKey);
    if (!installed) return false;
    for (const { faceKey, strike } of installed) {
      const remaining = (_bitmapFonts.get(faceKey) || []).filter(item => item !== strike);
      if (remaining.length) _bitmapFonts.set(faceKey, remaining);
      else _bitmapFonts.delete(faceKey);
    }
    _bitmapFontPaths.delete(pathKey);
    return true;
  };
  ...
  // Resolve an installed FNT strike and its logical, viewport-scaled height.
  const _resolveBitmapFont = (hdc) => {
    const dc = _getDcState(hdc);
    const font = _resolveSelectedFont(hdc);
    if (!font || !font.bitmapFont) return null;
    const sy = Math.abs((dc.vpExtY || 1) / (dc.winExtY || 1));
    const height = Math.max(1, Math.round((font.height || font.bitmapFont.height) *
      (Number.isFinite(sy) && sy > 0 ? sy : 1)));
    return { strike: font.bitmapFont, font, height };
  };

  const _bitmapGlyph = (strike, code) => {
    let c = code | 0;
    if (c < strike.firstChar || c > strike.lastChar) c = strike.defaultChar;
    if (c < strike.firstChar || c > strike.lastChar) c = strike.firstChar;
    return strike.glyphs[c - strike.firstChar];
  };

  const _bitmapGlyphWidth = (info, glyph) =>
    Math.max(1, Math.round(glyph.width * info.height / info.strike.height));

  const _bitmapTextWidth = (info, text) => {
    let width = 0;
    for (let i = 0; i < text.length; i++) {
      width += _bitmapGlyphWidth(info, _bitmapGlyph(info.strike, text.charCodeAt(i) & 0xFF));
    }
    return width;
  };

  const _drawBitmapGlyph = (c, info, glyph, x, y) => {
    const strike = info.strike;
    const destW = _bitmapGlyphWidth(info, glyph);
    const destH = info.height;
    const ix = Math.round(x), iy = Math.round(y);
    for (let dy = 0; dy < destH; dy++) {
      const sy = Math.min(strike.height - 1, Math.floor(dy * strike.height / destH));
      let run = -1;
      for (let dx = 0; dx <= destW; dx++) {
        let on = false;
        if (dx < destW) {
          const sx = Math.min(glyph.width - 1, Math.floor(dx * glyph.width / destW));
          // FNT 2.x/3.x stores a glyph as 8-pixel-wide vertical strips: all
          // scanlines for byte column 0, then all scanlines for byte column 1.
          const value = strike.bits[glyph.offset + (sx >> 3) * strike.height + sy];
          on = !!(value & (0x80 >> (sx & 7)));
        }
        if (on && run < 0) run = dx;
        if (!on && run >= 0) {
          c.fillRect(ix + run, iy + dy, dx - run, 1);
          run = -1;
        }
      }
    }
  ...
  function _drawWithClip(hdc, drawFn, dirtyRect) {
    ...
    for (const band of bands) {
      const mapped = _gdiMapRect(dc, band.l, band.t, band.r, band.b);
      if (mapped.w <= 0 || mapped.h <= 0) continue;
      c.save();
      c.beginPath();
      c.rect(t.ox + mapped.l, t.oy + mapped.t, mapped.w, mapped.h);
      c.clip();
      t.clipActive = true;
  ...
  function _gdiTextOut(hdc, x, y, options, rectWA, textPtr, nCount, isWide) {
    ...
    const bitmapInfo = _resolveBitmapFont(hdc);
    const fontHeight = bitmapInfo ? bitmapInfo.height : (parseInt(font.match(/(\d+)px/)?.[1]) || 13);
    ...
    const textWidth = (c) => {
      const glyphWidth = bitmapInfo ? _bitmapTextWidth(bitmapInfo, text) : c.measureText(text).width;
    ...
    const drawGlyphs = (c, dx, dy) => {
      ...
      if (bitmapInfo) {
        ...
        let top = dy;
        if (vAlign === 8) top -= fontHeight;
        else if (vAlign === 24) {
          top -= Math.round(bitmapInfo.strike.ascent * fontHeight / bitmapInfo.strike.height);
        }
        cursor = Math.round(cursor);
        top = Math.round(top);
        c.fillStyle = `rgb(${r},${g},${b})`;
        let remainingBreaks = Math.max(0, dc.justifyCount | 0);
        let remainingExtra = dc.justifyExtra | 0;
        for (let i = 0; i < text.length; i++) {
          const code = text.charCodeAt(i) & 0xFF;
          const glyph = _bitmapGlyph(bitmapInfo.strike, code);
          cursor += _drawBitmapGlyph(c, bitmapInfo, glyph, cursor, top);
          if (i + 1 < text.length) cursor += dc.charExtra | 0;
          if (code === 0x20 && remainingBreaks > 0) {
    ...
    const drawText = (c, dx, dy) => {
      ...
      if (bitmapInfo) {
        drawGlyphs(c, dx, dy);
      } else if (!_drawBinaryCanvasText(c, bounds, r, g, b,
  ...
  const host = {
    ...
    gdi_surface_upload: (id, left, top, right, bottom) => {
      ...
      if (presentation.targetDesktop && ctx.renderer && ctx.renderer.scheduleRepaint) {
        ctx.renderer.scheduleRepaint();
      }
      return 1;
    },
    ...
    add_font_resource: pathPtr => {
      const path = pathPtr ? readStr(pathPtr, 1024) : '';
      const bytes = _readVfsFile(path);
      const strikes = _parseFonStrikes(bytes);
      if (!strikes.length) return 0;
      const pathKey = path.toLowerCase().replace(/\//g, '\\');
      _removeBitmapFontPath(pathKey);
      const installedFaces = [];
      for (const strike of strikes) {
        const faceKey = strike.face.toLowerCase();
        const existing = _bitmapFonts.get(faceKey) || [];
        existing.push(strike);
        existing.sort((a, b) => a.height - b.height);
        _bitmapFonts.set(faceKey, existing);
        installedFaces.push({ faceKey, strike });
        console.log(`[GDI] loaded FNT bitmap face "${strike.face}" (${strike.glyphs.length} glyphs, ${strike.height}px)`);
      }
      _bitmapFontPaths.set(pathKey, installedFaces);
      return strikes.length;
    },
    remove_font_resource: pathPtr => {
      const path = pathPtr ? readStr(pathPtr, 1024) : '';
      return _removeBitmapFontPath(path.toLowerCase().replace(/\//g, '\\')) ? 1 : 0;
    },
    create_font: (height, weight, italic, facePtr) => {
      ...
      const strikes = _bitmapFonts.get(face.toLowerCase());
      let bitmapFont = null;
      if (strikes && strikes.length) {
        bitmapFont = strikes.reduce((best, candidate) =>
          Math.abs(candidate.height - normalizedHeight) < Math.abs(best.height - normalizedHeight)
            ? candidate : best, strikes[0]);
      }
      return _allocFontResource({ type: 'font', height: normalizedHeight, weight, italic, face, css, bitmapFont });
    },
    measure_text: (token, textPtr, nCount, isWide) => {
      ...
      const bitmapInfo = _resolveBitmapFont(token);
      if (bitmapInfo) return _bitmapTextWidth(bitmapInfo, text);
    ...
    get_text_metrics: (hdc) => {
      ...
      const bitmapInfo = _resolveBitmapFont(hdc);
      const font = _resolveFont(hdc);
      const height = bitmapInfo ? bitmapInfo.height : (parseInt(font.match(/(\d+)px/)?.[1]) || 13);
      let aveW = bitmapInfo
        ? Math.max(1, Math.round(bitmapInfo.strike.avgWidth * height / bitmapInfo.strike.height))
        : Math.round(height * 0.6); // reasonable default
      if (ctx.renderer && !bitmapInfo) {
        const c = ctx.renderer.ctx;
        c.font = font;
