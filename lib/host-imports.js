...
function createHostImports(ctx) {
  ...
  function _valRectHwnd(hwnd, l, t, r, b) {
    const prev = _updateRgns.get(hwnd);
    if (!prev) return;
    // Fast path: subtracting rect contains prev.bbox → result is empty.
    // _combineRgn(4) carries A.bbox forward unchanged and produces non-empty
    // negated branch chains, so the bbox-based _rgnIsEmpty miss this case.
    if (l <= prev.bbox.l && t <= prev.bbox.t && r >= prev.bbox.r && b >= prev.bbox.b) {
  ...
  function _valRgnHwnd(hwnd, rgn) {
    const prev = _updateRgns.get(hwnd);
    if (!prev) return;
    if (rgn.bbox.l <= prev.bbox.l && rgn.bbox.t <= prev.bbox.t &&
        rgn.bbox.r >= prev.bbox.r && rgn.bbox.b >= prev.bbox.b &&
        rgn.branches.length === 1 && rgn.simpleRect) {
      _updateRgns.delete(hwnd);
      return;
    }
  ...
  function _drawWithClip(hdc, drawFn) {
    ...
    const applyOwnWindowClip = () => {
      if (!hwndForClip || !ctx.renderer) return false;
      const we = ctx.renderer.wasm && ctx.renderer.wasm.exports;
      if (!we || !we.ctrl_get_wh) return false;
      const wh = we.ctrl_get_wh(hwndForClip) >>> 0;
      const w = wh & 0xFFFF;
      const h = wh >>> 16;
      if (w <= 0 || h <= 0) return false;
      t.ctx.save();
      t.ctx.beginPath();
      t.ctx.rect(t.ox, t.oy, w, h);
      t.ctx.clip();
      return true;
    };
    if (!rgn) {
      const ownClipped = applyOwnWindowClip();
      const excluded = _excludeChildrenClip(t, hwndForClip);
      try { drawFn(t); } finally { if (excluded) t.ctx.restore(); if (ownClipped) t.ctx.restore(); }
      return true;
    }
    const c = t.ctx;
    if (rgn.simpleRect) {
      ...
      const ownClipped = applyOwnWindowClip();
      const excluded = _excludeChildrenClip(t, hwndForClip);
      try { drawFn(t); } finally { if (excluded) c.restore(); if (ownClipped) c.restore(); c.restore(); }
      return true;
    }
    for (const chain of rgn.branches) {
      ...
      try {
        ...
        const ownClipped = applyOwnWindowClip();
        const excluded = _excludeChildrenClip(t, hwndForClip);
        try { drawFn(t); } finally { if (excluded) c.restore(); if (ownClipped) c.restore(); }
      } finally { c.restore(); }
    }
    return true;
  }
  ...
  const host = {
    ...
    invalidate: (hwnd) => {
      ...
      if (!hwnd) { if (ctx.renderer) ctx.renderer.invalidate(hwnd); return; }
      // Seed update rgn with full client rect so region-driven WM_PAINT
      // delivery (host_next_dirty_hwnd) sees this hwnd as dirty. Without
      // this, $invalidate_hwnd → host_invalidate would be invisible to the
      // region pump.
      const we = ctx.renderer && ctx.renderer.wasm && ctx.renderer.wasm.exports;
      let cw = 0, ch = 0;
      if (we && we.get_client_rect_wh) {
        const packed = we.get_client_rect_wh(hwnd) | 0;
        cw = packed & 0xFFFF; ch = (packed >>> 16) & 0xFFFF;
      }
      if ((!cw || !ch) && we && we.ctrl_get_wh) {
        const wh = we.ctrl_get_wh(hwnd) >>> 0;
        cw = wh & 0xFFFF; ch = (wh >>> 16) & 0xFFFF;
      }
      if (!cw || !ch) { cw = 32767; ch = 32767; }
      _invalRectHwnd(hwnd, 0, 0, cw, ch);
    ...
    next_dirty_hwnd: () => {
      ...
      for (const hwnd of _updateRgns.keys()) {
        ...
        // Phase A.2 prerequisite: skip non-dialog BS_OWNERDRAW children.
        // Dialog-owned owner-draw controls, such as calc.exe's keypad,
        // need their bootstrap WM_PAINT so the button wndproc can send
        // WM_DRAWITEM to the dialog proc. Toolbar-style owner-draw
        // children keep the old skip: their parent paint path owns the
        // pixels, and direct WM_PAINT can erase icons.
        if (we && we.wnd_get_style_export) {
          const style = we.wnd_get_style_export(hwnd) >>> 0;
          // BS_OWNERDRAW = 0x0B in low nibble; only meaningful for buttons,
          // but harmless to filter on the kind alone — non-button windows
          // with kind=0xB don't exist in this codebase.
          if ((style & 0xF) === 0xB) {
            const parent = we.wnd_get_parent ? (we.wnd_get_parent(hwnd) >>> 0) : 0;
            const parentWin = parent && wins[parent];
            if (!parentWin || !parentWin.isDialog) continue;
          }
        }
        const w = wins[hwnd];
    ...
    // Phase A.2 prerequisite: when WAT dispatches WM_PAINT to a parent,
    // call this first to seed _updateRgns for every WS_CHILD intersecting
    // the parent's update rgn. Subsequent next_dirty_hwnd iterations then
    // dispatch each child's WM_PAINT in z-order. No-op if no children
    // intersect or the WAT exports aren't available.
    seed_child_paints: (parentHwnd) => {
      parentHwnd = parentHwnd >>> 0;
      const parentRgn = _updateRgns.get(parentHwnd);
      if (!parentRgn || _rgnIsEmpty(parentRgn)) return 0;
      const we = ctx.renderer && ctx.renderer.wasm && ctx.renderer.wasm.exports;
      if (!we || !we.wnd_next_child_slot || !we.wnd_slot_hwnd ||
          !we.ctrl_get_xy || !we.ctrl_get_wh) return 0;
      const pb = parentRgn.bbox;
      let seeded = 0, slot = 0;
      for (let i = 0; i < 256; i++) {
        const next = we.wnd_next_child_slot(parentHwnd, slot) | 0;
        if (next < 0) break;
        const ch = we.wnd_slot_hwnd(next) >>> 0;
        slot = next + 1;
        if (!ch) continue;
        ...
        const wh = we.ctrl_get_wh(ch) >>> 0;
        const cx = xy & 0xFFFF, cy = (xy >>> 16) & 0xFFFF;
        const cw = wh & 0xFFFF, chh = (wh >>> 16) & 0xFFFF;
        if (cw <= 0 || chh <= 0) continue;
        // Translate parent-update-bbox into child-client-local; intersect.
        const il = Math.max(0, pb.l - cx);
        const it = Math.max(0, pb.t - cy);
        const ir = Math.min(cw, pb.r - cx);
        const ib = Math.min(chh, pb.b - cy);
        if (ir <= il || ib <= it) continue;
        _invalRectHwnd(ch, il, it, ir, ib);
        seeded++;
      }
      return seeded;
    },
    begin_paint_clip: (hdc, hwnd, rectWA) => {
      ...
      // Don't clear _updateRgns here. The clip is cloned into dc.clipRgn
      // for guest queries (GetClipBox, IntersectClipRect). EndPaint validates
      // the actually-painted rcPaint via host_validate_rect; under Phase A
      // GetMessageA's region-driven WM_PAINT dispatch already pre-validates
      // full-client, so by the time BeginPaint runs the rgn is empty and
      // the early-return branch above fires.
      return 1;
    },
    ...
    gdi_stretch_dib_bits: (hdc, xDst, yDst, wDst, hDst, xSrc, ySrc, wSrc, hSrc, bitsWA, bmiWA, colorUse, rop) => {
      ...
      try {
        ...
        const hwndForClip = _hwndFromDC(hdc);
        let drew = false;
        if (sw === dstW && sh === dstH && typeof t.ctx.putImageData === 'function') {
          const _clipExcluded = _excludeChildrenClip(t, hwndForClip);
          if (!_clipExcluded) {
