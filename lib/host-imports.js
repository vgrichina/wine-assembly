...
function createHostImports(ctx) {
  ...
  const _getDrawTarget = (hdc, hwnd) => {
    if (_isWindowDC(hdc)) {
      ...
      if (acd && _hwndFromDC(hdc) === acd.hwnd) {
        // We're already inside a repaint() call (the renderer dispatched
        // WM_PAINT to this control). Do NOT call scheduleRepaint() — it
        // would recurse straight back into _drawWatChildren in Node mode.
  ...
  const host = {
    ...
    gdi_draw_text: (hdc, textPtr, nCount, rectWA, uFormat, isWide) => {
      ...
      // We don't underline yet, but stripping matches the JS render path
      // (drawStaticText / drawButton / drawMenuBar all call _stripMnemonic).
    ...
    gdi_stretch_dib_bits: (hdc, xDst, yDst, wDst, hDst, xSrc, ySrc, wSrc, hSrc, bitsWA, bmiWA, colorUse, rop) => {
      ...
      try {
        if (process.env.SDB_DEBUG && globalThis._sdbCount <= 12) console.log(`[sdb #${globalThis._sdbCount}] bmi=0x${bmiWA.toString(16)} bits=0x${bitsWA.toString(16)} biW=${biWidth} biH=${biHeight} bpp=${biBitCount} rowBytes=${rowBytes} src=(${xSrc},${ySrc} ${sw}x${sh}) dst=(${xDst},${yDst} ${wDst}x${hDst})`);
        let tmpCanvas;
      ...
      } catch (e) { if (process.env.SDB_DEBUG) console.log('[sdb] err', e.message); }
      ...
      if (process.env.SDB_FULL_BLIT && biBitCount === 8) {
        try {
      ...
      if (ctx.dumpSdb) {
        ...
        ctx.dumpSdb.log.push(
          `#${callIdx} key=${key} biH=${biHeight} (${topDown?'top-down':'bot-up'}) ` +
