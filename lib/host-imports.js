...
function createHostImports(ctx) {
  ...
  const _surfaceForBitmap = (bmp) => {
    if (!bmp || bmp.type !== 'bitmap' || !bmp.dibSection || !_gdiSurface.GdiSurface) return null;
  ...
  const _markDrawTargetDirty = (target, dirtyRect) => {
    ...
    if (target.selectedBitmap) {
      ...
      if (bmp && bmp.dibSection) {
        _syncCanvasToDibSection(bmp, target, rect);
      }
    }
  };
  ...
  const _syncCanvasToDibSection = (bmp, target, dirtyRect) => {
    if (!bmp || !bmp.dibSection || !bmp.canvas || !bmp.lpBitsWa) return;
    const { w, h, bpp, lpBitsWa, palette } = bmp;
    if (w <= 0 || h <= 0) return;
    const mem = new Uint8Array(ctx.getMemory());
    const rowBytes = ((w * bpp + 31) >> 5) << 2;
  ...
  const host = {
    ...
    gdi_create_compat_bitmap: (hdc, w, h) => {
      ...
      return _gdiAlloc({ type: 'bitmap', w, h, bpp: 32, pixels: _opaqueBlackPixels(w, h), canvas, _opaque: true });
    },
    ...
    gdi_get_line_descriptor: (hdc, descWa) => {
      ...
      if (!descWa || !bmp || !bmp.dibSection || !bmp.lpBitsWa ||
