...
function createHostImports(ctx) {
  ...
  const _gdiObjects = {
    ...
    0x30021: { type: 'font', height: 11, weight: 400, italic: 0, face: 'MS Sans Serif', css: '11px "Microsoft Sans Serif", "MS Sans Serif", Tahoma, Arial, sans-serif' }, // DEFAULT_GUI_FONT (17) — Win98 dialog font
    0x30022: { type: 'font', height: 11, weight: 700, italic: 0, face: 'MS Sans Serif', css: 'bold 11px "Microsoft Sans Serif", "MS Sans Serif", Tahoma, Arial, sans-serif' }, // CAPTION_FONT — used by $defwndproc_ncpaint title text
  ...
  const _isWindowDC = (hdc) => (hdc >= 0x50001 && hdc < 0x200000) || (hdc >= 0xD0001 && hdc < 0x200000);
  ...
  const host = {
    ...
    // ---- waveOut audio bridge ----
    wave_out_open: (sampleRate, channels, bitsPerSample, callbackType) => {
      const handle = 0x000B0001;
      ctx._waveOut = { sampleRate, channels, bitsPerSample, bytesWritten: 0, startTime: 0 };
      if (ctx.log) ctx.log(`[waveOut] open: ${sampleRate}Hz ${channels}ch ${bitsPerSample}bit`);
      // Browser: init AudioContext
      if (typeof AudioContext !== 'undefined' || typeof webkitAudioContext !== 'undefined') {
        try {
          const AC = typeof AudioContext !== 'undefined' ? AudioContext : webkitAudioContext;
          if (!ctx._audioCtx) ctx._audioCtx = new AC({ sampleRate });
          ctx._waveOut.nextTime = ctx._audioCtx.currentTime;
          ctx._waveOut.startTime = Date.now();
        } catch (_) {}
      }
      return handle;
    },
    wave_out_write: (handle, pcmDataWA, byteLength) => {
      if (!ctx._waveOut) return 0;
      const wo = ctx._waveOut;
      wo.bytesWritten += byteLength;
      // Browser: decode PCM and schedule via Web Audio API
      if (ctx._audioCtx) {
        try {
          const mem = new Uint8Array(ctx.getMemory());
          const bytesPerSample = wo.bitsPerSample / 8;
          const numSamples = byteLength / (bytesPerSample * wo.channels);
          const audioBuffer = ctx._audioCtx.createBuffer(wo.channels, numSamples, wo.sampleRate);
          for (let ch = 0; ch < wo.channels; ch++) {
            const channelData = audioBuffer.getChannelData(ch);
            for (let i = 0; i < numSamples; i++) {
              const offset = pcmDataWA + (i * wo.channels + ch) * bytesPerSample;
              if (wo.bitsPerSample === 16) {
                const sample = (mem[offset] | (mem[offset + 1] << 8));
                channelData[i] = ((sample > 32767 ? sample - 65536 : sample)) / 32768;
              } else if (wo.bitsPerSample === 8) {
                channelData[i] = (mem[offset] - 128) / 128;
              }
            }
          }
          const source = ctx._audioCtx.createBufferSource();
          source.buffer = audioBuffer;
          source.connect(ctx._audioCtx.destination);
          const now = ctx._audioCtx.currentTime;
          const scheduleTime = Math.max(now, wo.nextTime);
          source.start(scheduleTime);
          wo.nextTime = scheduleTime + audioBuffer.duration;
        } catch (_) {}
      }
      // Node.js: write raw PCM to file if --audio-out specified
    ...
    wave_out_close: (handle) => {
      if (ctx.log) ctx.log(`[waveOut] close`);
      ctx._waveOut = null;
      return 0;
    },
    wave_out_get_pos: (handle) => {
      if (!ctx._waveOut) return 0;
      // Estimate bytes played based on elapsed time
      const wo = ctx._waveOut;
      if (!wo.startTime) return 0;
      const elapsed = (Date.now() - wo.startTime) / 1000;
      const bytesPerSec = wo.sampleRate * wo.channels * (wo.bitsPerSample / 8);
      return Math.min(Math.floor(elapsed * bytesPerSec), wo.bytesWritten);
    },
    ...
    get_control_count: (dlgHwnd) => {
      if (!ctx.renderer) return 0;
      const win = ctx.renderer.windows[dlgHwnd];
      return (win && win.controls) ? win.controls.length : 0;
    },
    get_control_info: (dlgHwnd, index) => {
      if (!ctx.renderer) return 0;
      const win = ctx.renderer.windows[dlgHwnd];
      if (!win || !win.controls || index >= win.controls.length) return 0;
      const ctrl = win.controls[index];
      const classMap = { 'Button': 1, 'Edit': 2, 'Static': 3, 'ListBox': 4, 'ComboBox': 5, 'ScrollBar': 6 };
      const classEnum = classMap[ctrl.className] || 0;
      return (classEnum << 16) | (ctrl.id & 0xFFFF);
    },
    register_control: (dlgHwnd, ctrlIndex, controlHwnd, ctrlId) => {
      if (!ctx.renderer) return;
      const win = ctx.renderer.windows[dlgHwnd];
      if (!win || !win.controls || ctrlIndex >= win.controls.length) return;
      win.controls[ctrlIndex].hwnd = controlHwnd;
      if (!ctx.renderer.controlHwndMap) ctx.renderer.controlHwndMap = new Map();
      ctx.renderer.controlHwndMap.set(controlHwnd, { dlgHwnd, ctrlIndex, ctrlId });
    },
