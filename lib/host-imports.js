  ...
  const host = {
    ...
    gdi_text_mask: (token, x, y, textPtr, nCount, isWide,
      ...
      try {
        ...
        mask.textAlign = spaced ? 'left' : canvasAlign;
        mask.textBaseline = canvasBaseline;
        if (!spaced) {
          mask.fillText(text, point.x, point.y);
        } else {
          let totalWidth = mask.measureText(text).width +
            Math.max(0, text.length - 1) * charExtra + justifyExtra;
          ...
          for (let i = 0; i < text.length; i++) {
            ...
            cursor += mask.measureText(ch).width;
