...
function createHostImports(ctx) {
  ...
  const _presentDxSurfaceToMainWindow = (surface) => {
    ...
    if (ctx.renderer && ctx.renderer.getWindowCanvas) {
      ...
      if (wc && wc.ctx) {
        const mem = new Uint8Array(ctx.getMemory());
        const img = wc.ctx.createImageData(surface.w, surface.h);
        const data = img.data;
        const palWa = _getPrimaryPalWa();
  ...
  const _presentBestDxOffscreen = () => {
    ...
    for (const p of primary) {
      if (_firstNonZeroByte(p.dibWa, p.bytes) >= 0) continue;
      const candidate = offscreen.find(s =>
        s.w === p.w &&
        s.h === p.h &&
        s.bpp === p.bpp &&
        _firstNonZeroByte(s.dibWa, s.bytes) >= 0);
      if (candidate) return _presentDxSurfaceToMainWindow(candidate);
    }
    return 0;
  };
  ...
  const host = {
    ...
    gdi_load_bitmap: (hInstance, resourceId) => {
      ...
      if (!bytes && ctx.dllResources) {
        for (const res of Object.values(ctx.dllResources)) {
          const candidate = res && res.bitmapBytes &&
            res.bitmapBytes[resourceId >>> 0];
          if (candidate) {
            bytes = candidate;
            break;
          }
