...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    gdi_bitblt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, rop) => {
      if (ctx._traceRop && (dstDC === 0x80062 || srcDC === 0x80060 || srcDC === 0x8005e || srcDC === 0x8005c || srcDC === 0x8005a))
        console.error(`[ROP] BitBlt dst=0x${dstDC.toString(16)} (${dx},${dy}) ${w}x${bh} src=0x${srcDC.toString(16)} (${sx},${sy}) rop=0x${rop.toString(16)}`);
