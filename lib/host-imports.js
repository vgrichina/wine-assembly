      ...
      if (we && we.wnd_window_screen_x && we.wnd_window_screen_y) {
        try {
          ...
          if (we.wnd_client_screen_x && we.wnd_client_screen_y) {
            x = (we.wnd_client_screen_x(hwnd) | 0) - baseX;
            y = (we.wnd_client_screen_y(hwnd) | 0) - baseY;
          } else {
            x = (we.wnd_window_screen_x(hwnd) | 0) - baseX;
            y = (we.wnd_window_screen_y(hwnd) | 0) - baseY;
          }
          if (we.wnd_screen_w && we.wnd_screen_h) {
            w = we.wnd_screen_w(hwnd) | 0;
            h = we.wnd_screen_h(hwnd) | 0;
          }
        } catch (_) {}
        ...
        x = (targetCr ? targetCr.x - target.x : 0) + (win.x | 0);
        y = (targetCr ? targetCr.y - target.y : 0) + (win.y | 0);
      } else if (ownSurface && win.clientRect) {
        x = win.clientRect.x - win.x;
        y = win.clientRect.y - win.y;
      }
