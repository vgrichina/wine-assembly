...
function createHostImports(ctx) {
  ...
  const _resolveFont = (hdc) => {
    ...
    if (dc.selectedFont) {
      const font = _fontResources[dc.selectedFont];
      if (font && font.css) {
  ...
  const host = {
    ...
    gdi_text_bind: (token, dcWa, originX, originY, auxWa, clipBandsWa, clipCount) => {
      ...
      // DirectDraw palettes can change independently of the surface pixels.
      // Refresh palette metadata here; _drawWithClip seeds only text's dirty
      // rectangle once its final device-space bounds are known.
      if (presentation && token >= 0x200000 && token < 0x300000 &&
    ...
    create_font: (height, weight, italic, facePtr) => {
      ...
      return _allocFontResource({ type: 'font', height: _normalizeFontHeight(height), weight, italic, face, css });
    },
