...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    gdi_draw_text: (hdc, textPtr, nCount, rectWA, uFormat, isWide) => {
      ...
      if (_accelIdx >= 0) {
        const prefix = text.substring(0, _accelIdx);
        const ch = text[_accelIdx];
        const ux = t.ox + x + Math.round(c.measureText(prefix).width);
        const uw = Math.max(1, Math.round(c.measureText(ch).width));
        const uy = t.oy + y + th - 1;
