...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    gdi_text_out: (hdc, x, y, textPtr, nCount, isWide) => {
      ...
      // Route every DC kind through the same resolver used by fills/blits.
      // Explicit DC-table window handles are not hwnd+0x40000 encoded, so
      // the old _isWindowDC gate silently skipped TextOut on real window DCs.
      const pt = _gdiMapPoint(dc, x, y);
      _drawWithClip(
        hdc,
        (target) => { drawText(target.ctx, target.ox + pt.x, target.oy + pt.y); },
        (target) => textBounds(target.ctx, pt.x, pt.y)
      );
      return 1;
    },
