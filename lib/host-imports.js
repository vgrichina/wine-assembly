...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    wave_out_open: (sampleRate, channels, bitsPerSample, callbackType) => {
      ...
      const wo = { sampleRate, channels, bitsPerSample, bytesWritten: 0, startTime: Date.now() };
      ctx._waveOut = wo;
      if (ctx._sharedAudio) ctx._sharedAudio.waveOut = wo;
      console.log(`[waveOut] open: ${sampleRate}Hz ${channels}ch ${bitsPerSample}bit`);
    ...
    wave_out_write: (handle, pcmDataWA, byteLength) => {
      const wo = ctx._waveOut || (ctx._sharedAudio && ctx._sharedAudio.waveOut);
      if (!wo) return 0;
    ...
    wave_out_close: (handle) => {
      console.log(`[waveOut] close`);
      ctx._waveOut = null;
      if (ctx._sharedAudio) ctx._sharedAudio.waveOut = null;
      return 0;
    },
    wave_out_get_pos: (handle) => {
      const wo = ctx._waveOut || (ctx._sharedAudio && ctx._sharedAudio.waveOut);
      if (!wo) return 0;
      // Pretend all written bytes have been played — avoids pacing issues
      // in emulation where wall-clock time doesn't match audio time.
      return wo.bytesWritten;
    },

    wave_out_set_volume: (handle, volume) => {
      const wo = ctx._waveOut || (ctx._sharedAudio && ctx._sharedAudio.waveOut);
      if (wo && wo.gainNode) {
        wo.gainNode.gain.value = volume / 65535;
      }
    },
    ...
    gdi_bitblt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, rop) => {
      ...
      if (rop === SRCCOPY) {
        ...
        if (srcIsMono && !dstIsMono) {
          for (let i = 0; i < imgData.data.length; i += 4) {
            const isBit0 = (imgData.data[i] === 0 && imgData.data[i+1] === 0 && imgData.data[i+2] === 0);
    ...
    gdi_text_out: (hdc, x, y, textPtr, nCount) => {
      ...
      drawText(dstBmp.canvas.getContext('2d'), x, y);
      return 1;
    },
  ...
  if (trace.has('gdi')) {
    ...
    if (host.gdi_text_out) wrap('gdi_text_out', host.gdi_text_out,
      ([dc,x,y,tp,n], r) => {
        let s = '';
        try { const mem = new Uint8Array(ctx.getMemory(), tp, Math.min(n, 32)); s = new TextDecoder('ascii').decode(mem); } catch (_) {}
        return `TextOut(${hex(dc)} ${x},${y} "${s}" n=${n}) → ${r}`;
      });
