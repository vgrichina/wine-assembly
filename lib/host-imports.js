...
function createHostImports(ctx) {
  ...
  // NOT read from _regionMap yet, though $DX_OBJECTS is declared: this literal
  // is one of the copies tools/check-wat-js-constants.js pins by regex — it
  // matches the name against a hex literal — and that clause has to be deleted
  // in the same change that converts it (docs/watx-region-safety-design.md §6).
  const DX_OBJECTS_WA = 0x07F60000;
