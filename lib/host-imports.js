...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    wave_out_open: (sampleRate, channels, bitsPerSample, callbackType) => {
      ...
      ctx._waveOut = { sampleRate, channels, bitsPerSample, bytesWritten: 0, startTime: Date.now() };
    ...
    wave_out_set_volume: (handle, volume) => {
      if (ctx._waveOut && ctx._waveOut.gainNode) {
        ctx._waveOut.gainNode.gain.value = volume / 65535;
      }
    },
