...
function createHostImports(ctx) {
  ...
  if (ctx.sharedAudio && ctx.sharedAudio.voices) {
    ...
    _voices = ctx._voices = {
      ...
      _installUnlock() {
        ...
        const unlock = () => {
          const ac = this._ac || ctx._audioCtx || this._ensureCtx(44100);
  ...
  const _rgbaToPngDataUrl = (w, h, pixels) => {
    let canvas = null;
    ...
    if (typeof canvas.toBuffer === 'function' && typeof Buffer !== 'undefined') {
  ...
  const _decodeCursorResource = (resourceId) => {
    ...
    const hotX = r16(group, best + 4);
    const hotY = r16(group, best + 6);
    const imageId = r16(group, best + 12);
    const image = _readRsrcBytes(1, imageId);
    if (!image || image.length < 12 || !_dib || typeof _dib.parseDIB !== 'function') return null;

    const dv = new DataView(image.buffer, image.byteOffset, image.byteLength);
    const biSize = dv.getUint32(0, true);
    let realW, storedH, realH, bpp, palBytes, headerBytes;
    const patched = new Uint8Array(image);
    if (biSize === 12) {
      realW = dv.getUint16(4, true);
      storedH = dv.getUint16(6, true);
      realH = storedH >> 1;
      bpp = dv.getUint16(10, true);
      headerBytes = 12;
      palBytes = (bpp <= 8 ? (1 << bpp) : 0) * 3;
      new DataView(patched.buffer).setUint16(6, realH, true);
    } else if (biSize >= 40) {
      realW = dv.getInt32(4, true);
      storedH = dv.getInt32(8, true);
      realH = Math.abs(storedH) >> 1;
      bpp = dv.getUint16(14, true);
      headerBytes = biSize;
      palBytes = (dv.getUint32(32, true) || (bpp <= 8 ? (1 << bpp) : 0)) * 4;
      new DataView(patched.buffer).setInt32(8, storedH < 0 ? -realH : realH, true);
    } else {
      return null;
    }
    if (realW <= 0 || realH <= 0) return null;
    const dib = _dib.parseDIB(patched);
    if (!dib) return null;

    const colorRow = Math.ceil(realW * bpp / 32) * 4;
    const maskRow = Math.ceil(realW / 32) * 4;
    const maskOff = headerBytes + palBytes + colorRow * realH;
    if (maskOff + maskRow * realH <= image.length) {
      for (let y = 0; y < realH; y++) {
        const srcRow = realH - 1 - y;
        for (let x = 0; x < realW; x++) {
          const bit = (image[maskOff + srcRow * maskRow + (x >> 3)] >> (7 - (x & 7))) & 1;
          if (bit) dib.pixels[(y * realW + x) * 4 + 3] = 0;
        }
      }
    }
    const dataUrl = _rgbaToPngDataUrl(realW, realH, dib.pixels);
  ...
  const _resolveDcRecord = (rec, hdc, hwndIn) => {
    ...
    // window / whole / screen — back-canvas of top-level hwnd.
    ...
    const topHwnd = _resolveTopHwnd(rec.hwnd);
    ...
    if (we && we.wnd_window_screen_x && we.wnd_window_screen_y) {
      try {
        ox = (we.wnd_window_screen_x(rec.hwnd) | 0) - (we.wnd_window_screen_x(topHwnd) | 0);
        oy = (we.wnd_window_screen_y(rec.hwnd) | 0) - (we.wnd_window_screen_y(topHwnd) | 0);
        if (rec.kind === 'window' && we.wnd_client_screen_x && we.wnd_client_screen_y) {
          ox = (we.wnd_client_screen_x(rec.hwnd) | 0) - (we.wnd_window_screen_x(topHwnd) | 0);
          oy = (we.wnd_client_screen_y(rec.hwnd) | 0) - (we.wnd_window_screen_y(topHwnd) | 0);
        }
      } catch (_) {}
    }
    ox += (rec.vpOrgX | 0) - (rec.winOrgX | 0);
  ...
  const _legacyGetDrawTarget = (hdc, hwnd) => {
    ...
    if (_isWindowDC(hdc)) {
      ...
      let ox = 0, oy = 0;
      if (we && we.wnd_window_screen_x && we.wnd_window_screen_y && topHwnd) {
        const topX = we.wnd_window_screen_x(topHwnd) | 0;
        const topY = we.wnd_window_screen_y(topHwnd) | 0;
      ...
      } else if (!(_isWholeWindowDC(hdc) && h === topHwnd)) {
  ...
  const host = {
    ...
    menu_track_popup: (hMenu, flags, x, y, hwnd) => {
      const TPM_RETURNCMD = 0x0100;
      let items = ctx._hostMenus && ctx._hostMenus.get(hMenu >>> 0) || [];
      if (!items.length) {
        const menuId = hMenu & 0xFFFF;
        const topIdx = ((hMenu >>> 16) & 0xFFFF) - 1;
        const e = ctx.exports;
        if (menuId && topIdx >= 0 && e && e.menu_load && e.menu_child_count &&
            e.menu_child_id && e.menu_child_flags &&
            e.menu_child_label_ptr && e.menu_child_label_len) {
          try {
            e.menu_load(hwnd >>> 0, menuId >>> 0);
            const n = e.menu_child_count(hwnd >>> 0, topIdx) | 0;
            const parsed = [];
            for (let i = 0; i < n; i++) {
              const itemFlags = e.menu_child_flags(hwnd >>> 0, topIdx, i) | 0;
              const labelPtr = e.menu_child_label_ptr(hwnd >>> 0, topIdx, i) | 0;
              const labelLen = e.menu_child_label_len(hwnd >>> 0, topIdx, i) | 0;
              let text = readBytesString(labelPtr, labelLen);
              if (e.menu_child_shortcut_ptr && e.menu_child_shortcut_len) {
                const scPtr = e.menu_child_shortcut_ptr(hwnd >>> 0, topIdx, i) | 0;
                const scLen = e.menu_child_shortcut_len(hwnd >>> 0, topIdx, i) | 0;
                const shortcut = readBytesString(scPtr, scLen);
                if (shortcut) text += '\t' + shortcut;
              }
              parsed.push({
                flags: itemFlags,
                id: e.menu_child_id(hwnd >>> 0, topIdx, i) >>> 0,
                submenu: 0,
                popup: false,
                separator: !!(itemFlags & 0x01),
                disabled: !!(itemFlags & 0x02),
                checked: !!(itemFlags & 0x04),
                text,
              });
            }
            items = parsed;
      ...
      if (flags & TPM_RETURNCMD) return 0;
      if (typeof document === 'undefined') return 1;
      if (ctx._hostPopupMenu && ctx._hostPopupMenu.parentNode) {
        try { ctx._hostPopupMenu.parentNode.removeChild(ctx._hostPopupMenu); } catch (_) {}
      }
      const canvas = ctx.renderer && ctx.renderer.canvas;
      const wrap = document.getElementById('screen-wrap') || (canvas && canvas.parentElement) || document.body;
      const rect = canvas && canvas.getBoundingClientRect ? canvas.getBoundingClientRect() : { left: 0, top: 0, width: 640, height: 480 };
      const hostRect = wrap && wrap.getBoundingClientRect ? wrap.getBoundingClientRect() : { left: 0, top: 0 };
      const sx = canvas && canvas.width ? rect.width / canvas.width : 1;
      const sy = canvas && canvas.height ? rect.height / canvas.height : 1;
      const el = document.createElement('div');
      el.className = 'wine-popup-menu';
      el.style.cssText = [
        'position:absolute',
        `left:${Math.round(rect.left - hostRect.left + (x | 0) * sx)}px`,
        `top:${Math.round(rect.top - hostRect.top + (y | 0) * sy)}px`,
        'min-width:180px',
        'background:#c0c0c0',
        ...
        'padding:2px',
        'z-index:10000',
        'font:12px "W95FA","MS Sans Serif",Tahoma,sans-serif',
        'color:#000',
        'user-select:none',
      ].join(';');
      const hide = () => {
        ...
        ctx._hostPopupMenu = null;
      };
      const sendCommand = (id) => {
        hide();
        if (!id || !ctx.renderer || !ctx.renderer.inputQueue) return;
        ctx.renderer.inputQueue.push({ type: 'key', hwnd: hwnd >>> 0, msg: 0x0111, wParam: id >>> 0, lParam: 0 });
      };
      for (const it of items) {
        const row = document.createElement('div');
        if (it.separator) {
          row.style.cssText = 'height:0;border-top:1px solid #808080;border-bottom:1px solid #fff;margin:3px 2px';
        } else {
          row.style.cssText = 'height:18px;line-height:18px;padding:0 22px 0 18px;white-space:nowrap;position:relative';
          const displayText = menuDisplayText(it.text || ' ');
          const tab = displayText.indexOf('\t');
          const label = document.createElement('span');
          label.textContent = tab >= 0 ? displayText.slice(0, tab) : displayText;
          row.appendChild(label);
          if (tab >= 0) {
            const shortcut = document.createElement('span');
            shortcut.textContent = displayText.slice(tab + 1);
            shortcut.style.cssText = 'position:absolute;right:22px';
            row.appendChild(shortcut);
          }
          if (it.disabled) {
            row.style.color = '#808080';
          } else {
            row.addEventListener('mouseenter', () => { row.style.background = '#000080'; row.style.color = '#fff'; });
            row.addEventListener('mouseleave', () => { row.style.background = ''; row.style.color = ''; });
            row.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); sendCommand(it.id); });
          }
          if (it.checked) {
            const check = document.createElement('span');
            check.textContent = '\u2713';
            check.style.cssText = 'position:absolute;left:5px';
            row.appendChild(check);
          }
          if (it.popup) {
            const arrow = document.createElement('span');
            arrow.textContent = '>';
            arrow.style.cssText = 'position:absolute;right:6px';
      ...
      wrap.appendChild(el);
      ctx._hostPopupMenu = el;
      const close = (ev) => {
        if (ctx._hostPopupMenu && !ctx._hostPopupMenu.contains(ev.target)) {
          hide();
    ...
    gdi_bitblt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, rop) => {
      ...
      if (globalThis.process && globalThis.process.env && globalThis.process.env.BLT_DBG) {
        ...
        if (globalThis._bltDbg++ < limit) {
          ...
          console.log(`[BLT] dst=${dstDC.toString(16)}(${isDstWindow?'win':'mem'}) src=${srcDC.toString(16)}(${isSrcWindow?'win':'mem'}) rop=${rop.toString(16)} dstT=${dstTarget?'Y':'N'} srcT=${srcTarget?'Y':'N'} dstC=${dd} srcC=${sd} src@${sx},${sy} dst@${dx},${dy} ${w}x${bh}`);
        }
      }
