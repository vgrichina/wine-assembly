...
function createHostImports(ctx) {
  ...
  // Window DC (0x50001) draws to the renderer canvas with client offset.
  ...
  const host = {
    ...
    crash_unimplemented: (namePtr, esp, eip, ebp) => {
      ...
      console.error('\n=== UNIMPLEMENTED API ===');
      console.error(`  Name: ${name || '(null/ordinal)'}`);
      console.error(`  EIP:  ${hex(eip)}  ESP: ${hex(esp)}  EBP: ${hex(ebp)}`);
      try {
        const e = ctx.instance.exports;
        console.error(`  EAX:  ${hex(e.get_eax())}  ECX: ${hex(e.get_ecx())}`);
        console.error(`  EDX:  ${hex(e.get_edx())}  EBX: ${hex(e.get_ebx())}`);
      ...
      } catch (err) { console.error('  (could not dump regs/stack:', err.message + ')'); }
    ...
    gdi_bitblt: (dstDC, dx, dy, w, bh, srcDC, sx, sy, rop, hwnd) => {
      ...
      const srcTarget = isSrcWindow
        ? (ctx.renderer ? { ctx: ctx.renderer.ctx, ox: _getClientOrigin(hwnd).x, oy: _getClientOrigin(hwnd).y, canvas: ctx.renderer.canvas } : null)
      ...
      const dstTarget = isDstWindow
        ? (ctx.renderer ? (() => { const o = _getClientOrigin(hwnd); const win = ctx.renderer.windows[hwnd];
            if (win) win.clientPainted = true;
    ...
    gdi_text_out: (hdc, x, y, textPtr, nCount, hwnd) => {
      ...
      if (bkMode === 2) { // OPAQUE
        ...
        bc.fillRect(x, y, text.length * charW, charH);
      }
      bc.fillStyle = `rgb(${r},${g},${b})`;
      bc.font = '13px monospace';
      bc.textBaseline = 'top';
