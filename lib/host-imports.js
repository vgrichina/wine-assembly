  ...
  const host = {
    ...
    gdi_fill_rect: (hdc, left, top, right, bottom, hbrush) => {
     _drawWithClip(hdc, (t) => {
      ...
      c.fillRect(x, y, w, h);
      if (typeof process !== 'undefined' && process.env && process.env.DBG_CAP
          && x <= 53 && y <= 8 && x + w > 53 && y + h > 8) {
        console.error(`[cap-wipe] hdc=0x${hdc.toString(16)} rect=${x},${y},${w},${h} color=${bc.toString(16)} hbrush=0x${hbrush.toString(16)}`);
