...
function createHostImports(ctx) {
  ...
  const _dcAlloc = (rec) => {
    const h = _dcHandleBox.free.length ? _dcHandleBox.free.pop() : _dcHandleBox.next++;
    _dcTable.set(h, rec);
    return h;
  };
  const _dcFree = (hdc) => {
    if (!_dcTable.has(hdc)) return 0;
    _dcTable.delete(hdc);
    _dcHandleBox.free.push(hdc);
    return 1;
  };
  ...
  const _resolveDcRecord = (rec, hdc, hwndIn) => {
    ...
    if (rec.kind === 'mem') {
      ...
      if (bmp && bmp.type === 'bitmap') {
        ...
        if (bmp.canvas) {
          return { ctx: bmp.canvas.getContext('2d'),
  ...
  const _legacyGetDrawTarget = (hdc, hwnd) => {
    ...
    if (bmp && bmp.type === 'bitmap') {
      ...
      if (bmp.dibSection) _syncDibSection(bmp);
      if (bmp.canvas) {
        return { ctx: bmp.canvas.getContext('2d'), ox: (dc.vpOrgX | 0) - (dc.winOrgX | 0), oy: (dc.vpOrgY | 0) - (dc.winOrgY | 0), canvas: bmp.canvas, selectedBitmap: bmpH, opaque: !!bmp._opaque };
      }
    }
    return null;
  };
  ...
  const host = {
    ...
    gdi_text_out: (hdc, x, y, textPtr, nCount, isWide) => {
      ...
      _drawWithClip(
        hdc,
        (target) => { drawText(target.ctx, target.ox + x, target.oy + y); },
        (target) => textBounds(target.ctx, x, y)
      );
      return 1;
    },
    ...
    gdi_draw_text: (hdc, textPtr, nCount, rectWA, uFormat, isWide) => {
      ...
      _drawWithClip(hdc, (tt) => {
        ...
        if (clippedToRect) {
          c.save();
          c.beginPath();
          c.rect(tt.ox + left, tt.oy + top, Math.max(0, right - left), Math.max(0, bottom - top));
          c.clip();
        }
        c.font = font;
        c.textBaseline = 'alphabetic';
  ...
  return { host, readStr, gdi: { _gdiObjects, _dcState, _gdiAlloc, _getDC, _getClientOrigin, handleBox: _handleBox, _dcTable, dcHandleBox: _dcHandleBox, screenCanvasState: _screenCanvasState, pixelCache: _pixelCache, presentBestDxOffscreen: _presentBestDxOffscreen } };
}
