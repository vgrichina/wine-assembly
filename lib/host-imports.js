...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    gdi_text_bind: (token, dcWa, originX, originY) => {
      ...
      // Text follows the same path for window and memory surfaces: seed the
      // derived offscreen Canvas from canonical WAT pixels, rasterize there,
      // then copy only text's dirty rectangle back through _markDrawTargetDirty.
      if (presentation) {
        const { width, height, surface, canvas } = presentation;
