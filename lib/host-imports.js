...
function createHostImports(ctx) {
    ...
    if (_isWindowDC(hdc)) {
      ...
      if (_isWholeWindowDC(hdc) && h === topHwnd) {
        ...
        if (childWin && childWin.isChild) {
          // For GetWindowDC on a child, childWin.x/y is already relative to
          // parent's client area (set by createWindow); for client DCs we
          // also need the parent's chrome offset computed above.
          if (_isWholeWindowDC(hdc)) {
            const topWin = ctx.renderer.windows[topHwnd];
            if (topWin && topWin.clientRect) {
              ox += topWin.clientRect.x - topWin.x;
  ...
  function _drawWithClip(hdc, drawFn) {
    ...
    if (typeof process !== 'undefined' && process.env.PROBE_FILL) {
      const b = rgn.bbox;
      console.error(`[clipRgn] hdc=${hdc.toString(16)} branches=${rgn.branches.length} bbox=(${b.l},${b.t})-(${b.r},${b.b}) rects=${rgn.rects?rgn.rects.length:'n'} simpleRect=${!!rgn.simpleRect}`);
      for (let bi = 0; bi < rgn.branches.length; bi++) {
        const ch = rgn.branches[bi];
        console.error(`  branch[${bi}] len=${ch.length} polarity=[${ch.map(e=>e.polarity).join(',')}] rule=[${ch.map(e=>e.rule).join(',')}]`);
      }
    }
    for (const chain of rgn.branches) {
      ...
      try {
        ...
        if (!(typeof process !== 'undefined' && process.env.SKIP_CLIP))
        ...
        c.translate(-t.ox, -t.oy);
        if (typeof process !== 'undefined' && process.env.PROBE_FILL) {
          const tx = c.getTransform();
          c.save();
          c.setTransform(1,0,0,1,0,0);
          c.fillStyle = 'magenta';
          c.fillRect(0, 0, 3, 3);
          const pp = c.getImageData(0, 0, 1, 1).data;
          c.restore();
          console.error(`  [inClipMagenta]=[${pp[0]},${pp[1]},${pp[2]},${pp[3]}] xform=(${tx.a},${tx.b},${tx.c},${tx.d},${tx.e},${tx.f})`);
        }
        drawFn(t);
      } finally { c.restore(); }
    }
  ...
  const host = {
    ...
    gdi_fill_rect: (hdc, left, top, right, bottom, hbrush) => {
     _drawWithClip(hdc, (t) => {
      ...
      if (typeof process !== 'undefined' && process.env.PROBE_FILL) {
        try {
          const id = c.getImageData(x + Math.floor(w/2), y + Math.floor(h/2), 1, 1).data;
          const expected = [bc & 0xFF, (bc>>8)&0xFF, (bc>>16)&0xFF];
          const match = id[0]===expected[0] && id[1]===expected[1] && id[2]===expected[2];
          const tx = c.getTransform ? c.getTransform() : null;
          const cvs = c.canvas;
          // Direct fresh fill probe on same context
          c.save();
          c.setTransform(1,0,0,1,0,0);
          c.fillStyle = 'red';
          c.fillRect(0, 0, 2, 2);
          const probe = c.getImageData(0, 0, 1, 1).data;
          c.restore();
          console.error(`[probe] hdc=${hdc.toString(16)} rect=(${x},${y})+${w}x${h} want=${expected} got=[${id[0]},${id[1]},${id[2]},${id[3]}] match=${match} redProbe=[${probe[0]},${probe[1]},${probe[2]},${probe[3]}] alpha=${c.globalAlpha} gco=${c.globalCompositeOperation} xform=${tx?`${tx.a},${tx.b},${tx.c},${tx.d},${tx.e},${tx.f}`:'n/a'} canvas=${cvs.width}x${cvs.height} tag=${c._tag||'?'} depth=${c._saveDepth||0}`);
    ...
    apply_window_clip: (hdc, hwnd) => {
      ...
      if (!clipChildren && !clipSiblings) return 0;
      const dc = _getDC(hdc);
    ...
    host.dx_trace = (kind, slot, a, b, c) => {
      switch (kind) {
        ...
        case 8: // Execute entry: slot=bufPtr(guest), a=instr_off, b=instr_len
          console.log(`[dx] ExecIn bufGuest=${hex(slot)} instrOff=${a} instrLen=${b}`);
          break;
        case 10: { // Device2 DrawPrimitive: slot=primType, a=vtxType, b=count, c=lpvVerts(guest)
          ...
          try {
            ...
            for (let i = 0; i < n; i++) {
              const o = i * 32;
              out.push(`(${dv.getFloat32(o, true).toFixed(0)},${dv.getFloat32(o + 4, true).toFixed(0)} c=${hex(dv.getUint32(o + 16, true))})`);
            }
            vs = out.join(' ');
          } catch (_) {}
          console.log(`[dx] DP2     primType=${slot} vtxType=${a} count=${b} lpv=${hex(c)} ${vs}`);
          break;
        }
        default:
