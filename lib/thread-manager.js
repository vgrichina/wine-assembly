...
const YIELD_NAMES = { 1: 'wait', 2: 'exit', 3: 'com_load_dll', 5: 'load_library', 6: 'modal_dialog', 7: 'message_wait', 8: 'net_wait', 9: 'critical_section', 10: 'send_message', 11: 'self_suspend' };
...
class ThreadManager {
  ...
  createSemaphore(initialCount, maxCount) {
    ...
    if (idx === -1) return 0;
    const handle = this._allocateSyncHandle(idx);
    Atomics.store(this.syncView, idx * 4 + 2, initialCount | 0); // count
    Atomics.store(this.syncView, idx * 4 + 3, (maxCount | 0) || 0x7FFFFFFF); // max
    Atomics.store(this.syncView, idx * 4 + 0, handle | 0); // issued handle mirror
