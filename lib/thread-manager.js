...
class ThreadManager {
  ...
  async runWorkerSlices(sliceSize) {
    ...
    const runnable = [];
    for (const [handle, thread] of this.threads) {
      if (thread.state !== 'active' || !thread.link) continue;
      if (thread.inFlight) continue;               // still executing its last slice
      if (thread.suspendCount > 0) continue;
      if (thread.sleepUntil && now < thread.sleepUntil) continue;
  ...
  async _runWorkerThread(handle, thread, sliceSize, sync) {
    ...
    if (r.trapped) {
      this._log(`[ThreadManager] worker thread ${thread.tid} trapped at EIP=0x${(r.eip >>> 0).toString(16)}: ${r.trapped}`);
  ...
  async _resolveWorkerWait(thread, r) {
    ...
    await thread.link.completeWait(result, waitStackBytes);
