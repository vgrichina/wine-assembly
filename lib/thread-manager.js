...
class ThreadManager {
  ...
  // object.  This compact handle model has no DuplicateHandle/reference-count
  // path, so closing the only handle frees its table slot immediately.  Storm
  // creates short-lived event triplets while streaming Diablo's MPQ; without
  // recycling, the fixed 64-slot table is exhausted after a few dozen reads.
