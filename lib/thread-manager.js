...
const YIELD_NAMES = { 1: 'wait', 2: 'exit', 3: 'com_load_dll', 4: 'help_load', 5: 'load_library', 6: 'modal_dialog', 7: 'message_wait', 8: 'net_wait', 9: 'critical_section' };
...
class ThreadManager {
  constructor(wasmModule, memory, mainInstance, makeImports, opts) {
    ...
    this._mainThreadState = { tid: 0, win32Tid: 1, state: 'active', suspendCount: 0, isMain: true };
    ...
    // Which scheduler is actually in charge. Today there is one: guest threads
    // are separate WASM instances round-robined on this JS thread. The Worker
    // backend is designed (docs/design-real-threads.md) and not built, and this
    // field exists so nothing can claim otherwise — a UI switch that says
    // "threads on" while this reads 'cooperative' is reporting a lie.
    this.backend = 'cooperative';
    // Set when the page asked for real threads: isolation is available and the
    // user opted in, but the broker for non-pure host imports does not exist.
    this.threadsRequested = !!opts.threadsRequested;
    if (this.threadsRequested) {
      this._log('[ThreadManager] real threads requested; worker backend is not implemented '
        + '(needs the host-import broker — docs/design-real-threads.md phase 1). '
    ...
    this._syncCurrentHandles = new Uint32Array(64);
    this._syncEverUsed = new Uint8Array(64);
  ...
  duplicateCurrentThread(win32Tid) {
    win32Tid = win32Tid >>> 0;
    let thread = win32Tid === 1 ? this._mainThreadState : null;
    if (!thread) {
      for (const candidate of this.threads.values()) {
        if ((((candidate.tid | 0) + 1) >>> 0) === win32Tid) {
          thread = candidate;
          break;
        }
      }
    ...
    if (!thread) {
      thread = this._pendingThreads.find(candidate =>
  ...
  waitSingleCooperative(handle, timeout) {
    ...
    const target = this._resolveThreadHandle(handle);
  ...
  waitMultipleCooperative(nCount, lpHandlesWA, bWaitAll, timeout) {
    let result = this.waitMultiple(nCount, lpHandlesWA, bWaitAll, timeout);
    if (result !== 0xFFFF || (timeout >>> 0) !== 0xFFFFFFFF) return result;
    // Never recursively enter a worker instance that is already executing.
    if (this._runningThreadHandle) return 0xFFFFFFFF;
    ...
    while (spent < STEP_BUDGET) {
      ...
      const stats = this.runSlice(100000, { quantumSteps: 50000, maxWallMs: 12 });
      result = this.waitMultiple(nCount, lpHandlesWA, bWaitAll, 0);
  ...
  async spawnPending() {
    for (const pending of this._pendingThreads) {
      ...
      // CPUID feature advertisement is a global, so it is per-instance. A
      // worker that disagreed with the main thread about MMX would decode the
      // same code differently.
      if (main.get_cpu_mmx && instance.exports.set_cpu_mmx) {
        instance.exports.set_cpu_mmx(main.get_cpu_mmx());
      }
      ...
      if (main.get_free_list && instance.exports.set_free_list) {
        instance.exports.set_free_list(main.get_free_list());
      }
      if (main.get_heap_sparse_ptr && instance.exports.set_heap_sparse_ptr) {
        instance.exports.set_heap_sparse_ptr(main.get_heap_sparse_ptr());
      }
      if (main.get_heap_sparse_end && instance.exports.set_heap_sparse_end) {
        instance.exports.set_heap_sparse_end(main.get_heap_sparse_end());
      }
      if (main.get_virtual_alloc_top && instance.exports.set_virtual_alloc_top) {
        instance.exports.set_virtual_alloc_top(main.get_virtual_alloc_top());
      }
      ...
      if (this._callDllMain && this._loadedDlls.length) {
        ...
        // DllMain may allocate through the worker's instance-local heap
        // globals. Publish those updated cursors before either thread runs.
        main.set_heap_ptr(instance.exports.get_heap_ptr());
        if (main.set_free_list && instance.exports.get_free_list) {
          main.set_free_list(instance.exports.get_free_list());
        }
        if (main.set_heap_sparse_ptr && instance.exports.get_heap_sparse_ptr) {
          main.set_heap_sparse_ptr(instance.exports.get_heap_sparse_ptr());
        }
        if (main.set_heap_sparse_end && instance.exports.get_heap_sparse_end) {
          main.set_heap_sparse_end(instance.exports.get_heap_sparse_end());
        }
        if (main.set_virtual_alloc_top && instance.exports.get_virtual_alloc_top) {
          main.set_virtual_alloc_top(instance.exports.get_virtual_alloc_top());
        }
      }
      ...
      if (this._faultUnmapped && instance.exports.set_fault_unmapped) {
        instance.exports.set_fault_unmapped(this._faultUnmapped);
      }
      ...
      // Propagate --count= hit counters. Without this a probe on an address
      // only a worker thread ever executes reads back whatever the main
      // instance's untouched slot happens to hold, which looks like a wrong
      // count rather than an unarmed instance.
      if (this._countAddrs.length && instance.exports.set_count) {
        for (let i = 0; i < this._countAddrs.length; i++) {
          if (!Number.isFinite(this._countAddrs[i])) continue;  // unresolved module+0xVA
          instance.exports.set_count(i, this._countAddrs[i] >>> 0);
        }
      }
  ...
  runSlice(batchSize, options) {
    ...
    for (let slice = 0; slice < numSlices; slice++) {
      ...
      for (const [handle, thread] of this._threadEntries(threadOrderOptions)) {
        ...
        if (main.get_free_list && e.set_free_list) {
          e.set_free_list(main.get_free_list());
        }
        if (main.get_heap_sparse_ptr && e.set_heap_sparse_ptr) {
          e.set_heap_sparse_ptr(main.get_heap_sparse_ptr());
        }
        if (main.get_heap_sparse_end && e.set_heap_sparse_end) {
          e.set_heap_sparse_end(main.get_heap_sparse_end());
        }
        if (main.get_virtual_alloc_top && e.set_virtual_alloc_top) {
          e.set_virtual_alloc_top(main.get_virtual_alloc_top());
        }
        if (main.get_dll_count) {
          const dllCount = main.get_dll_count() | 0;
        ...
        try { e.run(sliceSize); } catch (err) {
          ...
          this._log(`[ThreadManager] Thread ${thread.tid} crashed at EIP=0x${e.get_eip().toString(16)} prev=0x${prev.toString(16)} prev2=0x${prev2.toString(16)} ESP=0x${e.get_esp().toString(16)} yield=${reason}: ${err.message}`);
        ...
        if (main.set_free_list && e.get_free_list) {
          main.set_free_list(e.get_free_list());
        }
        if (main.set_heap_sparse_ptr && e.get_heap_sparse_ptr) {
          main.set_heap_sparse_ptr(e.get_heap_sparse_ptr());
        }
        if (main.set_heap_sparse_end && e.get_heap_sparse_end) {
          main.set_heap_sparse_end(e.get_heap_sparse_end());
        }
        if (main.set_virtual_alloc_top && e.get_virtual_alloc_top) {
          main.set_virtual_alloc_top(e.get_virtual_alloc_top());
        }
        ...
        if (postYield === 2) {
          ...
          // The call is parked on its import thunk. Clear the yield after this
          // slice; another thread in the round can release the shared owner.
          if (this._traceThread && thread.waitPolls < 3) {
            const cs = e.get_wait_handle() >>> 0;
            const esp = e.get_esp() >>> 0;
  ...
  checkMainYield() {
    ...
    // runSlice(); the main instance needs an equivalent wall-clock gate here.
    ...
    if (result === 0xFFFF || result === 0x102) {
      ...
      if (
        ...
        // `this._now`, not `Date.now`: the host supplies the clock, and the
