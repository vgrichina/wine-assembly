...
const YIELD_NAMES = { 1: 'wait', 2: 'exit', 3: 'com_load_dll', 5: 'load_library', 6: 'modal_dialog', 7: 'message_wait', 8: 'net_wait', 9: 'critical_section' };
...
class ThreadManager {
  ...
  resolveWait(r, state, opts) {
    ...
    if (waitStackBytes === 24 && !r.waitHandlesPtr) {
      ...
      result = this.waitMultiple(r.waitHandle, r.waitHandlesPtr, false, 0);
    } else {
      ...
      for (const [handle, thread] of this._threadEntries(threadOrderOptions)) {
        ...
        try { e.run(sliceSize); } catch (err) {
          ...
          this._log(`[ThreadManager] Thread ${thread.tid} crashed at `
            ...
            + `\n  prev_eip=${this._describeAddr(e.get_dbg_prev_eip ? e.get_dbg_prev_eip() : 0)} `
  ...
  _dispatchCooperativeSend(target, request, depth) {
    ...
    for (let round = 0; round < 64; round++) {
      ...
      if (y) break;
      if (!(target.get_eip() >>> 0)) { result = target.thread_send_end() | 0; break; }
    }
