...
const YIELD_NAMES = { 1: 'wait', 2: 'exit', 3: 'com_load_dll', 4: 'help_load' };
...
class ThreadManager {
  constructor(wasmModule, memory, mainInstance, makeImports, opts) {
    ...
    this._maxWorkerThreads = 7; // fixed decoded-cache layout reserves T1..T7
  ...
  createThread(startAddr, param, stackSize) {
    if (this._pendingThreads.length + this.threads.size >= this._maxWorkerThreads) {
  ...
  runSlice(batchSize) {
    ...
    for (let slice = 0; slice < numSlices; slice++) {
      for (const [handle, thread] of this.threads) {
        ...
        if (thread.sleepUntil && Date.now() < thread.sleepUntil) continue;
        ...
        if (postYield === 2) {
          ...
          const ms = e.get_sleep_timeout ? (e.get_sleep_timeout() >>> 0) : 0;
          thread.sleepUntil = ms ? Date.now() + ms : 0;
          thread.sleepCount++;
        } else {
          thread.sleepUntil = 0;
          thread.sleepCount = 0;
  ...
  checkMainYield() {
    ...
    const waitStackBytes = e.get_wait_stack_bytes ? (e.get_wait_stack_bytes() | 0) : (waitHandlesPtr ? 20 : 12);
    let result;
    if (waitHandlesPtr && this._hasMessage && this._hasMessage()) {
