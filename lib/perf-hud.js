...
(function (global) {
  ...
  class PerfHud {
    constructor() {
      ...
      // x86 throughput. Presentation rate is deliberately not called game
    ...
    _draw() {
      ...
      if (snap.logicalFps) {
        ...
        ctx.fillText(`page ${snap.fps.toFixed(0)}  ${(snap.stepsPerSec / 1e6).toFixed(1)}M steps/s`, pad, 22);
      } else {
        ctx.fillStyle = snap.guestFps < 20 ? '#f87171' : '#4ade80';
        ctx.fillText(`PRESENT ${snap.guestFps.toFixed(1)}/s`, pad, 10);
        ctx.fillStyle = '#94a3b8';
        ctx.fillText(`page ${snap.fps.toFixed(0)}  ${(snap.stepsPerSec / 1e6).toFixed(1)}M steps/s`, pad + 102, 10);
      }
