...
(function (global) {
  ...
  class PerfHud {
    constructor() {
      ...
      // machine is running. These two are the guest's presentation rate and
      // retired basic-block throughput. Presentation rate is deliberately not called game
      // FPS: an application may present more than once per logical frame.
      this.guestFrames = [];      // ring of timestamps of presented guest frames
      this.guestBlocks = [];      // ring of {at, blocks}; blocks vary in instruction count
      this._blockTotal = 0;
      this.logicalFrameMetric = null;
      this.logicalFrameCounts = []; // ring of {at, count, verifier}
      // Pointer input, which the phase bars cannot see at all. A step
      // histogram can be perfect while the game still feels laggy, because
      // pointer-to-paddle latency is not made of step time: it is how long a
      // move sits in the queue before the guest's message pump takes it, and
      // how many moves the guest gets per second once it does. Those are the
      // two numbers that separate "the emulator is slow" from "the emulator
      // is fine and the input path is behind".
      this.inputMoves = [];       // ring of timestamps of enqueued pointer moves
      this.inputCoalesced = [];   // ring of timestamps of moves replaced while queued
      this.inputTaken = [];       // ring of {at, ageMs} for moves the guest consumed
      this._cur = null;
      this._lastStepEnd = 0;
      this._lastFrameAt = 0;
      this._rafHandle = 0;
    ...
    // One guest presentation boundary: an explicit DirectDraw/GL present, or
    // a non-empty canonical GDI surface flush. Called from host imports.
    guestFrame() {
    ...
    setLogicalFrameMetric(metric) {
      this.logicalFrameMetric = metric ? {
        label: String(metric.label || 'GAME').slice(0, 12) || 'GAME',
        verifier: !!metric.verifier,
      } : null;
      this.logicalFrameCounts.length = 0;
    }

    logicalFrameCount(count, verifier) {
      if (!this.enabled || !this.logicalFrameMetric) return;
      this.logicalFrameCounts.push({
        at: now(),
        count: count >>> 0,
        verifier: verifier == null ? null : verifier >>> 0,
      });
      if (this.logicalFrameCounts.length > HISTORY) this.logicalFrameCounts.shift();
    }

    // Actual retired blocks from the guest result, including zero-work slices.
    countBlocks(n) {
      if (!this.enabled || !Number.isFinite(n) || n < 0) return;
      this._blockTotal += n;
      this.guestBlocks.push({ at: now(), blocks: n, total: this._blockTotal });
      if (this.guestBlocks.length > HISTORY) this.guestBlocks.shift();
    }

    // Deprecated compatibility seam; the unit is blocks, never instructions.
    countSteps(n) { this.countBlocks(n); }

    // ---- pointer input seam (called from lib/renderer-input.js) ---------
    //
    // Three separate events, because collapsing them would hide the failure
    // mode. A move ENQUEUED that is later COALESCED away is a browser sample
    // the guest never saw -- normal and desirable in bulk, since Windows
    // coalesces too, but a high coalesce rate next to a low taken rate means
    // the guest is sampling the pointer far below the rate the hand is
    // moving it. A move TAKEN carries its queue age, which is the part of
    // pointer-to-photon latency that belongs to us rather than to the
    // display.
    inputMove(coalesced) {
      if (!this.enabled) return;
      const ring = coalesced ? this.inputCoalesced : this.inputMoves;
      ring.push(now());
    ...
    inputTake(queuedAt) {
      if (!this.enabled) return;
      this.inputTaken.push({ at: now(), ageMs: queuedAt > 0 ? Math.max(0, now() - queuedAt) : 0 });
      if (this.inputTaken.length > HISTORY) this.inputTaken.shift();
    }

    // Whether this step's worker budget ran out with work still pending.
    markThrottled(hit) {
      if (this._cur) this._cur.throttled = !!(this._cur.throttled || hit);
    }
    ...
    reset() {
      ...
      this.logicalFrameCounts.length = 0;
      this.guestFrames.length = 0;
      this.guestBlocks.length = 0;
      this._blockTotal = 0;
      this.inputMoves.length = 0;
      this.inputCoalesced.length = 0;
      this.inputTaken.length = 0;
      this._cur = null;
      this._lastStepEnd = 0;
      this._lastFrameAt = 0;
      if (this._streamSteps) this._streamSteps.length = 0;
      if (this._streamFrames) this._streamFrames.length = 0;
      this._streamGuest = 0;
      this._startedAt = now();
    }
    ...
    _counterRate(samples, key, windowMs = 2000) {
      const t = now();
      let newest = null, oldest = null;
      for (let i = samples.length - 1; i >= 0; i--) {
        const sample = samples[i];
        if (!newest) newest = sample;
        if (t - sample.at > windowMs) break;
        oldest = sample;
      }
      if (!newest || !oldest || newest === oldest) return 0;
      const span = newest.at - oldest.at;
      const delta = ((newest[key] >>> 0) - (oldest[key] >>> 0)) >>> 0;
      return span > 0 ? (delta * 1000) / span : 0;
    }
    ...
    snapshot() {
      ...
      const sampledAt = now();
      const recentBlocks = this.guestBlocks.filter(s => sampledAt - s.at <= 2000);
      const blockSpan = recentBlocks.length ? sampledAt - recentBlocks[0].at : 0;
      // The earliest sample is the interval boundary, not work performed
      // inside that interval. Counter differences stay correct when the ring
      // evicts old samples and when zero-work slices extend the interval.
      const blocksPerSec = recentBlocks.length >= 2 && blockSpan > 0
        ? (recentBlocks[recentBlocks.length - 1].total - recentBlocks[0].total) * 1000 / blockSpan : 0;
      const logicalRate = this._counterRate(this.logicalFrameCounts, 'count');
      const logicalVerifierRate = this.logicalFrameMetric && this.logicalFrameMetric.verifier
        ? this._counterRate(this.logicalFrameCounts.filter(s => s.verifier != null), 'verifier')
        : 0;
      return {
        ...
        // page FPS is compositor cadence; guestFps is retained as the external
        // field name for compatibility but measures guest presentations/s.
        // It need not equal an application's own logical animation-loop FPS.
        guestFps: this._rate(this.guestFrames),
        logicalFps: this.logicalFrameMetric ? {
          label: this.logicalFrameMetric.label,
          fps: logicalRate,
          verifierFps: logicalVerifierRate,
          mismatch: !!(this.logicalFrameMetric.verifier &&
            Math.abs(logicalRate - logicalVerifierRate) > Math.max(1, logicalRate * 0.05)),
        } : null,
        blocksPerSec,
        stepsPerSec: blocksPerSec, // Deprecated alias; this has units of blocks/s.
        // movesPerSec is the hand, takenPerSec is what the guest actually
        // sampled, and ageMs is how stale each sample was when it got there.
        // A paddle that trails the pointer shows up here and nowhere else.
        input: (() => {
          const ages = this.inputTaken.map(t => t.ageMs).sort((a, b) => a - b);
          return {
            movesPerSec: this._rate(this.inputMoves),
            coalescedPerSec: this._rate(this.inputCoalesced),
            takenPerSec: this._rate(this.inputTaken.map(t => t.at)),
            ageMs: { p50: percentile(ages, 50), p90: percentile(ages, 90), p99: percentile(ages, 99), max: ages.length ? ages[ages.length - 1] : 0 },
          };
        })(),
    ...
    _ensureDom() {
      ...
      if (!document.getElementById('perf-hud-style')) {
        ...
        const hudCanvasSelectors = [
          '#perf-hud canvas',
          '#screen-wrap:fullscreen #perf-hud canvas',
          '#screen-wrap:-webkit-full-screen #perf-hud canvas',
          'body.page-fullscreen #perf-hud canvas',
          'body.no-debug.exclusive-fullscreen #perf-hud canvas',
        ].join(',');
        style.textContent = '#perf-hud {'
          + 'position:absolute !important; top:6px !important; right:6px !important;'
          + 'left:auto !important; bottom:auto !important; z-index:40 !important;'
    ...
    _draw() {
      ...
      // Never an ambiguous bare "fps". A proven app-specific logical counter
      // gets the lead; otherwise PRESENT remains the guest-side readout.
      if (snap.logicalFps) {
        ctx.fillStyle = snap.logicalFps.mismatch ? '#f87171' : '#4ade80';
        ctx.fillText(`${snap.logicalFps.label} ${snap.logicalFps.fps.toFixed(1)}/s`, pad, 10);
        ctx.fillStyle = snap.guestFps < 20 ? '#f87171' : '#94a3b8';
        ctx.fillText(`PRESENT ${snap.guestFps.toFixed(1)}/s`, pad + 90, 10);
        ctx.fillStyle = '#94a3b8';
        ctx.fillText(`page ${snap.fps.toFixed(0)}  ${(snap.blocksPerSec / 1e6).toFixed(1)}M blocks/s`, pad, 22);
      } else {
        ctx.fillStyle = snap.guestFps < 20 ? '#f87171' : '#4ade80';
        ctx.fillText(`PRESENT ${snap.guestFps.toFixed(1)}/s`, pad, 10);
        ctx.fillStyle = '#94a3b8';
        ctx.fillText(`page ${snap.fps.toFixed(0)}  ${(snap.blocksPerSec / 1e6).toFixed(1)}M blocks/s`, pad + 102, 10);
      }
      ctx.fillStyle = blockedPct > 5 ? '#f87171' : '#94a3b8';
      ctx.fillText(`step p50 ${snap.stepMs.p50.toFixed(1)} p99 ${snap.stepMs.p99.toFixed(1)}ms  blocked ${blockedPct.toFixed(0)}%`, pad, 34);
      // Pointer line, only once there is pointer traffic to report -- an
      // idle row of zeros would just be noise on a keyboard-driven app.
      // Red when the guest samples the pointer at under half the rate the
      // hand is moving it, or when a move waits longer than a frame.
      if (snap.input.movesPerSec > 0) {
        const starved = snap.input.takenPerSec < snap.input.movesPerSec / 2;
        ctx.fillStyle = (starved || snap.input.ageMs.p99 > 16.7) ? '#f87171' : '#94a3b8';
        ctx.fillText(`mouse ${snap.input.movesPerSec.toFixed(0)}/s in`
          + ` ${snap.input.takenPerSec.toFixed(0)}/s taken`
          + `  age p50 ${snap.input.ageMs.p50.toFixed(1)} p99 ${snap.input.ageMs.p99.toFixed(1)}ms`, pad, 46);
      }
      // Which scheduler produced these numbers. Every timing here means
      // something different between the cooperative scheduler and real
      // threads, so a reading without the mode attached is not comparable.
      if (typeof window !== 'undefined' && window.WINE_THREADS) {
        const iso = typeof crossOriginIsolated !== 'undefined' && crossOriginIsolated;
        ctx.fillStyle = '#fbbf24';
        ctx.fillText(iso ? 'threads: isolated, cooperative sched' : 'threads: NOT isolated', pad, H - 26);
      }
      ...
      // The pointer line and optional logical-FPS line need reserved space:
      // a graph that moves up and down as app hints or mouse traffic appear is
      // unreadable.
      const g1y = 52, g1h = 50;
