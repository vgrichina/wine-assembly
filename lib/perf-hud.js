...
(function (global) {
  ...
  class PerfHud {
    ...
    _draw() {
      ...
      // PRESENT first, PAGE second, and never an ambiguous bare "fps". The
      // application's own counter remains authoritative for logical game FPS.
      ...
      if (snap.input.movesPerSec > 0) {
        ...
        ctx.fillText(`mouse ${snap.input.movesPerSec.toFixed(0)}/s in`
          ...
          + `  age p50 ${snap.input.ageMs.p50.toFixed(1)} p99 ${snap.input.ageMs.p99.toFixed(1)}ms`, pad, 34);
      }
      ...
      // Row 3 is the pointer line, so the bars start below it. The space is
      // reserved whether or not that line has anything to say -- a graph
      // that moves up and down as the mouse starts and stops is unreadable.
      const g1y = 40, g1h = 62;
