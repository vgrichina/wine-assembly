...
class Win98Renderer {
  ...
  repaint() {
    ...
    // Fill desktop, clipping out window client areas
    ctx.save();
    ctx.beginPath();
    ...
    for (const win of sorted) {
      if (win.clientRect) {
        ...
        ctx.rect(cr.x + cr.w, cr.y, -cr.w, cr.h); // winding-rule cutout
      }
    }
    ctx.clip('evenodd');
    ...
    // Draw non-client chrome for each window (WM_NCPAINT)
    for (const win of sorted) {
      this.drawWindow(win);
    }
  ...
  handleWheel(canvasX, canvasY, deltaY) {
