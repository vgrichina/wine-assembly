...
class Win98Renderer {
  ...
  _drawWatChildren(win, clientX, clientY) {
    ...
    // Route child WM_PAINT drawing into the parent window's back-canvas.
    // _activeChildDraw offsets are in back-canvas space (window-local), so
    // the subsequent repaint() blit composites everything coherently.
    const wc = this.getWindowCanvas(win.hwnd);
    if (!wc) return false;
    const bctx = wc.ctx;
    ...
    if (cr) {
      bctx.save();
      bctx.beginPath();
      bctx.rect(cr.x - win.x, cr.y - win.y, cr.w, cr.h);
      bctx.clip();
    }
    ...
    while ((slot = e.wnd_next_child_slot(win.hwnd, slot)) !== -1) {
      ...
      const cx2 = clientX + (xy & 0xFFFF) - win.x;
      const cy2 = clientY + ((xy >>> 16) & 0xFFFF) - win.y;
      // cls 1=Button 2=Edit 3=Static 4=ListBox 6=ColorGrid 7=ScrollBar —
      // all have WM_PAINT in their WAT wndproc; the WAT code composes via
      // gdi_* primitives, and _activeChildDraw pins those writes to the
      // parent's back-canvas at the child's (cx2, cy2).
      if ((cls === 1 || cls === 2 || cls === 3 || cls === 4 || cls === 6 || cls === 7) && e.send_message) {
        this._activeChildDraw = { canvas: wc.canvas, ctx: bctx, ox: cx2, oy: cy2, hwnd: h };
  ...
  drawWindow(win) {
    ...
    // Fill dialog client area with button face color. Stays on screen
    // canvas (not back-canvas) so it acts as a fallback visible only
    // through transparent areas of the back-canvas blit. Filling the
    // back-canvas here would wipe child paints from the guest's own
    // message loop (owner-draw buttons, custom controls) that happened
    // before this repaint cycle.
