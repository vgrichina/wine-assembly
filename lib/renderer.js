...
class Win98Renderer {
  ...
  createDialog(hwnd, parentHwnd) {
    ...
    // Walk controls. $dlg_load allocates ctrl hwnds contiguously from
    // dlgHwnd+1, so we can enumerate without a dedicated export.
    const CLASS_NAMES = { 1: 'Button', 2: 'Edit', 3: 'Static', 4: 'ListBox', 5: 'ComboBox', 6: 'ScrollBar' };
    if (!this.controlHwndMap) this.controlHwndMap = new Map();
    for (let i = 0; i < ctrlCount; i++) {
      const ctrlHwnd = hwnd + 1 + i;
      const classEnum = e.ctrl_get_class(ctrlHwnd);
      const ctrlId = e.ctrl_get_id(ctrlHwnd);
      const xyPacked = e.ctrl_get_xy(ctrlHwnd) >>> 0;
      const whPacked = e.ctrl_get_wh(ctrlHwnd) >>> 0;
      win.controls.push({
        hwnd: ctrlHwnd,
        id: ctrlId,
        className: CLASS_NAMES[classEnum] || '',
        type: '',
        text: '',
        x: xyPacked & 0xFFFF,
      ...
      this.controlHwndMap.set(ctrlHwnd, { dlgHwnd: hwnd, ctrlIndex: i, ctrlId });
    }
