...
class Win98Renderer {
  ...
  _computeSingleAppZoom(topLevels) {
    ...
    const boardOnly = bounded && topLevels.filter(w => w && w.w > 0 && w.h > 0).length === 1 && !menu;
    const source = zoomMode && (!bounded || boardOnly)
      ? this._zoomModeCrop(rect, inset, canvasW, canvasH) : rect;
    const transform = this._computeExclusiveTransform(source);
    if (!transform) return null;
    if (bounded) {
      ...
      const area = overlay && overlay.getBoardArea ? overlay.getBoardArea() : { x: 0, y: 0, w: 1, h: 1 };
