...
class Win98Renderer {
  ...
  createWindow(hwnd, style, x, y, cx, cy, title, menuId) {
    const win = {
      hwnd, style, title,
      x: Math.max(0, x === -2147483648 ? 20 : x), // CW_USEDEFAULT
      y: Math.max(0, y === -2147483648 ? 20 : y),
      w: cx === -2147483648 ? 400 : cx,
      h: cy === -2147483648 ? 300 : cy,
      visible: false,
      menu: null,
      controls: [],
      editText: '',
    ...
    if (menuId && this.resources && this.resources.menus) {
  ...
  drawWindow(win) {
    ...
    if (win.menu) {
      const mh = this.drawMenuBar(x + 3, cy, w - 6, win.menu);
      cy += mh;
    }
    ...
    // For notepad-style: draw edit area if no controls
    if (win.controls.length === 0 && !win.isDialog) {
