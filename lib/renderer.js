...
class Win98Renderer {
  ...
  _repaintOnce() {
    ...
    for (const win of sorted) {
      ...
      // Legacy JS edit fallback (for children not yet WAT-managed).
      // WAT-managed edit children paint via their own WM_PAINT dispatched
      // by the guest's message loop into the parent's back-canvas.
      if (win._backCanvas) {
        ...
        for (const child of Object.values(this.windows)) {
          if (child.parentHwnd === win.hwnd && child.visible && child.isEdit) {
            if (we3 && we3.ctrl_get_class && we3.ctrl_get_class(child.hwnd) === 2) continue;
