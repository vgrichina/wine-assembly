  ...
  constructor(canvas) {
    ...
    this._createOffscreen = (w, h) => {
      ...
      if (typeof OffscreenCanvas !== 'undefined') cvs = new OffscreenCanvas(w, h);
        ...
        cvs = new Canvas(w, h);
        // skia-canvas defaults to a GPU surface (Metal on macOS), which is
        // much faster and is what we want almost everywhere. But GPU surfaces
        // are driver allocations that our own churn does not bound: a long
        // animated run keeps handing skia new surfaces and the process grows
        // without limit. test-aoe-menu runs 328k batches and peaked at 6.4GB
        // RSS, dying with a bare status=1 and no crash marker; the same run on
        // raster peaks at 1.3GB and finishes.
        //
        // Raster costs roughly 3x the CPU (EmPipe: 12s -> 35s), so it is opt-in
        // per run via WA_CANVAS_GPU=0 rather than the default.
