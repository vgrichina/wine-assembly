...
class Win98Renderer {
  ...
  _usesOwnWindowSurface(win) {
    ...
    if (className === 'systreeview32' || className === 'syslistview32' ||
        className === 'toolbarwindow32' || className === 'msctls_statusbar32' ||
        className === 'msctls_trackbar32' || className === 'slider1') return true;
    const e = (win.wasm || this.wasm) && (win.wasm || this.wasm).exports;
    try {
      const cls = e && e.ctrl_get_class ? (e.ctrl_get_class(win.hwnd) | 0) : 0;
      return cls === 1 || cls === 5 || cls === 8 || cls === 18 || cls === 19 || cls === 21;
    } catch (_) {
      return false;
    }
  }
    ...
    try {
      for (const child of children) {
        ...
        if (ownSurface) {
          ...
          this._ensureCommonControlFallbackSurface(child);
  ...
  _ensureStatusBarFallbackSurface(win) {
    const className = String(win && win.className || '').toLowerCase();
    if (className !== 'msctls_statusbar32' || win.w <= 0 || win.h <= 0) return;
    const text = String(win.title || '');
    const wc = this.getWindowCanvas(win.hwnd);
    if (!wc || !wc.ctx) return;
    const ctx = wc.ctx;
    const w = Math.max(1, win.w | 0);
    ...
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = '#808080';
    ctx.fillRect(2, 2, Math.max(0, w - 4), 1);
    ctx.fillRect(2, 2, 1, Math.max(0, h - 4));
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(2, h - 3, Math.max(0, w - 4), 1);
    ctx.fillRect(w - 3, 2, 1, Math.max(0, h - 4));
    ctx.save();
    ctx.beginPath();
    ctx.rect(4, 3, Math.max(0, w - 8), Math.max(0, h - 6));
    ctx.clip();
    ctx.fillStyle = '#000000';
    ctx.font = this.font;
    ctx.textAlign = 'left';
    ctx.textBaseline = 'top';
    ctx.fillText(text, 5, Math.max(2, Math.floor((h - 13) / 2)));
    ctx.restore();
  }

  _ensureCommonControlFallbackSurface(win) {
    this._ensureListViewFallbackSurface(win);
    this._ensureStatusBarFallbackSurface(win);
  }
  ...
  _paintCaretOverlay() {
    ...
    for (const wasm of wasms) {
      ...
      if (!visible || !hwnd) {
        this._caretBlinkState.set(wasm, { key: 'hidden', start: this._profileNow() });
        continue;
      }
      ...
      if (!state || state.key !== blinkKey) {
        state = { key: blinkKey, start: now };
        this._caretBlinkState.set(wasm, state);
      }
      const blinkOn = (Math.floor(Math.max(0, now - state.start) / this._caretBlinkMs) & 1) === 0;
      if (!blinkOn) continue;
    ...
    this._scheduleCaretBlink(activeCaret);
  }
