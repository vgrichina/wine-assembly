...
class Win98Renderer {
  ...
  drawWindow(win) {
    ...
    for (const child of Object.values(this.windows)) {
      if (child.parentHwnd === win.hwnd && child.visible && child.isEdit) {
        ...
        this.drawEditArea(clientX + child.x, clientY + child.y, ew, eh, child.editText);
        hasEditChild = true;
      }
    }
  ...
  handleMouseDown(canvasX, canvasY) {
    ...
    for (const win of Object.values(this.windows)) {
      ...
      if (win.hasCaption) {
        const bw = 16, bh = 14, by = win.y + 3 + 2;
        const closeX = win.x + 3 + win.w - 6 - bw - 2;
        if (canvasX >= closeX && canvasX < closeX + bw && canvasY >= by && canvasY < by + bh) {
