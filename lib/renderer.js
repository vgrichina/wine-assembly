  ...
  _drawWatChildren(win, clientX, clientY) {
    ...
    while ((slot = e.wnd_next_child_slot(win.hwnd, slot)) !== -1) {
      ...
      if (cls === 1) {
        ...
        // ColorGrid: 8 cols × 3 rows, 24x20 cells. Selection is read from
        // ColorGridState[0] (sel_idx). Colors are 0x00BBGGRR COLORREFs
        // matching Win32 convention — we swap to CSS rgb(r,g,b).
        const ctx = this.ctx;
        const selIdx = e.colorgrid_get_sel ? e.colorgrid_get_sel(h) : -1;
        const CELL_W = 24, CELL_H = 20;
        for (let row = 0; row < 3; row++) {
          for (let col = 0; col < 8; col++) {
            const idx = row * 8 + col;
            const cr = e.colorgrid_color ? e.colorgrid_color(idx) : 0;
            const r = cr & 0xFF, g = (cr >>> 8) & 0xFF, b = (cr >>> 16) & 0xFF;
            const rx = cx2 + col * CELL_W;
            const ry = cy2 + row * CELL_H;
            ctx.fillStyle = `rgb(${r},${g},${b})`;
            ctx.fillRect(rx + 1, ry + 1, CELL_W - 2, CELL_H - 2);
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 1;
            ctx.strokeRect(rx + 0.5, ry + 0.5, CELL_W - 1, CELL_H - 1);
            if (idx === selIdx) {
              // Heavy selection border
              ctx.strokeStyle = '#fff';
              ctx.lineWidth = 2;
        ...
        // ListBox: inset frame, white background, item rows. Selected row
        // gets the system highlight (blue + white text). Item height = 16px.
        const ctx = this.ctx;
        this.drawInsetBorder(cx2, cy2, ww, hh);
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(cx2 + 2, cy2 + 2, ww - 4, hh - 4);
        const count  = e.listbox_get_count(h);
        const selIdx = e.listbox_get_cur_sel(h);
        const topIdx = e.listbox_get_top_index(h);
        const rowH   = 16;
        const innerW = ww - 4;
        const innerH = hh - 4;
        const visible = Math.floor(innerH / rowH);
        ...
        for (let row = 0; row < visible; row++) {
          ...
          if (idx >= count) break;
          const ry = cy2 + 2 + row * rowH;
          const n = e.listbox_get_item_text(h, idx, scratchG, 255);
          let s = '';
          for (let i = 0; i < n; i++) s += String.fromCharCode(u8[scratchWa + i]);
          if (idx === selIdx) {
            ctx.fillStyle = '#000080';
            ctx.fillRect(cx2 + 2, ry, innerW, rowH);
            ctx.fillStyle = '#ffffff';
          } else {
            ctx.fillStyle = '#000000';
          }
          ctx.fillText(s, cx2 + 4, ry + rowH / 2 + 1);
        }
      }
      drewAny = true;
    }
