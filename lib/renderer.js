...
class Win98Renderer {
  ...
  drawWindow(win) {
    ...
    // Only redraw chrome when window is dirty (creation, resize, title
    // change, etc.). The back-canvas persists between repaints, so chrome
    // drawn previously is still there. repaint() just blits.
    if (hasBorder && this.dirtyWindows.has(win.hwnd)) {
      let cy = y + 3;
