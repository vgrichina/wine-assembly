...
class Win98Renderer {
  constructor(canvas) {
    ...
    if (!this._isNode && typeof setInterval !== 'undefined') {
      this._caretBlinkTimer = setInterval(() => this._tickCaretBlink(), 530);
    }
  ...
  _tickCaretBlink() {
    ...
    if (!e || !e.get_focus_hwnd || !e.ctrl_get_class || !e.send_message) return;
    const hwnd = e.get_focus_hwnd() | 0;
    if (!hwnd || (e.ctrl_get_class(hwnd) | 0) !== 2) return;
    e.send_message(hwnd, 0x000F, 0, 0); // WM_PAINT: repaint edit/caret only
  ...
  _profileInput(label, data) {
    ...
    const profile = {
      ...
      data: data || {},
      t0: now,
      marks: [{ name: 'browser-input', t: now, data: data || {} }],
    };
  ...
  _computeClientRect(win) {
    // Phase 3: prefer WAT-computed client rect (written by $defwndproc_do_nccalcsize).
    // WAT stores window-local l/t/r/b; JS stores screen coords, so add win.x/win.y.
    ...
    const hasBorder = !!(win.style & 0x00C00000) || !!(win.style & 0x00800000);
  ...
  getWindowCanvas(hwnd) {
    ...
    if (!win._backCanvas || win._backW !== w || win._backH !== h) {
      ...
      win._backW = w;
      win._backH = h;
      const borderlessTopLevel = !win.isChild && !(win.style & 0x00C00000) && !this._hasMenuBar(win);
  ...
  _defwndprocNcpaint(win, x, y, w, h) {
    ...
    // ncpaint uses whole-window DC (hwnd + 0xC0000), so _getDrawTarget
    // routes to the back-canvas at ox=0,oy=0 automatically — no
    // _activeChildDraw override needed.
    this.getWindowCanvas(win.hwnd);  // ensure back-canvas exists
