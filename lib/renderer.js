...
class Win98Renderer {
  ...
  _isExclusiveFullscreenWindow(win) {
    ...
    // Everything below is a *guess* from window geometry, and a guess that
    // fires on an ordinary GDI app blacks out the whole page: the compositor
    // fills the canvas with #000, and body.no-debug.exclusive-fullscreen hides
    // the taskbar and the desktop icons with !important. Maximized Notepad is
    // exactly that shape. So the shape only counts for a window DirectDraw is
    // actually presenting into -- a game that took the primary surface without
    // DDSCL_EXCLUSIVE still gets the fullscreen treatment, and nothing that
    // draws through GDI ever does.
    if (!win._dxFrameLayer) return false;
    const hasCaption = this._hasCaption(win);
  ...
  caretRect() {
    return this._focusCaretRect || null;
  }
