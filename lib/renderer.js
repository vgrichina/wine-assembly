...
class Win98Renderer {
  ...
  _repaintOnce() {
    ...
    for (const win of sorted) {
      // If window has a full-window back canvas (from GetWindowDC drawing),
      // blit it at window position — it replaces standard chrome
      if (win._fullBackCanvas) {
        ctx.drawImage(win._fullBackCanvas, win.x, win.y);
      } else {
        this.drawWindow(win);
      }
      // Blit per-window client-area canvas onto display canvas
