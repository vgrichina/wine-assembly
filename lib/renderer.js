...
class Win98Renderer {
  ...
  repaint() {
    ...
    // If a menu was covering client area, restore the saved pixels first
    for (const win of sorted) {
      if (win._savedClient) {
        const cr = win.clientRect;
        ctx.putImageData(win._savedClient, cr.x, cr.y);
        win._savedClient = null;
      }
    }
    ...
    if (this._openMenu) {
      // Save client area before menu overwrites it
      for (const win of sorted) {
        if (win.clientRect && win.hwnd === this._openMenu.winHwnd) {
          const cr = win.clientRect;
          if (cr.w > 0 && cr.h > 0) {
            win._savedClient = ctx.getImageData(cr.x, cr.y, cr.w, cr.h);
          }
        }
      }
