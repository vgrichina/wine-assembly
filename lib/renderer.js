...
class Win98Renderer {
  ...
  createWindow(hwnd, style, x, y, cx, cy, title, menuId) {
    ...
    if (this.resources && this.resources.menus) {
      // CreateWindowExA in WAT now resolves the class lpszMenuName when the
      // explicit hMenu arg is 0, so menuId already reflects the class menu
      // (for apps that wire it via WNDCLASSA). No JS-side guessing — apps
      // like Winamp that have menu resources only for popup TrackPopupMenu
      // use MUST NOT get a menu bar auto-attached.
      if (menuId && this.resources.menus[menuId]) {
        win._menuId = menuId;
      }
    }
  ...
  createDialog(hwnd, dlgId, parentHwnd) {
    ...
    if (dlg.menu && this.resources.menus && this.resources.menus[dlg.menu]) {
      win._menuId = dlg.menu;
  ...
  setMenu(hwnd, menuResId) {
    ...
    if (win && this.resources && this.resources.menus) {
      win._menuId = this.resources.menus[menuResId] ? menuResId : 0;
      this._setWatMenu(win);
      this.invalidate(hwnd);
    }
  }
  ...
  scheduleRepaint() {
    ...
    // Re-entrancy guard: _getDrawTarget calls scheduleRepaint from inside
    // gdi_draw_text, which may itself run inside an in-flight repaint (e.g.
    // a paint triggers the app to draw more text into a different window).
    // In Node mode scheduleRepaint() calls repaint() synchronously, which
    // would recurse. Mark the request as pending and let the outer repaint
    // re-drain at its end.
    if (this._repainting) {
  ...
  _repaintOnce() {
    ...
    for (const win of sorted) {
      ...
      if (win._backCanvas && win.clientRect) {
        const cr = win.clientRect;
        if (process.env.DBG_BLIT) console.log(`[blit] hwnd=0x${win.hwnd.toString(16)} cr=${cr.x},${cr.y},${cr.w}x${cr.h} back=${win._backW}x${win._backH}`);
        ctx.drawImage(win._backCanvas, cr.x, cr.y);
      } else if (process.env.DBG_BLIT) {
        console.log(`[blit] hwnd=0x${win.hwnd.toString(16)} NO BACK (back=${!!win._backCanvas}, cr=${!!win.clientRect})`);
      }
  ...
  _menuPaintDropdown() {
    const e = this.wasm && this.wasm.exports;
    if (!e || !e.menu_paint_dropdown || !e.menu_open_hwnd) return;
  ...
  _drawWatChildren(win, clientX, clientY) {
    ...
    while ((slot = e.wnd_next_child_slot(win.hwnd, slot)) !== -1) {
      ...
      if (cls === 1) {
        ...
        // ListBox / ColorGrid — both have a WM_PAINT handler in their
  ...
  drawWindow(win) {
    ...
    // The WAT side fills the four btnFace strips, draws the outset border,
    // then composes the caption gradient + sysbuttons + title text via
    // gdi_* host primitives. Falls back to the legacy JS path if the WAT
    // export is unavailable (e.g. wasm not yet instantiated).
    let cy = y + 3;
    ...
    for (const child of Object.values(this.windows)) {
      if (child.parentHwnd === win.hwnd && child.visible && child.isEdit) {
        const ew = Math.min(child.w || clientW, clientW - child.x);
        const eh = Math.min(child.h || clientH, clientH - child.y);
        // WAT-managed edit: dispatch WM_PAINT to WAT wndproc using JS dimensions
        console.log('[drawWindow] edit child hwnd=0x'+child.hwnd.toString(16), 'ew='+ew, 'eh='+eh, 'cls='+((we2&&we2.ctrl_get_class)?we2.ctrl_get_class(child.hwnd):'?'));
        if (we2 && we2.ctrl_get_class && we2.ctrl_get_class(child.hwnd) === 2 && we2.send_message) {
          // Update ctrl_geom from JS dimensions so WAT WM_PAINT knows the size
          ...
          try { we2.send_message(child.hwnd, 0x000F, 0, 0); }
          finally { this._activeChildDraw = null; }
          hasEditChild = true;
          continue;
        }
