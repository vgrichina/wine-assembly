  ...
  _paintWallpaper(ctx) {
    ...
    if (!this._wallpaperTiled) {
      ctx.drawImage(wallpaper,
        Math.floor((this.canvas.width - wallpaper.width) / 2),
        Math.floor((this.canvas.height - wallpaper.height) / 2));
      return true;
    }
    ...
    for (let y = 0; y < this.canvas.height; y += wallpaper.height) {
      for (let x = 0; x < this.canvas.width; x += wallpaper.width) {
