...
class Win98Renderer {
  ...
  _isExclusiveFullscreenWindow(win) {
    ...
    if (this._dxExclusiveHwnd(win) === win.hwnd) return true;
  ...
  _computeExclusivePresentationViewport(transform, cropFromSource) {
    ...
    const physicalFit = Math.min(outputW / nativeW, outputH / nativeH);
    const multiplier = physicalFit >= 1 ? Math.max(1, Math.floor(physicalFit)) : 0;
    let dstW;
    let dstH;
    if (mode === 'integer' && multiplier > 0) {
      dstW = nativeW * multiplier;
      dstH = nativeH * multiplier;
    } else if (outputW * nativeH <= outputH * nativeW) {
      dstW = outputW;
    ...
    return {
      ...
      dstY: Math.floor((outputH - dstH) / 2),
      dstW,
      dstH,
      outputW,
      outputH,
  ...
  _computeSingleAppZoom(topLevels) {
    ...
    if (rect.w >= canvasW && rect.h >= canvasH) return null;
    const transform = this._computeExclusiveTransform(rect);
    if (!transform) return null;
    return {
      transform,
      viewport: this._computeExclusivePresentationViewport(transform, true),
    };
  }
  ...
  _repaintOnce() {
    ...
    if (exclusive && top) {
      this._exclusiveTransform = this._computeExclusiveTransform(top);
  ...
  caretRect() {
    if (this._focusCaretRect) return this._focusCaretRect;
    return this._nativeEditCaretRect();
  }

  // The WAT-native EDIT owns and blinks its own caret and never calls
  // CreateCaret/ShowCaret, so the USER caret globals stay zero while the user
  // types in Notepad or WordPad. It publishes the rect it painted instead;
  // this reads that, in the same canvas pixels caretRect() promises. Nothing
  // is drawn from it -- the control already drew the caret -- so this must
  // stay out of _compositeCarets.
  _nativeEditCaretRect() {
    for (const win of Object.values(this.windows || {})) {
      const e = win && win.wasm && win.wasm.exports;
      if (!e || !e.get_edit_caret_hwnd) continue;
      let hwnd = 0;
      try { hwnd = e.get_edit_caret_hwnd() >>> 0; } catch (_) { continue; }
      if (!hwnd || hwnd !== (win.hwnd >>> 0) || !win.visible) continue;
      const origin = this._windowClientOriginOnCanvas(win);
      if (!origin) continue;
      try {
        return {
          x: origin.x + (e.get_edit_caret_x() | 0),
          y: origin.y + (e.get_edit_caret_y() | 0),
          w: Math.max(1, e.get_edit_caret_w() | 0),
          h: Math.max(1, e.get_edit_caret_h() | 0),
        };
      } catch (_) { continue; }
    }
    return null;
  ...
  _scheduleCaretBlink(active) {
    ...
    this._caretBlinkTimer = setTimeout(() => {
      ...
      else this._scheduleCaretBlink(true);
    }, this._caretBlinkMs);
  ...
  _openMenuGeometry() {
    ...
    const W = 180;
    const rects = [];
    if (height > 0) rects.push({ x: dx, y: dy, w: W, h: height });
    if (hover >= 0 && e.menu_child_sub_count) {
      const count = e.menu_child_sub_count(hwnd, top, hover) | 0;
      if (count > 0) rects.push({ x: dx + W, y: dy + 2 + hover * 20, w: W, h: count * 20 + 4 });
    }
