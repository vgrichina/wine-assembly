...
class Win98Renderer {
  ...
  repaint() {
    ...
    // Fill desktop, clipping out window rects (system only paints exposed desktop)
    ctx.save();
    ctx.beginPath();
    ...
    for (const win of sorted) {
      // Cut out client area from desktop fill (non-client is redrawn anyway)
      if (win.clientRect) {
  ...
  handleKeyDown(vkCode) {
    ...
    if (vkCode === 27) { // Escape
      if (this._openMenu) { const wh = this._openMenu.winHwnd; this._openMenu = null; this._menuMode = false; this.queuePaint(wh); this.repaint(); return; }
    ...
    if (this._openMenu) {
      ...
      if (items.length && (vkCode === 40 || vkCode === 38)) { // Down/Up
        ...
        if (h >= 0 && items[h] && !items[h].separator) {
          const sub = items[h];
          if (this._handleEditCommand(sub.id)) { const wh = this._openMenu.winHwnd; this._openMenu = null; this.queuePaint(wh); this.repaint(); return; }
          ...
          const wh = this._openMenu.winHwnd; this._openMenu = null; this.queuePaint(wh); this.repaint(); return;
        }
