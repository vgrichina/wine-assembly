...
class Win98Renderer {
  ...
  _workerPublicationHeld() {
    return this._workerGuestSliceDepth > 0 || this._workerGdiPaintDepth > 0;
  }

  beginWorkerGdiPaint(hwnd) {
    // Cooperative execution already makes the whole slice atomic by blocking
    // the browser. Only a brokered Worker import needs a cross-slice hold.
    if (this._workerGuestSliceDepth <= 0) return;
    const key = hwnd >>> 0;
  ...
  repaint() {
    if (this._workerGdiPaintDepth > 0) {
      this._repaintScheduled = true;
      this._workerRepaintDeferred = true;
      return;
    }
