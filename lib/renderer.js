...
class Win98Renderer {
  ...
  showFindDialog(hwnd, ownerHwnd, frGuestAddr) {
    // The dialog parent window is created here so the renderer has a frame
    // to draw the title bar / borders around. All children (Static label,
    // Edit, checkbox, groupbox, radios, Find Next, Cancel) live entirely
    // on the WAT side now — drawn via _drawWatChildren, hit-tested via
    // wnd_next_child_slot, and reacted to by $findreplace_wndproc. The
    // JS controls[] used to mirror the WAT state but became redundant
    // after the WAT click chain landed; controls[] stays empty.
    const w = 340, h = 128;
    ...
    this.windows[hwnd] = {
      ...
      visible: true, isChild: false, menu: null, controls: [],
      isDialog: true, isFindDialog: true,
