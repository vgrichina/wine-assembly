...
class Win98Renderer {
  constructor(canvas) {
    ...
    this._createOffscreen = (w, h) => {
      ...
      else try { const { createCanvas: cc } = require('canvas'); cvs = cc(w, h); }
        catch (e) { return null; }
  ...
  getWindowCanvas(hwnd) {
    ...
    if (!win._backCanvas || win._backW !== w || win._backH !== h) {
      ...
      win._backW = w;
      win._backH = h;
      // Fill opaque COLOR_3DFACE grey so (a) screensavers that BitBlt from
      // the desktop DC read meaningful opaque pixels instead of alpha-0, and
      // (b) dialog-style apps (e.g. calc) whose WM_ERASEBKGND only paints
      // some sub-rects don't show black bleed-through between buttons.
      win._backCtx.fillStyle = '#c0c0c0';
