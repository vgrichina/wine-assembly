...
class Win98Renderer {
  ...
  handleKeyDown(vkCode) {
    if (vkCode === 18) { this._altDown = true; return; } // Alt
    if (vkCode === 27 && this._openMenu) { this._openMenu = null; this.repaint(); return; } // Escape

    // Alt+letter: open menu by accelerator key
    if (this._altDown && vkCode >= 65 && vkCode <= 90) {
      const ch = String.fromCharCode(vkCode);
      // Find a top-level window with menu rects
      for (const w of Object.values(this.windows)) {
  ...
  handleKeyUp(vkCode) {
    ...
    if (vkCode === 18) this._altDown = false;
  }
