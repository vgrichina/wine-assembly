...
class Win98Renderer {
  ...
  createWindow(hwnd, style, x, y, cx, cy, title, menuId) {
    ...
    if (this.resources && this.resources.menus) {
      if (menuId) {
        ...
        if (win.menu) win._menuId = menuId;
        ...
        if (keys.length) {
          ...
          win._menuId = parseInt(keys[0], 10) || 0;
    ...
    if (win.menu) this._setWatMenu(win);
    return hwnd;
  }
  ...
  setMenu(hwnd, menuResId) {
    ...
    if (win && this.resources && this.resources.menus) {
      ...
      win._menuId = win.menu ? menuResId : 0;
      this._setWatMenu(win);
      this.invalidate(hwnd);
    }
  }
  ...
  repaint() {
    ...
    if (this._openMenu) {
      this._menuPaintDropdown(this._openMenu);
    }
  ...
  _menuPaintBar(win, x, y, w) {
    ...
    const openIdx = (this._openMenu && this._openMenu.winHwnd === win.hwnd)
      ? this._openMenu.index : -1;
  ...
  // Paint the open dropdown via WAT (replaces _drawDropdown).
  _menuPaintDropdown(menu) {
    const e = this.wasm && this.wasm.exports;
    if (!e || !e.menu_paint_dropdown) return false;
    const dx = menu.x;
    const dy = menu.y + menu.h;
    this._activeChildDraw = { canvas: this.canvas, ctx: this.ctx, ox: 0, oy: 0, hwnd: menu.winHwnd };
    try {
      e.menu_paint_dropdown(menu.winHwnd, menu.index, dx, dy,
                             menu.hoverIndex == null ? -1 : menu.hoverIndex);
    } finally { this._activeChildDraw = null; }
    return true;
  }
