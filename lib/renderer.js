...
class Win98Renderer {
  ...
  getWindowCanvas(hwnd) {
    ...
    if (!win._backCanvas || win._backW !== w || win._backH !== h) {
      ...
      win._backW = w;
      win._backH = h;
      // Fill opaque black so screensavers that BitBlt from the desktop DC
      // (which resolves to this canvas) read meaningful pixels instead of
      // transparent. Regular apps overwrite this in WM_ERASEBKGND / WM_PAINT.
      win._backCtx.fillStyle = '#000';
