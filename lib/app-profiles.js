...
// Compatibility patches can touch DLL or sparse guest addresses. Keep their
// translation policy shared with the emulator instead of assuming that every
// address is image-relative.
var _memUtilsProfiles = typeof require !== 'undefined' ? require('./mem-utils')
...
const EXE_PATCHES = {
  // Atomic Bomberman alpha demo (_BOMB.EXE SHA-256
  // 0ff14a352d6626660ceb66ea0e6743cd33c457e754cfd5705120bacae0530638).
  //
  // The executable deliberately accepts only 1997-04-18 through 1997-06-17
  // and otherwise shows "Software expired. Contact Interplay." Wine-Assembly's
  // deterministic Win32 calendar is 2000-01-01, so the unmodified alpha
  // expires even though its gameplay remains usable. Extend only the upper
  // bound for this byte-verified build; changing the process-wide clock would
  // alter every application's date handling and saved data.
  '_bomb.exe': [
    {
      key: 'atomic-bomberman-alpha-expiry',
      addr: 0x00416e42,
      expected: [0xe7, 0xf2, 0xa6, 0x33], // 1997-06-17 20:26:15 UTC
      replacement: [0xff, 0xff, 0xff, 0x7f],
      label: 'Atomic Bomberman alpha upper expiry date',
    },
  ],
...
function cleanupWinampVisualizerThread(info, exports, memoryBuffer, log) {
  ...
  const g2w = ptr => _memUtilsProfiles.guestToWasm(ptr, exports, memoryBuffer, imageBase);
  const read32 = (ptr) => {
    const wa = g2w(ptr);
...
function applyBytePatches(patches, exports, memoryBuffer, opts) {
  ...
  for (const patch of patches) {
    ...
    const wa = _memUtilsProfiles.guestToWasm(patch.addr, exports, memoryBuffer, imageBase);
...
// Presentation-only crops for exact legacy executables. These do not change
// guest geometry or pixels: they describe a temporary sub-frame that an app
// deliberately centers inside its exclusive display. Alpha Centauri's TQI
// opening is 400x192 inside a 640x480 primary; once the game enters its menu,
// the backing surface changes size and the crop stops matching automatically.
const EXCLUSIVE_CROPS = {
  'terran.exe': { sourceW: 640, sourceH: 480, x: 120, y: 144, w: 400, h: 192 },
};
...
const appProfilesApi = {
  ...
  LAUNCH_PREFS, applyLaunchPreferences, EXCLUSIVE_CROPS,
};
