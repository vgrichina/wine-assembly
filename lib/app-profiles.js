...
const LAUNCH_PREFS = {
  ...
  // Custom resolutions, too. video_init (0x401220) is a jump table whose cases
  // are all the same shape — `push <height imm32>; push <width imm32>; push 2;
  // call 0x405ee2` and then two `mov dword [ebp-N], <same imm32>` — so the
  // 1024x768 case at 0x40136f can be re-pointed at any size, which is how the
  // browser window (16:10, 1512x850, whatever it is) becomes a real mode. Its
  // DirectDraw counterpart is slot 5 of $enum_modes_dispatch, the host screen
  // itself: RCT compares the mode it wants against the list EnumDisplayModes
  // gave it and refuses an exact-match miss, so both halves have to agree, and
  // both round the same way (width to a multiple of 4, height to 2).
  'rct.exe': ({ screenW, screenH }) => {
    const le32 = (n) => [n & 0xff, (n >>> 8) & 0xff, (n >>> 16) & 0xff, (n >>> 24) & 0xff];
    // Rounded and clamped exactly as `$enum_mode_host_w`/`_h` in
    // src/09a8-handlers-directx.wat do it — the game will only accept a mode we
    // also enumerated, so if these two disagree by a pixel it silently falls
    // back down the ladder. Both limits are measured: RCT repaints in 8-row
    // blocks (a 850-tall mode leaves the last two rows black forever), and past
    // 1280 wide its engine simply stops drawing, which is also the widest mode
    // it ships.
    const clamp = (v, lo, hi) => Math.min(hi, Math.max(lo, v & ~7));
    const custom = {
      w: clamp(screenW, 640, 1280),
      h: clamp(screenH, 480, 1024),
    };
    // Worth re-pointing the case only when the screen is bigger than the stock
    // 800x600 *and* is not already 1024x768. Below that, the stock ladder is
    // right and the case is left alone.
    const useCustom = (custom.w > 800 || custom.h > 600) &&
      !(custom.w === 1024 && custom.h === 768);
    const index = useCustom ? 3
      : (custom.w >= 1024 && custom.h >= 768) ? 3
      : (custom.w >= 800 && custom.h >= 600) ? 2
      : 1;
    const patches = [{
      key: 'rct-resolution',
      addr: 0x0056fd6b,
      expected: [0xff],
      replacement: [index],
      label: `RollerCoaster Tycoon startup resolution = ${
        useCustom ? `${custom.w}x${custom.h}`
          : ['windowed 640x480', '640x480', '800x600', '1024x768'][index]}`,
    }];
    if (useCustom) {
      // The four immediates of the 1024x768 case: pushed height, pushed width,
      // and the two the case keeps for itself in its own frame.
      for (const [addr, was, now] of [
        [0x0040137c, 768, custom.h],
        [0x00401381, 1024, custom.w],
        [0x00401395, 1024, custom.w],
        [0x0040139c, 768, custom.h],
      ]) {
        patches.push({
          key: 'rct-custom-mode',
          addr,
          expected: le32(was),
          replacement: le32(now),
          label: `RollerCoaster Tycoon video_init case 3 ${was} -> ${now}`,
        });
      }
    }
    return patches;
