...
const LAUNCH_PREFS = {
  ...
  //
  // Only the default. An earlier version of this hook also re-pointed the four
  // immediates of video_init's 1024x768 case (0x40137c/81/95/9c) at the canvas
  // size, to get a mode that filled the browser window exactly. But that mode
  // has to *take over* one of the four rows RCT's Display Mode dropdown offers
  // (Window / 640x480 / 800x600 / 1024x768), and it took the largest — so
  // "Full Screen 1024x768" silently meant something else and true 1024x768 was
  // unreachable. $enum_modes_dispatch advertises the whole stock list now, so
  // all four rows do what they say and a mode larger than the canvas is scaled
  // to fit when it is presented.
  'rct.exe': ({ screenW, screenH }) => {
    // The largest stock mode that fits; RCT's own ladder handles the rest.
