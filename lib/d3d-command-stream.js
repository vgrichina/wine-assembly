...
(function (root, factory) {
  ...
  const CTRL = { READY: 0, COMPLETED: 1, ERROR: 2, SUBMITTED: 3 };
  ...
  class Encoder {
    constructor(options) {
      ...
      this.buffers = Array.from({ length: options.bufferCount || DEFAULT_BUFFERS }, () => ({
        buffer: new SharedArrayBuffer(this.capacity), used: 0, commands: 0, seq: 0,
      }));
      this.current = 0;
      this.lastSubmitted = 0;
      this.stats = { queued: 0, submissions: 0, fences: 0, waits: 0, fallbacks: 0 };
    ...
    enqueue(descriptorWa) {
      ...
      const descriptor = new DataView(memory, descriptorWa, 24);
      const thisGuest = descriptor.getUint32(0, true);
      const primitive = descriptor.getUint32(4, true);
      const vertexType = descriptor.getUint32(8, true);
      const verticesGuest = descriptor.getUint32(12, true);
      const count = descriptor.getUint32(16, true);
      const stateGuest = descriptor.getUint32(20, true);
      ...
      bytes.set(new Uint8Array(memory, stateWa, STATE_BYTES), start + HEADER_BYTES);
      bytes.set(new Uint8Array(memory, verticesWa, vertexBytes),
