...
(() => {
  ...
  // Is the drive loop actually parked on THIS device?
  //
  // The host now sleeps between guest slices instead of re-arming a
  // MessageChannel as fast as the browser will run one, and every measurement
  // of that so far was taken on a desktop with `ps`. iOS has no `ps`, and
  // Safari's per-tab figure in Activity Monitor bundles a GPU process shared
  // with every other tab -- measured on this Mac, that process sits at ~100%
  // of a core with our tab on about:blank, so it is not evidence about us at
  // all. So the page counts its own scheduling instead: an idle app that is
  // parking shows single-digit steps/s, and the pre-fix behaviour is tens of
  // thousands. That distinction needs no profiler and no cable.
  let batteryCount = 0;
  let batteryAt = 0;
  let batteryDelay = -1;
  let batteryHost = null;
  const batteryState = () => {
    const apps = peek('runningApps', null);
    const app = apps && apps.length ? apps[apps.length - 1] : null;
    const wine = app && app.wine;
    if (!wine) { batteryHost = null; return 'no-app'; }
    if (batteryHost !== wine) {
      batteryHost = wine;
      batteryCount = 0;
      batteryDelay = -1;
      batteryAt = performance.now();
      // Wrap the one call every scheduled step goes through, parked or not,
      // and read the sleep out of the argument it is already given. Calling
      // _parkedSleepMs() from here instead would be a second opinion with
      // side effects -- it takes the window lock and writes the guest tick.
      if (!wine.__diagStepWrapped && typeof wine._scheduleStep === 'function') {
        wine.__diagStepWrapped = true;
        const orig = wine._scheduleStep.bind(wine);
        wine._scheduleStep = (step, delayMs) => {
          batteryCount++;
          batteryDelay = delayMs || 0;
          return orig(step, delayMs);
        };
      }
    }
    const dt = Math.max(1, performance.now() - batteryAt) / 1000;
    const rate = batteryCount / dt;
    batteryCount = 0;
    batteryAt = performance.now();
    return `steps=${rate.toFixed(1)}/s park=${batteryDelay}ms ` +
      `hiddenPaused=${wine._hiddenPaused ? 1 : 0} ` +
      `audio=${wine._audioCtx ? wine._audioCtx.state : 'none'} ` +
      `req=${wine._audioRequested ? 1 : 0} wrapped=${wine.__diagStepWrapped ? 1 : 0}`;
  };
  ...
  const tick = () => {
    ...
    const key = now.verdict + '|' + now.why + '|' + now.hit + '|' + now.running + '|' + now.classes +
      ...
      // The step rate jitters every tick, so it goes into the key by order of
      // magnitude only. That still makes "parked at ~20/s" and "spinning at
      // ~10000/s" different keys -- which is the whole question -- without
      // turning the stream into a rate readout.
      '|b' + now.battery.replace(/steps=([\d.]+)/,
        (_m, n) => 'e' + Math.round(Math.log10(Math.max(1, Number(n)))))
        .replace(/park=-?\d+ms/, '');
