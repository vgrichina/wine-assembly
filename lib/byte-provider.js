...
(function () {
  ...
  function clampRange(size, off, len) {
    let start = Math.max(0, Math.min(size, off | 0));
    let end = Math.max(start, Math.min(size, start + Math.max(0, len | 0)));
  ...
  class SliceProvider {
    constructor(parent, offset, length, name) {
      ...
      const avail = Math.max(0, (parent.size | 0) - this.offset);
      this.size = length === undefined ? avail : Math.max(0, Math.min(avail, length | 0));
