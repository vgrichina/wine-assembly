...
(function (root, factory) {
  ...
  function appendTriangle(out, vertices, a, b, c, flat) {
    const colorStart = flat ? c * VERTEX_FLOATS + 3 : -1;
  ...
  function normalizeImmediate(mode, vertices, shadeModel) {
    ...
    if (mode === GL.QUADS) {
      ...
      for (let i = 0; i < edges; i++) {
        appendVertex(out, vertices, i, -1);
        appendVertex(out, vertices, (i + 1) % count, -1);
      }
      mode = GL.LINES;
    } else {
  ...
  class Encoder {
    ...
    call(opcode, stackWa, aux) {
      ...
      if (opcode === 19) this._state().shadeModel = u32At(memoryView, stackWa, 0);
