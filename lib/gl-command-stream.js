...
(function (root, factory) {
  ...
  function appendVertex(out, vertices, index, color) {
    const start = index * VERTEX_FLOATS;
    for (let i = 0; i < VERTEX_FLOATS; i++) {
      out.push(color && i >= 3 && i < 7 ? color[i - 3] : vertices[start + i]);
    }
  }

  function appendTriangle(out, vertices, a, b, c, flat) {
    const colorStart = c * VERTEX_FLOATS + 3;
    const color = flat ? vertices.slice(colorStart, colorStart + 4) : null;
    appendVertex(out, vertices, a, color);
    appendVertex(out, vertices, b, color);
    appendVertex(out, vertices, c, color);
  }
  ...
  function normalizeImmediate(mode, vertices, shadeModel) {
    ...
    if (mode === GL.QUADS) {
      for (let i = 0; i + 3 < count; i += 4) {
        appendTriangle(out, vertices, i, i + 1, i + 2, flat);
        appendTriangle(out, vertices, i, i + 2, i + 3, flat);
      }
      mode = GL.TRIANGLES;
    } else if (mode === GL.QUAD_STRIP) {
      for (let i = 0; i + 3 < count; i += 2) {
        appendTriangle(out, vertices, i, i + 1, i + 3, flat);
        appendTriangle(out, vertices, i, i + 3, i + 2, flat);
      }
      mode = GL.TRIANGLES;
    } else if (mode === GL.POLYGON || mode === GL.TRIANGLE_FAN) {
      for (let i = 1; i + 1 < count; i++) appendTriangle(out, vertices, 0, i, i + 1, flat);
      mode = GL.TRIANGLES;
      ...
      for (let i = 0; i < edges; i++) {
        appendVertex(out, vertices, i);
        appendVertex(out, vertices, (i + 1) % count);
      }
      mode = GL.LINES;
    } else {
  ...
  class Encoder {
    ...
    _pointerFloats(stackWa, count) {
      const pointer = u32At(this.getMemory(), stackWa, 0);
    ...
    call(opcode, stackWa, aux) {
      ...
      if (opcode === 56) {
        this._state().color = [0, 1, 2, 3]
          .map(i => (u32At(memory, stackWa, i) & 0xFF) / 255);
        return 0;
      }
      if (opcode === 58) {
        const pointer = u32At(memory, stackWa, 0);
        const wa = this.guestToWasm(pointer) >>> 0;
      ...
      if (opcode === 21) {
        ...
        this.immediate = { mode: u32At(memory, stackWa, 0), vertices: [] };
        return 0;
      }
      if (opcode >= 29 && opcode <= 31 && this._appendVertex(opcode, stackWa)) return 0;
      if (opcode === 22 && this.immediate) {
        this._finishImmediate();
        return 0;
      }
      if (this.immediate) throw new RangeError(`GL opcode ${opcode} inside glBegin/glEnd`);
      if (opcode === 19) this._state().shadeModel = u32At(memory, stackWa, 0);
      ...
      if (opcode === 51 && result) this.currentContext = u32At(memory, stackWa, 1);
      if (opcode === 49 && result) {
        const deleted = u32At(memory, stackWa, 0);
