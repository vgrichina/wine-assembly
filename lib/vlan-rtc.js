...
(function (root, factory) {
  ...
  async function roomKeyFor(secret) {
    const digest = await crypto.subtle.digest('SHA-256', enc.encode('vln-room:' + secret));
  ...
  async function joinRoom(options) {
    ...
    if (!secret) throw new Error('joinRoom: a room secret is required');
    ...
    const crypt = await cryptoKeyFor(secret);
    ...
    try {
      ...
      if (waiting.length === 0) {
        ...
        await gathered(pc);
        await signaling.publish(key, await sealed(crypt, {
        ...
        while (!answered && Date.now() < deadline) {
          for (const peer of await others()) {
            ...
            const msg = rec && await opened(crypt, rec.value);
        ...
        cleanup();
        return { wire, pc, roomKey: key, role: 'offer' };
      }
      ...
      await connected(pc, channel, Math.max(5000, deadline - Date.now()));
      cleanup();
      return { wire, pc, roomKey: key, role: 'answer' };
    } catch (err) {
      cleanup();
