  ...
  const imports = {
    ...
    get_window_rect: (hwnd, rectPtr) => {
      ...
      if (win) {
        ...
        let y = win.y | 0;
        let cur = win;
        const seen = new Set([hwnd >>> 0]);
        for (let guard = 0; cur.isChild && cur.parentHwnd && guard < 64; guard++) {
          const parent = ctx.renderer.windows[cur.parentHwnd];
          if (!parent || seen.has(parent.hwnd >>> 0)) break;
          seen.add(parent.hwnd >>> 0);
          if (typeof ctx.renderer._computeClientRect === 'function') {
            ctx.renderer._computeClientRect(parent);
          }
