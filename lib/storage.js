...
function createStorageImports(ctx) {
  ...
  return {
    ...
    reg_open_key: (hKey, subKeyWA, isWide) => {
      ...
      const raw = _store.get('reg:' + path);
      if (ctx.verbose) console.log(`  [reg_open] ${path} -> ${raw !== null ? 'found' : 'not found'}`);
    ...
    reg_query_value: (hKey, nameWA, typeGA, dataGA, cbDataGA, isWide) => {
      ...
      if (ctx.verbose) console.log(`  [reg_query] ${path}\\${name} -> ${val ? JSON.stringify(val.data) : 'not found'}`);
    ...
    reg_enum_key: (hKey, dwIndex, lpNameWA, cchName, isWide) => {
      ...
      if (dwIndex >= sorted.length) return 259; // ERROR_NO_MORE_ITEMS
