...
(function () {
  const DESKTOP_APPS = [
      ...
      // Both were wired up and reachable only from the app list. They draw
      // their real screens -- Runenlegen its board and score panel, Tile World
      // its level picker -- which tools/wep32-compare.js checks and photographs.
      ['runenlegen',  'Runenlegen',  '\u{1FAA8}'],
      ['tworld',      'Tile World',  '\u{1F511}'],
      ['winamp',      'Winamp',      '\u{1F3B5}'],
    ];
    // Local-only until the LAN party is wired end to end: the client is
    // playable solo against its own bots, but the interesting mode needs a
    // second browser and a room to meet in.
  ...
  const wineApps = { APPS, DESKTOP_APPS, LOCAL_CANDIDATE_APPS, appFileUrl };
