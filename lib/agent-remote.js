...
function shellQuote(value) {
  // location.href is usually percent-encoded, but URL fragments may retain a
  // literal apostrophe. The handoff is explicitly paste-ready shell, so an
  // unescaped fragment such as #';id;' must stay one argv value rather than
  // terminating the quote and becoming a command. POSIX shells represent an
  // apostrophe inside a single-quoted word by closing it, emitting an escaped
  // apostrophe, then reopening it: '\''.
  return "'" + String(value).replace(/'/g, "'\\''") + "'";
}

export function handoffText() {
  return 'Drive my live wine-assembly session:\n'
    + `  node tools/ctl.js -s ${shellQuote(location.href)} snapshot\n`
