...
(function () {
  ...
  function createBrowserShell(deps) {
    ...
    function stopBrowserOverlay(wine) {
      ...
      void flushBrowserOverlay(wine, 'stop');
    }
    ...
    async function attachDynamicOverlay(wine, app, appId) {
      ...
      wine._vfsOverlayTimer = setInterval(() => {
    ...
    async function launchAppInner(appKey) {
      ...
      // Writable C: state belongs above the immutable import mount. Restore
      // after ZIP/ISO/CUE populated the base and before resolving the EXE, so
      // an installed replacement wins and a recorded deletion stays deleted.
        ...
        const fetchDll = async (spec) => {
          ...
          if (!url && mountedExeDir) {
            ...
            if (vfs && vfs.files.has(mountedPath)) {
              return { name, bytes: await vfs.materialize(mountedPath) };
            }
          }
          if (!url) return null;
    ...
    function launchVfsExe(fileName, callerWine, workDir, args) {
      const vfs = callerWine && callerWine._helpCtx && callerWine._helpCtx.vfs;
      if (!vfs || typeof vfs._normPath !== 'function' || typeof vfs.adoptFrom !== 'function') {
        return false;
      }
      const norm = vfs._normPath(fileName);
      if (!vfs.files.has(norm)) return false;
      const base = norm.split('\\').pop();
      const key = 'vfs:' + norm;
      const cwd = (workDir && workDir.trim())
        || (/\\/.test(fileName) ? fileName.replace(/[\\/][^\\/]*$/, '') + '\\' : '');
      const snapshot = {
      ...
      apps[key] = {
        ...
        exeBytes: async (newVfs) => newVfs.materialize(fileName),
      };
      if (SINGLE_APP() && runningApps.length) {
        let waited = 0;
        const tick = setInterval(() => {
