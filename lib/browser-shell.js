...
(function () {
  // Diablo Shareware's Storm UI currently depends on cooperative execution:
  // under the experimental guest-Worker backend its secondary UI thread runs,
  // but never publishes the MPQ-backed menu art and leaves native gray button
  // surfaces on screen. Keep the user's global Threads preference intact for
  // every other app while this title uses the known-good backend.
  function appRequiresCooperativeBackend(appKey) {
    return appKey === 'diablo_shareware';
  }

  async function initWineForApp(wine, canvas, appKey, onCompatibilityFallback) {
    const requestedThreads = typeof window !== 'undefined' && window.WINE_THREADS;
    const forceCooperative = !!requestedThreads && appRequiresCooperativeBackend(appKey);
    if (forceCooperative) {
      window.WINE_THREADS = false;
      wine.cooperativeCompatibilityReason = 'Diablo Shareware menu artwork';
      if (onCompatibilityFallback) onCompatibilityFallback(
        'Threads: using cooperative mode for Diablo Shareware artwork compatibility');
    }
    try {
      await wine.init(canvas);
    } finally {
      // launchInFlight serializes boots, so no other WineAssembly can observe
      // this temporary override. Restore the preference even when init fails.
      if (forceCooperative) window.WINE_THREADS = requestedThreads;
    }
  }
  ...
  function createBrowserShell(deps) {
    ...
    function autoRunSliceFor(appKey, compatDispatch, workerMode) {
      switch (appKey) {
        ...
        // Uplink discovers valve/dlls/hl.dll by wildcard at map load, then
        // builds the server export table before producing its first frame.
        // Keep that scan/load cooperative with browser input and repaint.
    ...
    function applyRunSlice() {
      ...
      for (const app of runningApps) {
        app.wine.stepsPerSlice = selectedRunSlice(app.name, !!app.wine.guestWorker);
    ...
    async function launchAppInner(appKey) {
      ...
      await initWineForApp(wine, canvas, sel, message => {
      ...
      if (ok) {
        ...
        const runSlice = selectedRunSlice(sel, !!wine.guestWorker);
