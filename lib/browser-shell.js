...
(function () {
  ...
  function createBrowserShell(deps) {
    ...
    function autoRunSliceFor(appKey, compatDispatch) {
      switch (appKey) {
        // Jazz's J2V/title-to-level transition can stay inside one guest run
        // for minutes with the generic 100k-block slice. Keep that decoder
        // cooperative so browser input, audio completion and repaint continue
        // to run while the level is being prepared.
        case 'jazz2_demo':
