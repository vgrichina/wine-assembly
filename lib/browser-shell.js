...
(function () {
  ...
  function createBrowserShell(deps) {
    ...
    const GUEST_PAGE_TRANSLATION = (() => {
      try { return new URLSearchParams(location.search).has('guest-page-translation'); }
    ...
    async function launchAppInner(appKey) {
      ...
      if (ok) {
        ...
        // Experimental A/B seam: apply before the guest's first slice and
        // record it for every later cooperative or real Worker instance.
        // Packed-mode PTE misses are authoritative; this does not enable a
        // hidden fallback or the separate future PAGE_* permission policy.
        if (GUEST_PAGE_TRANSLATION && wine.hasGuestExport('set_guest_page_translation')) {
          if (wine.threadManager && wine.threadManager.recordInheritedWasmGlobal) {
            wine.threadManager.recordInheritedWasmGlobal('set_guest_page_translation', 1);
          }
          await wine.callGuest('set_guest_page_translation', 1);
          log.textContent += 'Packed sparse guest-page translation enabled\n';
        }
