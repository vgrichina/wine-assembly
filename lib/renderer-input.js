    ...
    for (const win of sortedWins) {
      ...
      if (win.hasCaption) {
        ...
        if (hit === 20 || hit === 8 || hit === 9) {
          if (hit === 20 && (win.isAboutDialog || win.isFindDialog)) {
            if (we && we.send_message) we.send_message(win.hwnd, 0x0010, 0, 0); // WM_CLOSE
            else delete this.windows[win.hwnd];
            this.repaint();
            return;
          }
          if (we && we.post_message_q) {
            const lParam = ((canvasY & 0xFFFF) << 16) | (canvasX & 0xFFFF);
            we.post_message_q(win.hwnd, 0x00A1, hit, lParam); // WM_NCLBUTTONDOWN
          } else if (hit === 20) {
      ...
      if ((win.isFindDialog || win.isAboutDialog || win.isDialog) && button !== 2) {
        ...
        if (we && we.dialog_route_mouse) {
          ...
          if (we.dialog_route_mouse(win.hwnd, 0x0201, 0x0001, lParam)) { // WM_LBUTTONDOWN
            we.dialog_route_mouse(win.hwnd, 0x0202, 0, lParam);          // WM_LBUTTONUP
