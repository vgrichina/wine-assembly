...
function installInputHandlers(R) {
  ...
  P.handleKeyDown = function(vkCode) {
    ...
    if ((this._menuMode || this._altDown) && vkCode >= 65 && vkCode <= 90) {
      if (we && we.menu_find_bar_accel) {
        for (const w of Object.values(this.windows)) {
          if (!w.visible || w.isChild || !w.menu) continue;
    ...
    // WAT-managed edit (Find dialog etc.): route WM_KEYDOWN directly to its
    // wndproc and skip the JS-side editor logic.
    {
      const we = this.wasm && this.wasm.exports;
      const watFocus = we && we.get_focus_hwnd ? we.get_focus_hwnd() : 0;
      if (watFocus && !this.windows[watFocus]) {
  ...
  P.handleKeyPress = function(charCode) {
    ...
    // If a WAT-managed control has focus (e.g. Find dialog edit), route the
    // WM_CHAR straight to its wndproc and skip JS-side editText handling —
    // the WAT EditState is the source of truth for those controls.
    const we = this.wasm && this.wasm.exports;
    const watFocus = we && we.get_focus_hwnd ? we.get_focus_hwnd() : 0;
    if (watFocus && !this.windows[watFocus]) {
