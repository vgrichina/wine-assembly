...
function installInputHandlers(R) {
  ...
  P.handleMouseDown = function(canvasX, canvasY, button) {
    ...
    for (const win of sortedWins) {
        ...
        if (canvasY >= bby && canvasY < bby + bh) {
          if (canvasX >= closeX && canvasX < closeX + bw) {
            ...
            if (win.isFindDialog) {
              ...
              this._focusedDialogEditWin = null;
              delete this.windows[win.hwnd];
              this.repaint();
              return;
            }
      ...
      for (const ctrl of win.controls) {
        ...
        if (canvasX >= cx && canvasX < cx + ctrl.w && canvasY >= cy && canvasY < cy + ctrl.h) {
          if (ctrl.className === 'Button' && ctrl.type !== 'groupbox') {
            ...
            if (win.isFindDialog) {
              // Dispatch through WAT: find the WAT-side child hwnd matching
              // this ctrl_id and send WM_LBUTTONDOWN/UP. $button_wndproc
              // posts WM_COMMAND, $findreplace_wndproc reacts (writes the
              // FR struct + posts the registered message to the owner).
              // Falls back to the legacy JS handler when wasm exports
              // aren't available.
              const we = this.wasm && this.wasm.exports;
              if (we && we.wnd_next_child_slot && we.send_message) {
                let s = 0, dispatched = false;
                while ((s = we.wnd_next_child_slot(win.hwnd, s)) !== -1) {
                  const ch = we.wnd_slot_hwnd(s);
                  if (we.ctrl_get_id(ch) === ctrl.id) {
                ...
                if (dispatched) {
                  // Mirror checkbox/radio state from WAT back to the JS
                  // controls[] so the (still-present) JS hit-test loop
                  // doesn't go stale and so the dialog repaints with the
                  // new check state.
                  const flags = we.button_get_flags(we.wnd_slot_hwnd(s));
                  ctrl.checked = (flags & 0x02) !== 0;
                  this.invalidate(win.hwnd);
                  return;
                }
              }
              this._handleFindDialogButton(win, ctrl);
              return;
            }
      ...
      // Check click on dialog edit controls (for focus)
      if (win.isFindDialog) {
        for (const ctrl of win.controls) {
          if (ctrl.className === 'Edit') {
            const cx = clientX + ctrl.x;
            const cy2 = clientY + ctrl.y;
            console.log('[find-edit-click] check', canvasX, canvasY, 'vs', cx, cy2, cx+ctrl.w, cy2+ctrl.h);
            if (canvasX >= cx && canvasX < cx + ctrl.w && canvasY >= cy2 && canvasY < cy2 + ctrl.h) {
              console.log('[find-edit-click] HIT! setting focus');
              this._focusedDialogEdit = ctrl;
              this._focusedDialogEditWin = win;
              if (ctrl.editText == null) ctrl.editText = ctrl.text || '';
              ctrl._cursor = ctrl.editText.length;
              ctrl._selStart = ctrl._cursor;
              this.invalidate(win.hwnd);
              return;
            }
          }
  ...
  P._handleFindDialogButton = function(win, ctrl) {
    if (ctrl.id === 2) {
      // Cancel — close dialog, send FR_DIALOGTERM to owner
      if (win.frGuestAddr && this._memory) {
        const mem = this._memory;
        const e = this._exports;
        if (mem && e) {
          const g2w = (a) => a - e.get_image_base() + 0x12000;
          const wa = g2w(win.frGuestAddr);
          const dv = new DataView(mem.buffer);
          // Set FR_DIALOGTERM (0x40) in Flags at offset 0x0C
          const oldFlags = dv.getUint32(wa + 0x0C, true);
          dv.setUint32(wa + 0x0C, (oldFlags & ~0x48) | 0x40, true);
          // Send registered message to owner with lParam = frGuestAddr
          this.inputQueue.push({
            type: 'findreplace', hwnd: win.ownerHwnd,
            msg: 0xC000, wParam: 0, lParam: win.frGuestAddr,
          });
        }
      }
      this._focusedDialogEdit = null;
    ...
    if (ctrl.id === 1) {
      // Find Next — update FINDREPLACE struct and send message
      // First handle checkbox/radio state
      for (const c of win.controls) {
        if (c.type === 'autocheckbox') c.checked = c.checked;
        if (c.type === 'autoradiobutton') c.checked = c.checked;
      }
      ctrl.pressed = true;
      this.invalidate(win.hwnd);
      if (win.frGuestAddr && this._memory) {
        ...
        if (mem && e) {
          ...
          // Build flags: FR_FINDNEXT=0x8, FR_DOWN=0x1, FR_MATCHCASE=0x4
          let flags = 0x08; // FR_FINDNEXT
          const matchCase = win.controls.find(c => c.id === 0x411);
          if (matchCase && matchCase.checked) flags |= 0x04;
          const downRadio = win.controls.find(c => c.id === 0x421);
          if (downRadio && downRadio.checked) flags |= 0x01;
          dv.setUint32(wa + 0x0C, flags, true);
          // Write search text to lpstrFindWhat buffer
          const findBufGuest = dv.getUint32(wa + 0x10, true);
          const findBufLen = dv.getUint16(wa + 0x18, true);
          const editCtrl = win.controls.find(c => c.id === 0x480);
          const searchText = editCtrl ? (editCtrl.editText || editCtrl.text || '') : '';
          const findBufWa = g2w(findBufGuest);
          for (let i = 0; i < Math.min(searchText.length, findBufLen - 1); i++) {
            m8[findBufWa + i] = searchText.charCodeAt(i) & 0xFF;
          }
          m8[findBufWa + Math.min(searchText.length, findBufLen - 1)] = 0;
          // Send registered message (0xC000) to owner with lParam = frGuestAddr
          this.inputQueue.push({
            type: 'findreplace', hwnd: win.ownerHwnd,
            msg: 0xC000, wParam: 0, lParam: win.frGuestAddr,
          });
    ...
    // Other buttons (checkbox/radio) — handle toggling
    if (ctrl.type === 'autocheckbox') {
      ctrl.checked = !ctrl.checked;
    } else if (ctrl.type === 'autoradiobutton') {
      for (const c of win.controls) {
        if ((c.type === 'autoradiobutton' || c.type === 'radiobutton') && c !== ctrl) c.checked = false;
      }
      ctrl.checked = true;
    }
    this.invalidate(win.hwnd);
