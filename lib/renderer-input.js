...
function installInputHandlers(R) {
  ...
  P.handleKeyDown = function(vkCode) {
    ...
    // _shiftDown/_ctrlDown locals only update after edit routing.
    {
      ...
      // Dialog focus traversal (Tab / Shift+Tab / Enter / Space).
      // Gated on the focused hwnd having a dialog ancestor. dlg_load
      // allocates child hwnds contiguously from dlgHwnd+1, so the tab
      // list is just that range filtered by WS_VISIBLE & WS_TABSTOP &
      // !WS_DISABLED.
      if (vkCode === 9 || vkCode === 13 || vkCode === 32) {
        const focus = (we && we.get_focus_hwnd) ? (we.get_focus_hwnd() | 0) : 0;
        let dlg = 0;
        for (let h = focus; h; ) {
          const w = this.windows[h];
          if (!w) break;
          if (w.isDialog) { dlg = h; break; }
          h = w.parentHwnd | 0;
        }
        if (dlg && we.dlg_get_ctrl_count && we.wnd_get_style_export) {
          ...
          if (list.length > 0) {
            if (vkCode === 9) {
              ...
              if (we.send_message) {
                if (focus) we.send_message(focus, 0x0008, 0, 0); // WM_KILLFOCUS
                we.send_message(next, 0x0007, 0, 0);             // WM_SETFOCUS
              } else if (we.set_focus_hwnd) {
                we.set_focus_hwnd(next);
              }
              this.invalidate(dlg);
              return;
            }
            if (vkCode === 13) {
              // Default pushbutton: button class with low-4 style == 1.
              let def = 0;
              ...
              if (def && we.post_message_q && we.ctrl_get_id) {
                ...
                we.post_message_q(dlg, 0x0111, id, def);          // WM_COMMAND, BN_CLICKED=0
                return;
              }
            }
            if (vkCode === 32) {
