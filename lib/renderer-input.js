...
function installInputHandlers(R) {
  ...
  P.handleMouseDown = function(canvasX, canvasY, button, opts) {
    ...
    for (const win of sortedWins) {
      ...
      // Clicking outside any WAT-managed dialog releases stale focus
      // (e.g. a Find dialog edit) so the next edit pickup re-scans for a
      // visible WAT Edit control. Use the focus round-trip when possible so
      // native edits receive WM_KILLFOCUS/EN_KILLFOCUS before another child
      // is focused; Paint commits its active text object during this step.
      ...
      if (!win.isFindDialog && !win.isAboutDialog && !win.isDialog) {
        ...
        const we = w && w.exports;
        if (we && we.set_focus) we.set_focus(0);
        else if (we && we.set_focus_hwnd) we.set_focus_hwnd(0);
      }
  ...
  P.handleMouseUp = function(canvasX, canvasY, button) {
    ...
    if (this._resizingWin) {
      ...
      const hit = we && we.hittest_sync
        ...
        : 0;
      this._clearStaleResizeCursor(hit);
      ...
      if (we && (we.dialog_route_mouse_screen || we.dialog_route_mouse)) {
        ...
        if (capture) {
          ...
          this._dispatchMouseEvent(capture.win, {
            ...
            msg: 0x0202,
            wParam: 0,
            lParam: (((ly & 0xFFFF) << 16) | (lx & 0xFFFF)) >>> 0,
          });
          this.scheduleRepaint();
          this.repaint();
          return;
  ...
  P.handleMouseMove = function(canvasX, canvasY) {
    ...
    for (const win of Object.values(this.windows)) {
      ...
      if (canvasX >= win.x && canvasX < win.x + win.w &&
        ...
        if (ownerExports && ownerExports.post_message_q) {
          // WindowFromPoint already establishes that a deep child contains
          // the point, so its message is client-area input. Top-level chrome
          // still needs the full non-client hit test for resize cursors.
          const hit = deep ? 1 : (ownerExports.hittest_sync
            ? (ownerExports.hittest_sync(win.hwnd, canvasX, canvasY) | 0)
            : 1);
          this._clearStaleResizeCursor(hit);
          const lp = (hit & 0xFFFF) | ((0x0200 & 0xFFFF) << 16);
    ...
    this._clearStaleResizeCursor(0);
  }
