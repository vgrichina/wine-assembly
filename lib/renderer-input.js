...
function installInputHandlers(R) {
  ...
  P._nativeScrollbarHit = function(topWin, deep, canvasX, canvasY, ownerWasm) {
    ...
    if (!looksRichEdit && !hasVScroll) return null;
    ...
    const bar = 16;
    if (w <= bar || h <= 0) return null;
    if (canvasX < sx + w - bar || canvasX >= sx + w || canvasY < sy || canvasY >= sy + h) return null;
    const info = this._nativeVerticalScrollInfo(ownerWasm, hwnd);
    if (!info) return null;
    const coord = (canvasY - sy) | 0;
    const part = this._scrollbarHitPartShared(ownerWasm, h, coord, info.pos, info.min, info.max);
    if (!part) return null;
    return { hwnd, wasm: ownerWasm, topWin, axis: 'v', sx, sy, w, h, coord, part, info };
  };
  ...
  P._handleNativeScrollbarDown = function(topWin, deep, canvasX, canvasY, ownerWasm) {
    ...
    const code = hit.part === 1 ? 0 :
      ...
      hit.part === 3 ? 2 :
      hit.part === 4 ? 3 : -1;
    if (code >= 0) this._sendNativeVerticalScroll(ownerWasm, hit.hwnd, code, hit.info.pos);
  ...
  P._handleNativeScrollbarMove = function(canvasX, canvasY) {
    ...
    if (drag.part === 5) {
      ...
      const next = this._scrollbarDragPosShared(
        drag.wasm,
        drag.longDim,
        coord,
        drag.anchorCoord,
      ...
      if (next !== drag.lastPos) {
        drag.lastPos = next;
        this._sendNativeVerticalScroll(drag.wasm, drag.hwnd, 5, next); // SB_THUMBTRACK
      }
    }
    return true;
  };
  ...
  P._handleNativeScrollbarUp = function(canvasX, canvasY) {
    ...
    if (drag.part === 5) {
      ...
      this._sendNativeVerticalScroll(drag.wasm, drag.hwnd, 4, drag.lastPos); // SB_THUMBPOSITION
    }
    this._nativeScrollbarDrag = null;
    this._sendNativeVerticalScroll(drag.wasm, drag.hwnd, 8, drag.lastPos); // SB_ENDSCROLL
    return true;
  };
    ...
    if (modalHwnd) {
      ...
      if (button !== 2 && modalExports && (modalExports.dialog_route_mouse_screen || modalExports.dialog_route_mouse)) {
        ...
        const routed = modalExports.dialog_route_mouse_screen
          ? (modalExports.dialog_route_mouse_screen(modalHwnd, msg, 0x0001, canvasX, canvasY) | 0)
          : (() => {
              ...
              return modalExports.dialog_route_mouse(modalHwnd, msg, 0x0001, lp) | 0;
            })();
        if (routed) {
          this.wasm = modalWasm;
