    ...
    for (const win of sortedWins) {
      ...
      if ((win.isFindDialog || win.isAboutDialog || win.isDialog) && button !== 2) {
        ...
        if (we && we.dialog_route_mouse) {
          ...
          // Send WM_LBUTTONDOWN now; defer WM_LBUTTONUP to handleMouseUp so
          // the user actually sees the button's pressed state while held.
          // (button_wndproc sets the pressed flag on DOWN and clears it on
          // UP; if both fire in the same tick, the pressed visual is never
          // composited.) Cache (parent, lParam) so mouseup can route UP to
          // the same child via dialog_route_mouse.
          if (we.dialog_route_mouse(win.hwnd, 0x0201, 0x0001, lParam)) { // WM_LBUTTONDOWN
    ...
    if (capHwnd) {
      ...
      if (win) {
        ...
        if (we && we.post_message_q) {
          const hit = (we.hittest_sync ? (we.hittest_sync(capHwnd, canvasX, canvasY) | 0) : 1);
          const lp = (hit & 0xFFFF) | ((0x0200 & 0xFFFF) << 16);
          we.post_message_q(capHwnd, 0x0020, capHwnd, lp >>> 0);
        }
        this.inputQueue.push({
          type: 'mouse',
          hwnd: capHwnd,
