...
function installInputHandlers(R) {
  ...
  P._getEditWindow = function() {
    // If a dialog edit control is focused, return a proxy object for it
    if (this._focusedDialogEdit) {
      ...
      if (dlg && this.windows[dlg.hwnd]) {
        // Create a proxy that maps editText/cursor to the control
        if (!ctrl._cursor) ctrl._cursor = (ctrl.editText || '').length;
        if (ctrl._selStart == null) ctrl._selStart = ctrl._cursor;
        return {
          editText: ctrl.editText || '',
          set editText(v) { ctrl.editText = v; ctrl.text = v; },
          get _cursor() { return ctrl._cursor || 0; },
          set _cursor(v) { ctrl._cursor = v; },
          get _selStart() { return ctrl._selStart != null ? ctrl._selStart : ctrl._cursor; },
          set _selStart(v) { ctrl._selStart = v; },
          hwnd: dlg.hwnd,
          _invalidate: () => this.invalidate(dlg.hwnd),
        };
      }
