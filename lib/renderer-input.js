...
function installInputHandlers(R) {
  ...
  P._clientOriginScreen = function(hwnd) {
    ...
    const y = win.y + 3 + ((win.style & 0x00C00000) ? 19 : 0) + (this._hasMenuBar(win) ? 18 : 0) + 1;
    return { x: win.x + 3, y };
  };
  ...
  P._hitTestDeepChild = function(topWin, cx, cy) {
    ...
    const topOrigin = topWin.isPopup
      ? this._windowOriginScreen(topWin.hwnd)
      : this._clientOriginScreen(topWin.hwnd);
    const topOx = topOrigin.x;
    const topOy = topOrigin.y;
  ...
  P._resolveCaptureTarget = function(capHwnd) {
    ...
    if (direct) {
      ...
      if (top.isPopup) {
        const origin = this._windowOriginScreen(top.hwnd);
        return { win: top, screenX: origin.x, screenY: origin.y, targetHwnd: capHwnd };
      }
      if (top.isChild) {
        const origin = this._windowOriginScreen(top.hwnd);
        return { win: top, screenX: origin.x, screenY: origin.y, targetHwnd: capHwnd };
      }
      const origin = this._clientOriginScreen(top.hwnd);
      return { win: top, screenX: origin.x, screenY: origin.y, targetHwnd: capHwnd };
    }
    const we = this.wasm && this.wasm.exports;
    ...
    for (let i = 0; i < 8 && cur; i++) {
      ...
      if (top) {
        const origin = (top.isPopup || top.isChild)
          ? this._windowOriginScreen(top.hwnd)
          : this._clientOriginScreen(top.hwnd);
        return { win: top, screenX: origin.x + dx, screenY: origin.y + dy, targetHwnd: capHwnd };
      }
      cur = parent;
    }
    return null;
  ...
  P.handleMouseDown = function(canvasX, canvasY, button) {
    ...
    for (const win of sortedWins) {
      ...
      if (typeof this._computeClientRect === 'function') this._computeClientRect(win);
      const clientOrigin = win.isPopup
        ? this._windowOriginScreen(win.hwnd)
        : this._clientOriginScreen(win.hwnd);
      ...
      if (button !== 2) {
        ...
        if (we && we.dialog_route_mouse) {
          ...
          // dialog_route_mouse.
          let routed = we.dialog_route_mouse(routeHwnd, msg, 0x0001, lParam) | 0;
          let routedLParam = lParam;
          let routedClientX = routeClientX;
          let routedClientY = routeClientY;
          if (!routed && routeWin && routeWin.isChild && we.wnd_get_parent) {
            let parentHwnd = routeHwnd;
            let parentWin = routeWin;
            for (let guard = 0; guard < 16 && parentWin && parentWin.isChild; guard++) {
              const p = we.wnd_get_parent(parentHwnd) | 0;
              const pWin = p ? this.windows[p] : null;
              if (!pWin) break;
              parentHwnd = p;
              parentWin = pWin;
            }
            if (parentHwnd !== routeHwnd) {
              if (typeof this._computeClientRect === 'function') this._computeClientRect(parentWin);
              const parentOrigin = parentWin && parentWin.isPopup
                ? this._windowOriginScreen(parentHwnd)
                : this._clientOriginScreen(parentHwnd);
              ...
              if (we.dialog_route_mouse(parentHwnd, msg, 0x0001, parentLParam)) {
                routed = 1;
      ...
      if (canvasX >= win.x && canvasX < win.x + win.w &&
        ...
        {
          const we = this.wasm && this.wasm.exports;
          if (button !== 2 && deep && we && we.dialog_route_mouse) {
            const routedLParam = ((relX & 0xFFFF) | ((relY & 0xFFFF) << 16)) >>> 0;
            if (we.dialog_route_mouse(targetHwnd, msg, 0x0001, routedLParam)) {
  ...
  P.handleMouseUp = function(canvasX, canvasY, button) {
    const modal = Object.values(this.windows).find(w => w.visible && w.isAboutDialog);
    if (modal && button !== 2) {
      if (this._dialogBtnDrag && this._dialogBtnDrag.parent !== modal.hwnd) {
    ...
    for (const win of sortedWins) {
      if (canvasX >= win.x && canvasX < win.x + win.w &&
          canvasY >= win.y && canvasY < win.y + win.h) {
        if (typeof this._computeClientRect === 'function') this._computeClientRect(win);
        const clientX = win.clientRect ? win.clientRect.x : (win.isPopup ? win.x : win.x + 3);
        const clientY = win.clientRect ? win.clientRect.y
          : (win.isPopup ? win.y : win.y + 3 + 18 + (this._hasMenuBar(win) ? 18 : 0) + 1);
        const deep = this._hitTestDeepChild(win, canvasX, canvasY);
        const targetHwnd = deep ? deep.hwnd : win.hwnd;
        const relX = deep ? (canvasX - deep.sx) : (canvasX - clientX);
        const relY = deep ? (canvasY - deep.sy) : (canvasY - clientY);
  ...
  P.handleKeyDown = function(vkCode) {
    ...
    if ((vkCode === 13 || vkCode === 27) && we && (we.send_message || we.post_message_q)) {
      const modal = Object.values(this.windows).find(w => w.visible && w.isAboutDialog);
      if (modal) {
        let id = vkCode === 27 ? 2 : 1;
        if (vkCode === 13 && we.dlg_get_default_btn && we.ctrl_get_id) {
          const def = we.dlg_get_default_btn(modal.hwnd) | 0;
          if (def) id = we.ctrl_get_id(def) & 0xFFFF;
        }
        if (we.send_message) we.send_message(modal.hwnd, 0x0111, id, 0);
        else we.post_message_q(modal.hwnd, 0x0111, id, 0);
        this.scheduleRepaint();
        return;
      }
    }
    ...
    // _shiftDown/_ctrlDown locals only update after edit routing.
    {
      ...
      if (vkCode === 9 || vkCode === 13 || vkCode === 27 || vkCode === 32
        ...
        if (dlg && we.dlg_get_ctrl_count && we.wnd_get_style_export) {
          ...
          if (vkCode === 27 && (we.send_message || we.post_message_q)) {
            if (we.send_message) we.send_message(dlg, 0x0111, 2, 0); // WM_COMMAND IDCANCEL
            else we.post_message_q(dlg, 0x0111, 2, 0);
            return;
          }
          ...
          if (vkCode === 37 || vkCode === 38 || vkCode === 39 || vkCode === 40) {
            if (focus && we.radio_group_step
              ...
              if (peer && peer !== focus) {
                ...
                if ((we.send_message || we.post_message_q) && we.ctrl_get_id) {
                  const id = we.ctrl_get_id(peer) & 0xFFFF;
                  if (we.send_message) we.send_message(dlg, 0x0111, id, peer);
                  else we.post_message_q(dlg, 0x0111, id, peer);
                }
                this.scheduleRepaint();
                return;
              }
          ...
          if (list.length > 0) {
            ...
            if (vkCode === 13) {
              ...
              if (def && (we.send_message || we.post_message_q) && we.ctrl_get_id) {
                const id = we.ctrl_get_id(def) & 0xFFFF;
                if (we.send_message) we.send_message(dlg, 0x0111, id, def);
                else we.post_message_q(dlg, 0x0111, id, def);
                return;
              }
            }
            if (vkCode === 32) {
              if (focus && we.ctrl_get_class && (we.ctrl_get_class(focus) | 0) === 1
                       && (we.send_message || we.post_message_q) && we.ctrl_get_id) {
                const id = we.ctrl_get_id(focus) & 0xFFFF;
                if (we.send_message) we.send_message(dlg, 0x0111, id, focus);
                else we.post_message_q(dlg, 0x0111, id, focus);
                return;
              }
            }
          }
