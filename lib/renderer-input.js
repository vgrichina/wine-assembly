...
function installInputHandlers(R) {
  ...
  P.handleMouseDown = function(canvasX, canvasY, button) {
    ...
    for (const win of sortedWins) {
      ...
      // Find dialog: hit-test against WAT-side children. The dialog has
      // no JS controls[] — all geometry / state lives in CONTROL_GEOM and
      // ButtonState/EditState. A hit on a button dispatches WM_LBUTTONDOWN
      // /UP through send_message; $button_wndproc posts WM_COMMAND;
      // $findreplace_wndproc writes the FR struct and posts 0xC000 to
      // the owner. A hit on the edit control sets focus via WAT.
      if (win.isFindDialog && button !== 2) {
