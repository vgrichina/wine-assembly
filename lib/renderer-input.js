...
function installInputHandlers(R) {
  ...
  P._openWorkerContextMenu = function(win, canvasX, canvasY) {
    ...
    const opened = e.menu_track_popup_open_module(win.hwnd >>> 0, hinst, 101, 0, popupX, popupY) | 0;
    ...
    if (e.menu_child_count && this.canvas) {
      ...
      if (nextX !== popupX || nextY !== popupY) {
        popupX = nextX;
        popupY = nextY;
        e.menu_track_popup_open_module(win.hwnd >>> 0, hinst, 101, 0, popupX, popupY);
      }
    }
    ...
    for (const win of Object.values(this.windows)) {
      ...
      if (canvasX >= win.x && canvasX < win.x + win.w &&
        ...
        if (clipped.active) {
          ...
          clientX = win.x + 3;
          clientY = win.y + 3 + 18 + (this._hasMenuBar(win) ? 18 : 0) + 1;
        }
