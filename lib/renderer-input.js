...
function installInputHandlers(R) {
  const P = R.prototype;
  // Process-shared emulator memory reserved beside DX_PROCESS_STATE. Browser
  ...
  const DI_MOUSE_INPUT_STATE = 0x07F0CEB0;
  const DI_MOUSE_BUTTON_QUEUE_CAPACITY = 8;
  ...
  // accumulators for GetDeviceState callers.
  P._queueDirectInputMouseEvent = function(memory, type, data) {
    ...
    if (!buffer || buffer.byteLength < DI_MOUSE_INPUT_STATE + 48) return false;
    ...
    if (((tail - head) >>> 0) >= DI_MOUSE_BUTTON_QUEUE_CAPACITY) return false;
    const code = type <= 4 ? type : ((type << 28) | ((data || 0) & 0x0FFFFFFF));
    store(base + 4 + (tail & (DI_MOUSE_BUTTON_QUEUE_CAPACITY - 1)), code);
