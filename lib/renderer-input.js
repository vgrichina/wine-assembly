...
function installInputHandlers(R) {
  ...
  P._resolveCaptureTarget = function(capHwnd) {
    ...
    if (we && we.wnd_top_level && we.wnd_mouse_msg_origin_x && we.wnd_mouse_msg_origin_y) {
      ...
      if (top) {
        return {
          win: top,
          screenX: we.wnd_mouse_msg_origin_x(capHwnd) | 0,
          screenY: we.wnd_mouse_msg_origin_y(capHwnd) | 0,
          targetHwnd: capHwnd,
        };
      }
    }
  ...
  P.handleMouseDown = function(canvasX, canvasY, button, opts) {
    ...
    // Menu tracking is WAT-side. JS only forwards the screen point.
    {
      ...
      if (we && we.menu_open_hwnd && we.menu_open_hwnd() !== 0) {
        ...
        if (trackedWin && we.menu_hittest_bar && we.menu_open) {
          const idx = we.menu_hittest_bar(
            tracked, (trackedWin.x | 0) + 3, (trackedWin.y | 0) + 22, canvasX, canvasY) | 0;
          if (idx >= 0) {
            we.menu_open(tracked, idx);
            this._menuMouseCapture = true;
            this.repaint();
            return;
          }
        }
        if (we.menu_handle_mouse_open) we.menu_handle_mouse_open(canvasX, canvasY);
        else we.menu_close();
        this._menuMouseCapture = true;
        this.repaint();
        return;
    ...
    {
      const we = this.wasm && this.wasm.exports;
      const dlg = we && we.get_findreplace_dlg ? (we.get_findreplace_dlg() | 0) : 0;
      if (dlg && button !== 2 && we.wnd_next_child_slot && we.wnd_slot_hwnd &&
        ...
        while ((slot = we.wnd_next_child_slot(dlg, slot)) !== -1) {
          ...
          this._dialogBtnDrag = { target: ch, sx: x, sy: y };
          this.scheduleRepaint();
          this.repaint();
          return;
        }
    ...
    for (const win of sortedWins) {
      ...
      if (win.hasCaption) {
        ...
        const hit = (we && we.nc_sysbutton_down)
          ...
          : 0;
        if (hit) {
          this._sysBtnDrag = { hwnd: win.hwnd };
          this.repaint();
          return;
        }
      }
      ...
      if (canvasX >= win.x && canvasX < win.x + win.w &&
        ...
        {
          const we = this.wasm && this.wasm.exports;
          if (button !== 2 && deep && we && (we.dialog_route_mouse_screen || we.dialog_route_mouse)) {
  ...
  P._findWatEditTarget = function() {
    ...
    return we.edit_command_target ? (we.edit_command_target() | 0) : 0;
  }
