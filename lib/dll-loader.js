...
function callDllMain(exports, loadAddr, dllMain, log, maxBlocks = 2000000) {
  ...
  // Run in chunks to detect infinite loops
  const chunkSize = 500;
  const maxChunks = 4000;  // 2M total
  let prevEip = 0;
  let stuckCount = 0;
  for (let chunk = 0; chunk < maxChunks; chunk++) {
    try {
      exports.run(chunkSize);
    } catch (e) {
    ...
    const curEip = exports.get_eip();
    if (curEip === 0) break;  // returned to sentinel
    if (curEip === prevEip) {
      stuckCount++;
      if (stuckCount >= 3) {
        if (log) log(`DllMain stuck at EIP=0x${curEip.toString(16)} after ${(chunk+1)*chunkSize} blocks`);
        exports.set_esp(savedEsp);
        exports.set_eip(savedEip);
        return;
      }
