  ...
  for (let i = 0; i < results.length; i++) {
    ...
    while (true) {
      ...
      for (let di = 0; di < dllCount; di++) {
        ...
        if (importedDllName.toLowerCase() === expName.toLowerCase() ||
            importedDllName.toLowerCase().replace('.dll', 'u.dll') === expName.toLowerCase() ||
            importedDllName.toLowerCase() === expName.toLowerCase().replace('u.dll', '.dll')) {
...
function initMsvcrtGlobals(exports, memory, msvcrtBase, log, options = {}) {
  ...
  // Build the cmdline string. CRT startup parses this to extract argv[0]
  // (the exe name) and lpCmdLine (everything after) for WinMain.
  const exeName = options.exeName || 'app.exe';
  const extraArgs = options.extraArgs || '';
  const cmdLineStr = extraArgs ? `${exeName} ${extraArgs}` : exeName;
  const cmdLineBytes = new TextEncoder().encode(cmdLineStr + '\0');

  // Wide command line
  const wCmdLine = exports.guest_alloc((cmdLineStr.length + 1) * 2);
  for (let i = 0; i < cmdLineStr.length; i++)
    exports.guest_write16(wCmdLine + i * 2, cmdLineStr.charCodeAt(i));
  exports.guest_write16(wCmdLine + cmdLineStr.length * 2, 0);

  // Narrow command line
  const aCmdLine = exports.guest_alloc(cmdLineBytes.length);
  const aCmdWa = g2w(aCmdLine);
  for (let i = 0; i < cmdLineBytes.length; i++) mem[aCmdWa + i] = cmdLineBytes[i];
  ...
  // and extracting the __sbh_threshold variable address from its code.
  // The sbh manages small allocations (<1KB) internally with its own metadata,
  // which can get corrupted by x86 emulation edge cases. Setting threshold=0
  // forces all allocations through HeapAlloc which we handle correctly.
  for (let i = 0; i < numNames; i++) {
    const nameRva = dv.getUint32(g2w(msvcrtBase + namesRva + i * 4), true);
    const name = readStr(mem, g2w(msvcrtBase + nameRva), 32);
    if (name === '_set_sbh_threshold') {
      ...
      const funcRva = dv.getUint32(g2w(msvcrtBase + funcsRva + ord * 4), true);
      const wa = g2w(msvcrtBase + funcRva);
      // Function starts: 55 8B EC A1 [state_addr] 56 83 F8 03 75 xx 8B 45 08 3D F8 03 00 00 0F 87 xx xx xx xx A3 [threshold_addr]
      // Look for the A3 (mov [imm32], eax) instruction which stores to __sbh_threshold
      for (let j = 0; j < 40; j++) {
        if (mem[wa + j] === 0xA3) {
          // Read the original absolute address, apply relocation delta
          const origAddr = dv.getUint32(wa + j + 1, true);
          // The address is already relocated by the DLL loader, so it's the guest address
          const thresholdGA = origAddr;
          exports.guest_write32(thresholdGA, 0);
          if (log) log(`Patched __sbh_threshold [0x${thresholdGA.toString(16)}] = 0 (sbh disabled)`);
          break;
        }
      }
      break;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { loadDll, loadDlls, patchExeImports, patchDllImports, callDllMain, detectRequiredDlls, DLL_CONFIGS };
} else if (typeof window !== 'undefined') {
  window.DllLoader = { loadDll, loadDlls, patchExeImports, callDllMain, detectRequiredDlls, DLL_CONFIGS };
}
