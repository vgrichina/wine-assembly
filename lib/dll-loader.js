...
// src/00-regions.wat (docs/watx-region-safety-design.md §6). The eight g2w
// translations below each spelled $GUEST_BASE out as 0x12000, which is eight
// independent copies of an ABI that lives in one place. `var`, and a name
// nothing else uses, deliberately: these lib/ files are classic scripts in the
// browser, where a second top-level `const GUEST_BASE` is a SyntaxError.
 ...
 */
function loadDll(exports, memory, dllBytes, modulePath) {
  const dllCount = exports.get_dll_count ? exports.get_dll_count() >>> 0 : 0;
  const dllCapacity = exports.get_dll_capacity ? exports.get_dll_capacity() >>> 0 : 16;
  if (dllCount >= dllCapacity) {
    throw new RangeError(`DLL table capacity ${dllCapacity} exhausted`);
  }
  ...
  if (modulePath && exports.guest_alloc && exports.set_dll_path) {
    const path = String(modulePath).replace(/\//g, '\\');
    const pathGuest = exports.guest_alloc(path.length + 1) >>> 0;
    const imageBase = exports.get_image_base() >>> 0;
    const pathWasm = pathGuest - imageBase + _regionMap.GUEST_BASE;
    const mem = new Uint8Array(memory);
    for (let i = 0; i < path.length; i++) mem[pathWasm + i] = path.charCodeAt(i) & 0xFF;
    mem[pathWasm + path.length] = 0;
    exports.set_dll_path((exports.get_dll_count() >>> 0) - 1, pathGuest);
  }
...
function patchExeImports(exports, memory, exeBytes, dlls, log) {
  ...
  const g2w = addr => addr - imageBase + _regionMap.GUEST_BASE;
...
function loadDlls(exports, memory, exeBytes, dlls, log, options = {}) {
  ...
  for (const dll of dlls) {
    const modulePath = dll.path || `C:\\${dll.name}`;
    const { loadAddr, dllMain } = loadDll(exports, memory, dll.bytes, modulePath);
  ...
  // PROCESS_ATTACH follows the loader dependency graph, not the manifest's
  // fetch/mapping order. Running a dependent first is observable: Diablo II's
  // D2CMP calls Storm during its entry point, and an uninitialized Storm heap
  // raises the game's SMemReAlloc critical-error dialog. Mapping every image
  // first is fine; initialize each dependency before the DLL that imports it.
  for (const r of orderDllInitializers(dlls, results)) {
    if (r.dllMain) {
      callDllMain(exports, r.loadAddr, r.dllMain, log, {
        lpReserved: 1,
        advanceGuestTime: options.advanceGuestTime,
      });
    }
  }
...
function callDllMain(exports, loadAddr, dllMain, log, options = {}) {
  ...
  const reason = typeof options === 'number'
    ? 1 : (options.reason == null ? 1 : options.reason);
  const advanceGuestTime = typeof options === 'number'
    ? null : (typeof options.advanceGuestTime === 'function' ? options.advanceGuestTime : null);
  const dv = new DataView(exports.memory.buffer);
  const imageBase = exports.get_image_base();
  // Worker stacks can live in sparse VirtualAlloc regions far above the PE
  // image. The historical image-base formula only covers the contiguous main
  // mapping and turns such an ESP into a DataView offset beyond linear memory.
  // Use the interpreter's real mapper when available; keep the formula solely
  // for small loader mocks and older external embedders.
  const g2w = typeof exports.guest_to_wasm === 'function'
    ? addr => exports.guest_to_wasm(addr) >>> 0
    : addr => addr - imageBase + _regionMap.GUEST_BASE;
  ...
  let esp = savedEsp;
  // Push args in reverse order: lpReserved, fdwReason, hModule, ret=0.
  esp -= 4; dv.setUint32(g2w(esp), lpReserved, true);  // static != 0, dynamic = 0
  esp -= 4; dv.setUint32(g2w(esp), reason, true);
  ...
  // Run up to 2M blocks per slice — DllMain should complete well within this.
  // A DLL initializer is allowed to call Sleep. The normal process scheduler
  // resumes that cooperative yield later, but callDllMain is deliberately
  // synchronous: restoring the caller's EIP/ESP while the initializer is only
  // half-finished abandons its stack frame and leaves the DLL corrupted. Treat
  // Sleep as a scheduling point here and immediately continue the initializer.
  // The host advances its guest clock by the requested interval first: old
  // DirectX DLLs calibrate RDTSC against GetTickCount around Sleep, and seeing
  // zero elapsed milliseconds sends their arithmetic through corrupt state.
  // The bounded count still catches a DLL that tries to sleep forever.
  try {
    let sleepYields = 0;
    for (;;) {
      exports.run(maxBlocks);
      if ((exports.get_eip() >>> 0) <= 2) break;
      const haltedForYield = !exports.get_last_run_halt ||
        (exports.get_last_run_halt() | 0) === 3;
      const slept = haltedForYield && exports.get_sleep_yielded &&
        !!exports.get_sleep_yielded();
      if (!slept) break;
      sleepYields++;
      if (sleepYields > 64) {
        if (log) log('WARNING: DllMain exceeded 64 cooperative Sleep yields');
        break;
      }
      const sleepMs = exports.get_sleep_timeout ? exports.get_sleep_timeout() >>> 0 : 0;
      if (advanceGuestTime && sleepMs) advanceGuestTime(sleepMs);
      if (log) log(`DllMain resumed after cooperative Sleep (${sleepYields})`);
    }
  } catch (e) {
    ...
    if (crashEip <= 2 && (reason !== 1 || crashEax === 1)) {
      if (log) log(`DllMain returned successfully for reason=${reason} (trapped on sentinel return addr)`);
  ...
  if (finalEip !== 0) {
    const yieldReason = exports.get_yield_reason ? (exports.get_yield_reason() >>> 0) : 0;
    if (log) log(`WARNING: DllMain did not return cleanly, EIP=0x${finalEip.toString(16)} yield=${yieldReason}`);
  } else {
...
function orderDllInitializers(dlls, results) {
  const key = name => String(name || '').replace(/^.*[\\/]/, '').toLowerCase();
  const byName = new Map();
  for (let i = 0; i < results.length; i++) {
    byName.set(key((dlls[i] && dlls[i].name) || results[i].name), i);
  }
  const ordered = [];
  ...
  const visit = i => {
    ...
    // A malformed/cyclic import graph is still mapped successfully. Preserve
    // manifest order at the cycle edge instead of recursing forever.
    if (active.has(i)) return;
    active.add(i);
    const bytes = dlls[i] && dlls[i].bytes;
    for (const imported of bytes ? detectRequiredDlls(bytes) : []) {
      const dependency = byName.get(key(imported));
      if (dependency !== undefined && dependency !== i) visit(dependency);
    }
    active.delete(i);
    done.add(i);
    ordered.push(results[i]);
  };
  for (let i = 0; i < results.length; i++) visit(i);
  return ordered;
}
...
function initMsvcrtGlobals(exports, memory, msvcrtBase, log, options = {}) {
  ...
  // Disable the CRT small-block heap through its public entry point.  Older
  // code tried to recognize one private _set_sbh_threshold implementation and
  // patch a supposed __active_heap global out of its first instruction.  The
  // Win98 MSVCRT shipped with MW3 has a different, perfectly ordinary body
  // (`mov eax,[esp+4] ... mov [threshold],eax`), so that byte-pattern silently
  // matched nothing and left the expensive x86 allocator enabled.
  //
  // A zero threshold is the CRT-supported way to route future allocations to
  // HeapAlloc.  Calling the authentic export also preserves any SBH pages that
  // DllMain already created, so free() can continue to recognize their owners;
  // writing a guessed private global cannot make that guarantee.
  const setSbhThreshold = findDllExport(exports, memory, msvcrtBase, '_set_sbh_threshold');
  if (setSbhThreshold) {
    const ok = callDllExport(exports, memory, setSbhThreshold, [0], log,
      'MSVCRT!_set_sbh_threshold(0)');
    if (log) {
      log(ok
        ? 'MSVCRT small-block heap disabled through _set_sbh_threshold(0)'
        : 'WARNING: MSVCRT rejected _set_sbh_threshold(0)');
    }
  }
}
...
const WIN16_EMULATED_MODULES = new Set(['DDEML', 'WIN87EM', 'VER']);
// The first module id an app's own DLLs can take. Must match $WIN16_DYNAMIC_BASE.
const WIN16_DYNAMIC_BASE = 13;
// Must match $WIN16_DYNAMIC_MODULES. Names no longer consume permanent
// staging slots: app-local NEs share one six-megabyte load image.
const WIN16_DYNAMIC_MODULES = 24;
...
function loadWin16Dlls(exports, memory, exeBytes, dir, readFile, log = () => {}, extraNames = []) {
  ...
  const appStride = exports.win16_app_dll_staging_size
    ? exports.win16_app_dll_staging_size()
    : exports.win16_dll_staging(WIN16_DYNAMIC_BASE + 1)
  ...
  if (exports.win16_dynamic_module_slot) {
    ...
    for (let i = 0; i < WIN16_DYNAMIC_MODULES; i++) {
      const name = slotName(i);
      if (!name) break;
      claimed.add(name);
      next = WIN16_DYNAMIC_BASE + i + 1;
      // Keep the dynamic id/name that load_pe assigned, since WAT uses it to
      // dispatch these APIs, but an emulated module has no file to stage.
      if (WIN16_EMULATED_MODULES.has(name.toUpperCase())) continue;
    ...
    for (const name of [...imported, ...extraNames]) {
      ...
      claimed.add(name);
      if (next >= WIN16_DYNAMIC_BASE + WIN16_DYNAMIC_MODULES) {
      ...
      if (!isImport) {
        const bytes = readFile(dir, name);
        if (!bytes) continue;
        if (bytes.length > appStride) {
          log(`[win16] ${name}: ${bytes.length} bytes exceeds the staging image`);
          continue;
        }
      } else if (!stage(name, next)) {
        log(`[win16] ${name}: no DLL found, imports from it will stop the task`);
        continue;
      }
      ...
      if (!isImport) {
        log(`[win16] registered ${name} as module ${next} for LoadLibrary`);
        next++;
        continue;
      }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { loadDll, loadDlls, patchExeImports, patchDllImports, callDllMain, resumeAfterLoadLibraryYield, detectRequiredDlls, orderDllInitializers, shouldReportNtForDlls, loadWin16Dlls, win16ModuleNames, win16StageableModules, DLL_CONFIGS,
    _test: { initMsvcrtGlobals } };
} else if (typeof self !== 'undefined') {
  // `self`, not `window`: DLL loading drives guest execution (it sets EIP/ESP
  // and calls run() for DllMain), so in worker mode it has to run in the worker
  // where the instance lives — and there is no `window` there.
  self.DllLoader = { loadDll, loadDlls, patchExeImports, patchDllImports, callDllMain, resumeAfterLoadLibraryYield, detectRequiredDlls, orderDllInitializers, shouldReportNtForDlls, loadWin16Dlls, win16ModuleNames, win16StageableModules, DLL_CONFIGS };
}
