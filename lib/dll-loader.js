...
function loadWin16Dlls(exports, memory, exeBytes, dir, readFile, log = () => {}) {
  if (!exports.load_ne_dll || !exports.is_win16 || !exports.is_win16()) return [];
  const mem = new Uint8Array(memory.buffer);
  const loaded = [];
  for (const name of win16ModuleNames(exeBytes)) {
    const id = WIN16_MODULE_IDS[name];
    if (!id || id <= WIN16_SYSTEM_MODULES) continue;
    const bytes = readFile(dir, name);
    if (!bytes) { log(`[win16] ${name}: no DLL found, imports from it will stop the task`); continue; }
    const stride = exports.win16_dll_staging(1) - exports.win16_dll_staging(0);
    if (bytes.length > stride) { log(`[win16] ${name}: ${bytes.length} bytes exceeds the staging slot`); continue; }
    mem.set(bytes, exports.win16_dll_staging(id));
    if (exports.load_ne_dll(id)) { loaded.push(name); log(`[win16] loaded ${name} (${bytes.length} bytes)`); }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { loadDll, loadDlls, patchExeImports, patchDllImports, callDllMain, resumeAfterLoadLibraryYield, detectRequiredDlls, shouldReportNtForDlls, loadWin16Dlls, win16ModuleNames, DLL_CONFIGS };
} else if (typeof window !== 'undefined') {
  window.DllLoader = { loadDll, loadDlls, patchExeImports, patchDllImports, callDllMain, resumeAfterLoadLibraryYield, detectRequiredDlls, shouldReportNtForDlls, loadWin16Dlls, win16ModuleNames, DLL_CONFIGS };
}
