...
const WIN16_EMULATED_MODULES = new Set(['DDEML']);
...
function win16StageableModules() {
  return Object.keys(WIN16_MODULE_IDS)
    .filter(name => WIN16_MODULE_IDS[name] > WIN16_SYSTEM_MODULES &&
                    !WIN16_EMULATED_MODULES.has(name));
}
...
function loadWin16Dlls(exports, memory, exeBytes, dir, readFile, log = () => {}) {
  ...
  const stage = (name, id) => {
    ...
    if (bytes.length > stride) { log(`[win16] ${name}: ${bytes.length} bytes exceeds the staging slot`); return false; }
