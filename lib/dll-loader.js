...
// DLL inspection runs in the browser host, CLI, and guest Worker. All three
// consume the same direct/DIB/packed-page policy through mem-utils; keeping a
// local affine formula here made sparse DLL pointers host-dependent.
var _memUtilsDll = typeof require !== 'undefined' ? require('./mem-utils')
...
function loadDll(exports, memory, dllBytes, modulePath) {
  ...
  if (modulePath && exports.guest_alloc && exports.set_dll_path) {
    ...
    const pathWasm = _memUtilsDll.guestToWasm(pathGuest, exports, memory, imageBase);
...
function patchExeImports(exports, memory, exeBytes, dlls, log) {
  ...
  const g2w = addr => _memUtilsDll.guestToWasm(addr, exports, memory, imageBase);
...
function callDllMain(exports, loadAddr, dllMain, log, options = {}) {
  ...
  // The shared helper uses the interpreter's mapper when available and its
  // packed-page JS twin for small loader mocks and older external embedders.
  const g2w = addr => _memUtilsDll.guestToWasm(addr, exports, memory, imageBase);
