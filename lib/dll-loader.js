...
function loadDlls(exports, memory, exeBytes, dlls, log) {
  ...
  // DEBUG: Check MFC42 DefWindowProcA IAT entry
  if (results.length > 0 && results[0].name === 'mfc42.dll') {
    const dv = new DataView(memory);
    const imageBase = exports.get_image_base();
    const g2w = addr => addr - imageBase + 0x12000;
    const iatAddr = results[0].loadAddr + 0xcf4fc + 27*4; // DefWindowProcA
    const val = dv.getUint32(g2w(iatAddr), true);
    if (log) log(`DEBUG: MFC42 DefWindowProcA IAT @ 0x${iatAddr.toString(16)} = 0x${val.toString(16)}`);
  }
