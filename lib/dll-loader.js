...
function loadWin16Dlls(exports, memory, exeBytes, dir, readFile, log = () => {}) {
  ...
  const appStride = exports.win16_dll_staging(13) - exports.win16_dll_staging(12);
  const imported = new Set(win16ModuleNames(exeBytes));
  if (exports.win16_dynamic_modules_reset) exports.win16_dynamic_modules_reset();
  const stage = (name, id) => {
    const bytes = readFile(dir, name);
    if (!bytes) return false;
    const room = id >= 12 ? appStride : stride;
  ...
  if (exports.win16_dynamic_module_slot) {
    ...
    for (const name of imported) {
      if (WIN16_MODULE_IDS[name] || WIN16_EMULATED_MODULES.has(name)) continue;
      if (next >= 16) { log(`[win16] ${name}: no module slot left`); continue; }
      if (!stage(name, next)) {
        log(`[win16] ${name}: no DLL found, imports from it will stop the task`);
        continue;
      }
      const slot = exports.win16_dynamic_module_slot(next - 12);
