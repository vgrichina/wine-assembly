...
`;
  ...
  const TouchControls = {
    ...
    _addButton(spec) {
      if (!spec || (!Number.isFinite(spec.vk) &&
      ...
      const hold = command === null && mouseButton === null && spec.hold !== false;
      const vk = spec.vk | 0;
      ...
      const end = (e) => {
        ...
        for (const t of changedTouches(e)) {
          ...
          } else if (entry.hold) this._release(entry.vk);
        }
      };
    ...
    layoutZones() {
      ...
      for (const corner of ['bl', 'br']) {
        if (this._corners && this._corners[corner]) {
          ...
          const landscape = this.layout && this.layout.boardLayout && usableHost.width > usableHost.height;
    ...
    _placeModeToggle(app, host) {
      ...
      if (this.layout && this.layout.boardLayout) {
        const action = this._widgets.find(w => w._tcCorner === 'br' && w.getBoundingClientRect);
        const r = action && action.getBoundingClientRect();
        if (r && r.width > 0) {
          const span = spanFor(pills.length);
          let x = r.right - host.left - span;
          for (const p of pills.reverse()) {
            p.style.left = x + 'px';
            p.style.top = Math.max(4, r.top - host.top - size - 12) + 'px';
            p.style.opacity = '1';
            x += size + PILL_GAP;
          }
          return;
      ...
      const bands = [
        { bottom: true, band: bottomBand },
        { bottom: false, band: topBand },
      ].filter(item => item.band >= MIN_BAND)
        .sort((a, b) => b.band - a.band);
      for (const choice of bands) {
        ...
        const bottom = choice.bottom;
        const band = choice.band;
    ...
    getBoardArea() {
      ...
      if (host.width > height) {
        const side = Math.min(204, host.width * 0.30);
        return this._boardArea = { x: side / host.width, y: 0, w: 1 - 2 * side / host.width, h: height / host.height };
      }
      return this._boardArea = { x: 0, y: 0, w: 1, h: Math.max(1, height - 204) / host.height };
    },

    getOccupiedHeight() {
      if (!this.isVisible()) return 0;
      const host = this.el.getBoundingClientRect ? this.el.getBoundingClientRect() : null;
      if (this.layout.boardLayout && host) {
        const vv = typeof window !== 'undefined' && window.visualViewport;
