...
(function () {
  ...
  function collectStormQueue(exports, modules, appName) {
    ...
    // Storm's queue-head global is preferred-base+0x2228c. Relocations move
    // the whole DLL, so the RVA remains valid in every browser load.
    const address = (storm.loadAddress + 0x2228C) >>> 0;
  ...
  function formatSnapshot(snapshot) {
    ...
    for (const app of snapshot.apps) {
      ...
      if (app.stormQueue) {
        lines.push('', `Storm queue head ${hex(app.stormQueue.address)} ` +
          `(module ${hex(app.stormQueue.moduleBase)})`, ...formatWordDump(app.stormQueue));
      }
