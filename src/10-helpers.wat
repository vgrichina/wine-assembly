          ...
          (if (i32.lt_s (local.get $level) (i32.const 0))
            (then
              ...
              (if (i32.or (i32.eqz (local.get $steps))
                    (i32.gt_u (local.get $steps) (local.get $save_depth))) (then (br $finish)))
              (if (i32.eqz (call $gdi_dc_restore (local.get $hdc) (local.get $level)))
                (then (br $finish)))
              (local.set $save_depth (i32.sub (local.get $save_depth) (local.get $steps))))
            (else
              (if (i32.or (i32.eqz (local.get $level))
                    (i32.gt_u (local.get $level) (local.get $save_depth))) (then (br $finish)))
              (if (i32.eqz (call $gdi_dc_restore (local.get $hdc)
                    (i32.add (local.get $outer_save) (local.get $level))))
                (then (br $finish)))
              (local.set $save_depth (i32.sub (local.get $level) (i32.const 1)))))))
