  ...
  (func $gdi_metafile_snapshot_wmf (param $hdc i32) (result i32)
    ...
    (local.set $total (i32.add (local.get $pixels) (i32.const 120)))
    ...
    ;; Set MM_ANISOTROPIC, origin 0,0 and a 640x480 logical window.
  ...
  ;; Parse the standard bitmap record emitted above, plus compatible
  ;; application-provided META_STRETCHDIB records. Unsupported WMF records are
  ;; skipped according to their declared size; malformed streams fail closed.
  (func $gdi_metafile_play_wmf (param $hdc i32) (param $handle i32) (result i32)
    ...
    (local.set $end (i32.add (local.get $data) (i32.shl (local.get $words) (i32.const 1))))
    (block $done (loop $records
      ...
      (if (i32.eqz (local.get $function))
        (then (local.set $saw_eof (i32.const 1)) (br $done)))
  ...
  (func $gdi_metafile_play_emf (param $hdc i32) (param $handle i32)
    ...
    (if (i32.or (i32.lt_u (local.get $header_size) (i32.const 88))
          (i32.or (i32.gt_u (local.get $header_size) (local.get $declared))
            (i32.gt_u (local.get $declared) (local.get $available))))
      (then (return (i32.const 0))))
    ...
    (block $done (loop $records
      ...
      (if (i32.eq (local.get $record_type) (i32.const 0x51))
        (then
          ...
          (if (i32.and (local.get $target_rect)
                (i32.and (i32.ne (local.get $bounds_w) (i32.const 0))
                  (i32.ne (local.get $bounds_h) (i32.const 0))))
            (then
              (local.set $dx (i32.add (local.get $target_l) (i32.div_s
                (i32.mul (i32.sub (local.get $dx) (local.get $bounds_l)) (local.get $target_w))
                (local.get $bounds_w))))
              (local.set $dy (i32.add (local.get $target_t) (i32.div_s
                (i32.mul (i32.sub (local.get $dy) (local.get $bounds_t)) (local.get $target_h))
                (local.get $bounds_h))))
              (local.set $dw (i32.div_s (i32.mul (local.get $dw) (local.get $target_w))
                (local.get $bounds_w)))
              (local.set $dh (i32.div_s (i32.mul (local.get $dh) (local.get $target_h))
                (local.get $bounds_h)))))
          (local.set $mapped_x (call $gdi_line_map_x (local.get $dst) (local.get $dx)))
          (local.set $mapped_y (call $gdi_line_map_y (local.get $dst) (local.get $dy)))
          (local.set $mapped_w (i32.sub
