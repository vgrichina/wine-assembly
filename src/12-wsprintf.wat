  ...
  ;; Right-justify a UTF-16 value within $width characters.
  ;; $written is a byte count, matching the wide integer helpers above.
  (func $apply_pad_w (param $dst i32) (param $written i32) (param $width i32) (param $pad i32) (result i32)
    (local $chars i32) (local $n i32) (local $i i32)
    (local.set $chars (i32.div_u (local.get $written) (i32.const 2)))
    ...
    (block $done_shift (loop $shift
      (br_if $done_shift (i32.eqz (local.get $i)))
      (local.set $i (i32.sub (local.get $i) (i32.const 1)))
      (call $gs16
        (i32.add (local.get $dst) (i32.mul (i32.add (local.get $i) (local.get $n)) (i32.const 2)))
        (call $gl16 (i32.add (local.get $dst) (i32.mul (local.get $i) (i32.const 2)))))
      (br $shift)))
    (local.set $i (i32.const 0))
    (block $done_fill (loop $fill
      (br_if $done_fill (i32.ge_u (local.get $i) (local.get $n)))
      (call $gs16 (i32.add (local.get $dst) (i32.mul (local.get $i) (i32.const 2))) (local.get $pad))
      (local.set $i (i32.add (local.get $i) (i32.const 1)))
      (br $fill)))
    (i32.mul (local.get $width) (i32.const 2)))
    ...
    (block $done (loop $loop
      ...
      (if (i32.eq (local.get $ch) (i32.const 46))
        (then
          ...
          (block $skip_p (loop $pl
            ...
            (local.set $ch (call $gl16 (i32.add (local.get $fmt) (local.get $fi))))
            (br $pl)))))
      ;; Length modifier: Win32 wsprintf accepts h/l/w; Media Player uses %ws.
      ...
      (if (i32.or (i32.eq (local.get $ch) (i32.const 100)) (i32.eq (local.get $ch) (i32.const 105)))
        (then
          ...
          (local.set $oi (i32.add (local.get $oi)
            (call $apply_pad_w (i32.add (local.get $out) (local.get $oi)) (local.get $written) (local.get $width)
              (select (i32.const 48) (i32.const 32) (local.get $pad_zero)))))
          (br $loop)))
      ;; 'u': unsigned decimal
      (if (i32.eq (local.get $ch) (i32.const 117))
      ...
      (if (i32.eq (local.get $ch) (i32.const 99))
        (then
          ...
          (local.set $oi (i32.add (local.get $oi) (i32.const 2)))
          (br $loop)))
      ;; 's': wide string, except %hs which converts an ANSI string.
      (if (i32.eq (local.get $ch) (i32.const 115))
        (then
          (if (i32.eqz (local.get $arg))
            (then
            ...
            (else
              (local.set $sptr (local.get $arg))
              (if (i32.eq (local.get $length) (i32.const 104))
                (then
                  (block $asd (loop $asl
                    (local.set $sch (call $gl8 (local.get $sptr)))
                    (br_if $asd (i32.eqz (local.get $sch)))
