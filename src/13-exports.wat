  ...
  (func $winamp_about_fast_sqrt
    ...
    (if (i32.ge_u (local.get $v) (i32.const 0x10000))
      (then
        (if (i32.ge_u (local.get $v) (i32.const 0x1000000))
	      (then
	        (if (i32.ge_u (local.get $v) (i32.const 0x10000000))
	          (then
	            (if (i32.ge_u (local.get $v) (i32.const 0x40000000))
	              (then
	                    (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 24)))) (i32.const 8)))
	                    (return))
	              (else
	                    (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 22)))) (i32.const 7)))
	                    (return)))))
	        (if (i32.ge_u (local.get $v) (i32.const 0x4000000))
	          (then
	                (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 20)))) (i32.const 6)))
	                (return))
	          (else
	                (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 18)))) (i32.const 5)))
	                (return)))))
	      (else
	        (if (i32.ge_u (local.get $v) (i32.const 0x100000))
	          (then
	            (if (i32.ge_u (local.get $v) (i32.const 0x400000))
	              (then
	                    (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 16)))) (i32.const 4)))
	                    (return))
	              (else
	                    (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 14)))) (i32.const 3)))
	                    (return)))))
	        (if (i32.ge_u (local.get $v) (i32.const 0x40000))
	          (then
	                (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 12)))) (i32.const 2)))
	                (return))
	          (else
	                (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 10)))) (i32.const 1)))
	                (return)))))))
    (if (i32.ge_u (local.get $v) (i32.const 0x100))
      (then
        (if (i32.ge_u (local.get $v) (i32.const 0x1000))
          (then
            (if (i32.ge_u (local.get $v) (i32.const 0x4000))
              (then
	                (global.set $eax (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 8)))))
	                (return))
              (else
	                (global.set $eax (i32.shr_u (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 6)))) (i32.const 1)))
	                (return)))))
        (if (i32.ge_u (local.get $v) (i32.const 0x400))
          (then
	            (global.set $eax (i32.shr_u (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 4)))) (i32.const 2)))
	            (return))
          (else
	            (global.set $eax (i32.shr_u (call $gl8 (i32.add (i32.const 0x446660) (i32.shr_u (local.get $v) (i32.const 2)))) (i32.const 3)))
	            (return)))))
    (if (i32.ge_u (local.get $v) (i32.const 0x10))
      (then
        (if (i32.ge_u (local.get $v) (i32.const 0x40))
          (then
	            (global.set $eax (i32.shr_u (call $gl8 (i32.add (i32.const 0x446660) (local.get $v))) (i32.const 4)))
	            (return))
          (else
	            (global.set $eax (i32.shl (call $gl8 (i32.add (i32.const 0x446660) (i32.shl (local.get $v) (i32.const 2)))) (i32.const 5)))
	            (return)))))
    (if (i32.ge_u (local.get $v) (i32.const 0x4))
      (then
	        (global.set $eax (i32.shl (call $gl8 (i32.const 0x446660)) (i32.const 6)))
	        (return)))
    (global.set $eax (i32.shl (call $gl8 (i32.const 0x446660)) (i32.const 7))))

  (func $winamp_about_pixel_step
    (local $radius i32) (local $sample i32) (local $map_x i32) (local $map_y i32)
    (local $tmp i32) (local $dst i32) (local $src i32) (local $remaining i32)
    (local.set $radius
      (i32.trunc_f64_s
        (f64.sqrt
          (f64.convert_i32_s (call $gl32 (i32.add (global.get $ebp) (i32.const -52)))))))
    (local.set $sample
      (call $gl32
        (i32.add (i32.add (global.get $ebp) (i32.const -4264))
                 (i32.shl (local.get $radius) (i32.const 2)))))

    (local.set $tmp
      (i32.add
        (i32.div_s
          (i32.mul (local.get $sample) (call $gl32 (i32.add (global.get $ebp) (i32.const -32))))
          (i32.const 65536))
        (i32.const 196)))
    (if (i32.lt_s (local.get $tmp) (i32.const 0))
      (then (local.set $tmp (i32.const 0))))
    (if (i32.ge_s (local.get $tmp) (i32.const 392))
      (then (local.set $tmp (i32.const 391))))
    (local.set $map_x (local.get $tmp))

    (local.set $tmp
      (i32.add
        (i32.div_s
          (i32.mul (call $gl32 (i32.add (global.get $ebp) (i32.const -20))) (local.get $sample))
          (i32.const 65536))
        (i32.const 96)))
    (if (i32.lt_s (local.get $tmp) (i32.const 0))
      (then (local.set $tmp (i32.const 0))))
    (if (i32.ge_s (local.get $tmp) (i32.const 192))
      (then (local.set $tmp (i32.const 191))))
    (local.set $map_y (local.get $tmp))

    (local.set $src
      (i32.add
        (call $gl32 (i32.add (global.get $ebp) (i32.const -24)))
        (i32.add
          (call $gl32
            (i32.add (i32.add (global.get $ebp) (i32.const -1956))
                     (i32.shl (local.get $map_y) (i32.const 2))))
          (local.get $map_x))))
    (local.set $dst (call $gl32 (i32.add (global.get $ebp) (i32.const -8))))
    (call $gs8 (local.get $dst) (call $gl8 (local.get $src)))
    (call $gs32 (i32.add (global.get $ebp) (i32.const -8)) (i32.add (local.get $dst) (i32.const 1)))

    (local.set $tmp (call $gl32 (i32.add (global.get $ebp) (i32.const -32))))
    (call $gs32
      (i32.add (global.get $ebp) (i32.const -32))
      (i32.add (local.get $tmp) (call $gl32 (i32.add (global.get $ebp) (i32.const -56)))))
    (local.set $tmp (call $gl32 (i32.add (global.get $ebp) (i32.const -16))))
    (call $gs32 (i32.add (global.get $ebp) (i32.const -16)) (i32.add (local.get $tmp) (i32.const 2)))
    (call $gs32
      (i32.add (global.get $ebp) (i32.const -52))
      (i32.add (call $gl32 (i32.add (global.get $ebp) (i32.const -52))) (local.get $tmp)))

    (local.set $remaining (i32.sub (call $gl32 (i32.add (global.get $ebp) (i32.const -12))) (i32.const 1)))
    (call $gs32 (i32.add (global.get $ebp) (i32.const -12)) (local.get $remaining))
    (if (local.get $remaining)
      (then (global.set $eip (i32.const 0x004073f0)))
      (else (global.set $eip (i32.const 0x00407484)))))

  (func $winamp_credits_rgb_distance
    (local $a i32) (local $b i32) (local $dr i32) (local $dg i32) (local $db i32)
    (local.set $a (call $gl32 (i32.add (global.get $esp) (i32.const 4))))
    (local.set $b (call $gl32 (i32.add (global.get $esp) (i32.const 8))))
    (local.set $dr
      (i32.sub
        (call $gl8 (local.get $a))
        (call $gl8 (local.get $b))))
    (local.set $dg
      (i32.sub
        (call $gl8 (i32.add (local.get $a) (i32.const 1)))
        (call $gl8 (i32.add (local.get $b) (i32.const 1)))))
    (local.set $db
      (i32.sub
        (call $gl8 (i32.add (local.get $a) (i32.const 2)))
        (call $gl8 (i32.add (local.get $b) (i32.const 2)))))
    (global.set $eax
      (i32.add
        (i32.add (i32.mul (local.get $dr) (local.get $dr))
                 (i32.mul (local.get $dg) (local.get $dg)))
        (i32.mul (local.get $db) (local.get $db))))
    (global.set $eip (call $gl32 (global.get $esp)))
    (global.set $esp (i32.add (global.get $esp) (i32.const 4))))

  (func $winamp_credits_distance_value (param $a i32) (param $b i32) (result i32)
    (local $dr i32) (local $dg i32) (local $db i32)
    (local.set $dr (i32.sub (call $gl8 (local.get $a)) (call $gl8 (local.get $b))))
    (local.set $dg (i32.sub (call $gl8 (i32.add (local.get $a) (i32.const 1)))
                            (call $gl8 (i32.add (local.get $b) (i32.const 1)))))
    (local.set $db (i32.sub (call $gl8 (i32.add (local.get $a) (i32.const 2)))
                            (call $gl8 (i32.add (local.get $b) (i32.const 2)))))
    (i32.add
      (i32.add (i32.mul (local.get $dr) (local.get $dr))
               (i32.mul (local.get $dg) (local.get $dg)))
  ...
  (func $winamp_credits_scan_step
    (local $dist i32) (local $n i32)
    (local.set $n (i32.const 0))
    (block $done (loop $scan
      (br_if $done (i32.ge_u (local.get $n) (i32.const 8192)))
      (br_if $done (i32.ge_u (global.get $esi) (call $gl32 (i32.add (global.get $esp) (i32.const 28)))))
      (if (i32.eqz (call $gl8 (global.get $edi)))
        (then
          (local.set $dist
            (call $winamp_credits_distance_value
              (i32.add (global.get $edi) (i32.const -3))
              (call $gl32 (i32.add (global.get $esp) (i32.const 24)))))
          (if (i32.lt_s (local.get $dist) (i32.const 5))
            (then
              (call $gs8 (global.get $edi) (i32.const 1))
              (global.set $ebp (i32.sub (global.get $ebp) (i32.const 1))))
            (else
              (if (i32.lt_s (local.get $dist) (call $gl32 (i32.add (global.get $esp) (i32.const 16))))
                (then
                  (call $gs32 (i32.add (global.get $esp) (i32.const 16)) (local.get $dist))
                  (call $gs32 (i32.add (global.get $esp) (i32.const 20)) (global.get $esi))))))))
      (global.set $esi (i32.add (global.get $esi) (i32.const 1)))
      (global.set $edi (i32.add (global.get $edi) (i32.const 8)))
      (local.set $n (i32.add (local.get $n) (i32.const 1)))
      (br $scan)))
    (if (i32.lt_u (global.get $esi) (call $gl32 (i32.add (global.get $esp) (i32.const 28))))
      (then (global.set $eip (i32.const 0x004040ff)))
      (else (global.set $eip (i32.const 0x0040413a)))))

  (func $winamp_credits_merge_scan_step
    (local $dist i32) (local $n i32)
    (local.set $n (i32.const 0))
    (block $done (loop $scan
      (br_if $done (i32.ge_u (local.get $n) (i32.const 8192)))
      (br_if $done (i32.eqz (global.get $ebp)))
      (local.set $dist (call $winamp_credits_distance_value (global.get $esi) (global.get $ebp)))
      (if (i32.gt_s (global.get $ebx) (local.get $dist))
        (then
          (global.set $edi (global.get $esi))
          (global.set $ebx (local.get $dist))))
      (global.set $esi (global.get $ebp))
      (global.set $ebp (call $gl32 (i32.add (global.get $esi) (i32.const 4))))
      (local.set $n (i32.add (local.get $n) (i32.const 1)))
      (br $scan)))
    (if (global.get $ebp)
      (then (global.set $eip (i32.const 0x0040418a)))
      (else (global.set $eip (i32.const 0x004041a5)))))

  (func $winamp_credits_palette_search
    (local $limit i32) (local $r i32) (local $g i32) (local $b i32)
    (local $dr i32) (local $dg i32) (local $db i32) (local $dist i32)
    (local.set $limit (call $gl32 (i32.add (global.get $esp) (i32.const 0x20))))
    (local.set $r (call $gl32 (i32.add (global.get $esp) (i32.const 0x38))))
    (local.set $g (call $gl32 (i32.add (global.get $esp) (i32.const 0x18))))
    (local.set $b (call $gl32 (i32.add (global.get $esp) (i32.const 0x1c))))
    (block $done (loop $search
      (br_if $done (i32.ge_s (global.get $ebp) (local.get $limit)))
      (local.set $dr (i32.sub (call $gl8 (global.get $esi)) (local.get $r)))
      (local.set $dg (i32.sub (call $gl8 (i32.add (global.get $esi) (i32.const 1))) (local.get $g)))
      (local.set $db (i32.sub (call $gl8 (i32.add (global.get $esi) (i32.const 2))) (local.get $b)))
      (local.set $dist
        (i32.add
          (i32.add (i32.mul (local.get $dr) (local.get $dr))
                   (i32.mul (local.get $dg) (local.get $dg)))
          (i32.mul (local.get $db) (local.get $db))))
      (if (i32.lt_s (local.get $dist) (global.get $edi))
        (then
          (global.set $edi (local.get $dist))
          (call $gs32 (i32.add (global.get $esp) (i32.const 0x30)) (global.get $ebp))
          (br_if $done (i32.eqz (local.get $dist)))))
      (global.set $ebp (i32.add (global.get $ebp) (i32.const 1)))
      (global.set $esi (i32.add (global.get $esi) (i32.const 3)))
      (br $search)))
    (global.set $ebx (call $gl32 (i32.add (global.get $esp) (i32.const 0x2c))))
    (global.set $ebp (call $gl32 (i32.add (global.get $esp) (i32.const 0x24))))
    (global.set $eip (i32.const 0x00403bea)))

  (func $winamp_credits_span_step
    (local $n i32) (local $depth i32) (local $shade i32) (local $u i32) (local $v i32)
    (local $texel i32) (local $dst i32) (local $color_base i32)
    (local.set $n (call $gl8 (i32.add (global.get $esp) (i32.const 0x23))))
    (if (i32.eqz (local.get $n))
      (then (local.set $n (i32.const 256))))
    (block $done (loop $px
      (br_if $done (i32.eqz (local.get $n)))

      (local.set $shade (call $gl32 (i32.add (global.get $esp) (i32.const 0x48))))
      (if (i32.le_s (local.get $shade) (i32.const 0))
        (then
          (local.set $depth
            (i32.load16_u
              (call $g2w (call $gl32 (i32.add (global.get $esp) (i32.const 0xa8)))))))
        (else
          (if (i32.ge_s (local.get $shade) (i32.const 0xff00))
            (then
              (local.set $depth
                (i32.load16_u
                  (call $g2w
                  (i32.add
                    (call $gl32 (i32.add (global.get $esp) (i32.const 0xa8)))
                    (i32.const 0x1fe))))))
            (else
              (local.set $depth
                (i32.load16_u
                  ...
                  (i32.add
                    (call $gl32 (i32.add (global.get $esp) (i32.const 0xa8)))
                    (i32.shl (i32.shr_s (local.get $shade) (i32.const 8)) (i32.const 1))))))))))

      (local.set $u
        (i32.and
          (i32.shr_s (global.get $ebp)
                     (i32.and (call $gl32 (i32.add (global.get $esp) (i32.const 0x94))) (i32.const 31)))
          (call $gl32 (i32.add (global.get $esp) (i32.const 0xb8)))))
      (local.set $v
        (i32.and
          (i32.shr_s (global.get $edi) (i32.const 16))
          (call $gl32 (i32.add (global.get $esp) (i32.const 0xbc)))))
      (local.set $texel
        (call $gl8
          (i32.add
            (i32.add
              (call $gl32 (i32.add (global.get $esp) (i32.const 0xc4)))
              (local.get $u))
            (local.get $v))))
      (local.set $color_base
        (i32.add
          (call $gl32 (i32.add (global.get $esp) (i32.const 0xcc)))
          (local.get $texel)))
      (local.set $dst (call $gl32 (i32.add (global.get $esp) (i32.const 0x80))))
      (call $gs8
        (local.get $dst)
        (call $gl8 (i32.add (local.get $depth) (local.get $color_base))))

      (call $gs32
        (i32.add (global.get $esp) (i32.const 0x48))
        (i32.add (local.get $shade) (call $gl32 (i32.add (global.get $esp) (i32.const 0x70)))))
      (call $gs32
        (i32.add (global.get $esp) (i32.const 0x80))
        (i32.add (local.get $dst) (i32.const 1)))
      (global.set $edi
        (i32.add (global.get $edi) (call $gl32 (i32.add (global.get $esp) (i32.const 0x58)))))
      (global.set $ebp
        (i32.add (global.get $ebp) (call $gl32 (i32.add (global.get $esp) (i32.const 0x64)))))
      (local.set $n (i32.sub (local.get $n) (i32.const 1)))
      (br $px)))
    (call $gs8 (i32.add (global.get $esp) (i32.const 0x23)) (i32.const 0))
    (global.set $ebx (call $gl32 (i32.add (global.get $esp) (i32.const 0x80))))
    (global.set $eip (i32.const 0x004050f4)))

  (func $winamp_credits_quantize_init
    (local $pal i32) (local $out i32) (local $scale i32) (local $base i32)
    (local $qscale i32) (local $i i32) (local $j i32)
    (local $tr i32) (local $tg i32) (local $tb i32)
    (local $dr i32) (local $dg i32) (local $db i32)
    (local $dist i32) (local $best_dist i32) (local $best i32) (local $p i32)
    (if (call $gl32 (i32.const 0x0044f8bc))
      (then
        (global.set $eip (call $gl32 (global.get $esp)))
        (global.set $esp (i32.add (global.get $esp) (i32.const 4)))
        (return)))
    (local.set $pal (i32.add (call $gl32 (i32.add (global.get $esp) (i32.const 4))) (i32.const 2)))
    (local.set $out (i32.const 0x0045e020))
    (local.set $scale (i32.const 32))
    (call $gs32 (i32.const 0x0044f8bc) (i32.const 1))
    (block $outer_done (loop $outer
      (br_if $outer_done (i32.le_s (local.get $scale) (i32.const 0)))
      (local.set $base
        (select (i32.const 0) (local.get $base) (i32.gt_s (local.get $scale) (i32.const 16))))
      (local.set $qscale
        (select
          (i32.sub (local.get $scale) (i32.const 16))
          (local.get $scale)
          (i32.gt_s (local.get $scale) (i32.const 16))))
      (local.set $i (i32.const 0))
      (block $entries_done (loop $entries
        (br_if $entries_done (i32.ge_u (local.get $i) (i32.const 256)))
        (local.set $p (i32.add (local.get $pal) (i32.mul (local.get $i) (i32.const 3))))
        (local.set $tr
          (i32.add
            (i32.shr_s
              (i32.add
                (i32.mul (call $gl8 (i32.add (local.get $p) (i32.const -2))) (local.get $qscale))
                (i32.const 15))
              (i32.const 4))
            (local.get $base)))
        (local.set $tg
          (i32.add
            (i32.shr_s
              (i32.add
                (i32.mul (call $gl8 (i32.add (local.get $p) (i32.const -1))) (local.get $qscale))
                (i32.const 15))
              (i32.const 4))
            (local.get $base)))
        (local.set $tb
          (i32.add
            (i32.shr_s
              (i32.add
                (i32.mul (call $gl8 (local.get $p)) (local.get $qscale))
                (i32.const 15))
              (i32.const 4))
            (local.get $base)))
        (local.set $best_dist (i32.const 0x200000))
        (local.set $best (i32.const 0))
        (local.set $j (i32.const 0))
        (block $search_done (loop $search
          (br_if $search_done (i32.ge_u (local.get $j) (i32.const 256)))
          (local.set $p (i32.add (local.get $pal) (i32.mul (local.get $j) (i32.const 3))))
          (local.set $dr (i32.sub (call $gl8 (i32.add (local.get $p) (i32.const -2))) (local.get $tr)))
          (local.set $dg (i32.sub (call $gl8 (i32.add (local.get $p) (i32.const -1))) (local.get $tg)))
          (local.set $db (i32.sub (call $gl8 (local.get $p)) (local.get $tb)))
          (local.set $dist
            (i32.add
              (i32.add (i32.mul (local.get $dr) (local.get $dr))
                       (i32.mul (local.get $dg) (local.get $dg)))
          ...
          (if (i32.lt_s (local.get $dist) (local.get $best_dist))
            (then
              ...
              (br_if $search_done (i32.eqz (local.get $dist)))))
          (local.set $j (i32.add (local.get $j) (i32.const 1)))
          (br $search)))
        (call $gs8 (i32.add (local.get $out) (local.get $i)) (local.get $best))
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (br $entries)))
      (local.set $base (i32.add (local.get $base) (i32.const 16)))
      (local.set $out (i32.add (local.get $out) (i32.const 256)))
      (local.set $scale (i32.sub (local.get $scale) (i32.const 1)))
      (br $outer)))
    (global.set $eip (call $gl32 (global.get $esp)))
    (global.set $esp (i32.add (global.get $esp) (i32.const 4))))

  (func $winamp_credits_image_init
    (local $src_base i32) (local $dst i32) (local $row_acc i32) (local $col_acc i32)
    (local $row i32) (local $col i32) (local $src_row i32)
    (local.set $src_base (call $gl32 (i32.add (global.get $esp) (i32.const 0x10))))
    (local.set $dst (global.get $ebx))
    (local.set $row_acc (call $gl32 (i32.add (global.get $esp) (i32.const 0x18))))
    (local.set $row (i32.const 0))
    (block $rows_done (loop $rows
      (br_if $rows_done (i32.ge_u (local.get $row) (i32.const 256)))
      (local.set $src_row
        (i32.add
          (local.get $src_base)
          (i32.mul
            (i32.mul (i32.shr_s (local.get $row_acc) (i32.const 16)) (i32.const 49))
            (i32.const 8))))
      (local.set $col_acc (i32.const 0))
      (local.set $col (i32.const 0))
      (block $cols_done (loop $cols
        (br_if $cols_done (i32.ge_u (local.get $col) (i32.const 256)))
        (call $gs8
          (local.get $dst)
          (call $gl8
            (i32.add
              (local.get $src_row)
              (i32.shr_s (local.get $col_acc) (i32.const 16)))))
        (local.set $dst (i32.add (local.get $dst) (i32.const 1)))
        (local.set $col_acc (i32.add (local.get $col_acc) (i32.const 0x18800)))
        (local.set $col (i32.add (local.get $col) (i32.const 1)))
        (br $cols)))
      (local.set $row_acc (i32.add (local.get $row_acc) (i32.const 0xc000)))
      (local.set $row (i32.add (local.get $row) (i32.const 1)))
      (br $rows)))
    (global.set $ebx (local.get $dst))
    (call $gs32 (i32.add (global.get $esp) (i32.const 0x18)) (local.get $row_acc))
    (global.set $ebp (i32.const 0))
    (global.set $eip (i32.const 0x00402c89)))
  ...
  (func (export "init_thread") (param $tid i32)
    ...
    (global.set $CACHE_INDEX (i32.add (i32.const 0x05E52000)
