...
- **SetDlgItemInt now real** (`src/09a-handlers.wat`): heap_allocs a 16-byte scratch, formats the value as decimal ASCII (handling bSigned), sends `WM_SETTEXT` to the child found by `ctrl_find_by_id`, then frees. `ctrl_text_dup` already copies the buffer inside the edit's WM_SETTEXT handler, so the scratch is safe to release immediately. Custom Field dialog now shows the defaults (9/9/10 on XP) in the edit boxes. Side effect: the extra paint queue traffic from the 3 WM_SETTEXTs also brings the Cancel button into the pre-yield paint pass, so the previously-missing Cancel now renders too — both listed open tasks closed with one change.
...
- **Menu dropdown clipped to window**: opening Game menu showed only 6 of 14 items, with labels truncated ("Intermediat"/"Custom.."). The old Canvas bridge reused a client DC clip while drawing the desktop popup overlay. Popup menus now use a dedicated WAT-backed screen overlay DC, so their canonical clip and coordinates are independent of the owner window. All 14 items render at full width.
