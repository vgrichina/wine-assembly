...
- **2026-08-16: the `MeshBuilder::Load` blocker is closed** — it was a fabricated
  `camera.x` (a ProgressiveMesh copied under a plain-mesh name), and d3rm was
  right to refuse it. Given a real plain `Mesh` file the DX SDK viewer loads and
  renders geometry; see `apps/screensavers.md` Task 3 and
  `test/fixtures/d3drm/README.md`. The note below predates that.
