...
| `--trace-loopmatch[=0xEIP]` | At **decode** time, dump every self-loop block the decoder emits (or just the one entered at `0xEIP`): entry, op count, and each `(handler index, operand)` — exactly the input the loop-idiom matcher in `src/07b-loop-match.wat` sees. Pipe the log through `node tools/loopmatch-decode.js <log> [--eip=] [--uniq]` to get handler names. Reach for this when asking "why did this loop not get lowered": the answer is a role the matcher does not recognize, and this shows which op it is. Pairs with `--loopmatch-stats` (self-loop/match counts at exit) and `--no-loop-superops` (match and count as usual, but emit the original ops — the A/B partner for measuring a lowering without a rebuild). |
