...
out.push('');
out.push('    ;; CreateWindowEx continuation thunk — WndProc(WM_CREATE) returned');
out.push('    (if (i32.eq (local.get $name_rva) (i32.const 0xCACA0001))');
out.push('      (then');
out.push('        (global.set $eax (global.get $createwnd_saved_hwnd))');
out.push('        (global.set $eip (global.get $createwnd_saved_ret))');
out.push('        (return)))');
out.push('');
out.push('    (local.set $name_ptr (i32.add (global.get $GUEST_BASE) (i32.add (local.get $name_rva) (i32.const 2))))');
out.push('');
out.push('    ;; Log API name for tracing');
out.push('    (call $host_log (local.get $name_ptr) (call $strlen (local.get $name_ptr)))');
out.push('');
