...
assert(indexHtml.includes('lib/renderer-input.js?v=154'), 'web host should cache-bust renderer input after desktop changes');
assert(indexHtml.includes('lib/host-imports.js?v=154'), 'web host should cache-bust host-imports after desktop changes');
assert(!hostJs.includes('?v=55'), 'host.js should not fetch stale WAT/API sources with v55');
assert(hostJs.includes("SOURCE_VERSION = '154'"), 'host.js should define the current WAT/API cache-buster');
...
assert(hostJs.includes('audioHot ? (menuOpen ? 20000 : 5000) : 50000'), 'web host should use larger non-audio worker quanta for visible compute workers');
assert(hostJs.includes('menuOpen ? (mainThreadWaiting ? 8 : 6) : (mainThreadWaiting ? 6 : 4)'), 'web host should use a smaller wall budget while waveOut audio is active');
