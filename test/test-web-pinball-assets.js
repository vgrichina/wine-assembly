...
assert(indexHtml.includes('lib/renderer-input.js?v=151'), 'web host should cache-bust renderer input after desktop changes');
assert(indexHtml.includes('lib/host-imports.js?v=151'), 'web host should cache-bust host-imports after desktop changes');
assert(!hostJs.includes('?v=55'), 'host.js should not fetch stale WAT/API sources with v55');
assert(hostJs.includes("SOURCE_VERSION = '151'"), 'host.js should define the current WAT/API cache-buster');
...
assert(hostJs.includes('audioHot ? 5000 : 50000'), 'web host should use larger non-audio worker quanta for visible compute workers');
assert(hostJs.includes('audioHot ? (mainThreadWaiting ? 6 : 4)'), 'web host should use a smaller wall budget while waveOut audio is active');
assert(hostJs.includes('prioritizeAudioThreads: audioHot'), 'web host should prioritize audio workers while waveOut audio is active');
