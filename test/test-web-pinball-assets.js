...
// inside index.html. They are lib/apps.js, lib/browser-shell.js and
// lib/browser-input.js now (lib/dll-registry.js owns the DLL name map), so the
// assertions below look at the whole browser app, not just the page markup.
const pageHtml = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const webApp = [
  pageHtml,
  fs.readFileSync(path.join(ROOT, 'lib', 'apps.js'), 'utf8'),
  fs.readFileSync(path.join(ROOT, 'lib', 'browser-shell.js'), 'utf8'),
  fs.readFileSync(path.join(ROOT, 'lib', 'browser-input.js'), 'utf8'),
  fs.readFileSync(path.join(ROOT, 'lib', 'dll-registry.js'), 'utf8'),
].join('\n');
const hostJs = fs.readFileSync(path.join(ROOT, 'host.js'), 'utf8');
// The GDI/window and audio halves of the host import layer are their own files
// now, so the import-layer assertions read all three.
const hostImportsJs = [
...
assert(pageHtml.includes('lib/browser-shell.js?v=9'),
...
assert(webApp.includes("if (typeof toggleRecording === 'function') toggleRecording();"), 'Start menu recorder should use default screen capture');
...
assert(/\[\s*'funtris'\s*,\s*'Funtris'/.test(webApp), 'default desktop whitelist should include Funtris');
for (const [id, label, exe] of [
  ...
  assert(new RegExp(`\\[\\s*'${id}'\\s*,\\s*'${label}'`).test(webApp), `default desktop whitelist should include ${label}`);
  assertBundled(exe);
  assertDeployFile(exe);
}
assert(/\[\s*'pyramid'\s*,\s*'Pyramid'/.test(webApp), 'default desktop whitelist should include Pyramid');
assert(/funtris:\s*\{[\s\S]*Funtris\\\\Options'[\s\S]*GetStarted'[\s\S]*data:\s*0[\s\S]*dismissStartupDialog:\s*\{\s*title:\s*'Funtris',\s*command:\s*1\s*\}/s.test(webApp), 'Funtris browser launch should suppress startup nag dialogs');
assert(/peaks:\s*\{[\s\S]*Peaks\\\\Options'[\s\S]*GetStarted'[\s\S]*data:\s*0/s.test(webApp), 'Peaks browser launch should suppress startup nag dialogs');
assert(/pyramid:\s*\{[\s\S]*iCDateCount'[\s\S]*value:\s*-1[\s\S]*GetStarted'[\s\S]*data:\s*0/s.test(webApp), 'Pyramid browser launch should suppress startup nag dialogs');
for (const rel of [
...
assert(/winamp:\s*\{[\s\S]*resetIniOnLaunch:\s*\['winamp\.ini'\]/s.test(webApp), 'Winamp web launch should reset stale persisted INI layout state');
...
assert(webApp.includes('lib/renderer-input.js?v=194'), 'web host should cache-bust renderer input after buffered DirectInput mouse edges');
...
assert(webApp.indexOf('lib/gdi-surface.js?v=2') < webApp.indexOf('lib/host-imports.js?v=210'),
  'web host should load the GDI surface module before host imports');
assert(webApp.includes('lib/host-imports.js?v=210'), 'web host should cache-bust the current host imports');
...
assert(webApp.includes('host.js?v=241'), 'web host should cache-bust host.js after the current source update');
assert(hostJs.includes("static SOURCE_VERSION = '232'"), 'web host should cache-bust WASM artifacts and WAT source compilation');
...
assert(webApp.includes('wine._availableDllFiles = new Set(Object.keys(availableDlls))'), 'web launch should tell host imports which DLLs can be dynamically fetched');
...
// Recent input used to zero the worker budget; that starved mouse-driven games
// (Blobby measured 0fps while the mouse moved), so it reserves most of the
// slice for input instead of all of it.
assert(hostJs.includes('recentInputWake ? Math.max(10000, activeStepsPerSlice >> 2)'),
  'web host should reserve most of the slice for synchronous dialog input without starving workers');
assert(hostJs.includes('recentInputWake ? 6'), 'web host should shorten, not remove, the worker quantum while input arrives');
...
assert(webApp.includes('Loading ${app.files.length} data file(s)...'), 'web launcher should log data-file preload progress');
assert(webApp.includes('onProgress: ({ loaded, failed, total }) =>'), 'web launcher should report data-file preload progress');
assert(webApp.includes('Data files ready: ${app.files.length}'), 'web launcher should log data-file preload completion');
assert(webApp.includes('Starting run slice=${runSlice}'), 'web launcher should log run-loop start');
...
for (const app of ['freecell', 'sol', 'cruel', 'golf']) {
  ...
  assert(re.test(webApp), `${app} web manifest should explicitly load cards.dll`);
}
