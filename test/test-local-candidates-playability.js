...
const tests = [
  {
    ...
    check(result, before, after, metricBefore, metricAfter, diff) {
      return [
        ...
        { name: 'Deal command visibly changes cards', pass: diff > 1000 },
  ...
  },
  {
    ...
    input: null,
    check(result, before, after, metricBefore, metricAfter) {
      return [
        { name: 'run exited cleanly', pass: result.exitCode === 0 },
        { name: 'about dialog was dismissed', pass: /dlg-click: id=1/.test(result.out) },
  ...
  },
  {
    ...
    after: fullPath('qblackjack', 'borrow'),
    input: null,
    check(result, before, after, metricBefore, metricAfter, diff) {
      return [
        { name: 'run exited cleanly', pass: result.exitCode === 0 },
        ...
        { name: 'Min bet opens expected borrow prompt', pass: /MessageBox] "You can't afford that!"/.test(result.out) },
        { name: 'house-limit modal is not shown', pass: !/Congratulations!/.test(result.out) },
        { name: 'borrow prompt visibly changes the table', pass: diff > 10000 },
        { name: 'no crash marker', pass: hasNoCrash(result.out) },
      ];
    },
  },
...
tests.find(t => t.id === 'peaks').input = [
  ...
  '21:dlg-click:1',
  `60:png:${tests.find(t => t.id === 'peaks').before}`,
  '80:post-cmd:40005',
  `130:png:${tests.find(t => t.id === 'peaks').after}`,
  '140:dump-windows:peaks',
  '150:stop',
].join(',');
...
tests.find(t => t.id === 'fourstones').input = [
  '20:wait-dlg-control:1:1000',
  '21:dlg-click:1',
  `60:png:${tests.find(t => t.id === 'fourstones').before}`,
  '80:post-cmd:40005',
  '140:post-cmd:40002',
  `180:png:${tests.find(t => t.id === 'fourstones').after}`,
  '190:dump-windows:four',
  '200:stop',
].join(',');
...
tests.find(t => t.id === 'qblackjack').input = [
  '20:wait-dlg-control:1:1000',
  '21:dlg-click:1',
  `60:png:${tests.find(t => t.id === 'qblackjack').before}`,
  '80:post-cmd:311',
  `120:png:${tests.find(t => t.id === 'qblackjack').after}`,
  '140:dump-windows:qbj',
  '150:stop',
].join(',');
...
tests.find(t => t.id === 'marbles').input = [
  ...
  '110:mousedown:320:240',
  '112:mouseup:320:240',
  `170:png:${tests.find(t => t.id === 'marbles').after}`,
  '180:dump-windows:marbles',
  '190:stop',
].join(',');
