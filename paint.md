...
- `$handle__initterm` now iterates [start, end), calls each non-NULL fn ptr
  via continuation thunk (0xCACA0003)
- 0xCACA0003 allocated in pe-loader, handler in gen_dispatch.js
- 0xCACA0002 (CBT hook continuation) also added to gen_dispatch.js
  (was lost when gen_dispatch.js regenerated the file)

**Current state:** EXE's `_initterm` (API #27) now works with real iteration.
But MFC42's DllMain `_initterm` runs via real msvcrt code during `callDllMain`
and the init functions still don't execute. Need to verify:
- Are relocations applied to .data init table entries? (dump guest memory at
  0x0111d040 after DLL loading to check if value is 0x0105c8e5 or stale
  0x5f4078e5)
- Does real msvcrt's `_initterm` iterate correctly? (may have code paths we
  don't handle)

The function at 0x0105c8ef sets `[module_state+0x103c] = AfxWndProc`:
```asm
push 0x01           ;; flag
push 0x421          ;; size?
...
- `09b-dispatch.wat` (hand-written) — thunk handlers (CACA0000-0003), arg
...
**_initterm now implemented (committed 5402e4b):** Handler iterates [start,
end) function pointer table, calls each non-NULL entry via 0xCACA0003
continuation thunk chain. Replaces the previous no-op stub.

**Next step:** Dump guest memory at 0x0111d040 after DLL loading to verify
relocation. If value is still 0x5f4078e5, relocations aren't applied to .data
section. If it's 0x0105c8e5, the issue is in msvcrt's `_initterm` code path.
Note: MFC42's DllMain `_initterm` goes through real msvcrt code (patched IAT),
not our thunk handler. The EXE's `_initterm` (API #27) uses our new handler.
